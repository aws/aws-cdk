...
constructor(scope: Construct, id: string, props: Ec2ServiceProps) {
  ...
  // インポートされたタスク定義の場合の情報メッセージはsuper()後に追加
...
**検証:**
- [x] ビルドが通ること
- [x] awsvpcモードのテストが通ること
- [x] bridge/host/noneモードのテストが通ること
...
- [x] インポートされたTaskDefinitionでEc2Serviceを作成するテストを追加
```typescript
...
- [x] defaultContainerバリデーションがスキップされることを確認
```typescript
...
- [x] インポートされたTaskDefinitionでネットワーク設定を使用した場合
```typescript
...
- [x] placement strategies/constraintsは引き続き使用可能であることを確認
```typescript
...
- [x] インポートされたタスク定義を使用する基本的なインテグレーションテストを作成
```typescript
...
- [x] Placement Strategyがインポートされたタスク定義でも機能することを確認するテストを追加
```typescript
...
- [x] Ec2Serviceでインポートされたタスク定義の使用例を追加
```markdown
...
- [x] `Ec2ServiceProps.taskDefinition`のドキュメントを更新
```typescript
/**
 * The task definition to use for tasks in the service.
 *
...
### Task 7.3: 後方互換性確認
- [x] 既存のEc2Serviceコードが変更なしで動作すること
- [x] 既存のテストがすべて通ること

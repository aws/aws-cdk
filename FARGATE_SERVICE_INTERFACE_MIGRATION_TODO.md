...
- [x] `TaskDefinition`のコンストラクタ内でSymbolを設定
```typescript
...
**検証:**
- [x] ビルドが通ること
- [x] `TaskDefinition.isTaskDefinition()`が正しく動作すること
...
- [x] `FargateServiceProps`インターフェースを修正
```typescript
...
- [x] ephemeralStorageGiBチェックを条件付きに変更
```typescript
...
- [x] referencesSecretJsonFieldバリデーションを条件付きに変更
```typescript
if (TaskDefinition.isTaskDefinition(props.taskDefinition)) {
...
- [x] defaultContainerバリデーションを条件付きに変更
```typescript
if (TaskDefinition.isTaskDefinition(props.taskDefinition)) {
  this.node.addValidation({
...
- [x] `taskRole` の null ガードを追加（インポートされたタスク定義では `taskRole` が undefined になりえる）
```typescript
// 変更前
...
- [x] `CODE_DEPLOY` および `taskDefinitionRevision` での `family` アクセスにガードを追加（`family` は `ITaskDefinition` に存在しない）
```typescript
// 変更後
...
- [x] `enableExecuteCommand()`メソッドを修正
```typescript
...
- [x] `executeCommandLogConfiguration()`メソッドを修正
```typescript
...
- [x] `enableExecuteCommandEncryption()`メソッドを修正
```typescript
...
- [x] `validateServiceConnectConfiguration()`メソッドを修正
```typescript
...
- [x] `enableServiceConnect()`メソッドのservices配列生成部分を修正
```typescript
...
- [x] `enableServiceConnect()`メソッドのlogConfig部分を修正
```typescript
...
- [x] `defaultLoadBalancerTarget`ゲッターを修正
```typescript
...
- [x] インポートされたTaskDefinitionでFargateServiceを作成するテストを追加
```typescript
...
- [x] インポートされたTaskDefinitionでバリデーションがスキップされることを確認
```typescript
...
- [x] Service Connectがインポートされたタスク定義でエラーになることを確認
```typescript
...
- [x] Execute Commandがインポートされたタスク定義で警告を出すことを確認
```typescript
...
- [x] Load Balancer Targetがインポートされたタスク定義でエラーになることを確認
```typescript
...
- [x] インポートされたタスク定義を使用するインテグレーションテストを作成
```typescript
...
- [x] `FargateServiceProps.taskDefinition`のドキュメントを更新
```typescript
/**
 * The task definition to use for tasks in the service.
 *

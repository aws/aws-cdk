...
export async function main() {
  const args = yargs
    .env('GEN')
    .usage('Usage: gen')
    .option('java', { type: 'string', alias: 'j', desc: 'Website root where JavaDocs are available' })
    .option('typescript', { type: 'string', alias: 't', desc: 'Website root where TypeScript docs are available' })
    .option('dotnet', { type: 'string', alias: 'n', desc: 'Website root where .NET docs are available' })
    .option('website', { type: 'string', alias: 'w', required: true, desc: 'Directory where to store sidebars.json' })
    .option('docs', { type: 'string', alias: 'd', required: true, desc: 'Directory where to store reference MarkDowns' })
    .argv;
  ...
  const docs = new Docs(args.website, args.docs);

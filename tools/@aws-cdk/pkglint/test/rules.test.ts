...
describe('BundledFilesArePresent', () => {
  let fakeModule: FakeModule | undefined;
  ...
  test('passes when the required bundled file exists and is non-empty', async () => {
    fakeModule = new FakeModule({
      files: {
        'package.json': {
          main: 'lib/index.js',
        ...
        'lib/handler.bundle/index.js': '// bundled handler code',
      },
    });
    const dirPath = await fakeModule.tmpdir();
  ...
  test('errors when a required bundled file is missing', async () => {
    fakeModule = new FakeModule({
      files: {
        'package.json': {
          main: 'lib/index.js',
    ...
    expect(pkgJson.hasReports).toBe(true);
    expect(pkgJson.reports.length).toEqual(1);
    expect(pkgJson.reports[0].ruleName).toEqual('package-info/require-bundled-files');
    expect(pkgJson.reports[0].message).toContain('missing');
    expect(pkgJson.reports[0].message).toContain('lib/handler.bundle/index.js');
  });

  test('errors when a required bundled file is empty', async () => {
    fakeModule = new FakeModule({
      files: {
        'package.json': {
          main: 'lib/index.js',
    ...
    expect(pkgJson.hasReports).toBe(true);
    expect(pkgJson.reports.length).toEqual(1);
    expect(pkgJson.reports[0].message).toContain('empty');
  });

  test('is skipped when the package has not been compiled yet', async () => {
    // `main` points to a file that does not exist -> package is not built,
    // so the bundled artifacts are expected to be absent too.
    fakeModule = new FakeModule({
      files: {
        'package.json': {
          main: 'lib/index.js',

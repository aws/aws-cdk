...
export function generateClassExample(typeSystem: TypeSystem, classType: spec.ClassType): string {
  ...
  if (classType.abstract) {
    ...
    example.push({
      tabs: 0,
      after: '\n',
      fragment: '// This example was autogenerated. You will have to change the values.'
    });
    example.push({
      tabs: 0,
      after: '',
      fragment: `new ${classType.name}`,
    });
    example.push({
      tabs: 0,
      before: '(',
      after: '\n)',
      fragment: buildExample(typeSystem, classType, 1),
    });
...
function renderExample(example: ExampleFragment[]): string {
  const res: string[] = [];
  for (const ex of example) {
    res.push(`${ex.before ?? ''}${tabLevel(ex.tabs)}${ex.fragment}${ex.after ?? ''}`);
  }
  return res.join('');
}
...
function buildExample(typeSystem: TypeSystem, type: spec.Type, level: number): string {
  ...
  if (spec.isClassType(type)) {
    for (const params of type.initializer?.parameters ?? []) {
      if (params.name === 'scope' || params.name === 'id') {
        example.push({
          tabs: 0,
          after: ', ',
          fragment: params.name === 'scope' ? 'this' : 'id',
        });
      } else {
        example.push({
          tabs: level,
          after: ',\n',
          fragment: `${params.name}: ${addProp(typeSystem, params.type, params.name, level)}`,
        });
      }
    }
    ...
    example.push({
      tabs: level,
      after: ',\n',
      fragment: `${type.name}.${type.members[0].name}`,
    });
    ...
    for (const props of type.properties ?? []) {
      example.push({
        tabs: level,
        after: ',',
        fragment: `${props.name}: ${addProp(typeSystem, props.type, props.name, level)}`,
      });
    }
    example.push({
      tabs: level-1,
...
function addProp(typeSystem: TypeSystem, typeReference: spec.TypeReference, name: string, level: number): string {
  ...
  if (spec.isNamedTypeReference(typeReference)) {
    ...
    if (nextType) {
      return `{\n${buildExample(typeSystem, nextType, level+1)}}`;
    }
    return 'DNE';
  }

...
test('if the queue is encrypted with a imported kms key, printout warning', () => {
  ...
  Annotations.fromStack(stack).hasWarning('/Default/ImportedKey', `S3Notifications:SQS:KMSPermissionsNotAdded: Can not change key policy of imported kms key. Ensure that your key policy contains the following permissions: \n${JSON.stringify({
    Action: [
      'kms:GenerateDataKey*',
      'kms:Decrypt',
    ],

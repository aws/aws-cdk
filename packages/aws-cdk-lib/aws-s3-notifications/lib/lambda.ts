...
export class LambdaDestination implements s3.IBucketNotificationDestination {
  ...
  public bind(scope: Construct, bucket: s3.IBucketRef): s3.BucketNotificationDestinationConfig {
    ...
    if (!(bucket instanceof Construct)) {
      throw new ValidationError('Lambdadestinationfunctiononlyconfigured', `LambdaDestination for function ${Names.nodeUniqueId(this.fn.permissionsNode)} can only be configured on a
    ...
    if (bucket.node.tryFindChild(permissionId) === undefined) {
      this.fn.addPermission(permissionId, {
        ...
        sourceArn: bucket.bucketRef.bucketArn,

...
export abstract class InputFormat {
  ...
  public static csv(options?: CsvOptions): InputFormat {
    ...
    if (options?.delimiter && (!this.validCsvDelimiters.includes(options.delimiter) || options.delimiter.length !== 1)) {
      throw new UnscopedValidationError('DelimiterSingleCharacterOne', [
...
export abstract class TableBase extends Resource implements ITable, ITableRef, iam.IResourceWithPolicy, IEncryptedResource {
  ...
  public get streamGrants(): StreamGrants {
    if (!this.tableStreamArn) {
      throw new ValidationError('DynamodbStreamsEnabledTable', `DynamoDB Streams must be enabled on the table ${this.node.path}`, this);
    }
  ...
  public metricSystemErrors(props?: cloudwatch.MetricOptions): cloudwatch.Metric {
    if (!props?.dimensions?.Operation && !props?.dimensionsMap?.Operation) {
      ...
      throw new ValidationError('OperationDimensionPassedSystemerrors', "'Operation' dimension must be passed for the 'SystemErrors' metric.", this);
    }
  ...
  public metricUserErrors(props?: cloudwatch.MetricOptions): cloudwatch.Metric {
    if (props?.dimensions) {
      throw new ValidationError('DimensionsSupportedUsererrorsMetric', "'dimensions' is not supported for the 'UserErrors' metric", this);
    }
  ...
  public metricSuccessfulRequestLatency(props?: cloudwatch.MetricOptions): cloudwatch.Metric {
    if (!props?.dimensions?.Operation && !props?.dimensionsMap?.Operation) {
      throw new ValidationError('OperationDimensionPassedSuccessfulrequestlatency', "'Operation' dimension must be passed for the 'SuccessfulRequestLatency' metric.", this);
    }
  ...
  private sumMetricsForOperations(metricName: string, expressionLabel: string, props?: OperationsMetricOptions): cloudwatch.IMetric {
    if (props?.dimensions?.Operation) {
      throw new ValidationError('OperationDimensionSupportedOperations', "The Operation dimension is not supported. Use the 'operations' property.", this);
    }
  ...
  private createMetricsForOperations(metricName: string, operations: Operation[],
    ...
    if (props?.dimensions?.Operation) {
      throw new ValidationError('InvalidPropertiesOperationDimension', 'Invalid properties. Operation dimension is not supported when calculating operational metrics', this);
    }
    ...
    for (const operation of operations) {
      ...
      if (firstChar === firstChar.toUpperCase()) {
        ...
        throw new ValidationError('MapperGeneratedIllegalOperation', `Mapper generated an illegal operation metric name: ${operationMetricName}. Must start with a lowercase letter`, this);
      }
...
export class Table extends TableBase {
  ...
  public static fromTableAttributes(scope: Construct, id: string, attrs: TableAttributes): ITable {
    ...
    if (!attrs.tableName) {
      if (!attrs.tableArn) {
        throw new ValidationError('OneTablenameTablearnRequired', 'One of tableName or tableArn is required!', scope);
      }

      arn = attrs.tableArn;
      const maybeTableName = stack.splitArn(attrs.tableArn, ArnFormat.SLASH_RESOURCE_NAME).resourceName;
      if (!maybeTableName) {
        throw new ValidationError('ArnDynamodbTableForm', 'ARN for DynamoDB table must be in the form: ...', scope);
      }
      name = maybeTableName;
    } else {
      if (attrs.tableArn) {
        throw new ValidationError('OneTablearnTablenameProvided', 'Only one of tableArn or tableName can be provided', scope);
      }
  ...
  constructor(scope: Construct, id: string, props: TableProps) {
    ...
    if (!props?.partitionKey) {
      throw new ValidationError('PartitionkeyRequiredTable', 'partitionKey is required for Table', this);
    }
    ...
    if (props.replicationRegions) {
      if (props.stream && props.stream !== StreamViewType.NEW_AND_OLD_IMAGES) {
        throw new ValidationError('StreamNewOldImages', '`stream` must be set to `NEW_AND_OLD_IMAGES` when specifying `replicationRegions`', this);
      }
  ...
  public autoScaleReadCapacity(props: EnableScalingProps): IScalableTableAttribute {
    if (this.tableScaling.scalableReadAttribute) {
      throw new ValidationError('ReadAutoscalingAlreadyEnabled', 'Read AutoScaling already enabled for this table', this);
    }
    if (this.billingMode === BillingMode.PAY_PER_REQUEST) {
      throw new ValidationError('AutoscalingAvailableTablesPay', 'AutoScaling is not available for tables with PAY_PER_REQUEST billing mode', this);
    }
  ...
  public autoScaleWriteCapacity(props: EnableScalingProps): IScalableTableAttribute {
    if (this.tableScaling.scalableWriteAttribute) {
      throw new ValidationError('WriteAutoscalingAlreadyEnabled', 'Write AutoScaling already enabled for this table', this);
    }
    if (this.billingMode === BillingMode.PAY_PER_REQUEST) {
      throw new ValidationError('AutoscalingAvailableTablesPay', 'AutoScaling is not available for tables with PAY_PER_REQUEST billing mode', this);
    }
  ...
  public autoScaleGlobalSecondaryIndexReadCapacity(indexName: string, props: EnableScalingProps): IScalableTableAttribute {
    ...
    if (!attributePair) {
      throw new ValidationError('GlobalSecondaryIndexName', `No global secondary index with name ${indexName}`, this);
    }
    if (attributePair.scalableReadAttribute) {
      throw new ValidationError('ReadAutoscalingAlreadyEnabled', 'Read AutoScaling already enabled for this index', this);
    }
  ...
  public autoScaleGlobalSecondaryIndexWriteCapacity(indexName: string, props: EnableScalingProps): IScalableTableAttribute {
    ...
    if (attributePair.scalableWriteAttribute) {
      throw new ValidationError('WriteAutoscalingAlreadyEnabled', 'Write AutoScaling already enabled for this index', this);
    }
  ...
  public schema(indexName?: string): SchemaOptions {
    ...
    if (!schema) {
      throw new ValidationError('FindSchemaIndexIndexname', `Cannot find schema for index: ${indexName}. Use 'addGlobalSecondaryIndex' or 'addLocalSecondaryIndex' to add index`, this);
    }

    if (schema.partitionKeys.length > 1 || schema.sortKeys.length > 1) {
      throw new ValidationError('IndexIndexnameUsesMulti', `Index ${indexName} uses multi-attribute keys and cannot be returned by schema(), use schemaV2() instead.`, this);
    }
  ...
  private validateProvisioning(props: { readCapacity?: number; writeCapacity?: number }): void {
    if (this.billingMode === BillingMode.PAY_PER_REQUEST) {
      if (props.readCapacity !== undefined || props.writeCapacity !== undefined) {
        throw new ValidationError('ProvisionReadWriteCapacity', 'you cannot provision read and write capacity for a table with PAY_PER_REQUEST billing mode', this);
      }
    }
  }
  ...
  private validateIndexName(indexName: string) {
    if (this.schemas.has(indexName)) {
      ...
      throw new ValidationError('DuplicateIndexNameIndexname', `a duplicate index name, ${indexName}, is not allowed`, this);
    }
  }
  ...
  private addKey(attribute: Attribute, keyType: string) {
    ...
    if (existingProp) {
      throw new ValidationError('UnableAttributeNameKeytype', `Unable to set ${attribute.name} as a ${keyType} key, because ${existingProp.attributeName} is a ${keyType} key`, this);
    }
  ...
  private registerAttribute(attribute: Attribute) {
    ...
    if (existingDef && existingDef.attributeType !== type) {
      throw new ValidationError('UnableSpecifyNameType', `Unable to specify ${name} as ${type} because it was already defined as ${existingDef.attributeType}`, this);
    }
    if (!existingDef) {
  ...
  private createReplicaTables(regions: string[], timeout?: Duration, waitForReplicationToFinish?: boolean, replicaRemovalPolicy?: RemovalPolicy) {
    ...
    if (!Token.isUnresolved(stack.region) && regions.includes(stack.region)) {
      throw new ValidationError('ReplicationregionsIncludeRegionWhere', '`replicationRegions` cannot include the region where this stack is deployed.', this);
    }
  ...
  private parseEncryption(props: TableProps): { sseSpecification: CfnTableProps['sseSpecification']; encryptionKey?: kms.IKey } {
    ...
    if (encryptionType != null && props.serverSideEncryption != null) {
      throw new ValidationError('OneEncryptionServersideencryptionSpecified', 'Only one of encryption and serverSideEncryption can be specified, but both were provided', this);
    }

    if (props.serverSideEncryption && props.encryptionKey) {
      throw new ValidationError('EncryptionkeySpecifiedServersideencryptionSpecified', 'encryptionKey cannot be specified when serverSideEncryption is specified. Use encryption instead', this);
    }
    ...
    if (encryptionType !== TableEncryption.CUSTOMER_MANAGED && props.encryptionKey) {
      throw new ValidationError('EncryptionkeySpecifiedUnlessEncryption', `encryptionKey cannot be specified unless encryption is set to TableEncryption.CUSTOMER_MANAGED (it was set to ${encryptionType})`, this);
    }

    if (encryptionType === TableEncryption.CUSTOMER_MANAGED && props.replicationRegions) {
      throw new ValidationError('TableencryptionCustomerManagedSupported', 'TableEncryption.CUSTOMER_MANAGED is not supported by DynamoDB Global Tables (where replicationRegions was set)', this);
    }
    ...
    switch (encryptionType) {
      ...
      default:
        throw new ValidationError('UnexpectedEncryptiontypeEncryptiontype', `Unexpected 'encryptionType': ${encryptionType}`, this);
    }
  }

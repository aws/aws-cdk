...
export class TableV2 extends TableBaseV2 {
  ...
  public static fromTableAttributes(scope: Construct, id: string, attrs: TableAttributesV2): ITableV2 {
    class Import extends TableBaseV2 {
      ...
      public constructor(tableArn: string, tableName: string, tableId?: string, tableStreamArn?: string, resourcePolicy?: PolicyDocument) {
        ...
        if (!resourceRegion) {
          throw new ValidationError('Mustbetableformarnpartitiondynamodbregionaccounttabletablename', 'Table ARN must be of the form: arn:<partition>:dynamodb:<region>:<account>:table/<table-name>', this);
        }
    ...
    if (!attrs.tableArn) {
      if (!attrs.tableName) {
        throw new ValidationError('MustBeLeastProvided', 'At least one of `tableArn` or `tableName` must be provided', scope);
      }
      ...
      if (attrs.tableName) {
        throw new ValidationError('OnlyProvided', 'Only one of `tableArn` or `tableName` can be provided, but not both', scope);
      }

      tableArn = attrs.tableArn;
      const resourceName = stack.splitArn(tableArn, ArnFormat.SLASH_RESOURCE_NAME).resourceName;
      if (!resourceName) {
        throw new ValidationError('Mustbetableformarnpartitiondynamodbregionaccounttabletablename', 'Table ARN must be of the form: arn:<partition>:dynamodb:<region>:<account>:table/<table-name>', scope);
      }
  ...
  public constructor(scope: Construct, id: string, props: TablePropsV2) {
    ...
    if (props.multiRegionConsistency === MultiRegionConsistency.STRONG) {
      ...
      throw new ValidationError('Witnessregioncannotspecified', 'Witness region cannot be specified for a Multi-Region Eventual Consistency (MREC) Global Table - Witness regions are only supported for Multi-Region Strong Consistency (MRSC) Global Tables.', this);
    }
  ...
  public replica(region: string): ITableV2 {
    if (Token.isUnresolved(this.stack.region)) {
      throw new ValidationError('Replicatablessupportedregion', 'Replica tables are not supported in a region agnostic stack', this);
    }

    if (Token.isUnresolved(region)) {
      throw new ValidationError('ProvidedCannotToken', 'Provided `region` cannot be a token', this);
    }
    ...
    if (!this.replicaTables.has(region)) {
      throw new ValidationError('Replicatableexistsregion', `No replica table exists in region ${region}`, this);
    }
  ...
  private configureIndexProjection(props: SecondaryIndexProps): CfnGlobalTable.ProjectionProperty {
    ...
    if (this.nonKeyAttributes.size > MAX_NON_KEY_ATTRIBUTES) {
      throw new ValidationError('MaximumNumberNonkeyattributesAcross', `The maximum number of 'nonKeyAttributes' across all secondary indexes is ${MAX_NON_KEY_ATTRIBUTES}`, this);
    }
  ...
  private addAttributeDefinition(attribute: Attribute) {
    ...
    if (existingAttributeDef && existingAttributeDef.attributeType !== type) {
      throw new ValidationError('Unabletounablespecifybecause', `Unable to specify ${name} as ${type} because it was already defined as ${existingAttributeDef.attributeType}`, this);
    }
  ...
  private validateIndexName(indexName: string) {
    if (this.globalSecondaryIndexes.has(indexName) || this.localSecondaryIndexes.has(indexName)) {
      throw new ValidationError('DuplicateDuplicateduplicatesecondaryindex', `Duplicate secondary index name, ${indexName}, is not allowed`, this);
    }
  }

  private validateIndexProjection(props: SecondaryIndexProps) {
    if (props.projectionType === ProjectionType.INCLUDE && !props.nonKeyAttributes) {
      throw new ValidationError('Shouldbenonkeyattributesshould', `Non-key attributes should be specified when using ${ProjectionType.INCLUDE} projection type`, this);
    }

    if (props.projectionType !== ProjectionType.INCLUDE && props.nonKeyAttributes) {
      throw new ValidationError('Nonkeyattributesshouldspecified', `Non-key attributes should not be specified when not using ${ProjectionType.INCLUDE} projection type`, this);
    }
  }
  ...
  private validateReplicaIndexOptions(options: { [indexName: string]: ReplicaGlobalSecondaryIndexOptions }) {
    for (const indexName of Object.keys(options)) {
      if (!this.globalSecondaryIndexes.has(indexName)) {
        throw new ValidationError('CannotCannotcannotconfigurereplica', `Cannot configure replica global secondary index, ${indexName}, because it is not defined on the primary table`, this);
      }

      const replicaGsiOptions = options[indexName];
      if (this.billingMode === BillingMode.PAY_PER_REQUEST && replicaGsiOptions.readCapacity) {
        throw new ValidationError('CannotCannotConfigureReadcapacity', `Cannot configure 'readCapacity' for replica global secondary index, ${indexName}, because billing mode is ${BillingMode.PAY_PER_REQUEST}`, this);
      }
    }
  }
  ...
  private validateReplica(props: ReplicaTableProps) {
    ...
    if (Token.isUnresolved(props.region)) {
      throw new ValidationError('Replicatableregiontoken', 'Replica table region must not be a token', this);
    }

    if (props.region === this.stack.region) {
      throw new ValidationError('CannotCannotreplicatablesame', `You cannot add a replica table in the same region as the primary table - the primary table region is ${this.region}`, this);
    }

    if (this.replicaTables.has(props.region)) {
      throw new ValidationError('DuplicateDuplicateduplicatereplicatable', `Duplicate replica table region, ${props.region}, is not allowed`, this);
    }

    if (this.billingMode === BillingMode.PAY_PER_REQUEST && props.readCapacity) {
      throw new ValidationError('CannotProvideReadcapacityReplica', `You cannot provide 'readCapacity' on a replica table when the billing mode is ${BillingMode.PAY_PER_REQUEST}`, this);
    }
  }
  ...
  private validateGlobalSecondaryIndex(props: GlobalSecondaryIndexPropsV2) {
    ...
    if (this.globalSecondaryIndexes.size === MAX_GSI_COUNT) {
      throw new ValidationError('Providemorethanglobal', `You may not provide more than ${MAX_GSI_COUNT} global secondary indexes`, this);
    }

    if (this.billingMode === BillingMode.PAY_PER_REQUEST && (props.readCapacity || props.writeCapacity)) {
      throw new ValidationError('CannotConfigureReadcapacityWritecapacity', `You cannot configure 'readCapacity' or 'writeCapacity' on a global secondary index when the billing mode is ${BillingMode.PAY_PER_REQUEST}`, this);
    }
  }
  ...
  private validateLocalSecondaryIndex(props: LocalSecondaryIndexProps) {
    ...
    if (!this.hasSortKey) {
      throw new ValidationError('Tablesortorderlocal', 'The table must have a sort key in order to add a local secondary index', this);
    }

    if (this.localSecondaryIndexes.size === MAX_LSI_COUNT) {
      throw new ValidationError('Providemorethanlocal', `You may not provide more than ${MAX_LSI_COUNT} local secondary indexes`, this);
    }
  }
  ...
  private validatePitr(props: TablePropsV2) {
    ...
    if (props.pointInTimeRecovery !== undefined && props.pointInTimeRecoverySpecification !== undefined) {
      throw new ValidationError('ValidationError', '`pointInTimeRecoverySpecification` and `pointInTimeRecovery` are set. Use `pointInTimeRecoverySpecification` only.', this);
    }

    if (!props.pointInTimeRecoverySpecification?.pointInTimeRecoveryEnabled && recoveryPeriodInDays) {
      throw new ValidationError('CannotCannotWhileFalse', 'Cannot set `recoveryPeriodInDays` while `pointInTimeRecoveryEnabled` is set to false.', this);
    }

    if (recoveryPeriodInDays !== undefined && (recoveryPeriodInDays < 1 || recoveryPeriodInDays > 35 )) {
      throw new ValidationError('MustBeValueBetween', '`recoveryPeriodInDays` must be a value between `1` and `35`.', this);
    }
  }
  ...
  private validateMrscConfiguration(props: TablePropsV2) {
    ...
    if (Token.isUnresolved(primaryRegion)) {
      throw new ValidationError('Mrscglobaltablesstrong', 'MRSC global tables with STRONG consistency are not supported in a region agnostic stack', this);
    }
    ...
    for (const region of allRegions) {
      if (Token.isUnresolved(region)) {
        throw new ValidationError('Mrscglobaltablesstrong', 'MRSC global tables with STRONG consistency do not support token-based regions', this);
      }
    }
    ...
    if (!regionSet || !regionSetName) {
      throw new ValidationError('PrimaryRegionSupportedMrsc', `Primary region '${primaryRegion}' is not supported for MRSC global tables with STRONG consistency. Supported regions: ${Object.values(regionSets).flat().join(', ')}`, this);
    }

    for (const region of allRegions) {
      if (!regionSet.includes(region)) {
        throw new ValidationError('RegionSameRegionPrimary', `Region '${region}' is not in the same region set (${regionSetName}) as the primary region '${primaryRegion}'. All regions must be within the same region set for MRSC global tables with STRONG consistency. Supported ${regionSetName} regions: ${regionSet.join(', ')}`, this);
      }
    }
    ...
    if (witnessRegion) {
      if (totalReplicas !== 2) {
        throw new ValidationError('MrscGlobalTableWitness', `MRSC global table with witness region must have exactly 2 replicas (including primary), but found ${totalReplicas}. Current configuration: primary region '${primaryRegion}', replica regions [${replicaRegions.join(', ')}], witness region '${witnessRegion}'`, this);
      }
    } else {
      if (totalReplicas !== 3) {
        throw new ValidationError('MrscGlobalTableWithout', `MRSC global table without witness region must have exactly 3 replicas (including primary), but found ${totalReplicas}. Current configuration: primary region '${primaryRegion}', replica regions [${replicaRegions.join(', ')}]`, this);
      }
    }
  }
...
export class TableV2MultiAccountReplica extends TableBaseV2 {
  ...
  public constructor(scope: Construct, id: string, props: TableV2MultiAccountReplicaProps = {}) {
    ...
    if (!props.replicaSourceTable) {
      throw new ValidationError('Isrequiredreplicasourcetablerequiredtablev2multiaccountreplica', 'replicaSourceTable is required for TableV2MultiAccountReplica', this);
    }

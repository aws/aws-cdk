...
test('scheduled scaling shows warning when minute is not defined in cron', () => {
  ...
  // THEN
  Annotations.fromStack(stack).hasWarning('/Default/MyTable/ReadScaling/Target', "AppAutoScaling:Schedule:DefaultRunEveryMinute: cron: If you don't pass 'minute', by default the event runs every minute. Pass 'minute: '*'' if that's what you intend, or 'minute: 0' to run once per hour instead.");
});

...
export class DataProtectionPolicy {
   ...
   * @internal
   */
  public _bind(_scope: Construct): any {
...
export class CustomDataIdentifier extends DataIdentifier {
  ...
  constructor(public readonly name: string, public readonly regex: string) {
    ...
    if (!name || name.trim().length === 0) {
      throw new Error('Custom data identifier name cannot be empty');
    }
    
    if (!regex || regex.trim().length === 0) {
      throw new Error('Custom data identifier regex cannot be empty');
    }
  }

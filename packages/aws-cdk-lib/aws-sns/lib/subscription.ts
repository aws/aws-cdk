...
export class Subscription extends Resource {
  ...
  constructor(scope: Construct, id: string, props: SubscriptionProps) {
    ...
    if (props.rawMessageDelivery &&
      ...
      throw new ValidationError(lit`RawMessageDeliveryEnabledFirehose`, 'Raw message delivery can only be enabled for HTTP, HTTPS, SQS, and Firehose subscriptions.', this);
    }

    if (props.filterPolicy) {
      if (Object.keys(props.filterPolicy).length > 5) {
        throw new ValidationError(lit`FilterPolicyMaximumAttributeNames`, 'A filter policy can have a maximum of 5 attribute names.', this);
      }
      ...
      if (total > 150) {
        throw new ValidationError(lit`TotalCombinationValues`, `The total combination of values (${total}) must not exceed 150.`, this);
      }
    ...
    if (props.protocol === SubscriptionProtocol.FIREHOSE && !props.subscriptionRoleArn) {
      throw new ValidationError(lit`SubscriptionRoleArnRequiredField`, 'Subscription role arn is required field for subscriptions with a firehose protocol.', this);
    }
  ...
  private renderDeliveryPolicy(deliveryPolicy: DeliveryPolicy, protocol: SubscriptionProtocol): any {
    if (![SubscriptionProtocol.HTTP, SubscriptionProtocol.HTTPS].includes(protocol)) {
      throw new ValidationError(lit`DeliveryPolicySupportedSubscriptions`, `Delivery policy is only supported for HTTP and HTTPS subscriptions, got: ${protocol}`, this);
    }
    ...
    if (healthyRetryPolicy) {
      ...
      if (minDelayTarget !== undefined) {
        if (minDelayTarget.toMilliseconds() % 1000 !== 0) {
          throw new ValidationError(lit`MinDelayTargetWholeNumber`, `minDelayTarget must be a whole number of seconds, got: ${minDelayTarget}`, this);
        }
        const minDelayTargetSecs = minDelayTarget.toSeconds();
        if (minDelayTargetSecs < 1 || minDelayTargetSecs > delayTargetLimitSecs) {
          throw new ValidationError(lit`MinDelayTarget`, `minDelayTarget must be between 1 and ${delayTargetLimitSecs} seconds inclusive, got: ${minDelayTargetSecs}s`, this);
        }
      }
      if (maxDelayTarget !== undefined) {
        if (maxDelayTarget.toMilliseconds() % 1000 !== 0) {
          throw new ValidationError(lit`MaxDelayTargetWholeNumber`, `maxDelayTarget must be a whole number of seconds, got: ${maxDelayTarget}`, this);
        }
        const maxDelayTargetSecs = maxDelayTarget.toSeconds();
        if (maxDelayTargetSecs < 1 || maxDelayTargetSecs > delayTargetLimitSecs) {
          throw new ValidationError(lit`MaxDelayTarget`, `maxDelayTarget must be between 1 and ${delayTargetLimitSecs} seconds inclusive, got: ${maxDelayTargetSecs}s`, this);
        }
        if ((minDelayTarget !== undefined) && minDelayTarget.toSeconds() > maxDelayTargetSecs) {
          throw new ValidationError(lit`MinDelayTargetExceedMax`, 'minDelayTarget must not exceed maxDelayTarget', this);
        }
      }
      ...
      if (healthyRetryPolicy.numRetries && (healthyRetryPolicy.numRetries < 0 || healthyRetryPolicy.numRetries > numRetriesLimit)) {
        throw new ValidationError(lit`NumRetries`, `numRetries must be between 0 and ${numRetriesLimit} inclusive, got: ${healthyRetryPolicy.numRetries}`, this);
      }
      ...
      if (numNoDelayRetries && (numNoDelayRetries < 0 || !Number.isInteger(numNoDelayRetries))) {
        throw new ValidationError(lit`NumDelayRetriesIntegerZero`, `numNoDelayRetries must be an integer zero or greater, got: ${numNoDelayRetries}`, this);
      }
      if (numMinDelayRetries && (numMinDelayRetries < 0 || !Number.isInteger(numMinDelayRetries))) {
        throw new ValidationError(lit`NumMinDelayRetriesInteger`, `numMinDelayRetries must be an integer zero or greater, got: ${numMinDelayRetries}`, this);
      }
      if (numMaxDelayRetries && (numMaxDelayRetries < 0 || !Number.isInteger(numMaxDelayRetries))) {
        throw new ValidationError(lit`NumMaxDelayRetriesInteger`, `numMaxDelayRetries must be an integer zero or greater, got: ${numMaxDelayRetries}`, this);
      }
    }
    if (throttlePolicy) {
      ...
      if (maxReceivesPerSecond !== undefined && (maxReceivesPerSecond < 1 || !Number.isInteger(maxReceivesPerSecond))) {
        throw new ValidationError(lit`MaxReceivesPerSecondInteger`, `maxReceivesPerSecond must be an integer greater than zero, got: ${maxReceivesPerSecond}`, this);
      }
    }
    return {
...
function buildFilterPolicyWithMessageBody(
  ...
  if (totalCombinationValues[0] > 150) {
    throw new ValidationError(lit`TotalCombinationValues`, `The total combination of values (${totalCombinationValues}) must not exceed 150.`, scope);
  }

...
export function validateProps(props: QueueProps) {
  ...
  validateRange(
    ...
    props.retentionPeriod && props.retentionPeriod.toSeconds(),
    60,
    1_209_600,
    'seconds'
  );
  ...
  validateRange(
    'visibility timeout',
    props.visibilityTimeout && props.visibilityTimeout.toSeconds(),
    0,
    43_200,
    'seconds'
  );
  validateRange(
    'dead letter target maximum receive count',
    props.deadLetterQueue && props.deadLetterQueue.maxReceiveCount,
    1,
    +Infinity
  );
}

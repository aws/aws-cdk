...
abstract class SecretBase extends Resource implements ISecret {
  ...
  public grantRead(grantee: iam.IGrantable, versionStages?: string[]): iam.Grant {
    ...
    if (this instanceof Secret && result.resourceStatement && (!this.encryptionKey && crossAccount === TokenComparison.DIFFERENT)) {
      throw new ValidationError('Mustbeprovidedcrossaccount', 'KMS Key must be provided for cross account access to Secret', this);
    }
...
export class Secret extends SecretBase {
  ...
  public static fromSecretAttributes(scope: Construct, id: string, attrs: SecretAttributes): ISecret {
    ...
    if (attrs.secretArn) {
      if (attrs.secretCompleteArn || attrs.secretPartialArn) {
        throw new ValidationError('CannotCannot', 'cannot use `secretArn` with `secretCompleteArn` or `secretPartialArn`', scope);
      }
  ...
  constructor(scope: Construct, id: string, props: SecretProps = {}) {
    ...
    if ((props.generateSecretString ? 1 : 0)
      ...
      > 1) {
      throw new ValidationError('CannotCannotSpecifyMore', 'Cannot specify more than one of `generateSecretString`, `secretStringValue`, `secretObjectValue`, and `secretStringBeta1`.', this);
    }
  ...
  public addReplicaRegion(region: string, encryptionKey?: kms.IKeyRef): void {
    ...
    if (!Token.isUnresolved(stack.region) && !Token.isUnresolved(region) && region === stack.region) {
      throw new ValidationError('CannotCannotcannotregionwhere', 'Cannot add the region where this stack is deployed as a replica region.', this);
    }
...
function parseSecretName(construct: IConstruct, secretArn: string) {
  ...
  throw new ValidationError('InvalidInvalidinvalidformatsecret', 'invalid ARN format; no secret name provided', construct);
}

...
export class DomainName extends Resource implements IDomainName {
  ...
  constructor(scope: Construct, id: string, props: DomainNameProps) {
    ...
    if (props.ownershipCertificate && !props.mtls) {
      throw new ValidationError('Ownershipcertificateonlyused', 'ownership certificate can only be used with mtls domains', scope);
    }
  ...
  private validateEndpointType(endpointType: string | undefined) : void {
    for (let config of this.domainNameConfigurations) {
      if (endpointType && endpointType == config.endpointType) {
        throw new ValidationError('Endpointtypealreadyexists', `an endpoint with type ${endpointType} already exists`, this);
      }
    }
  }

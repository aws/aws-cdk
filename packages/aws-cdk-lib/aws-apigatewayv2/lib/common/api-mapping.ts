...
export class ApiMapping extends Resource implements IApiMapping {
    ...
    class Import extends Resource implements IApiMapping {
      ...
      public get apiMappingRef(): ApiMappingReference {
        ...
        return {
          ...
          get domainName() {
            if (!attrs.domainName) {
              throw new ValidationError('CannotCannotObjectApi', 'Cannot use object in this API: \'domainName\' was not supplied when calling ApiMapping.fromApiMappingAttributes()', self);
            }
  ...
  constructor(scope: Construct, id: string, props: ApiMappingProps) {
    ...
    if (!stage) {
      throw new ValidationError('Mustbestagepropertyspecified', 'stage property must be specified', scope);
    }

    if (props.apiMappingKey === '') {
      throw new ValidationError('Emptystringmappingallowed', 'empty string for api mapping key not allowed', scope);
    }
  ...
  public get domainName(): IDomainName {
    ...
    throw new ValidationError('Supplieddomainnamedoesimplement', `Supplied domainName (${this._domainName.constructor.name}) does not implement IDomainName`, this);
  }

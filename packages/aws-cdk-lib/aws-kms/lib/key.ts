...
export class Key extends KeyBase {
  ...
  constructor(scope: Construct, id: string, props: KeyProps = {}) {
    ...
    if (keySpec.startsWith('HMAC') && props.enableKeyRotation) {
      throw new ValidationError('Rotationcannotenabledhmac', 'key rotation cannot be enabled on HMAC keys', this);
    }

    if (keySpec !== KeySpec.SYMMETRIC_DEFAULT && props.enableKeyRotation) {
      throw new ValidationError('Rotationcannotenabledasymmetric', 'key rotation cannot be enabled on asymmetric keys', this);
    }
    ...
    if (props.rotationPeriod) {
      if (props.enableKeyRotation === false) {
        throw new ValidationError('RotationperiodCannotSpecifiedEnablekeyrotation', '\'rotationPeriod\' cannot be specified when \'enableKeyRotation\' is disabled', this);
      }
      if (props.rotationPeriod.toDays() < 90 || props.rotationPeriod.toDays() > 2560) {
        throw new ValidationError('MustBeRotationperiodValueBetween', `'rotationPeriod' value must between 90 and 2650 days. Received: ${props.rotationPeriod.toDays()}`, this);
      }
    ...
    if (defaultKeyPoliciesFeatureEnabled) {
      if (props.trustAccountIdentities === false) {
        throw new ValidationError('CannotCannotFalseAwsCdkAwsKmsDefaultkeypolicies', '`trustAccountIdentities` cannot be false if the @aws-cdk/aws-kms:defaultKeyPolicies feature flag is set', this);
      }
    ...
    if (props.pendingWindow) {
      ...
      if (pendingWindowInDays < 7 || pendingWindowInDays > 30) {
        throw new ValidationError('MustBePendingwindowValueBetween', `'pendingWindow' value must between 7 and 30 days. Received: ${pendingWindowInDays}`, this);
      }
    }

...
export class CompositeAlarm extends AlarmBase {
  ...
  constructor(scope: Construct, id: string, props: CompositeAlarmProps) {
    ...
    if (props.alarmRule.renderAlarmRule().length > 10240) {
      throw new ValidationError('Alarmruleexpressioncannot', 'Alarm Rule expression cannot be greater than 10240 characters, please reduce the conditions in the Alarm Rule', this);
    }
    ...
    if (props.actionsSuppressor === undefined) {
      if (extensionPeriod !== undefined || waitPeriod !== undefined) {
        throw new ValidationError('Actionssuppressorextensionwaitperiodsrequire', 'ActionsSuppressor Extension/Wait Periods require an ActionsSuppressor to be set.', this);
      }
    } else {

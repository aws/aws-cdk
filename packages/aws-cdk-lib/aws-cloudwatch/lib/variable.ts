...
export abstract class Values {
  ...
  public static fromSearchComponents(components: SearchComponents): Values {
    if (components.dimensions.length === 0) {
      throw new UnscopedValidationError('EmptyDimensionsProvided', 'Empty dimensions provided. Please specify one dimension at least');
    }
    if (!components.dimensions.includes(components.populateFrom)) {
      throw new UnscopedValidationError('PopulatefromPresentDimensions', `populateFrom (${components.populateFrom}) is not present in dimensions`);
    }
...
export class DashboardVariable implements IVariable {
  public constructor(private readonly options: DashboardVariableOptions) {
    if (options.inputType !== VariableInputType.INPUT && !options.values) {
      throw new UnscopedValidationError('RequiresVariableInputtypeRequires', `Variable with inputType (${options.inputType}) requires values to be set`);
    }
    if (options.inputType == VariableInputType.INPUT && options.values) {
      throw new UnscopedValidationError('InputtypeInputCannotCombined', 'inputType INPUT cannot be combined with values. Please choose either SELECT or RADIO or remove \'values\' from options.');
    }
  }

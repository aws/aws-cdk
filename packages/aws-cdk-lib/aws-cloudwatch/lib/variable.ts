...
export abstract class Values {
  ...
  public static fromSearchComponents(components: SearchComponents): Values {
    if (components.dimensions.length === 0) {
      throw new UnscopedValidationError('EmptyDimensionsProvidedPlease', 'Empty dimensions provided. Please specify one dimension at least');
    }
    if (!components.dimensions.includes(components.populateFrom)) {
      throw new UnscopedValidationError('PopulatefromComponentsPopulatefromPresent', `populateFrom (${components.populateFrom}) is not present in dimensions`);
    }
  ...
  public static fromValues(...values: VariableValue[]): Values {
    if (values.length == 0) {
      throw new UnscopedValidationError('EmptyValuesAllowed', 'Empty values is not allowed');
    }
...
export class DashboardVariable implements IVariable {
  public constructor(private readonly options: DashboardVariableOptions) {
    if (options.inputType !== VariableInputType.INPUT && !options.values) {
      throw new UnscopedValidationError('VariableInputtypeOptionsInputtype', `Variable with inputType (${options.inputType}) requires values to be set`);
    }
    if (options.inputType == VariableInputType.INPUT && options.values) {
      throw new UnscopedValidationError('InputtypeInputCombinedValues', 'inputType INPUT cannot be combined with values. Please choose either SELECT or RADIO or remove \'values\' from options.');
    }
  }

...
export class DedicatedIpPool extends Resource implements IDedicatedIpPool {
  ...
  constructor(scope: Construct, id: string, props: DedicatedIpPoolProps = {}) {
    ...
    if (props.dedicatedIpPoolName && !/^[a-z0-9_-]{0,64}$/.test(props.dedicatedIpPoolName)) {
      throw new ValidationError('InvalidInvalidDedicatedippoolname', `Invalid dedicatedIpPoolName "${props.dedicatedIpPoolName}". The name must only include lowercase letters, numbers, underscores, hyphens, and must not exceed 64 characters.`, this);
    }

...
export class LustreFileSystem extends FileSystemBase {
  ...
  private validateDriveCacheType(deploymentType: LustreDeploymentType, storageType?: StorageType, driveCacheType?: DriveCacheType): void {
    ...
    if (deploymentType !== LustreDeploymentType.PERSISTENT_1 || storageType !== StorageType.HDD) {
      throw new ValidationError('Drivecachetypeonlypersistent1storage', `driveCacheType can only be set for PERSISTENT_1 HDD storage type, got: ${deploymentType} and ${storageType}`, this);
    }
  }
  ...
  private validateStorageType(deploymentType: LustreDeploymentType, storageType?: StorageType): void {
    ...
    if (storageType === StorageType.HDD && deploymentType !== LustreDeploymentType.PERSISTENT_1) {
      throw new ValidationError('Storagetypeonlysupported', `Storage type HDD is only supported for PERSISTENT_1 deployment type, got: ${deploymentType}`, this);
    }
  }
  ...
  private validateFiileSystemTypeVersion(deploymentType: LustreDeploymentType, fileSystemTypeVersion?: FileSystemTypeVersion): void {
    ...
    if (fileSystemTypeVersion === FileSystemTypeVersion.V_2_10) {
      if (!deploymentType.startsWith('SCRATCH') && deploymentType !== LustreDeploymentType.PERSISTENT_1) {
        throw new ValidationError('Filesystemtypeversionv210onlysupported', 'fileSystemTypeVersion V_2_10 is only supported for SCRATCH and PERSISTENT_1 deployment types', this);
      }
    }
  ...
  private validateAutoImportPolicy(deploymentType: LustreDeploymentType, importPath?: string, autoImportPolicy?: LustreAutoImportPolicy): void {
    ...
    if (importPath === undefined) {
      throw new ValidationError('RequiresRequiresautoimportpolicyrequiresimportpath', 'autoImportPolicy requires importPath to be defined', this);
    }
    if (deploymentType === LustreDeploymentType.PERSISTENT_2) {
      throw new ValidationError('Autoimportpolicysupportedpersistent2deployments', 'autoImportPolicy is not supported with PERSISTENT_2 deployments', this);
    }
  }
  ...
  private validateExportPath(exportPath?: string, importPath?: string): void {
    ...
    if (importPath === undefined) {
      throw new ValidationError('CannotCannotcannotdefineexport', 'Cannot define an export path without also defining an import path', this);
    }
    ...
    if (!exportPath.startsWith(importPath)) {
      throw new ValidationError('ExportPathInvalid', `The export path "${exportPath}" is invalid. Expecting the format: s3://{IMPORT_PATH}/optional-prefix`, this);
    }
    if (exportPath.length > 900) {
      throw new ValidationError('ExportPathExceedsMaximum', `The export path "${exportPath}" exceeds the maximum length of 900 characters`, this);
    }
  }
  ...
  private validateImportedFileChunkSize(importedFileChunkSize?: number): void {
    ...
    if (importedFileChunkSize < 1 || importedFileChunkSize > 512000) {
      throw new ValidationError('Importedfilechunksizecannot', `importedFileChunkSize cannot be ${importedFileChunkSize} MiB. It must be a value from 1 to 512, 000 MiB`, this);
    }
  }
  ...
  private validateImportPath(importPath?: string): void {
    ...
    if (importPath.search(regexp) === -1) {
      throw new ValidationError('ImportPathInvalid', `The import path "${importPath}" is invalid. Expecting the format: s3://{BUCKET_NAME}/optional-prefix`, this);
    }
    if (importPath.length > 900) {
      throw new ValidationError('ImportPathExceedsMaximum', `The import path "${importPath}" exceeds the maximum length of 900 characters`, this);
    }
  }
  ...
  private validatePerUnitStorageThroughput(
    ...
    if (deploymentType === LustreDeploymentType.PERSISTENT_1) {
      if (storageType === StorageType.HDD && ![12, 40].includes(perUnitStorageThroughput)) {
        throw new ValidationError('Mustbeperunitstoragethroughputmbstibpersistent1', `perUnitStorageThroughput must be 12 or 40 MB/s/TiB for PERSISTENT_1 HDD storage, got: ${perUnitStorageThroughput}`, this);
      }
    ...
    if (deploymentType === LustreDeploymentType.PERSISTENT_2) {
      if (![125, 250, 500, 1000].includes(perUnitStorageThroughput)) {
        throw new ValidationError('Mustbeperunitstoragethroughput', `perUnitStorageThroughput must be 125, 250, 500 or 1000 MB/s/TiB for PERSISTENT_2 deployment type, got: ${perUnitStorageThroughput}`, this);
      }
    }
  }
  ...
  private validateStorageCapacity(
    ...
    if (deploymentType === LustreDeploymentType.SCRATCH_1) {
      if (![1200, 2400, 3600].includes(storageCapacity) && storageCapacity % 3600 !== 0) {
        throw new ValidationError('Mustbestoragecapacity', `storageCapacity must be 1, 200, 2,400, 3,600, or a multiple of 3,600 for SCRATCH_1 deployment type, got ${storageCapacity}.`, this);
      }
    }
    ...
    if (
      ...
      if (![1200, 2400].includes(storageCapacity) && storageCapacity % 2400 !== 0) {
        throw new ValidationError('Mustbestoragecapacity', `storageCapacity must be 1, 200, 2,400, or a multiple of 2,400 for SCRATCH_2 and PERSISTENT_2 deployment types, got ${storageCapacity}`, this);
      }
    }
    ...
    if (deploymentType === LustreDeploymentType.PERSISTENT_1) {
      if (storageType === StorageType.HDD) {
        if (perUnitStorageThroughput === 12 && storageCapacity % 6000 !== 0) {
          throw new ValidationError('Mustbestoragecapacitymultiple', `storageCapacity must be a multiple of 6, 000 for PERSISTENT_1 HDD storage with 12 MB/s/TiB throughput, got ${storageCapacity}`, this);
        }
        if (perUnitStorageThroughput === 40 && storageCapacity % 1800 !== 0) {
          throw new ValidationError('Mustbestoragecapacitymultiple', `storageCapacity must be a multiple of 1, 800 for PERSISTENT_1 HDD storage with 40 MB/s/TiB throughput, got ${storageCapacity}`, this);
        }
      } else {
        if (![1200, 2400].includes(storageCapacity) && storageCapacity % 2400 !== 0) {
          throw new ValidationError('Mustbestoragecapacity', `storageCapacity must be 1, 200, 2,400, or a multiple of 2,400 for PERSISTENT_1 SSD storage, got ${storageCapacity}`, this);
        }
      }
    }
  }
  ...
  private validateAutomaticBackupRetention(deploymentType: LustreDeploymentType, automaticBackupRetention?: Duration): void {
    if (automaticBackupRetention) {
      ...
      if (automaticBackupRetentionDays > 90) {
        throw new ValidationError('Mustbeautomaticbackupretentionperiodbetween', `automaticBackupRetention period must be between 0 and 90 days. received: ${automaticBackupRetentionDays}`, this);
      }
    }
  }
  ...
  private validateDailyAutomaticBackupStartTime(automaticBackupRetention?: Duration,
    ...
    if (dailyAutomaticBackupStartTime && automaticBackupDisabled) {
      throw new ValidationError('Mustbeautomaticbackupretentionperiodnonzero', 'automaticBackupRetention period must be set a non-zero day when dailyAutomaticBackupStartTime is set', this);
    }
  }
}

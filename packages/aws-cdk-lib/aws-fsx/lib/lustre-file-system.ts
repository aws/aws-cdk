...
export class LustreFileSystem extends FileSystemBase {
  ...
  private validateDriveCacheType(deploymentType: LustreDeploymentType, storageType?: StorageType, driveCacheType?: DriveCacheType): void {
    ...
    if (deploymentType !== LustreDeploymentType.PERSISTENT_1 || storageType !== StorageType.HDD) {
      throw new ValidationError('DrivecachetypePersistentHddStorage', `driveCacheType can only be set for PERSISTENT_1 HDD storage type, got: ${deploymentType} and ${storageType}`, this);
    }
  }
  ...
  private validateStorageType(deploymentType: LustreDeploymentType, storageType?: StorageType): void {
    ...
    if (storageType === StorageType.HDD && deploymentType !== LustreDeploymentType.PERSISTENT_1) {
      throw new ValidationError('StorageTypeHddSupported', `Storage type HDD is only supported for PERSISTENT_1 deployment type, got: ${deploymentType}`, this);
    }
  }
  ...
  private validateFiileSystemTypeVersion(deploymentType: LustreDeploymentType, fileSystemTypeVersion?: FileSystemTypeVersion): void {
    ...
    if (fileSystemTypeVersion === FileSystemTypeVersion.V_2_10) {
      if (!deploymentType.startsWith('SCRATCH') && deploymentType !== LustreDeploymentType.PERSISTENT_1) {
        throw new ValidationError('FilesystemtypeversionSupportedScratchPersistent', 'fileSystemTypeVersion V_2_10 is only supported for SCRATCH and PERSISTENT_1 deployment types', this);
      }
    }
  ...
  private validateAutoImportPolicy(deploymentType: LustreDeploymentType, importPath?: string, autoImportPolicy?: LustreAutoImportPolicy): void {
    ...
    if (importPath === undefined) {
      throw new ValidationError('AutoimportpolicyRequiresImportpathDefined', 'autoImportPolicy requires importPath to be defined', this);
    }
    if (deploymentType === LustreDeploymentType.PERSISTENT_2) {
      throw new ValidationError('AutoimportpolicySupportedPersistentDeployments', 'autoImportPolicy is not supported with PERSISTENT_2 deployments', this);
    }
  }
  ...
  private validateExportPath(exportPath?: string, importPath?: string): void {
    ...
    if (importPath === undefined) {
      throw new ValidationError('DefineExportPathWithout', 'Cannot define an export path without also defining an import path', this);
    }
    ...
    if (Token.isUnresolved(importPath) !== Token.isUnresolved(exportPath)) {
      throw new ValidationError('ImportpathExportpathEachTokens', 'The importPath and exportPath must each be Tokens or not Tokens, you cannot use a mix', this);
    }
    if (!exportPath.startsWith(importPath)) {
      throw new ValidationError('ExportPathExportpathInvalid', `The export path "${exportPath}" is invalid. Expecting the format: s3://{IMPORT_PATH}/optional-prefix`, this);
    }
    if (exportPath.length > 900) {
      throw new ValidationError('ExportPathExportpathExceeds', `The export path "${exportPath}" exceeds the maximum length of 900 characters`, this);
    }
  }
  ...
  private validateImportedFileChunkSize(importedFileChunkSize?: number): void {
    ...
    if (importedFileChunkSize < 1 || importedFileChunkSize > 512000) {
      throw new ValidationError('ImportedfilechunksizeImportedfilechunksizeMibValue', `importedFileChunkSize cannot be ${importedFileChunkSize} MiB. It must be a value from 1 to 512,000 MiB`, this);
    }
  }
  ...
  private validateImportPath(importPath?: string): void {
    ...
    if (importPath.search(regexp) === -1) {
      throw new ValidationError('ImportPathImportpathInvalid', `The import path "${importPath}" is invalid. Expecting the format: s3://{BUCKET_NAME}/optional-prefix`, this);
    }
    if (importPath.length > 900) {
      throw new ValidationError('ImportPathImportpathExceeds', `The import path "${importPath}" exceeds the maximum length of 900 characters`, this);
    }
  }
  ...
  private validatePerUnitStorageThroughput(
    ...
    if (deploymentType !== LustreDeploymentType.PERSISTENT_1 && deploymentType !== LustreDeploymentType.PERSISTENT_2) {
      throw new ValidationError('PerunitstoragethroughputPersistentPersistentDeployment', 'perUnitStorageThroughput can only be set for the PERSISTENT_1/PERSISTENT_2 deployment types, received: ' + deploymentType, this);
    }
    ...
    if (deploymentType === LustreDeploymentType.PERSISTENT_1) {
      if (storageType === StorageType.HDD && ![12, 40].includes(perUnitStorageThroughput)) {
        throw new ValidationError('PerunitstoragethroughputTibPersistentHdd', `perUnitStorageThroughput must be 12 or 40 MB/s/TiB for PERSISTENT_1 HDD storage, got: ${perUnitStorageThroughput}`, this);
      }
      if ((storageType === undefined || storageType === StorageType.SSD) && ![50, 100, 200].includes(perUnitStorageThroughput)) {
        throw new ValidationError('Perunitstoragethroughput100200Tib', 'perUnitStorageThroughput must be 50, 100, or 200 MB/s/TiB for PERSISTENT_1 SSD storage, got: ' + perUnitStorageThroughput, this);
      }
    }

    if (deploymentType === LustreDeploymentType.PERSISTENT_2) {
      if (![125, 250, 500, 1000].includes(perUnitStorageThroughput)) {
        throw new ValidationError('Perunitstoragethroughput125250500', `perUnitStorageThroughput must be 125, 250, 500 or 1000 MB/s/TiB for PERSISTENT_2 deployment type, got: ${perUnitStorageThroughput}`, this);
      }
    }
  }
  ...
  private validateStorageCapacity(
    ...
    if (deploymentType === LustreDeploymentType.SCRATCH_1) {
      if (![1200, 2400, 3600].includes(storageCapacity) && storageCapacity % 3600 !== 0) {
        throw new ValidationError('Storagecapacity200400600', `storageCapacity must be 1,200, 2,400, 3,600, or a multiple of 3,600 for SCRATCH_1 deployment type, got ${storageCapacity}.`, this);
      }
    }
    ...
    if (
      ...
      if (![1200, 2400].includes(storageCapacity) && storageCapacity % 2400 !== 0) {
        throw new ValidationError('Storagecapacity200400Multiple', `storageCapacity must be 1,200, 2,400, or a multiple of 2,400 for SCRATCH_2 and PERSISTENT_2 deployment types, got ${storageCapacity}`, this);
      }
    }
    ...
    if (deploymentType === LustreDeploymentType.PERSISTENT_1) {
      if (storageType === StorageType.HDD) {
        if (perUnitStorageThroughput === 12 && storageCapacity % 6000 !== 0) {
          throw new ValidationError('StoragecapacityMultiple000Persistent', `storageCapacity must be a multiple of 6,000 for PERSISTENT_1 HDD storage with 12 MB/s/TiB throughput, got ${storageCapacity}`, this);
        }
        if (perUnitStorageThroughput === 40 && storageCapacity % 1800 !== 0) {
          throw new ValidationError('StoragecapacityMultiple800Persistent', `storageCapacity must be a multiple of 1,800 for PERSISTENT_1 HDD storage with 40 MB/s/TiB throughput, got ${storageCapacity}`, this);
        }
      } else {
        if (![1200, 2400].includes(storageCapacity) && storageCapacity % 2400 !== 0) {
          throw new ValidationError('Storagecapacity200400Multiple', `storageCapacity must be 1,200, 2,400, or a multiple of 2,400 for PERSISTENT_1 SSD storage, got ${storageCapacity}`, this);
        }
      }
    }
  }
  ...
  private validateAutomaticBackupRetention(deploymentType: LustreDeploymentType, automaticBackupRetention?: Duration): void {
    if (automaticBackupRetention) {
      ...
      if ([LustreDeploymentType.SCRATCH_1, LustreDeploymentType.SCRATCH_2].includes(deploymentType) && automaticBackupRetentionDays > 0) {
        throw new ValidationError('AutomaticBackupsSupportedScratch', 'automatic backups is not supported on scratch file systems', this);
      }

      if (automaticBackupRetentionDays > 90) {
        throw new ValidationError('AutomaticbackupretentionPeriodDaysReceived', `automaticBackupRetention period must be between 0 and 90 days. received: ${automaticBackupRetentionDays}`, this);
      }
    }
  }
  ...
  private validateDailyAutomaticBackupStartTime(automaticBackupRetention?: Duration,
    ...
    if (dailyAutomaticBackupStartTime && automaticBackupDisabled) {
      throw new ValidationError('AutomaticbackupretentionPeriodNonZero', 'automaticBackupRetention period must be set a non-zero day when dailyAutomaticBackupStartTime is set', this);
    }
  }
}

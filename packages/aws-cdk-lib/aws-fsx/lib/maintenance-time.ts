...
export class LustreMaintenanceTime {
  ...
  private validate(hour: number, minute: number) {
    if (!Number.isInteger(hour) || hour < 0 || hour > 23) {
      throw new UnscopedValidationError('MustBeMaintenanceTimeHour', 'Maintenance time hour must be an integer between 0 and 23');
    }
    if (!Number.isInteger(minute) || minute < 0 || minute > 59) {
      throw new UnscopedValidationError('MustBeMaintenanceTimeMinute', 'Maintenance time minute must be an integer between 0 and 59');
    }
  }
}

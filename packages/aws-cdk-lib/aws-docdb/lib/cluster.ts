...
export class DatabaseCluster extends DatabaseClusterBase {
    ...
    class Import extends DatabaseClusterBase implements IDatabaseCluster {
      ...
      public get instanceIdentifiers(): string[] {
        if (!this._instanceIdentifiers) {
          throw new UnscopedValidationError('CannotCannotAccessImported', 'Cannot access `instanceIdentifiers` of an imported cluster without provided instanceIdentifiers');
        }
      ...
      public get clusterEndpoint(): Endpoint {
        if (!this._clusterEndpoint) {
          throw new UnscopedValidationError('CannotCannotAccessImported', 'Cannot access `clusterEndpoint` of an imported cluster without an endpoint address and port');
        }
      ...
      public get clusterReadEndpoint(): Endpoint {
        if (!this._clusterReadEndpoint) {
          throw new UnscopedValidationError('CannotCannotAccessImported', 'Cannot access `clusterReadEndpoint` of an imported cluster without a readerEndpointAddress and port');
        }
      ...
      public get instanceEndpoints(): Endpoint[] {
        if (!this._instanceEndpoints) {
          throw new UnscopedValidationError('CannotCannotAccessImported', 'Cannot access `instanceEndpoints` of an imported cluster without instanceEndpointAddresses and port');
        }
      ...
      public get securityGroupId(): string {
        if (!this._securityGroupId) {
          throw new UnscopedValidationError('CannotCannotAccessImported', 'Cannot access `securityGroupId` of an imported cluster without securityGroupId');
        }
  ...
  constructor(scope: Construct, id: string, props: DatabaseClusterProps) {
    ...
    if (!props.instanceType && !props.serverlessV2ScalingConfiguration) {
      throw new ValidationError('Mustbeeitherinstancetypefor', 'Either instanceType (for provisioned clusters) or serverlessV2ScalingConfiguration (for serverless clusters) must be specified', this);
    }
    const isServerless = !!props.serverlessV2ScalingConfiguration;
    if (isServerless && props.instanceType) {
      throw new ValidationError('CannotCannotcannotspecifyboth', 'Cannot specify both instanceType and serverlessV2ScalingConfiguration', this);
    }
    ...
    if (subnetIds.length < 2) {
      throw new ValidationError('RequiresRequiresclusterrequiresleast', `Cluster requires at least 2 subnets, got ${subnetIds.length}`, this);
    }
    ...
    if (props.kmsKey && !storageEncrypted) {
      throw new ValidationError('Suppliedstorageencryptedfalse', 'KMS key supplied but storageEncrypted is false', this);
    }
    ...
    if (props.engineVersion !== undefined && !validEngineVersionRegex.test(props.engineVersion)) {
      throw new ValidationError('InvalidInvalidEngineVersion', `Invalid engine version: '${props.engineVersion}'. Engine version must be in the format x.y.z`, this);
    }
    ...
    if (
      ...
      throw new ValidationError('IOOptimizedStorageSupportedStarting', `I/O-optimized storage is supported starting with engine version 5.0.0, got '${props.engineVersion}'`, this);
    }
    ...
    if (isServerless && props.engineVersion !== undefined && Number(props.engineVersion.split('.')[0]) < MIN_ENGINE_VERSION_FOR_SERVERLESS) {
      throw new ValidationError('RequiresDocumentdbServerlessRequires', `DocumentDB serverless requires engine version 5.0.0 or higher, got '${props.engineVersion}'`, this);
    }
    ...
    if (!isServerless) {
      ...
      if (instanceCount < 1) {
        throw new ValidationError('Isrequiredleastinstancerequired', 'At least one instance is required for provisioned clusters', this);
      }
  ...
  private getInstanceRemovalPolicy(props: DatabaseClusterProps) {
    if (props.instanceRemovalPolicy === RemovalPolicy.SNAPSHOT) {
      throw new ValidationError('Awsdocdbdbinstancedoessupportsnapshot', 'AWS::DocDB::DBInstance does not support the SNAPSHOT removal policy', this);
    }
  ...
  private getSecurityGroupRemovalPolicy(props: DatabaseClusterProps) {
    if (props.securityGroupRemovalPolicy === RemovalPolicy.SNAPSHOT) {
      throw new ValidationError('Awsec2securitygroupdoessupportsnapshot', 'AWS::EC2::SecurityGroup does not support the SNAPSHOT removal policy', this);
    }

...
export class DatabaseCluster extends DatabaseClusterBase {
    ...
    class Import extends DatabaseClusterBase implements IDatabaseCluster {
      ...
      public get instanceIdentifiers(): string[] {
        if (!this._instanceIdentifiers) {
          throw new UnscopedValidationError('AccessInstanceidentifiersImportedCluster', 'Cannot access `instanceIdentifiers` of an imported cluster without provided instanceIdentifiers');
        }
      ...
      public get clusterEndpoint(): Endpoint {
        if (!this._clusterEndpoint) {
          throw new UnscopedValidationError('AccessClusterendpointImportedCluster', 'Cannot access `clusterEndpoint` of an imported cluster without an endpoint address and port');
        }
      ...
      public get clusterReadEndpoint(): Endpoint {
        if (!this._clusterReadEndpoint) {
          throw new UnscopedValidationError('AccessClusterreadendpointImportedCluster', 'Cannot access `clusterReadEndpoint` of an imported cluster without a readerEndpointAddress and port');
        }
      ...
      public get instanceEndpoints(): Endpoint[] {
        if (!this._instanceEndpoints) {
          throw new UnscopedValidationError('AccessInstanceendpointsImportedCluster', 'Cannot access `instanceEndpoints` of an imported cluster without instanceEndpointAddresses and port');
        }
      ...
      public get securityGroupId(): string {
        if (!this._securityGroupId) {
          throw new UnscopedValidationError('AccessSecuritygroupidImportedCluster', 'Cannot access `securityGroupId` of an imported cluster without securityGroupId');
        }
  ...
  constructor(scope: Construct, id: string, props: DatabaseClusterProps) {
    ...
    if (!props.instanceType && !props.serverlessV2ScalingConfiguration) {
      throw new ValidationError('EitherInstancetypeProvisionedClusters', 'Either instanceType (for provisioned clusters) or serverlessV2ScalingConfiguration (for serverless clusters) must be specified', this);
    }
    const isServerless = !!props.serverlessV2ScalingConfiguration;
    if (isServerless && props.instanceType) {
      throw new ValidationError('SpecifyInstancetypeServerlessv2scalingconfiguration', 'Cannot specify both instanceType and serverlessV2ScalingConfiguration', this);
    }
    ...
    if (props.engineVersion !== undefined && !validEngineVersionRegex.test(props.engineVersion)) {
      throw new ValidationError('InvalidEngineVersionProps', `Invalid engine version: '${props.engineVersion}'. Engine version must be in the format x.y.z`, this);
    }
    ...
    if (
      ...
      throw new ValidationError('OptimizedStorageSupportedStarting', `I/O-optimized storage is supported starting with engine version 5.0.0, got '${props.engineVersion}'`, this);
    }
    ...
    if (isServerless && props.engineVersion !== undefined && Number(props.engineVersion.split('.')[0]) < MIN_ENGINE_VERSION_FOR_SERVERLESS) {
      throw new ValidationError('DocumentdbServerlessRequiresEngine', `DocumentDB serverless requires engine version 5.0.0 or higher, got '${props.engineVersion}'`, this);
    }
    ...
    if (!isServerless) {
      ...
      if (instanceCount < 1) {
        throw new ValidationError('LeastOneInstanceRequired', 'At least one instance is required for provisioned clusters', this);
      }
  ...
  private getInstanceRemovalPolicy(props: DatabaseClusterProps) {
    if (props.instanceRemovalPolicy === RemovalPolicy.SNAPSHOT) {
      throw new ValidationError('AwsDocdbDbinstanceSupport', 'AWS::DocDB::DBInstance does not support the SNAPSHOT removal policy', this);
    }
  ...
  private getSecurityGroupRemovalPolicy(props: DatabaseClusterProps) {
    if (props.securityGroupRemovalPolicy === RemovalPolicy.SNAPSHOT) {
      throw new ValidationError('AwsEc2SecuritygroupSupport', 'AWS::EC2::SecurityGroup does not support the SNAPSHOT removal policy', this);
    }
  ...
  public addRotationSingleUser(automaticallyAfter?: Duration): secretsmanager.SecretRotation {
    if (!this.secret) {
      throw new ValidationError('AddSingleUserRotation', 'Cannot add single user rotation for a cluster without secret.', this);
    }
  ...
  public addRotationMultiUser(id: string, options: RotationMultiUserOptions): secretsmanager.SecretRotation {
    if (!this.secret) {
      throw new ValidationError('AddMultiUserRotation', 'Cannot add multi user rotation for a cluster without secret.', this);
    }
    return new secretsmanager.SecretRotation(this, id, {

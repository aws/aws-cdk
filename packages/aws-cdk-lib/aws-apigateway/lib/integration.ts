...
export class Integration {
  constructor(private readonly props: IntegrationProps) {
    ...
    if (options.connectionType === ConnectionType.VPC_LINK && options.vpcLink === undefined && options.vpcLinkV2 === undefined) {
      throw new UnscopedValidationError('VpcLinkRequired', '\'connectionType\' of VPC_LINK requires \'vpcLink\' or \'vpcLinkV2\' prop to be set');
    }

    if (options.connectionType === ConnectionType.INTERNET && (options.vpcLink !== undefined || options.vpcLinkV2 !== undefined)) {
      throw new UnscopedValidationError('CannotSetVpcLinkConnection', 'cannot set \'vpcLink\' or \'vpcLinkV2\' where \'connectionType\' is INTERNET');
    }

    if (options.vpcLink !== undefined && options.vpcLinkV2 !== undefined) {
      throw new UnscopedValidationError('VpcLinkMutuallyExclusive', '\'vpcLink\' and \'vpcLinkV2\' are mutually exclusive');
    }

    if (options.integrationTarget !== undefined && options.vpcLinkV2 === undefined) {
      throw new UnscopedValidationError('IntegrationTargetRequiresVpcLinkV2', '\'integrationTarget\' requires \'vpcLinkV2\' to be set');
    }

...
describe('ec2 task definition', () => {
  describe('When creating an ECS TaskDefinition', () => {
    ...
    test('warns when setting containers from ECR repository using fromRegistry method', () => {
      ...
      // THEN
      Annotations.fromStack(stack).hasWarning('/Default/Ec2TaskDef/web', "ECS:RepositoryImage:ECRImageRequiresPolicy: Proper policies need to be attached before pulling from ECR repository, or use 'fromEcrRepository'.");
    });

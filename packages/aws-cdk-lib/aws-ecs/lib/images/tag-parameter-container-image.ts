    ...
    return cdk.Lazy.string({
      produce: () => {
        if (this.imageTagParameter) {
          ...
          throw new cdk.UnscopedValidationError('TagParameterContainerImage must be used in a container definition when using tagParameterName', 'TagParameterNotBound');
        }
      },
    });
  }
    ...
    return cdk.Lazy.string({
      produce: () => {
        if (this.imageTagParameter) {
          ...
          throw new cdk.UnscopedValidationError('TagParameterContainerImage must be used in a container definition when using tagParameterValue', 'TagParameterNotBound');
        }
      },
    });
  }

...
export abstract class BaseService extends Resource
  ...
  public static fromServiceArnWithCluster(scope: Construct, id: string, serviceArn: string): IBaseService {
    ...
    if (Token.isUnresolved(resourceName)) {
      ...
      if (resourceNameParts.length !== 2) {
        throw new ValidationError('ResourceNameResourcenameService', `resource name ${resourceName} from service ARN: ${serviceArn} is not using the ARN cluster format`, scope);
      }
  ...
  constructor(
    ...
    if (props.propagateTags && props.propagateTaskTagsFrom) {
      throw new ValidationError('SpecifyEitherPropagatetagsPropagatetasktagsfrom', 'You can only specify either propagateTags or propagateTaskTagsFrom. Alternatively, you can leave both blank', this);
    }
    ...
    if (props.deploymentAlarms && !this.isEcsDeploymentController) {
      throw new ValidationError('DeploymentAlarmsRequiresEcs', 'Deployment alarms requires the ECS deployment controller.', this);
    }
    ...
    if (
      ...
      throw new ValidationError('CodeDeployDeploymentcontrollerUsed', 'CODE_DEPLOY deploymentController can only be used with the `latest` task definition revision', this);
    }
    ...
    if (props.deploymentAlarms) {
      if (props.deploymentAlarms.alarmNames.length === 0) {
        throw new ValidationError('LeastOneAlarmName', 'at least one alarm name is required when specifying deploymentAlarms, received empty array', this);
      }
    ...
    if (props.lifecycleHooks) {
      if (this.isEcsDeploymentController) {
        ...
        throw new ValidationError('DeploymentLifecycleHooksRequires', 'Deployment lifecycle hooks requires the ECS deployment controller.', this);
      }
    }
  ...
  private renderVolumes(): CfnService.ServiceVolumeConfigurationProperty[] {
    if (this.volumes.length > 1) {
      throw new ValidationError('OneEbsVolumeSpecified', `Only one EBS volume can be specified for 'volumeConfigurations', got: ${this.volumes.length}`, this);
    }
  ...
  public enableDeploymentAlarms(alarmNames: string[], options?: DeploymentAlarmOptions) {
    if (alarmNames.length === 0) {
      throw new ValidationError('LeastOneAlarmName', 'at least one alarm name is required when calling enableDeploymentAlarms(), received empty array', this);
    }
  ...
  public enableServiceConnect(config?: ServiceConnectProps) {
    if (this._serviceConnectConfig) {
      throw new ValidationError('ServiceConnectConfigurationSpecified', 'Service connect configuration cannot be specified more than once.', this);
    }
    ...
    const services = cfg.services?.map(svc => {
      ...
      if (!containerPort) {
        throw new ValidationError('PortMappingNameSvc', `Port mapping with name ${svc.portMappingName} does not exist.`, this);
      }
      const alias = {
  ...
  private validateServiceConnectConfiguration(config?: ServiceConnectProps) {
    if (!this.taskDefinition.defaultContainer) {
      throw new ValidationError('TaskDefinitionLeastOne', 'Task definition must have at least one container to enable service connect.', this);
    }
    ...
    if ((!config || !config.namespace) && !this.cluster.defaultCloudMapNamespace) {
      throw new ValidationError('NamespaceDefinedEitherServiceconnectconfig', 'Namespace must be defined either in serviceConnectConfig or cluster.defaultCloudMapNamespace', this);
    }
    ...
    config.services.forEach(serviceConnectService => {
      ...
      if (!this.taskDefinition.findPortMappingByName(serviceConnectService.portMappingName)) {
        throw new ValidationError('PortMappingServiceconnectservicePortmappingname', `Port Mapping '${serviceConnectService.portMappingName}' does not exist on the task definition.`, this);
      }
      ...
      if (portNames.get(serviceConnectService.portMappingName)?.includes(discoveryName)) {
        throw new ValidationError('CreateMultipleServicesDiscoveryname', `Cannot create multiple services with the discoveryName '${discoveryName}'.`, this);
      }
      ...
      if (serviceConnectService.ingressPortOverride && !this.isValidPort(serviceConnectService.ingressPortOverride)) {
        throw new ValidationError('IngressportoverrideServiceconnectserviceIngressportoverrideValid', `ingressPortOverride ${serviceConnectService.ingressPortOverride} is not valid.`, this);
      }
      ...
      if (serviceConnectService.port &&
        ...
        throw new ValidationError('ClientAliasPortServiceconnectservice', `Client Alias port ${serviceConnectService.port} is not valid.`, this);
      }
      ...
      if (awsPcaAuthorityArn && !Token.isUnresolved(awsPcaAuthorityArn) && !awsPcaAuthorityArn.startsWith('arn:')) {
        throw new ValidationError('AwspcaauthorityarnStartArnLeast', `awsPcaAuthorityArn must start with "arn:" and have at least 6 components; received ${awsPcaAuthorityArn}`, this);
      }
    });
  }
  ...
  private validateCanaryConfiguration(config: TrafficShiftConfig) {
    if (this.deploymentStrategy !== DeploymentStrategy.CANARY) {
      throw new ValidationError('CanaryConfigurationRequiresDeploymentstrategy', 'Canary configuration requires deploymentStrategy to be set to CANARY', this);
    }
    ...
    if (config.stepPercent !== undefined && !Token.isUnresolved(config.stepPercent)) {
      if (!Number.isFinite(config.stepPercent) || config.stepPercent < 0.1 || config.stepPercent > 100.0) {
        throw new ValidationError('CanaryDeploymentSteppercent100', `Canary deployment stepPercent must be between 0.1 and 100.0, received ${config.stepPercent}`, this);
      }
      if (!Number.isInteger(config.stepPercent * 10)) {
        throw new ValidationError('CanaryDeploymentSteppercentMultiple', `Canary deployment stepPercent must be a multiple of 0.1, received ${config.stepPercent}`, this);
      }
    }
    ...
    if (config.stepBakeTime !== undefined && !config.stepBakeTime.isUnresolved()) {
      ...
      if (!Number.isInteger(minutes)) {
        throw new ValidationError('CanaryDeploymentStepbaketimeWhole', `Canary deployment stepBakeTime must be a whole number of minutes, received ${minutes} minutes`, this);
      }
      if (minutes < 0 || minutes > 1440) {
        throw new ValidationError('CanaryDeploymentStepbaketime1440', `Canary deployment stepBakeTime must be between 0 and 1440 minutes, received ${minutes} minutes`, this);
      }
    }
  }
  ...
  private validateLinearConfiguration(config: TrafficShiftConfig) {
    if (this.deploymentStrategy !== DeploymentStrategy.LINEAR) {
      throw new ValidationError('LinearConfigurationRequiresDeploymentstrategy', 'Linear configuration requires deploymentStrategy to be set to LINEAR', this);
    }

    if (config.stepPercent !== undefined && !Token.isUnresolved(config.stepPercent)) {
      if (!Number.isFinite(config.stepPercent) || config.stepPercent < 3.0 || config.stepPercent > 100.0) {
        throw new ValidationError('LinearDeploymentSteppercent100', `Linear deployment stepPercent must be between 3.0 and 100.0, received ${config.stepPercent}`, this);
      }
      if (!Number.isInteger(config.stepPercent * 10)) {
        throw new ValidationError('LinearDeploymentSteppercentMultiple', `Linear deployment stepPercent must be a multiple of 0.1, received ${config.stepPercent}`, this);
      }
    }
    ...
    if (config.stepBakeTime !== undefined && !config.stepBakeTime.isUnresolved()) {
      const minutes = config.stepBakeTime.toMinutes({ integral: false });
      if (!Number.isInteger(minutes)) {
        throw new ValidationError('LinearDeploymentStepbaketimeWhole', `Linear deployment stepBakeTime must be a whole number of minutes, received ${minutes} minutes`, this);
      }
      if (minutes < 0 || minutes > 1440) {
        throw new ValidationError('LinearDeploymentStepbaketime1440', `Linear deployment stepBakeTime must be between 0 and 1440 minutes, received ${minutes} minutes`, this);
      }
    }
  }
  ...
  public autoScaleTaskCount(props: appscaling.EnableScalingProps) {
    if (this.scalableTaskCount) {
      throw new ValidationError('AutoscalingTaskCountAlready', 'AutoScaling of task count already enabled for this service', this);
    }
  ...
  public enableCloudMap(options: CloudMapOptions): cloudmap.Service {
    ...
    if (sdNamespace === undefined) {
      throw new ValidationError('EnableServiceDiscoveryCloudmap', 'Cannot enable service discovery if a Cloudmap Namespace has not been created in the cluster.', this);
    }

    if (sdNamespace.type === cloudmap.NamespaceType.HTTP) {
      throw new ValidationError('EnableDnsServiceDiscovery', 'Cannot enable DNS service discovery for HTTP Cloudmap Namespace.', this);
    }

    // Determine DNS type based on network mode
    const networkMode = this.taskDefinition.networkMode;
    if (networkMode === NetworkMode.NONE) {
      throw new ValidationError('ServiceDiscoveryNetworkmodeNone', 'Cannot use a service discovery if NetworkMode is None. Use Bridge, Host or AwsVpc instead.', this);
    }
    ...
    if (networkMode === NetworkMode.BRIDGE || networkMode === NetworkMode.HOST) {
      ...
      if (dnsRecordType !== cloudmap.DnsRecordType.SRV) {
        throw new ValidationError('SrvRecordsUsedNetwork', 'SRV records must be used when network mode is Bridge or Host.', this);
      }
    }
  ...
  private attachToELB(loadBalancer: elb.LoadBalancer, containerName: string, containerPort: number): void {
    if (this.taskDefinition.networkMode === NetworkMode.AWS_VPC) {
      throw new ValidationError('ClassicLoadBalancerNetworkmode', 'Cannot use a Classic Load Balancer if NetworkMode is AwsVpc. Use Host or Bridge instead.', this);
    }
    if (this.taskDefinition.networkMode === NetworkMode.NONE) {
      throw new ValidationError('ClassicLoadBalancerNetworkmode', 'Cannot use a Classic Load Balancer if NetworkMode is None. Use Host or Bridge instead.', this);
    }
  ...
  private attachToELBv2(
    ...
    if (this.taskDefinition.networkMode === NetworkMode.NONE) {
      throw new ValidationError('LoadBalancerNetworkmodeNone', 'Cannot use a load balancer if NetworkMode is None. Use Bridge, Host or AwsVpc instead.', this);
    }
  ...
  private addServiceRegistry(registry: ServiceRegistry) {
    if (this.serviceRegistries.length >= 1) {
      throw new ValidationError('AssociateGivenServiceDiscovery', 'Cannot associate with the given service discovery registry. ECS supports at most one service registry per service.', this);
    }
  ...
  private renderTimeout(idleTimeout?: Duration, perRequestTimeout?: Duration): CfnService.TimeoutConfigurationProperty | undefined {
    ...
    if (idleTimeout && idleTimeout.toMilliseconds() > 0 && idleTimeout.toMilliseconds() < Duration.seconds(1).toMilliseconds()) {
      throw new ValidationError('IdletimeoutLeastSecondDisable', `idleTimeout must be at least 1 second or 0 to disable it, got ${idleTimeout.toMilliseconds()}ms.`, this);
    }
    if (perRequestTimeout && perRequestTimeout.toMilliseconds() > 0 && perRequestTimeout.toMilliseconds() < Duration.seconds(1).toMilliseconds()) {
      throw new ValidationError('PerrequesttimeoutLeastSecondDisable', `perRequestTimeout must be at least 1 second or 0 to disable it, got ${perRequestTimeout.toMilliseconds()}ms.`, this);
    }
    return {
...
function determineContainerNameAndPort(scope: IConstruct, options: DetermineContainerNameAndPortOptions) {
  ...
  if (options.dnsRecordType === cloudmap.DnsRecordType.SRV) {
    ...
    if (options.container && options.container.taskDefinition != options.taskDefinition) {
      throw new ValidationError('AddDiscoveryContainerAnother', 'Cannot add discovery for a container from another task definition', scope);
    }
    ...
    if (options.containerPort && !container.portMappings.some(mapping => mapping.containerPort === options.containerPort)) {
      throw new ValidationError('AddDiscoveryContainerPort', 'Cannot add discovery for a container port that has not been mapped', scope);
    }

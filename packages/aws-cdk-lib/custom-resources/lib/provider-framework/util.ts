...
export function calculateRetryPolicy(scope: IConstruct, props: { totalTimeout?: Duration; queryInterval?: Duration } = { }) {
  ...
  if (Math.round(maxAttempts) !== maxAttempts) {
    throw new ValidationError('CannotCannotcannotdetermineretry', `Cannot determine retry count since totalTimeout=${totalTimeout.toSeconds()}s is not integrally dividable by queryInterval=${interval.toSeconds()}s`, scope);
  }

...
export class AwsCustomResource extends Construct implements iam.IGrantable {
  ...
  private static breakIgnoreErrorsCircuit(sdkCalls: Array<AwsSdkCall | undefined>, caller: string) {
    for (const call of sdkCalls) {
      if (call?.ignoreErrorCodesMatching) {
        throw new cdk.UnscopedValidationError('Caller', `\`${caller}\`` + ' cannot be called along with `ignoreErrorCodesMatching`.');
      }
    }
  }
  ...
  constructor(scope: Construct, id: string, props: AwsCustomResourceProps) {
    ...
    if (!props.onCreate && !props.onUpdate && !props.onDelete) {
      throw new cdk.ValidationError('LeastOncreateOnupdateOndelete', 'At least `onCreate`, `onUpdate` or `onDelete` must be specified.', this);
    }

    if (!props.role && !props.policy) {
      throw new cdk.ValidationError('LeastOnePolicyRole', 'At least one of `policy` or `role` (or both) must be specified.', this);
    }

    if (props.onCreate && !props.onCreate.physicalResourceId) {
      throw new cdk.ValidationError('PhysicalresourceidSpecifiedOncreateCall', "'physicalResourceId' must be specified for 'onCreate' call.", this);
    }

    if (!props.onCreate && props.onUpdate && !props.onUpdate.physicalResourceId) {
      throw new cdk.ValidationError('PhysicalresourceidSpecifiedOnupdateCall', "'physicalResourceId' must be specified for 'onUpdate' call when 'onCreate' is omitted.", this);
    }
    ...
    for (const call of [props.onCreate, props.onUpdate, props.onDelete]) {
      if (call?.externalId && !call?.assumedRoleArn) {
        throw new cdk.ValidationError('ExternalidProvidedAssumedrolearnSpecified', 'ExternalId can only be provided when assumedRoleArn is specified.', this);
      }
    }

    if (includesPhysicalResourceIdRef(props.onCreate?.parameters)) {
      throw new cdk.ValidationError('PhysicalresourceidreferenceSpecifiedOncreateParameters', '`PhysicalResourceIdReference` must not be specified in `onCreate` parameters.', this);
    }

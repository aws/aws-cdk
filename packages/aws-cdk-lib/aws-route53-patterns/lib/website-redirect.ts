...
export class HttpsRedirect extends Construct {
  constructor(scope: Construct, id: string, props: HttpsRedirectProps) {
    ...
    if (props.certificate) {
      ...
      if (!Token.isUnresolved(certificateRegion) && certificateRegion !== 'us-east-1') {
        throw new ValidationError('Mustbecertificateuseast1region', `The certificate must be in the us-east-1 region and the certificate you provided is in ${certificateRegion}.`, this);
      }
    }
  ...
  private certificateScope(): Construct {
    ...
    if (!parent) {
      throw new ValidationError('Mustbestackcreatedscope', `Stack ${stack.stackId} must be created in the scope of an App or Stage`, this);
    }
    if (Token.isUnresolved(stack.region)) {
      throw new ValidationError('Enabled', `When ${ROUTE53_PATTERNS_USE_CERTIFICATE} is enabled, a region must be defined on the Stack`, this);
    }

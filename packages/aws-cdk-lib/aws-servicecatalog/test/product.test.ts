...
describe('Product', () => {
  ...
  test('default product test', () => {
        ...
        productVersions: [
          {
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromAsset(
              path.join(__dirname, 'product1.template.json')
            ),
          },
        ],
      });
      ...
      new servicecatalog.CloudFormationProduct(stack, 'MyProduct', {
        ...
        productVersions: [
          ...
          },
          {
            ...
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromAsset(
              path.join(__dirname, 'product2.template.json')
            ),
          },
        ],
      });
        ...
        productVersions: [
          {
            ...
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromProductStack(
              new TestProductStack(stack, 'v1')
            ),
          },
          {
            productVersionName: 'v2',
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromProductStack(
              new TestProductStack(stack, 'v2')
            ),
          },
          {
            productVersionName: 'v3',
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromProductStack(
              new TestProductStack(stack, 'v3')
            ),
          },
        ],
      });
    ...
    test('product test from product stack history', () => {
      ...
      const expectedTemplateFileKey =
        'StackMyProductStackHistory8F05371C.StackProductStack190B56DE.v1.product.template.json';
      const snapshotExists = fs.existsSync(
        path.join(DEFAULT_PRODUCT_STACK_SNAPSHOT_DIRECTORY, expectedTemplateFileKey)
      );
    ...
    test('product test from product stack history snapshot', () => {
      ...
      new servicecatalog.CloudFormationProduct(stack, 'MyProduct', {
        productName: 'testProduct',
        owner: 'testOwner',
        productVersions: [productStackHistory.versionFromSnapshot('v1')],
      });

      const assembly = app.synth();
      expect(assembly.artifacts.length).toEqual(2);
      expect(assembly.stacks[0].assets.length).toBe(2);
      expect(assembly.stacks[0].assets[0].path).toEqual(
        'asset.434625edc7e017d93f388b5f116c2ebcf11a38457cfb89a9b00d4e551c0bf731.json'
      );
    }),
    test('fails product from product stack history snapshot not found', () => {
      ...
      expect(() => {
        new servicecatalog.CloudFormationProduct(stack, 'MyProduct', {
          productName: 'testProduct',
          owner: 'testOwner',
          productVersions: [productStackHistory.versionFromSnapshot('v3')],
        });
      }).toThrow(
        'Template StackMyProductStackHistory8F05371C.StackProductStack190B56DE.v3.product.template.json cannot be found in product-stack-snapshots'
      );
    }),
      ...
      new servicecatalog.CloudFormationProduct(stack, 'MyProduct', {
        ...
        productVersions: [
          ...
          },
          {
            productVersionName: 'v2',
            cloudFormationTemplate: servicecatalog.CloudFormationTemplate.fromUrl(
              'https://awsdocs.s3.amazonaws.com/servicecatalog/development-environment-v2.template'
            ),
          },
          {
            productVersionName: 'v3',
    ...
    test('product from attributes', () => {
      const product = servicecatalog.Product.fromProductArn(
        stack,
        'MyProduct',
        'arn:aws:catalog:region:account-id:product/prod-djh8932wr'
      );

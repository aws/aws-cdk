  ...
  describe('portfolio creation and importing', () => {
    test('default portfolio creation', () => {
      ...
      test('portfolio from arn', () => {
        const portfolio = servicecatalog.Portfolio.fromPortfolioArn(
          stack,
          'MyPortfolio',
          'arn:aws:catalog:region:account-id:portfolio/port-djh8932wr'
        );
      ...
      test('fails portfolio from arn without resource name in arn', () => {
        expect(() => {
          servicecatalog.Portfolio.fromPortfolioArn(
            stack,
            'MyPortfolio',
            'arn:aws:catalog:region:account-id:portfolio'
          );
      ...
      test('portfolio arn formatting', () => {
        ...
        expect(portfolio.portfolioArn).toEqual(
          `arn:${stack.partition}:catalog:${stack.region}:${stack.account}:portfolio/${portfolio.portfolioId}`
        );
      }),
    ...
    describe('portfolio constraints that have roles', () => {
      ...
      beforeEach(() => {
        ...
        test('set a launch role constraint using imported local role', () => {
          ...
          const importedLaunchRole = iam.Role.fromRoleArn(
            portfolio.stack,
            'ImportedLaunchRole',
            'arn:aws:iam::123456789012:role/ImportedLaunchRole'
          );

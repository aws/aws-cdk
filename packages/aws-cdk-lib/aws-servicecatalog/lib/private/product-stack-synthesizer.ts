...
export class ProductStackSynthesizer extends cdk.StackSynthesizer {
  ...
  public addFileAsset(asset: cdk.FileAssetSource): cdk.FileAssetLocation {
    if (!this.assetBucket) {
      throw new UnscopedValidationError('MustBeAssetBucketProvided', 'An Asset Bucket must be provided to use Assets');
    }
    ...
    if (this.serverSideEncryption === ServerSideEncryption.AWS_KMS && !this.serverSideEncryptionAwsKmsKeyId) {
      throw new UnscopedValidationError('MustBeProvidedSseKms', 'A KMS Key must be provided to use SSE_KMS');
    }
    if (this.serverSideEncryption !== ServerSideEncryption.AWS_KMS && this.serverSideEncryptionAwsKmsKeyId) {
      throw new UnscopedValidationError('MustBeSseKmsEncryptionEnabled', 'A SSE_KMS encryption must be enabled if you provide KMS Key');
    }
  ...
  private physicalNameOfBucket(bucket: IBucket) {
    ...
    if (resolvedName === undefined) {
      throw new UnscopedValidationError('MustBeBucketnameProvidedAssets', 'A bucketName must be provided to use Assets');
    }
    return resolvedName;
  }

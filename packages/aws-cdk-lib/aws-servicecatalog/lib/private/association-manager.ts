...
export class AssociationManager {
  ...
  public static constrainTagUpdates(portfolio: IPortfolio, product: IProduct, options: TagUpdateConstraintOptions): void {
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new ValidationError('CannotCannotcannotmultipleupdate', `Cannot have multiple tag update constraints for association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
  }
  ...
  public static notifyOnStackEvents(portfolio: IPortfolio, product: IProduct, topic: sns.ITopic, options: CommonConstraintOptions): void {
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new ValidationError('Topicalreadysubscribedassociation', `Topic ${topic.node.path} is already subscribed to association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
  }
  ...
  public static deployWithStackSets(portfolio: IPortfolio, product: IProduct, options: StackSetsConstraintOptions) {
    ...
    if (portfolio.node.tryFindChild(this.launchRoleConstraintLogicalId(association.associationKey))) {
      throw new ValidationError('CannotCannotcannotconfigurestackset', `Cannot configure StackSet deployment when a launch role is already defined for association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new ValidationError('CannotCannotcannotconfiguremultiple', `Cannot configure multiple StackSet deployment constraints for association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
  }
  ...
  private static setLaunchRoleConstraint(
    ...
    if (portfolio.node.tryFindChild(this.stackSetConstraintLogicalId(association.associationKey))) {
      throw new ValidationError('CannotCannotcannotlaunchrole', `Cannot set launch role when a StackSet rule is already defined for association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new ValidationError('CannotCannotcannotmultiplelaunch', `Cannot set multiple launch roles for association ${this.prettyPrintAssociation(portfolio, product)}`, portfolio);
    }
  }

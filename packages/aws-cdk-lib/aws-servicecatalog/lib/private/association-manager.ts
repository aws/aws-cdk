...
export class AssociationManager {
  ...
  public static constrainTagUpdates(
    portfolio: IPortfolio,
    product: IProduct,
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new Error(
        `Cannot have multiple tag update constraints for association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
  }

  public static notifyOnStackEvents(
    portfolio: IPortfolio,
    product: IProduct,
    topic: sns.ITopic,
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new Error(
        `Topic ${topic.node.path} is already subscribed to association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
  }
  ...
  public static constrainCloudFormationParameters(
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new Error(
        `Provisioning rule ${options.rule.ruleName} already configured on association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
  }

  public static setLaunchRole(
    portfolio: IPortfolio,
    product: IProduct,
  ...
  public static setLocalLaunchRoleName(
    portfolio: IPortfolio,
    product: IProduct,
  ...
  public static deployWithStackSets(portfolio: IPortfolio, product: IProduct, options: StackSetsConstraintOptions) {
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new Error(
        `Cannot configure multiple StackSet deployment constraints for association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
  }
  ...
  private static setLaunchRoleConstraint(
    ...
    if (portfolio.node.tryFindChild(this.stackSetConstraintLogicalId(association.associationKey))) {
      throw new Error(
        `Cannot set launch role when a StackSet rule is already defined for association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
    ...
    if (!portfolio.node.tryFindChild(constructId)) {
      ...
      throw new Error(
        `Cannot set multiple launch roles for association ${this.prettyPrintAssociation(portfolio, product)}`
      );
    }
  }

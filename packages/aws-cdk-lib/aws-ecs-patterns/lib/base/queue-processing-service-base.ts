...
export abstract class QueueProcessingServiceBase extends Construct {
  ...
  constructor(scope: Construct, id: string, props: QueueProcessingServiceBaseProps) {
    ...
    if (props.queue && (props.retentionPeriod || props.visibilityTimeout || props.maxReceiveCount)) {
      ...
      throw new ValidationError('ErrorpropsJoinQueueSpecify', `${errorProps.join(', ')} can be set only when queue is not set. Specify them in the QueueProps of the queue`, this);
    }
    ...
    if (props.cooldown && props.cooldown.toSeconds() > 999999999) {
      throw new ValidationError('Cooldown999999999FoundProps', `cooldown cannot be more than 999999999, found: ${props.cooldown.toSeconds()}`, this);
    }
    ...
    if (!this.desiredCount && !this.maxCapacity) {
      throw new ValidationError('MaxscalingcapacityGreaterDesiredcount', 'maxScalingCapacity must be set and greater than 0 if desiredCount is 0', this);
    }

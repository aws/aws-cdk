...
export class ApplicationLoadBalancedFargateService extends ApplicationLoadBalancedServiceBase {
  ...
  constructor(scope: Construct, id: string, props: ApplicationLoadBalancedFargateServiceProps = {}) {
    ...
    if (
      ...
      throw new ValidationError('MinimumHealthyPercentMaximum', 'Minimum healthy percent must be less than maximum healthy percent.', this);
    }
  ...
  private validateHealthyPercentage(name: string, value?: number) {
    ...
    if (!Number.isInteger(value) || value < 0) {
      throw new ValidationError('NameNonNegativeInteger', `${name}: Must be a non-negative integer; received ${value}`, this);
    }
  }
  ...
  private validateContainerCpu(cpu: number, containerCpu?: number) {
    ...
    if (containerCpu > cpu) {
      throw new ValidationError('ContainercpuCpuReceivedContainercpu', `containerCpu must be less than to cpu; received containerCpu: ${containerCpu}, cpu: ${cpu}`, this);
    }
  ...
  private validateContainerMemoryLimitMiB(memoryLimitMiB: number, containerMemoryLimitMiB?: number) {
    ...
    if (containerMemoryLimitMiB > memoryLimitMiB) {
      throw new ValidationError('ContainermemorylimitmibMemorylimitmibReceivedContainermemorylimitmib', `containerMemoryLimitMiB must be less than to memoryLimitMiB; received containerMemoryLimitMiB: ${containerMemoryLimitMiB}, memoryLimitMiB: ${memoryLimitMiB}`, this);
    }
    if (containerMemoryLimitMiB <= 0 || !Number.isInteger(containerMemoryLimitMiB)) {
      throw new ValidationError('ContainermemorylimitmibPositiveIntegerReceived', `containerMemoryLimitMiB must be a positive integer; received ${containerMemoryLimitMiB}`, this);
    }
  }
}

    ...
    test('creates admin role if not specified', () => {
      stack.deployStage.addAction(
        new cpactions.CloudFormationDeployStackSetAction({
          ...defaultOpts(),
          stackInstances: cpactions.StackInstances.fromArtifactPath(stack.sourceOutput.atPath('accounts.json'), [
            'us-east-1',
            'us-west-1',
            'ca-central-1',
          ]),
          ...
          Statement: Match.arrayWith([
            {
              ...
              Resource: {
                'Fn::Join': [
                  '',
                  ['arn:', { Ref: 'AWS::Partition' }, ':cloudformation:us-east-1:111111111111:stackset/MyStack:*'],
                ],
              },
            },
            {
            ...
            Actions: [
              {
                Configuration: {
                  ...
                  AdministrationRoleArn: {
                    'Fn::GetAtt': ['PipelineDeployStackSetUpdateStackSetAdministrationRole183434B0', 'Arn'],
                  },
                  ExecutionRoleName: 'Exec',
          ...
          Actions: [
            {
              Configuration: {
                ...
                TemplatePath: 'SourceArtifact::template.yaml',
                Parameters:
                  'ParameterKey=key0,ParameterValue=val0 ParameterKey=key1,ParameterValue=val1 ParameterKey=key2,UsePreviousValue=true ParameterKey=key3,UsePreviousValue=true',
                Capabilities: 'CAPABILITY_NAMED_IAM',
                DeploymentTargets: 'SourceArtifact::accounts.json',
                FailureTolerancePercentage: 50,
                MaxConcurrentPercentage: 25,

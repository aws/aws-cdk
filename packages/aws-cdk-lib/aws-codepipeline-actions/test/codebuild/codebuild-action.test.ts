...
describe('CodeBuild Action', () => {
  describe('CodeBuild action', () => {
    ...
    describe('auto-create CodeBuild service role for Full Clone sources (feature flag)', () => {
      ...
      test('auto-create succeeds: emits an info message that the role is scoped-down and how to add extra permissions', () => {
        const { stack, action } = setup();

        // the scoped role was created
        expect(action.serviceRole).toBeDefined();

        // an info annotation surfaces the minimal-role gap and points at both escape hatches
        Annotations.fromStack(stack).hasInfo('*', Match.stringLikeRegexp('codeBuildServiceRoleAutoScoped'));
        Annotations.fromStack(stack).hasInfo('*', Match.stringLikeRegexp('.*scoped-down CodeBuild service role.*'));
        Annotations.fromStack(stack).hasInfo('*', Match.stringLikeRegexp('.*buildAction.serviceRole.*'));
        Annotations.fromStack(stack).hasInfo('*', Match.stringLikeRegexp('.*serviceRoleOverride.*'));
      });

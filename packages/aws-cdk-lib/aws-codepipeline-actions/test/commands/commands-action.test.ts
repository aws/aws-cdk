...
describe('Commands Action', () => {
  ...
  describe('grant policy', () => {
    ...
    test('grant kms decrypt policy for pipeline bucket key if the action has input without output', () => {
      // GIVEN
      stack = new Stack();
      sourceOutput1 = new codepipeline.Artifact('SourceArtifact');
      sourceOutput2 = new codepipeline.Artifact('SourceArtifact2');
      pipeline = new codepipeline.Pipeline(stack, 'Pipeline', {
        artifactBucket: new Bucket(stack, 'PipelineBucket', {
          encryptionKey: new Key(stack, 'PipelineBucketKey'),
        }),
      });
      pipeline.addStage({
        stageName: 'Source',
  ...
  test('grant kms encrypt and decrypt policy for pipeline bucket key if the action has input and output', () => {
    // GIVEN
    stack = new Stack();
    sourceOutput1 = new codepipeline.Artifact('SourceArtifact');
    sourceOutput2 = new codepipeline.Artifact('SourceArtifact2');

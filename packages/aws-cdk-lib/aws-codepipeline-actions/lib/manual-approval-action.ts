...
export class ManualApprovalAction extends Action {
  ...
  constructor(props: ManualApprovalActionProps) {
    ...
    if (props.timeout && (props.timeout.toMinutes() < 5 || props.timeout.toMinutes() > 86400)) {
      throw new UnscopedValidationError('MustBeTimeoutBetweenMinutes', `timeout must be between 5 minutes and 86400 minutes (60 days), got ${props.timeout.toMinutes()} minutes`);
    }
  ...
  public grantManualApproval(grantable: iam.IGrantable): void {
    if (!this.stage) {
      throw new UnscopedValidationError('CannotCannotGrantPermissions', 'Cannot grant permissions before binding action to a stage');
    }

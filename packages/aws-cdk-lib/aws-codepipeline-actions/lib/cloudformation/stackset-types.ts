...
export abstract class StackInstances {
  ...
  public static fromArtifactPath(artifactPath: codepipeline.ArtifactPath, regions: string[]): StackInstances {
    if (regions.length === 0) {
      throw new cdk.UnscopedValidationError('RegionsEmptyList', "'regions' may not be an empty list");
    }
  ...
  private static fromList(targets: string[], regions: string[]): StackInstances {
    if (targets.length === 0) {
      throw new cdk.UnscopedValidationError('TargetsEmptyList', "'targets' may not be an empty list");
    }

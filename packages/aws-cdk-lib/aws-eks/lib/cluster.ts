...
export interface ComputeConfig {
  ...
  /**
   * IAM role arn for the nodePools.
   *
...
export interface ClusterProps extends ClusterOptions {
   ...
   *
   * When `autoMode` is enabled, the default capacity is `0`.
   *
   * @default 2
   */
...
export class Cluster extends ClusterBase {
  ...
  constructor(scope: Construct, id: string, props: ClusterProps) {
    ...
    this.role = props.role ?? this.createClusterRole(autoModeEnabled);
    ...
    const resource = this._clusterResource = new ClusterResource(this, 'Resource', {
      ...
      computeConfig: {
        ...
        nodePools: autoModeEnabled ? props.compute?.nodePools ?? ['system', 'general-purpose'] : undefined,
        nodeRoleArn: autoModeEnabled ? props.compute?.nodeRole?.roleArn ?? this.createNodePoolRole(`${id}nodePoolRole`).roleArn : undefined,
      },
      storageConfig: {
        blockStorage: {
          enabled: autoModeEnabled,
    ...
    /**
     * EKS Automode should also support Nodegroup creation, so we need to handle the following cases:
     * autoMode defaultCapacity minCapacity value
     *  true    undefined        0
     *  true    defined          defined capacity
     *  false   defined          defined capacity
     *  false   undefined        DEFAULT_CAPACITY_COUNT
     */
    const minCapacity = autoModeEnabled && !props.defaultCapacity ? 0 : (props.defaultCapacity ?? DEFAULT_CAPACITY_COUNT);
    if (minCapacity > 0) {
  ...
  private isValidAutoModeConfig(props: ClusterProps) {
    ...
    if (autoModeEnabled && props.compute?.nodePools) {
      this.validateNodePools(props.compute.nodePools);
    }
  ...
  private createClusterRole(autoModeEnabled: boolean): iam.IRole {
    const baseRole = this.createBaseRole();

    if (autoModeEnabled) {
      return this.enhanceRoleForAutoMode(baseRole);
    }

    return baseRole;
  }

  private createBaseRole(): iam.Role {
  ...
  /**
   * Configure additional permission and policies for the role
   * For more information, see https://docs.aws.amazon.com/eks/latest/userguide/automode-get-started-cli.html#auto-mode-create-roles
   */
  private enhanceRoleForAutoMode(role: iam.Role): iam.Role {
    ...
    autoModePolicies.forEach(policyName => {
  ...
  /**
   * The required policies are listed [here](https://docs.aws.amazon.com/eks/latest/userguide/automode-get-started-cli.html#auto-mode-create-roles)
   */
  private createNodePoolRole(id: string): iam.Role {
  ...
  private validateNodePools(nodepools: string[]): void {
    if (nodepools) {
      const validNodePools = ['general-purpose', 'system'];
      const invalidPools = nodepools.filter(pool => !validNodePools.includes(pool));

      if (invalidPools.length > 0) {
        throw new Error(
          `Invalid node pool values: ${invalidPools.join(', ')}. ` +
          `Valid values are: ${validNodePools.join(', ')}`,
        );
      }
    }
  }

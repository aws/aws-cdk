...
abstract class ClusterBase extends Resource implements ICluster {
  ...
  public addCdk8sChart(id: string, chart: Construct, options: KubernetesManifestOptions = {}): KubernetesManifest {
    ...
    if (typeof cdk8sChart.toJson !== 'function') {
      throw new ValidationError('InvalidCdk8sChartContain', `Invalid cdk8s chart. Must contain a 'toJson' method, but found ${typeof cdk8sChart.toJson}`, this);
    }
  ...
  public connectAutoScalingGroupCapacity(autoScalingGroup: autoscaling.AutoScalingGroup, options: AutoScalingGroupOptions) {
    ...
    if (options.bootstrapOptions && !bootstrapEnabled) {
      throw new ValidationError('SpecifyBootstrapoptionsBootstrapenabledFalse', 'Cannot specify "bootstrapOptions" if "bootstrapEnabled" is false', this);
    }
...
export class Cluster extends ClusterBase {
  ...
  constructor(scope: Construct, id: string, props: ClusterProps) {
    ...
    if (selectedSubnetIdsPerGroup.some(Token.isUnresolved) && selectedSubnetIdsPerGroup.length > 1) {
      throw new ValidationError('EksClusterSelectMultiple', 'eks.Cluster: cannot select multiple subnet groups from a VPC imported from list tokens with unknown length. Select only one subnet group, pass a length to Fn.split, or switch to Vpc.fromLookup.', this);
    }
    ...
    if (!hasPendingLookup) {
      if (privateSubnets.length === 0 && publicAccessDisabled) {
        ...
        throw new ValidationError('VpcContainPrivateSubnets', 'Vpc must contain private subnets when public endpoint access is disabled', this);
      }

      if (privateSubnets.length === 0 && publicAccessRestricted) {
      // no private subnets and public access is restricted, no good.
        throw new ValidationError('VpcContainPrivateSubnets', 'Vpc must contain private subnets when public endpoint access is restricted', this);
      }
    }
    ...
    if (!hasPendingLookup) {
      if (placeClusterHandlerInVpc && privateSubnets.length === 0) {
        throw new ValidationError('PlaceClusterHandlerVpc', 'Cannot place cluster handler in the VPC since no private subnets could be selected', this);
      }
    }

    if (props.clusterHandlerSecurityGroup && !placeClusterHandlerInVpc) {
      throw new ValidationError('SpecifyClusterhandlersecuritygroupWithoutPlaceclusterhandlerinvpc', 'Cannot specify clusterHandlerSecurityGroup without placeClusterHandlerInVpc set to true', this);
    }

    if (props.serviceIpv4Cidr && props.ipFamily == IpFamily.IP_V6) {
      throw new ValidationError('SpecifyServiceipv4cidrIpfamilyEqual', 'Cannot specify serviceIpv4Cidr with ipFamily equal to IpFamily.IP_V6', this);
    }

    // Check if the cluster name exceeds 100 characters
    if (!Token.isUnresolved(this.physicalName) && this.physicalName.length > 100) {
      throw new ValidationError('ClusterName100Characters', 'Cluster name cannot be more than 100 characters', this);
    }
    ...
    if (this.endpointAccess._config.privateAccess && privateSubnets.length !== 0) {
      ...
      if (this.vpc instanceof ec2.Vpc && !(this.vpc.dnsHostnamesEnabled && this.vpc.dnsSupportEnabled)) {
        throw new ValidationError('PrivateEndpointAccessRequires', 'Private endpoint access requires the VPC to have DNS support and DNS hostnames enabled. Use `enableDnsHostnames: true` and `enableDnsSupport: true` when creating the VPC.', this);
      }
  ...
  private defineKubectlProvider(removalPolicy?: RemovalPolicy) {
    ...
    if (this.stack.node.tryFindChild(uid)) {
      throw new ValidationError('SingleEksClusterDefined', 'Only a single EKS cluster can be defined within a CloudFormation stack', this);
    }
...
class ImportedCluster extends ClusterBase {
  ...
  public get clusterSecurityGroup(): ec2.ISecurityGroup {
    if (!this._clusterSecurityGroup) {
      throw new ValidationError('ClustersecuritygroupDefinedImportedCluster', '"clusterSecurityGroup" is not defined for this imported cluster', this);
    }
  ...
  public get clusterSecurityGroupId(): string {
    if (!this.props.clusterSecurityGroupId) {
      throw new ValidationError('ClustersecuritygroupidDefinedImportedCluster', '"clusterSecurityGroupId" is not defined for this imported cluster', this);
    }
  ...
  public get clusterCertificateAuthorityData(): string {
    if (!this.props.clusterCertificateAuthorityData) {
      throw new ValidationError('ClustercertificateauthoritydataDefinedImportedCluster', '"clusterCertificateAuthorityData" is not defined for this imported cluster', this);
    }
  ...
  public get clusterEncryptionConfigKeyArn(): string {
    if (!this.props.clusterEncryptionConfigKeyArn) {
      throw new ValidationError('ClusterencryptionconfigkeyarnDefinedImportedCluster', '"clusterEncryptionConfigKeyArn" is not defined for this imported cluster', this);
    }
  ...
  public get openIdConnectProvider(): iam.IOpenIdConnectProvider {
    if (!this.props.openIdConnectProvider) {
      throw new ValidationError('OpenidconnectproviderDefinedImportedCluster', '"openIdConnectProvider" is not defined for this imported cluster', this);
    }

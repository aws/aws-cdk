...
export class AwsAuth extends Construct {
  ...
  constructor(scope: Construct, id: string, props: AwsAuthProps) {
    ...
    if (!supportConfigMap) {
      throw new ValidationError('Configmapsupportedauthenticationmode', 'ConfigMap not supported in the AuthenticationMode', this);
    }
  ...
  private assertSameStack(construct: IConstruct) {
    ...
    if (Stack.of(construct) !== thisStack) {
      ...
      throw new ValidationError('Shouldbeshoulddefinedscope', `${construct.node.path} should be defined in the scope of the ${thisStack.stackName} stack to prevent circular dependencies`, this);
    }
  }

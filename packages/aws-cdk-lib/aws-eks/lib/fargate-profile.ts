...
export class FargateProfile extends Construct implements ITaggable {
  ...
  constructor(scope: Construct, id: string, props: FargateProfileProps) {
    ...
    if (props.subnetSelection && !props.vpc) {
      Annotations.of(this).addWarningV2('EKS:FargateProfile:DefaultToPrivateSubnets', 'Vpc must be defined to use a custom subnet selection. All private subnets belonging to the EKS cluster will be used by default');
    }

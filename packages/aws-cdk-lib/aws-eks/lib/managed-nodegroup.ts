...
function getPossibleAmiTypes(scope: Construct, instanceTypes: InstanceType[]): NodegroupAmiType[] {
  ...
  if (architectures.size === 0) { // protective code, the current implementation will never result in this.
    throw new ValidationError('CannotDetermineCompatibleAmiType', `Cannot determine any ami type compatible with instance types: ${instanceTypes.map(i => i.toString()).join(', ')}`, scope);
  }

  if (architectures.size > 1) {
    throw new ValidationError('InstanceTypesDifferentArchitecturesNotAllowed', 'instanceTypes of different architectures is not allowed', scope);
  }

...
function migrateJson() {
  ...
  return ['```json', JSON.stringify({ context }, undefined, 2), '```'].join('\n');
}
...
function oldBehavior(flag: FlagInfo): string | undefined {
  switch (flag.type) {
    case FlagType.ApiDefault:
      return flag.compatibilityWithOldBehaviorMd;
    case FlagType.BugFix:
      return flag.compatibilityWithOldBehaviorMd;
    case FlagType.VisibleContext:
      return undefined;
  }
}
...
function flags(pred: (x: FlagInfo) => boolean) {
  const entries = Object.entries(feats.FLAGS).filter(([_, flag]) => pred(flag));
...
async function updateRecommendedFlagsFile(filename: string) {
  await fs.writeFile(filename, JSON.stringify(feats.CURRENTLY_RECOMMENDED_FLAGS, undefined, 2), {
    encoding: 'utf-8',
  });
}

...
*When enabled, the trust policy for the cross-account action role is scoped to the pipeline role.* (default)

When this feature flag is enabled, the trust policy of the cross-account action role will be scoped to the pipeline role. If you are providing a custom role, you will need to ensure 'roleName' is specified or set to PhysicalName.GENERATE_IF_NEEDED.
...
**Compatibility with old behavior:** Disable the feature flag to keep the trust policy as the root account principal

...
export const FLAGS: Record<string, FlagInfo> = {
  ...
  //////////////////////////////////////////////////////////////////////
  [CDK_DIFF_USE_CHANGESET_PROPERTY_VALUES_BY_DEFAULT]: {
    type: FlagType.BugFix,
    summary: 'When enabled, make `cdk diff` default to using values from DescribeChangeSet(IncludePropertyValues: true) for resource property differences.',
    detailsMd: `
      When this flag is enabled, the default behavior of \`cdk diff\` will be to use the output of DescribeChangeSet(IncludePropertyValues: true),  which is
      more accurate than just computing the differences between the before and after of the template that is generated by \`cdk synth\`. However, this behavior
      is new and may interact with the cdk diff formatting logic in unforeseen ways, and so at release, this will be behind a feature flag.
    `,
    introducedIn: { v2: '2.144.0' },
    recommendedValue: true,
  },
};

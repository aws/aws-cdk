...
export class CachePolicy extends Resource implements ICachePolicy {
  ...
  constructor(scope: Construct, id: string, props: CachePolicyProps = {}) {
    ...
    if (!Token.isUnresolved(cachePolicyName) && !cachePolicyName.match(/^[\w-]+$/i)) {
      throw new ValidationError('CachepolicynameIncludeAlphanumericCharacters', `'cachePolicyName' can only include '-', '_', and alphanumeric characters, got: '${cachePolicyName}'`, this);
    }

    if (cachePolicyName.length > 128) {
      throw new ValidationError('CachepolicynameLonger128Characters', `'cachePolicyName' cannot be longer than 128 characters, got: '${cachePolicyName.length}'`, this);
    }

    if (props.comment && !Token.isUnresolved(props.comment) && props.comment.length > 128) {
      throw new ValidationError('CommentLonger128Characters', `'comment' cannot be longer than 128 characters, got: ${props.comment.length}`, this);
    }

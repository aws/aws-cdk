...
export class OriginRequestPolicy extends Resource implements IOriginRequestPolicy {
  ...
  constructor(scope: Construct, id: string, props: OriginRequestPolicyProps = {}) {
    ...
    if (!Token.isUnresolved(originRequestPolicyName) && !originRequestPolicyName.match(/^[\w-]+$/i)) {
      throw new ValidationError('OriginrequestpolicynameIncludeAlphanumericCharacters', `'originRequestPolicyName' can only include '-', '_', and alphanumeric characters, got: '${props.originRequestPolicyName}'`, this);
    }
...
export class OriginRequestCookieBehavior {
  ...
  public static denyList(...cookies: string[]) {
    if (cookies.length === 0) {
      throw new UnscopedValidationError('LeastOneCookieDeny', 'At least one cookie to deny must be provided');
    }
  ...
  public static allowList(...cookies: string[]) {
    if (cookies.length === 0) {
      throw new UnscopedValidationError('LeastOneCookieAllow', 'At least one cookie to allow must be provided');
    }
...
export class OriginRequestHeaderBehavior {
  ...
  public static all(...cloudfrontHeaders: string[]) {
    if (cloudfrontHeaders.length > 0) {
      if (!cloudfrontHeaders.every(header => header.startsWith('CloudFront-'))) {
        throw new UnscopedValidationError('AdditionalCloudfrontHeadersPassed', 'additional CloudFront headers passed to `OriginRequestHeaderBehavior.all()` must begin with \'CloudFront-\'');
      }
  ...
  public static allowList(...headers: string[]) {
    if (headers.length === 0) {
      throw new UnscopedValidationError('LeastOneHeaderAllow', 'At least one header to allow must be provided');
    }
    if (headers.map(header => header.toLowerCase()).some(header => ['authorization', 'accept-encoding'].includes(header))) {
      throw new UnscopedValidationError('PassAuthorizationAcceptEncoding', 'you cannot pass `Authorization` or `Accept-Encoding` as header values; use a CachePolicy to forward these headers instead');
    }
  ...
  public static denyList(...headers: string[]) {
    if (headers.length === 0) {
      throw new UnscopedValidationError('LeastOneHeaderDeny', 'At least one header to deny must be provided');
    }
...
export class OriginRequestQueryStringBehavior {
  ...
  public static allowList(...queryStrings: string[]) {
    if (queryStrings.length === 0) {
      throw new UnscopedValidationError('LeastOneQueryString', 'At least one query string to allow must be provided');
    }
  ...
  public static denyList(...queryStrings: string[]) {
    if (queryStrings.length === 0) {
      throw new UnscopedValidationError('LeastOneQueryString', 'At least one query string to deny must be provided');
    }

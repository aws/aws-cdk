...
export class OriginRequestPolicy extends Resource implements IOriginRequestPolicy {
  ...
  constructor(scope: Construct, id: string, props: OriginRequestPolicyProps = {}) {
    ...
    if (!Token.isUnresolved(originRequestPolicyName) && !originRequestPolicyName.match(/^[\w-]+$/i)) {
      throw new ValidationError('OriginrequestpolicynameOnlyInclude', `'originRequestPolicyName' can only include '-', '_', and alphanumeric characters, got: '${props.originRequestPolicyName}'`, this);
    }
...
export class OriginRequestHeaderBehavior {
  ...
  public static all(...cloudfrontHeaders: string[]) {
    if (cloudfrontHeaders.length > 0) {
      if (!cloudfrontHeaders.every(header => header.startsWith('CloudFront-'))) {
        throw new UnscopedValidationError('MustBeAdditionalCloudfrontHeaders', 'additional CloudFront headers passed to `OriginRequestHeaderBehavior.all()` must begin with \'CloudFront-\'');
      }
  ...
  public static allowList(...headers: string[]) {
    ...
    if (headers.map(header => header.toLowerCase()).some(header => ['authorization', 'accept-encoding'].includes(header))) {
      throw new UnscopedValidationError('CannotPassHeaderValues', 'you cannot pass `Authorization` or `Accept-Encoding` as header values; use a CachePolicy to forward these headers instead');
    }
  ...
  public static denyList(...headers: string[]) {
    if (headers.length === 0) {
      throw new UnscopedValidationError('MustBeLeastHeaderDeny', 'At least one header to deny must be provided');
    }

...
export class CacheBehavior {
  ...
  constructor(originId: string, private readonly props: CacheBehaviorProps) {
    ...
    if (props.enableGrpc) {
      if (props.allowedMethods !== AllowedMethods.ALLOW_ALL) {
        throw new UnscopedValidationError('AllowedmethodsOnlyAllowedmethods', '\'allowedMethods\' can only be AllowedMethods.ALLOW_ALL if \'enableGrpc\' is true');
      }
      if (props.edgeLambdas !== undefined && props.edgeLambdas.length > 0) {
        throw new UnscopedValidationError('EdgelambdasCannotSpecifiedEnablegrpc', '\'edgeLambdas\' cannot be specified if \'enableGrpc\' is true');
      }
    }
  ...
  private validateEdgeLambdas(edgeLambdas?: EdgeLambda[]) {
    ...
    if (edgeLambdas && edgeLambdas.some(lambda => lambda.includeBody && !includeBodyEventTypes.includes(lambda.eventType))) {
      throw new UnscopedValidationError('IncludebodyOnlyTrueOriginRequest', '\'includeBody\' can only be true for ORIGIN_REQUEST or VIEWER_REQUEST event types.');
    }
  }

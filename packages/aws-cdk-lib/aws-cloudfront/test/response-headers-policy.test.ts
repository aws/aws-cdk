...
describe("ResponseHeadersPolicy", () => {
  ...
  test("import existing policy by id", () => {
    ...
    expect(responseHeadersPolicy.responseHeadersPolicyId).toEqual(
      responseHeadersPolicyId
    );
  });
  ...
  test("throws with out of range sampling rate", () => {
    expect(
      ...
      "Sampling rate must be between 0 and 100 (inclusive), received 110"
    );
  });
  ...
  test("throws with sampling rate with more than 4 decimal places", () => {
    expect(
      ...
      "Sampling rate can have up to four decimal places, received 50.12345"
    );
  });

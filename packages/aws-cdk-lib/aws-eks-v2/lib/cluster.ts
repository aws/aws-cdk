...
export class Cluster extends ClusterBase {
  ...
  constructor(scope: Construct, id: string, props: ClusterProps) {
    ...
    if (selectedSubnetIdsPerGroup.some(Token.isUnresolved) && selectedSubnetIdsPerGroup.length > 1) {
      throw new UnscopedValidationError('ValidationError', 'eks.Cluster: cannot select multiple subnet groups from a VPC imported from list tokens with unknown length. Select only one subnet group, pass a length to Fn.split, or switch to Vpc.fromLookup.');
    }
    ...
    if (privateSubnets.length === 0 && publicAccessDisabled) {
      ...
      throw new UnscopedValidationError('ContainPrivateSubnetsPublic', 'Vpc must contain private subnets when public endpoint access is disabled');
    }

    if (privateSubnets.length === 0 && publicAccessRestricted) {
      // no private subnets and public access is restricted, no good.
      throw new UnscopedValidationError('ContainPrivateSubnetsPublic', 'Vpc must contain private subnets when public endpoint access is restricted');
    }

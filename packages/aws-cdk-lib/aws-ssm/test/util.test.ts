...
describe('arnForParameterName', () => {
  describe('simple names', () => {
    ...
    test('token parameterName and concrete physical name (no additional "/")', () => {
      const stack = new Stack();
      expect(
        stack.resolve(arnForParameterName(stack, Token.asString({ Ref: 'Boom' }), { physicalName: 'myParam' }))
      ).toEqual({
        'Fn::Join': [
          '',
          [
  ...
  describe('path names', () => {
    ...
    test('token parameterName and concrete physical name (no sep)', () => {
      const stack = new Stack();
      expect(
        stack.resolve(arnForParameterName(stack, Token.asString({ Ref: 'Boom' }), { physicalName: '/foo/bar' }))
      ).toEqual({
        'Fn::Join': [
          '',
          [
  ...
  test('fails if explicit separator is not defined and parameterName is a token', () => {
    const stack = new Stack();
    expect(() => arnForParameterName(stack, Token.asString({ Ref: 'Boom' }))).toThrow(

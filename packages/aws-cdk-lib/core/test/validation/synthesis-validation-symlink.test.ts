...
describe('policy validation plugins tolerate readdir/lstat disagreement (#38653)', () => {
  ...
  test('synth succeeds when readdir classifies a directory as a symlink (Windows reparse-point behavior)', () => {
    ...
    // Prove the code under test actually walked the assembly through the mock:
    // if it did not, synth never encountered the spoofed reparse point and the
    // "not.toThrow()" above would be meaningless.
    expect(spoofInvocations).toBeGreaterThan(0);

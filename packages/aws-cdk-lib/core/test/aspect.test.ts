...
describe('aspect', () => {
  ...
  test('Warn if an Aspect is added via another Aspect', () => {
    ...
    expect(root.node.metadata[0].data).toEqual({ id: 'Core:IgnoredAspect', message: 'We detected an Aspect was added via another Aspect, and will not be applied', scope: 'MyConstruct' });

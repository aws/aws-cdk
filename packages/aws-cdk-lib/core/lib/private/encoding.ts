...
export function createTokenDouble(x: number) {
  if (Math.floor(x) !== x || x < 0) {
    throw new UnscopedValidationError('EncodePositiveIntegers', 'Can only encode positive integers');
  }
  if (x > MAX_ENCODABLE_INTEGER) {
    throw new UnscopedValidationError('GotIndexTooLarge', `Got an index too large to encode: ${x}`);
  }

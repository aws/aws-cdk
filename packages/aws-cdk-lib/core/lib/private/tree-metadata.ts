...
class FragmentedTreeWriter {
  ...
  public addNode(construct: IConstruct, parent: IConstruct | undefined, node: Node) {
    ...
    if (parent === undefined) {
      if (this.forest.length > 0) {
        throw new AssumptionError('OnlyExactlyNodeWithout', 'Can only add exactly one node without a parent');
      }
  ...
  private addToExistingTree(node: Node, parent: Node) {
    ...
    if (this.isTreeFull(tree)) {
      ...
      if (!grandParent) {
        throw new AssumptionError('CouldFindParent', `Could not find parent of ${JSON.stringify(parent)}`);
      }
  ...
  private treeForNode(node: Node): Tree {
    ...
    throw new AssumptionError('CouldFindTreeNode', `Could not find tree for node: ${JSON.stringify(node)}, tried ${tried}`);
  }
}

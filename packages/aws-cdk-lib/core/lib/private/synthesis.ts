...
function invokeValidationPlugins(root: IConstruct, outdir: string, assembly: private_cxapi.CloudAssembly) {
  ...
  // Collect all template paths for plugins that need them
  let allTemplatePaths: string[] = [];
  for (const paths of templatePathsByPlugin.values()) {
    allTemplatePaths.push(...paths);
  }
  if (allTemplatePaths.length === 0) {
    for (const asm of assemblies.values()) {
      allTemplatePaths.push(...asm.stacksRecursively.map(stack => stack.templateFullPath));
    }
  }
  allTemplatePaths = [...new Set(allTemplatePaths)];
  ...
  //
  // Rule matching: violations are matched as <pluginName>::<ruleName>.
  // Users suppress with: Validations.of(x).acknowledge({ id: '<pluginName>::<ruleId>' })
  ...
  if (acknowledgedRuleIds.size > 0) {
    for (let i = 0; i < reports.length; i++) {
      ...
      const filtered = reports[i].violations.filter(v => {
        ...
        return !acknowledgedRuleIds.has(`${pluginName}::${v.ruleName}`);
      });

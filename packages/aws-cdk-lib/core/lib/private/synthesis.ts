...
function validateTree(root: IConstruct) {
  ...
  if (errors.length > 0) {
    ...
    throw new UnscopedValidationError('ValidationFailedFollowingErrors', `Validation failed with the following errors:\n  ${errorList}`);
  }
}

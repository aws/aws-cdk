...
export function generatePhysicalName(resource: IResource): string {
  ...
  if (Token.isUnresolved(region) || !region) {
    throw new ValidationError('CannotCannotcannotgeneratephysical', `Cannot generate a physical name for ${Node.of(resource).path}, because the region is un-resolved or missing`, resource);
  }

  const account: string = stack.account;
  if (Token.isUnresolved(account) || !account) {
    throw new ValidationError('CannotCannotcannotgeneratephysical', `Cannot generate a physical name for ${Node.of(resource).path}, because the account is un-resolved or missing`, resource);
  }
...
export class GeneratedWhenNeededMarker implements IResolvable {
  ...
  public resolve(_ctx: IResolveContext): never {
    throw new UnscopedValidationError('InvalidInvalidPhysicalName', 'Invalid physical name passed to CloudFormation. Use "this.physicalName" instead');
  }

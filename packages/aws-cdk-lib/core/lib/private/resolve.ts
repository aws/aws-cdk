...
export function resolve(obj: any, options: IResolveOptions): any {
  ...
  if (typeof(obj) === 'function') {
    throw new UnscopedValidationError('TryingResolveNonDataObject', `Trying to resolve a non-data object. Only token are supported for lazy evaluation. Path: ${pathName}. Object: ${obj}`);
  }
  ...
  for (const key of Object.keys(obj)) {
    ...
    if (typeof(resolvedKey) === 'string') {
      ...
      if (!options.allowIntrinsicKeys) {
        throw new UnscopedValidationError('UsedResolveString', `"${String(key)}" is used as the key in a map so must resolve to a string, but it resolves to: ${JSON.stringify(resolvedKey)}. Consider using "CfnJson" to delay resolution to deployment-time`);
      }

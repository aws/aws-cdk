...
function tokenAwareStringify(root: any, space: number, ctx: IResolveContext) {
  ...
  function recurse(obj: any): void {
    ...
    if (Token.isUnresolved(obj)) {
      throw new UnscopedValidationError('ShouldnTHappenAnymore', "This shouldn't happen anymore");
    }
  ...
  function renderIntrinsic(intrinsic: any) {
    ...
    throw new UnscopedValidationError('UnexpectedUnexpectedTypeHint', `Unexpected type hint: ${resolvedTypeHint(intrinsic)}`);
  }

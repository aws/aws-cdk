...
function resolveValue(consumer: Stack, reference: CfnReference): IResolvable {
  ...
  if (producer.node.root !== consumer.node.root) {
    throw new UnscopedValidationError('CannotCannotReferenceAcross', 'Cannot reference across apps. Consuming and producing stacks must be defined within the same CDK app.');
  }
...
export function referenceNestedStackValueInParent(reference: Reference, targetStack: Stack): Intrinsic {
  ...
  if (currentStack !== targetStack && !isNested(currentStack, targetStack)) {
    throw new UnscopedValidationError('MustBeReferencedResourceStack', `Referenced resource must be in stack '${targetStack.node.path}', got '${reference.target.node.path}'`);
  }

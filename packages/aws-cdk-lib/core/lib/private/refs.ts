...
function resolveValue(consumer: Stack, reference: CfnReference): IResolvable {
  ...
  if (producer.node.root !== consumer.node.root) {
    throw new UnscopedValidationError('ReferenceAcrossAppsConsuming', 'Cannot reference across apps. Consuming and producing stacks must be defined within the same CDK app.');
  }

  // unsupported: stacks are not in the same account
  if (producerAccount !== consumerAccount) {
    throw new UnscopedValidationError('StackConsumerNodePath', `Stack "${consumer.node.path}" cannot reference ${renderReference(reference)} in stack "${producer.node.path}". ` +
...
function createCrossRegionImportValue(reference: Reference, importStack: Stack): Intrinsic {
  ...
  if (Token.isUnresolved(exportName)) {
    throw new UnscopedValidationError('UnresolvedTokenGeneratedExport', `unresolved token in generated export name: ${JSON.stringify(exportingStack.resolve(exportName))}`);
  }
...
function createNestedStackParameter(nested: Stack, reference: CfnReference, value: IResolvable) {
  ...
  if (!param) {
    ...
    if (!('setParameter' in nested)) {
      throw new UnscopedValidationError('AssertionFailedNestedStack', 'assertion failed: nested stack should have a "setParameter" method');
    }
...
export function referenceNestedStackValueInParent(reference: Reference, targetStack: Stack): Intrinsic {
  ...
  if (currentStack !== targetStack && !isNested(currentStack, targetStack)) {
    throw new UnscopedValidationError('ReferencedResourceStackTargetstack', `Referenced resource must be in stack '${targetStack.node.path}', got '${reference.target.node.path}'`);
  }

...
export abstract class Resource extends Construct implements IResource {
  ...
  constructor(scope: Construct, id: string, props: ResourceProps = {}) {
    ...
    if ((props.account !== undefined || props.region !== undefined) && props.environmentFromArn !== undefined) {
      throw new ValidationError('SupplyMostOneAccount', `Supply at most one of 'account'/'region' (${props.account}/${props.region}) and 'environmentFromArn' (${props.environmentFromArn})`, this);
    }
  ...
  public _enableCrossEnvironment(): void {
    if (!this._allowCrossEnvironment) {
      ...
      throw new ValidationError('ResourceNodePathCross', `Cannot use resource '${this.node.path}' in a cross-environment fashion, ` +
  ...
  public applyRemovalPolicy(policy: RemovalPolicy) {
    ...
    if (!child || !CfnResource.isCfnResource(child)) {
      throw new ValidationError('ApplyRemovalpolicyChildCfnresource', 'Cannot apply RemovalPolicy: no child or not a CfnResource. Apply the removal policy on the CfnResource directly.', this);
    }

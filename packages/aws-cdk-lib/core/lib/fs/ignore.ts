...
import { CopyOptions, IgnoreMode, DockerIgnoreOptions } from './options';
...
export abstract class IgnoreStrategy {
   ...
   * @param absoluteRootPath the absolute path to the root directory of the paths to be considered
   */
  public static docker(absoluteRootPath: string, patterns: string[], options?: DockerIgnoreOptions): DockerIgnoreStrategy {
    return new DockerIgnoreStrategy(absoluteRootPath, patterns, options);
  }
  ...
  public static fromCopyOptions(options: CopyOptions, absoluteRootPath: string): IgnoreStrategy {
    ...
    switch (ignoreMode) {
      ...
      case IgnoreMode.DOCKER:
        return this.docker(absoluteRootPath, exclude, options.dockerIgnoreOptions);
    }
  }
...
export class DockerIgnoreStrategy extends IgnoreStrategy {
  ...
  constructor(absoluteRootPath: string, patterns: string[], options?: DockerIgnoreOptions) {
    super();
    ...
    this.absoluteRootPath = absoluteRootPath;
    if (options?.copyNestedExcludes ?? true) this.completeIgnore = dockerIgnore();
  ...
  private completelyAdd(pattern: string) {
    if (this.completeIgnore === undefined) return;
  ...
  public completelyIgnores(absoluteFilePath: string): boolean {
    ...
    return this.ignore.ignores(relativePath) && (this.completeIgnore === undefined || this.completeIgnore.ignores(relativePath));
  }
}

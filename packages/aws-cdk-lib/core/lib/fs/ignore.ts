...
export class GlobIgnoreStrategy extends IgnoreStrategy {
  ...
  public ignores(absoluteFilePath: string): boolean {
    if (!path.isAbsolute(absoluteFilePath)) {
      throw new UnscopedValidationError('GlobignorestrategyIgnoresExpectsAbsolute', 'GlobIgnoreStrategy.ignores() expects an absolute path');
    }
...
export class GitIgnoreStrategy extends IgnoreStrategy {
  ...
  public ignores(absoluteFilePath: string): boolean {
    if (!path.isAbsolute(absoluteFilePath)) {
      throw new UnscopedValidationError('GitignorestrategyIgnoresExpectsAbsolute', 'GitIgnoreStrategy.ignores() expects an absolute path');
    }
  ...
  public completelyIgnores(absoluteDirectoryPath: string): boolean {
    if (!path.isAbsolute(absoluteDirectoryPath)) {
      throw new UnscopedValidationError('GitignorestrategyCompletelyignoresExpectsAbsolute', 'GitIgnoreStrategy.completelyIgnores() expects an absolute path');
    }
...
export class DockerIgnoreStrategy extends IgnoreStrategy {
  ...
  private getRelativePath(absoluteFilePath: string): string {
    if (!path.isAbsolute(absoluteFilePath)) {
      throw new UnscopedValidationError('DockerignorestrategyIgnoresExpectsAbsolute', 'DockerIgnoreStrategy.ignores() expects an absolute path');
    }

    ...
    for (const mixin of mixins) {
      for (const construct of allConstructs) {
        if (mixin.supports(construct)) {
          ...
          throw new ValidationError('Mixincouldappliedrequired', `Mixin ${mixin.constructor.name} could not be applied to ${construct.constructor.name} but was required to.`, this.scope);
        }
      }
    }

    if (this.expectAny && applications.length <= 0) {
      throw new ValidationError('Leastmixinapplicationrequired', 'At least one mixin application was required, but none of the provided mixins could be applied to any selected construct.', this.scope);
    }

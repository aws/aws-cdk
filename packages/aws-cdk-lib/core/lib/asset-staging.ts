  ...
  switch (outputType) {
    ...
    case BundlingOutput.SINGLE_FILE:
      if (!archiveFile) {
        ...
        throw new ValidationError(lit`SymlinkInBundlingOutput`, 'The output from bundling is not allowed to be a symlink. Ensure that no logic has been added to your app which would cause the output of bundling to be a symlink.', scope);
      }

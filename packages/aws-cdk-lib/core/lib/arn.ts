...
export class Arn {
  ...
  public static format(components: ArnComponents, stack?: Stack): string {
    ...
    if (partition == null || region == null || account == null) {
      throw new UnscopedValidationError('ArnFormatPartitionPartition', `Arn.format: partition (${partition}), region (${region}), and account (${account}) must all be passed if stack is not passed.`);
    }
    ...
    if (sep !== '/' && sep !== ':' && sep !== '') {
      throw new UnscopedValidationError('ResourcepathsepEmptyString', 'resourcePathSep may only be ":", "/" or an empty string');
    }
  ...
  public static extractResourceName(arn: string, resourceType: string): string {
    ...
    if (!Token.isUnresolved(parsed.resource) && parsed.resource !== resourceType) {
      throw new UnscopedValidationError('ExpectedResourceTypeResourcetype', `Expected resource type '${resourceType}' in ARN, got '${parsed.resource}' in '${arn}'`);
    }
    if (!parsed.resourceName) {
      throw new UnscopedValidationError('ExpectedResourceNameArn', `Expected resource name in ARN, didn't find one: '${arn}'`);
    }
...
function parseArnShape(arn: string): 'token' | string[] {
  ...
  if (!looksLikeArn) {
    if (Token.isUnresolved(arn)) {
      ...
      throw new UnscopedValidationError('ArnsStartArnLeast', `ARNs must start with "arn:" and have at least 6 components: ${arn}`);
    }
  }
  ...
  if (!partition) {
    throw new UnscopedValidationError('PartitionComponent2ndComponent', 'The `partition` component (2nd component) of an ARN is required: ' + arn);
  }

  const service = components.length > 2 ? components[2] : undefined;
  if (!service) {
    throw new UnscopedValidationError('ServiceComponent3rdComponent', 'The `service` component (3rd component) of an ARN is required: ' + arn);
  }

  const resource = components.length > 5 ? components[5] : undefined;
  if (!resource) {
    throw new UnscopedValidationError('ResourceComponent6thComponent', 'The `resource` component (6th component) of an ARN is required: ' + arn);
  }

...
export class CfnMapping extends CfnRefElement {
    ...
    if (!Token.isUnresolved(key1)) {
      if (!(key1 in this.mapping)) {
        if (defaultValue === undefined) {
          throw new ValidationError('MappingDoesnTContainTopLevel', `Mapping doesn't contain top-level key '${key1}'`, this);
        } else {
          notInMap = true;
        }
        ...
        if (!(key2 in this.mapping[key1])) {
          if (defaultValue === undefined) {
            throw new ValidationError('MappingDoesnTContainSecondLevel', `Mapping doesn't contain second-level key '${key2}'`, this);
          } else {
            notInMap = true;
          }
        }
  ...
  private validateAlphanumeric(this: void, value: any) {
    ...
    if (value.match(/[^a-zA-Z0-9]/g)) {
      throw new UnscopedValidationError('AttributeNameContainOnly', `Attribute name '${value}' must contain only alphanumeric characters.`);
    }
  }
}

...
export class CfnMapping extends CfnRefElement {
  ...
  public setValue(key1: string, key2: string, value: any) {
    if ([key1, key2].some(k => ['__proto__', 'constructor'].includes(k))) {
      throw new ValidationError('ProtoConstructorKeys', 'Cannot use \'__proto__\' or \'constructor\' as keys', this);
    }
    ...
    if (!Token.isUnresolved(key1)) {
      if (!(key1 in this.mapping)) {
        if (defaultValue === undefined) {
          throw new ValidationError('MappingDoesnContainTop', `Mapping doesn't contain top-level key '${key1}'`, this);
        } else {
          notInMap = true;
        }
        ...
        if (!(key2 in this.mapping[key1])) {
          if (defaultValue === undefined) {
            throw new ValidationError('MappingDoesnContainSecond', `Mapping doesn't contain second-level key '${key2}'`, this);
          } else {
            notInMap = true;
          }
        }
  ...
  private validateAlphanumeric(this: void, value: any) {
    ...
    if (value.match(/[^a-zA-Z0-9]/g)) {
      throw new UnscopedValidationError('AttributeNameValueContain', `Attribute name '${value}' must contain only alphanumeric characters.`);
    }
  }
}

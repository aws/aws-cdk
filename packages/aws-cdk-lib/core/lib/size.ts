...
export class Size {
  ...
  private constructor(amount: number, unit: StorageUnit) {
    if (!Token.isUnresolved(amount) && amount < 0) {
      throw new UnscopedValidationError('StorageAmountsNegativeReceived', `Storage amounts cannot be negative. Received: ${amount}`);
    }
    this.amount = amount;
    this.unit = unit;
  }
...
function convert(amount: number, fromUnit: StorageUnit, toUnit: StorageUnit, options: SizeConversionOptions = {}) {
  ...
  if (Token.isUnresolved(amount)) {
    throw new UnscopedValidationError('SizeSpecifiedSizeTounit', `Size must be specified as 'Size.${toUnit}()' here since its value comes from a token and cannot be converted (got Size.${fromUnit})`);
  }

...
export class DefaultTokenResolver implements ITokenResolver {
  ...
  public resolveList(xs: string[], context: IResolveContext) {
    ...
    if (xs.length !== 1) {
      throw new UnscopedValidationError('CannotCannotElementsList', `Cannot add elements to list token, got: ${xs}`);
    }
    ...
    if (fragments.length !== 1) {
      throw new UnscopedValidationError('CannotCannotConcatenateStrings', `Cannot concatenate strings in a tokenized string array, got: ${xs[0]}`);
    }

...
export class Duration {
  ...
  public static parse(duration: string): Duration {
    ...
    if (!matches) {
      throw new UnscopedValidationError('ValidIsoDurationDuration', `Not a valid ISO duration: ${duration}`);
    }
  ...
  private constructor(amount: number, unit: TimeUnit) {
    if (!Token.isUnresolved(amount) && amount < 0) {
      throw new UnscopedValidationError('DurationAmountsNegativeReceived', `Duration amounts cannot be negative. Received: ${amount}`);
    }
...
function convert(amount: number, fromUnit: TimeUnit, toUnit: TimeUnit, { integral = true }: TimeConversionOptions) {
  if (fromUnit.inMillis === toUnit.inMillis) {
    if (integral && !Token.isUnresolved(amount) && !Number.isInteger(amount)) {
      throw new UnscopedValidationError('AmountWholeNumberTounit', `${amount} must be a whole number of ${toUnit}.`);
    }
    return amount;
  }
  if (Token.isUnresolved(amount)) {
    throw new UnscopedValidationError('DurationSpecifiedDurationTounit', `Duration must be specified as 'Duration.${toUnit}()' here since its value comes from a token and cannot be converted (got Duration.${fromUnit})`);
  }
  const value = (amount * fromUnit.inMillis) / toUnit.inMillis;
  if (!Number.isInteger(value) && integral) {
    throw new UnscopedValidationError('AmountFromunitConvertedInto', `'${amount} ${fromUnit}' cannot be converted into a whole number of ${toUnit}.`);
  }
  return value;
}

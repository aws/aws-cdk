  ...
  private doAddDockerImageAsset(asset: DockerImageAssetSource): DockerImageAssetLocation {
    ...
    if (!this.addedImageAssets.has(assetId)) {
      if (!asset.directoryName) {
        throw new UnscopedValidationError('LegacystacksynthesizerDoesSupportType', `LegacyStackSynthesizer does not support this type of file asset: ${JSON.stringify(asset)}`);
      }

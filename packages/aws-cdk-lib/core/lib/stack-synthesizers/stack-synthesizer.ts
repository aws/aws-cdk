...
export abstract class StackSynthesizer implements IStackSynthesizer {
  ...
  public bind(stack: Stack): void {
    if (this._boundStack !== undefined) {
      throw new ValidationError('Stacksynthesizeronlyusedstack', 'A StackSynthesizer can only be used for one Stack: create a new instance to use with a different Stack', stack);
    }
  ...
  protected get boundStack(): Stack {
    if (!this._boundStack) {
      throw new UnscopedValidationError('MustBeStacksynthesizerBoundStack', 'The StackSynthesizer must be bound to a Stack first before boundStack() can be called');
    }
...
function stackTemplateFileAsset(stack: Stack, session: ISynthesisSession): FileAssetSource {
  ...
  if (!fs.existsSync(templatePath)) {
    throw new ValidationError('Stacktemplatewrittenyet', `Stack template ${stack.stackName} not written yet: ${templatePath}`, stack);
  }

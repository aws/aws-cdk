...
export class NonIpInstance extends InstanceBase {
  ...
  constructor(scope: Construct, id: string, props: NonIpInstanceProps) {
    ...
    if (discoveryType !== DiscoveryType.API) {
      throw new ValidationError('Typeinstanceonlyregistered', 'This type of instance can only be registered for HTTP namespaces.', this);
    }

    if (props.customAttributes === undefined || Object.keys(props.customAttributes).length === 0) {
      throw new ValidationError('Specifyleastcustomattribute', 'You must specify at least one custom attribute for this instance type.', this);
    }

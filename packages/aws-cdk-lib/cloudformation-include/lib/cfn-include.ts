...
export class CfnInclude extends core.CfnElement {
  ...
  constructor(scope: Construct, id: string, props: CfnIncludeProps) {
    ...
    for (const logicalId of this.dehydratedResources) {
      if (!Object.keys(this.template.Resources).includes(logicalId)) {
        throw new core.ValidationError(lit`DehydratedResourceNotFound`, `Logical ID '${logicalId}' was specified in 'dehydratedResources', but does not belong to a resource in the template.`, this);
      }
    }
    ...
    for (const logicalId of Object.keys(this.parametersToReplace)) {
      if (!(logicalId in (this.template.Parameters || {}))) {
        throw new core.ValidationError(lit`ParameterNotFoundInTemplate`, `Parameter with logical ID '${logicalId}' was not found in the template`, this);
      }
    }
    ...
    for (const nestedStackId of Object.keys(props.loadNestedStacks || {})) {
      if (!(nestedStackId in this.resources)) {
        throw new core.ValidationError(lit`NestedStackNotFoundInTemplate`, `Nested Stack with logical ID '${nestedStackId}' was not found in the template`, this);
      }
    }
  ...
  public getResource(logicalId: string): core.CfnResource {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`ResourceNotFoundInTemplate`, `Resource with logical ID '${logicalId}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getCondition(conditionName: string): core.CfnCondition {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`ConditionNotFoundInTemplate`, `Condition with name '${conditionName}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getParameter(parameterName: string): core.CfnParameter {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`ParameterNotFoundInTemplate`, `Parameter with name '${parameterName}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getMapping(mappingName: string): core.CfnMapping {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`MappingNotFoundInTemplate`, `Mapping with name '${mappingName}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getOutput(logicalId: string): core.CfnOutput {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`OutputNotFoundInTemplate`, `Output with logical ID '${logicalId}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getRule(ruleName: string): core.CfnRule {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`RuleNotFoundInTemplate`, `Rule with name '${ruleName}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getHook(hookLogicalId: string): core.CfnHook {
    ...
    if (!ret) {
      throw new core.ValidationError(lit`HookNotFoundInTemplate`, `Hook with logical ID '${hookLogicalId}' was not found in the template`, this);
    }
    return ret;
  }
  ...
  public getNestedStack(logicalId: string): IncludedNestedStack {
    if (!this.nestedStacks[logicalId]) {
      if (!this.template.Resources[logicalId]) {
        throw new core.ValidationError(lit`NestedStackNotFoundInTemplate`, `Nested Stack with logical ID '${logicalId}' was not found in the template`, this);
      } else if (this.template.Resources[logicalId].Type !== 'AWS::CloudFormation::Stack') {
        throw new core.ValidationError(lit`ResourceNotCloudFormationStack`, `Resource with logical ID '${logicalId}' is not a CloudFormation Stack`, this);
      } else {
        throw new core.ValidationError(lit`NestedStackNotIncluded`, `Nested Stack '${logicalId}' was not included in the parent template. ` +
  ...
  public loadNestedStack(logicalId: string, nestedStackProps: CfnIncludeProps): IncludedNestedStack {
    if (logicalId in this.nestedStacks) {
      throw new core.ValidationError(lit`NestedStackAlreadyIncluded`, `Nested Stack '${logicalId}' was already included in its parent template`, this);
    }
    ...
    if (cfnStack instanceof core.CfnStack) {
      ...
      throw new core.ValidationError(lit`ResourceNotCloudFormationStack`, `Nested Stack with logical ID '${logicalId}' is not an AWS::CloudFormation::Stack resource`, this);
    }
  }
  ...
  private createMapping(mappingName: string): void {
    ...
    const cfnParser = new cfn_parse.CfnParser({
      finder: {
        findCondition() { throw new core.ValidationError(lit`ConditionsNotAllowedInMappings`, 'Referring to Conditions in Mapping definitions is not allowed', self); },
        findMapping() { throw new core.ValidationError(lit`MappingsNotAllowedInMappings`, 'Referring to other Mappings in Mapping definitions is not allowed', self); },
        findRefTarget() { throw new core.ValidationError(lit`RefNotAllowedInMappings`, 'Using Ref expressions in Mapping definitions is not allowed', self); },
        findResource() { throw new core.ValidationError(lit`GetAttNotAllowedInMappings`, 'Using GetAtt expressions in Mapping definitions is not allowed', self); },
      },
      parameters: {},
    });
  ...
  private createParameter(logicalId: string): void {
    ...
    const expression = new cfn_parse.CfnParser({
      finder: {
        findResource() { throw new core.ValidationError(lit`GetAttNotAllowedInParameters`, 'Using GetAtt expressions in Parameter definitions is not allowed', self); },
        findRefTarget() { throw new core.ValidationError(lit`RefNotAllowedInParameters`, 'Using Ref expressions in Parameter definitions is not allowed', self); },
        findCondition() { throw new core.ValidationError(lit`ConditionsNotAllowedInParameters`, 'Referring to Conditions in Parameter definitions is not allowed', self); },
        findMapping() { throw new core.ValidationError(lit`MappingsNotAllowedInParameters`, 'Referring to Mappings in Parameter definitions is not allowed', self); },
      },
      parameters: {},
  ...
  private createRule(ruleName: string): void {
    ...
    const cfnParser = new cfn_parse.CfnParser({
      finder: {
        ...
        findResource() { throw new core.ValidationError(lit`GetAttNotAllowedInRules`, 'Using GetAtt expressions in Rule definitions is not allowed', self); },
        findCondition(conditionName: string): core.CfnCondition | undefined {
          return self.conditions[conditionName];
        },
  ...
  private createOutput(logicalId: string, scope: Construct): void {
    ...
    const cfnOutput = new core.CfnOutput(scope, logicalId, {
      ...
      condition: (() => {
        ...
        throw new core.ValidationError(lit`OutputReferencesUnknownCondition`, `Output with name '${logicalId}' refers to a Condition with name ` +
  ...
  private getOrCreateCondition(conditionName: string): core.CfnCondition {
    ...
    const cfnParser = new cfn_parse.CfnParser({
      finder: {
        findResource() { throw new core.ValidationError(lit`GetAttNotAllowedInConditions`, 'Using GetAtt in Condition definitions is not allowed', self); },
        findRefTarget(elementName: string): core.CfnElement | undefined {
  ...
  private getOrCreateResource(logicalId: string, cycleChain: string[] = []): core.CfnResource {
    ...
    if (cycleChain.length !== new Set(cycleChain).size) {
      if (!this.allowCyclicalReferences) {
        throw new core.ValidationError(lit`CyclicalReferenceDetected`, `Found a cycle between resources in the template: ${cycleChain.join(' depends on ')}`, this);
      }
    ...
    if (this.nestedStacksToInclude[logicalId] && this.dehydratedResources.includes(logicalId)) {
      throw new core.ValidationError(lit`NestedStackCannotBeDehydrated`, `nested stack '${logicalId}' was marked as dehydrated - nested stacks cannot be dehydrated`, this);
  ...
  private createNestedStack(nestedStackId: string, cfnParser: cfn_parse.CfnParser): core.CfnResource {
    ...
    if (nestedStackAttributes.Type !== 'AWS::CloudFormation::Stack') {
      throw new core.ValidationError(lit`ResourceNotCloudFormationStack`, `Nested Stack with logical ID '${nestedStackId}' is not an AWS::CloudFormation::Stack resource`, this);
    }
    if (nestedStackAttributes.CreationPolicy) {
      throw new core.ValidationError(lit`CreationPolicyNotSupportedForNestedStack`, 'CreationPolicy is not supported by the AWS::CloudFormation::Stack resource', this);
    }
    if (nestedStackAttributes.UpdatePolicy) {
      throw new core.ValidationError(lit`UpdatePolicyNotSupportedForNestedStack`, 'UpdatePolicy is not supported by the AWS::CloudFormation::Stack resource', this);
    }

...
describe('CDK Include', () => {
  ...
  test('can ingest a template with only an empty S3 Bucket, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('only-empty-bucket.json'));
  });
  ...
  test('can ingest a template with only an S3 Bucket with complex properties, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('only-bucket-complex-props.json'));
  });
  ...
  test('can ingest a template with a Bucket Ref-erencing a KMS Key, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('bucket-with-encryption-key.json'));
  });
              ...
              ServerSideEncryptionConfiguration: [
                {
                  ServerSideEncryptionByDefault: {
                    KMSMasterKeyID: {
                      'Fn::GetAtt': ['MyScopeKey7673692F', 'Arn'],
                    },
  ...
  test('can ingest a template with an Fn::If expression for simple values, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('if-simple-property.json'));
  });
  ...
  test('can ingest a template with an Fn::If expression for complex values, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('if-complex-property.json'));
  });

  test('can ingest a template using Fn::If in Tags, and output it unchanged', () => {
    includeTestTemplate(stack, 'if-in-tags.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('if-in-tags.json'));
  });

  test('can ingest a UserData script, and output it unchanged', () => {
    includeTestTemplate(stack, 'user-data.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('user-data.json'));
  });
  ...
  test('can correctly ingest a resource with a property of type: Map of Lists of primitive types', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('ssm-association.json'));
  ...
  test('can ingest a template with intrinsic functions and conditions, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('functions-and-conditions.json'));
  });
  ...
  test('can ingest a template with fn:: intrinsic function used in deletion policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsic-deletion-policy.json'));
  });
  ...
  test('can ingest a template with ref intrinsic functions used in deletion policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsic-deletion-policy-ref.json'));
  });
  ...
  test('can ingest a JSON template with string-form Fn::GetAtt, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('get-att-string-form.json'));
  });
  ...
  test('can ingest a template with Fn::Sub in string form with escaped and unescaped references and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-string.json'));
  });
  ...
  test('can ingest a template with Fn::Sub using dotted attributes in map form and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-map-dotted-attributes.json'));
  });

  test('preserves an empty map passed to Fn::Sub', () => {
    includeTestTemplate(stack, 'fn-sub-map-empty.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-map-empty.json'));
  });
  ...
  test('can ingest a template with Fn::Sub shadowing a logical ID from the template and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-shadow.json'));
  });
  ...
  test('can ingest a template with Fn::Sub attribute expression shadowing a logical ID from the template, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-shadow-attribute.json'));
  });
  ...
  test('can ingest a template with Fn::Sub with brace edge cases and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-sub-brace-edges.json'));
  });
          ...
          Properties: {
            BucketName: {
              'Fn::Sub': [
                ...
                {
                  MyParam: {
                    'Fn::Join': ['', ["it's_a_", { Ref: 'AnotherParam' }, '_concatenation']],
                  },
                },
              ],
            },
  ...
  test('can ingest a template with a Ref expression for an array value, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('ref-array-property.json'));
  });

  test('renders non-Resources sections unchanged', () => {
    includeTestTemplate(stack, 'only-empty-bucket-with-parameters.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('only-empty-bucket-with-parameters.json'));
  });
  ...
  test('correctly parses Conditions and the Condition resource attribute', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('resource-attribute-condition.json'));
  });

  test('allows Conditions to reference Mappings', () => {
    includeTestTemplate(stack, 'condition-using-mapping.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('condition-using-mapping.json'));
  });
    ...
    Template.fromStack(stack).templateMatches({
      ...
      Resources: {
        AlwaysTrue: {
          ...
          Properties: {
            BucketName: {
              'Fn::If': ['TotallyFalse', { Ref: 'Param' }, { Ref: 'AWS::NoValue' }],
            },
          },
        },
      },
  ...
  test('Intrinsics can be used in the leaf nodes of autoscaling creation policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsics-create-policy-autoscaling.json'));
  });
  ...
  test('Nested intrinsics can be used in the leaf nodes of autoscaling creation policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsics-create-policy-complex.json'));
  });
  ...
  test('intrinsics can be used in the leaf nodes of autoscaling resource signal creation policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsics-create-policy-resource-signal.json'));
  });
  ...
  test('Nested Intrinsics can be used in the leaf nodes of autoscaling rolling update policy', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('intrinsics-update-policy-complex.json'));
  });

  test('preserves unknown resource attributes', () => {
    includeTestTemplate(stack, 'non-existent-resource-attribute.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('non-existent-resource-attribute.json'));
  });

  test('preserves unknown policy attributes', () => {
    const cfnTemplate = includeTestTemplate(stack, 'non-existent-policy-attribute.json');
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('non-existent-policy-attribute.json'));
  });
  ...
  test('correctly handles string arrays in policy attributes', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('string-arrays-in-policy.json'));
  });
  ...
  test('can include a template with a custom resource that uses attributes', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('custom-resource-with-attributes.json'));
  ...
  test('can ingest a template that contains outputs and modify them', () => {
    ...
    Template.fromStack(stack).templateMatches({
      Conditions: {
        ...
        MyCondition: {
          'Fn::If': ['AlwaysFalseCond', { Ref: 'AWS::NoValue' }, true],
        },
      },
      Parameters: {
        ...originalTemplate.Parameters,
  ...
  test('can ingest a template that contains outputs and get those outputs', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('outputs-with-references.json'));
  });
    ...
    Template.fromStack(stack).templateMatches({
      ...
      Resources: {
        Bucket: {
          ...
          Properties: {
            BucketName: {
              'Fn::FindInMap': ['SomeMapping', { Ref: 'AWS::Region' }, 'key1'],
            },
          },
        },
      },
  ...
  test('can ingest a template that uses Fn::FindInMap with the first argument being a dynamic reference', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('find-in-map-with-dynamic-mapping.json'));
  });
    ...
    Template.fromStack(stack).templateMatches({
      ...
      Resources: {
        Bucket: {
          ...
          Properties: {
            BucketName: {
              'Fn::FindInMap': ['DifferentMapping', { Ref: 'AWS::Region' }, 'key1'],
            },
          },
        },
      },
  ...
  test('can ingest a template that uses Fn::FindInMap for the value of a boolean property', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('find-in-map-for-boolean-property.json'));
  });
  ...
  test('can ingest a template that contains Rules, and allows retrieving those Rules', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('only-parameters-and-rule.json'));
  });
  ...
  test('can ingest a template that contains Hooks, and allows retrieving those Hooks', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('hook-code-deploy-blue-green-ecs.json'));
  });
  ...
  test('replaces references to parameters with the user-specified values in Resources, Conditions, Metadata, and Options sections', () => {
    ...
    Template.fromStack(stack).templateMatches({
      ...
      Metadata: {
        Field: {
          'Fn::If': ['AlwaysFalse', 'AWS::NoValue', 'my-s3-bucket'],
        },
      },
      Conditions: {
        AlwaysFalse: {
  ...
  test('can ingest a template that contains properties not in the current CFN spec, and output it unchanged', () => {
    ...
    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('properties-not-in-cfn-spec.json'));
  });

  test('roundtrip a fn-select with a fn-if/ref-novalue in it', () => {
    includeTestTemplate(stack, 'fn-select-with-novalue.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('fn-select-with-novalue.json'));
  });

  test('Fn::If can be used in Tags', () => {
    includeTestTemplate(stack, 'tags-with-intrinsics.json');

    Template.fromStack(stack).templateMatches(loadTestFileToJsObject('tags-with-intrinsics.json'));
  });
});
...
function includeTestTemplate(
  ...
  testTemplate: string,
  props: IncludeTestTemplateProps = {}
): inc.CfnInclude {

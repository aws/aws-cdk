...
export class Resolver extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ResolverProps) {
    ...
    if (pipelineConfig && props.dataSource) {
      throw new ValidationError('Pipelineresolvercannotdata', `Pipeline Resolver cannot have data source. Received: ${props.dataSource.name}`, scope);
    }

    if (props.cachingConfig?.ttl && (props.cachingConfig.ttl.toSeconds() < 1 || props.cachingConfig.ttl.toSeconds() > 3600)) {
      throw new ValidationError('Mustbecachingconfigbetween', `Caching config TTL must be between 1 and 3600 seconds. Received: ${props.cachingConfig.ttl.toSeconds()}`, scope);
    }
    ...
    if (props.cachingConfig?.cachingKeys) {
      if (props.cachingConfig.cachingKeys.find(cachingKey =>
        ...
        throw new ValidationError('Mustbecachingconfigkeys', `Caching config keys must begin with $context.arguments, $context.source or $context.identity. Received: ${props.cachingConfig.cachingKeys}`, scope);
      }
    }

...
describe('States Language', () => {
  test('A single task is a State Machine', () => {
    ...
    test('A sequence of two tasks is a State Machine', () => {
      ...
      const chain = stepfunctions.Chain.start(task1).next(task2);
    ...
    test('A chain can be appended to', () => {
      ...
      // WHEN
      const chain = stepfunctions.Chain.start(task1).next(task2).next(task3);
    ...
    test('A state machine can be appended to another state machine', () => {
      ...
      // WHEN
      const chain = stepfunctions.Chain.start(task1).next(stepfunctions.Chain.start(task2).next(task3));
    ...
    test('Chaining does not chain onto error handler, extended', () => {
      ...
      // WHEN
      const chain = task1.addCatch(errorHandler).next(task2.addCatch(errorHandler)).next(task3.addCatch(errorHandler));
    ...
    test('No duplicate state IDs even across Parallel branches', () => {
      ...
      const parallel = new stepfunctions.Parallel(stack, 'Parallel').branch(state1).branch(state2);
    ...
    test('No cross-parallel jumps', () => {
      ...
      const parallel = new stepfunctions.Parallel(stack, 'Parallel').branch(state1.next(state2)).branch(state2);

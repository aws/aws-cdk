...
describe('Fields', () => {
  ...
  test('infinitely recursive object graphs do not break referenced path finding', () => {
    ...
    expect(FieldUtils.findReferencedPaths(paths)).toStrictEqual(['$.listField', '$.numField', '$.stringField']);
  });

  test('rendering a non-object value should just return itself', () => {
    expect(FieldUtils.renderObject(TaskInput.fromText('Hello World').value)).toEqual('Hello World');
    expect(FieldUtils.renderObject('Hello World' as any)).toEqual('Hello World');

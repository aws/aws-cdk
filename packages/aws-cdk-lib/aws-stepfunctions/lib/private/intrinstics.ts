...
export class IntrinsicParser {
  ...
  private raiseError(message: string): never {
    throw new UnscopedValidationError('InvalidInvalidJsonpathExpression', `Invalid JSONPath expression: ${message} at index ${this.i} in ${JSON.stringify(this.expression)}`);
  }
}

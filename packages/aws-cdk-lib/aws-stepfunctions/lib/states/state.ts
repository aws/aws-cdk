...
export abstract class State extends Construct implements IChainable {
  ...
  public bindToGraph(graph: StateGraph) {
    ...
    if (this.containingGraph) {
      throw new UnscopedValidationError('TryingState', `Trying to use state '${this.stateId}' in ${graph}, but is already in ${this.containingGraph}. Every state can only be used in one graph.`);
    }
...
function validateErrors(errors?: string[]) {
  if (errors?.includes(Errors.ALL) && errors.length > 1) {
    throw new UnscopedValidationError('AppearAloneErrorList', `${Errors.ALL} must appear alone in an error list`);
  }
}

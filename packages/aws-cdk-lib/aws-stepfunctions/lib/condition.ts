...
class VariableComparison extends Condition {
  constructor(private readonly variable: string, private readonly comparisonOperator: ComparisonOperator, private readonly value: any) {
    super();
    if (!/^\$|(\$[.[])/.test(variable)) {
      throw new UnscopedValidationError('MustBeVariableReference', `Variable reference must be '$', start with '$.', or start with '$[', got '${variable}'`);
    }
  }
...
class CompoundCondition extends Condition {
  ...
  constructor(private readonly operator: CompoundOperator, ...conditions: Condition[]) {
    ...
    if (conditions.length === 0) {
      throw new UnscopedValidationError('SupplyLeastInnerCondition', 'Must supply at least one inner condition for a logical combination');
    }
  }
...
class JsonataCondition extends Condition {
  constructor(private readonly condition: string) {
    ...
    if (!isValidJsonataExpression(condition)) {
      throw new UnscopedValidationError('JsonataExpressionStart', `JSONata expression must start with '{%' and end with '%}', got '${condition}'`);
    }
  }

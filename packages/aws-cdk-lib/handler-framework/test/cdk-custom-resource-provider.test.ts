...
describe('cdk custom resource provider', () => {
  describe('customize roles', () => {
    test('role is not created if preventSynthesis!=false', () => {
      ...
      const handler = new CdkHandler(stack, 'Handler', {
        ...
        compatibleRuntimes: [STANDARD_PROVIDER],
      });

      // WHEN
      const cr = CdkCustomResourceProvider.getOrCreateProvider(stack, 'Custom:MyResourceType', { handler });
      cr.addToRolePolicy({
        Action: 's3:GetBucket',
        Effect: 'Allow',
  ...
  test('minimal configuration', () => {
    ...
    // WHEN
    CdkCustomResourceProvider.getOrCreate(stack, 'Custom:MyResourceType', { handler });
  ...
  test('policyStatements can be used to add statements to the inline policy', () => {
    ...
    // WHEN
    CdkCustomResourceProvider.getOrCreate(stack, 'Custom:MyResourceType', {
      handler,
      policyStatements: [
        { statement1: 123 },
        { statement2: { foo: 111 } },
  ...
  test('addToRolePolicy() can be used to add statements to the inline policy', () => {
    ...
    // WHEN
    const provider = CdkCustomResourceProvider.getOrCreateProvider(stack, 'Custom:MyResourceType', {
      handler,
      policyStatements: [
        { statement1: 123 },
        { statement2: { foo: 111 } },

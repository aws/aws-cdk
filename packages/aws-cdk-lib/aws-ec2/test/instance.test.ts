...
describe('instance', () => {
  ...
  describe('blockDeviceMappings', () => {
    ...
    test('throws if throughput is specified for an EBS volume', () => {
      // THEN
      expect(() => {
        new Instance(stack, 'Instance', {
          vpc,
      ...
      }).toThrow(/The throughput property is not supported on EC2 instances. Use a Launch Template instead/);
    });

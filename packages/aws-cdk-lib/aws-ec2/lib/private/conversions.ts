...
export function asNetworkAcl(x: INetworkAclRef, scope: IConstruct): INetworkAcl {
  ...
  throw new ValidationError('ProvidedNetworkAclInstanceNetwork', `Provided networkAcl is not an instance of INetworkAcl: ${x.constructor.name}`, scope);
}
...
export function asSubnet(x: ISubnetRef, scope: IConstruct): ISubnet {
  ...
  throw new ValidationError('ProvidedSubnetInstanceSubnet', `Provided subnet is not an instance of ISubnet: ${x.constructor.name}`, scope);
}

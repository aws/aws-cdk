...
export class CloudFormationInit {
  ...
  public addConfigSet(configSetName: string, configNames: string[] = []) {
    ...
    if (unk.length > 0) {
      throw new UnscopedValidationError('UnknownUnknownConfigsReferenced', `Unknown configs referenced in definition of '${configSetName}': ${unk}`);
    }
  ...
  public attach(attachedResource: CfnResource, attachOptions: AttachInitOptions) {
    if (attachOptions.platform === OperatingSystemType.UNKNOWN) {
      throw new ValidationError('CannotCannotcannotattachcloudformationinit', 'Cannot attach CloudFormationInit to an unknown OS type', attachedResource);
    }

    const CFN_INIT_METADATA_KEY = 'AWS::CloudFormation::Init';

    if (attachedResource.getMetadata(CFN_INIT_METADATA_KEY) !== undefined) {
      throw new ValidationError('CannotCannotBindCfninit', `Cannot bind CfnInit: resource '${attachedResource.node.path}' already has '${CFN_INIT_METADATA_KEY}' attached`, attachedResource);
    }
...
export class InitConfig {
  ...
  private initPlatformFromOSType(osType: OperatingSystemType): InitPlatform {
    switch (osType) {
      ...
      default: {
        throw new UnscopedValidationError('CannotCannotAttachCloudformationinit', 'Cannot attach CloudFormationInit to an unknown OS type');
      }
    }
  }
}

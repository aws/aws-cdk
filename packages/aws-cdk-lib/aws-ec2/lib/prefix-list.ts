...
export class PrefixList extends PrefixListBase {
  ...
  public static fromLookup(scope: Construct, id: string, options: PrefixListLookupOptions): IPrefixList {
    if (Token.isUnresolved(options.prefixListName)) {
      throw new ValidationError('Mustbeargumentslookmanaged', 'All arguments to look up a managed prefix list must be concrete (no Tokens)', scope);
    }
    ...
    if (response.length !== 1) {
      throw new ValidationError('UnexpectedUnexpectedResponseReceived', 'Unexpected response received from the context provider. Please clear out the context key using `cdk context --remove` and try again.', scope);
    }
  ...
  constructor(scope: Construct, id: string, props?: PrefixListProps) {
    ...
    if (props?.prefixListName) {
      ...
      if (props.prefixListName.length > 255 ) {
        throw new ValidationError('Lengthsexceedingcharacterscannot', 'Lengths exceeding 255 characters cannot be set.', this);
      }
    }
      ...
      if (props?.addressFamily === AddressFamily.IP_V6) {
        ...
        for (const entry of entries) {
          if (!ipv6Regex.test(entry.cidr)) {
            throw new ValidationError('InvalidInvalidinvalidipv6address', `Invalid IPv6 address range: ${entry.cidr}`, this);
          }
        }
      // Regular expressions for validating IPv4 addresses
        ...
        for (const entry of entries) {
          if (!ipv4Regex.test(entry.cidr)) {
            throw new ValidationError('InvalidInvalidinvalidipv4address', `Invalid IPv4 address range: ${entry.cidr}`, this);
          }
        }
      }

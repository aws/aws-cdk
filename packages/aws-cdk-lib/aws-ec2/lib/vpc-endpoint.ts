...
export class InterfaceVpcEndpoint extends VpcEndpoint implements IInterfaceVpcEndpoint {
  ...
  private endpointSubnets(props: InterfaceVpcEndpointProps) {
    ...
    if (!subnetSelection.isPendingLookup && subnetSelection.subnets.length == 0) {
      throw new ValidationError('CannotCannotcannotcreateendpoint', 'Cannot create a VPC Endpoint with no subnets', this);
    }
    ...
    if (filteredSubnets.length == 0) {
      throw new ValidationError('Lookupsupportedazsreturnedsubnets', `lookupSupportedAzs returned ${availableAZs} but subnets have AZs ${subnets.map(s => s.availabilityZone)}`, this);
    }
  ...
  private validateCanLookupSupportedAzs(subnets: ISubnet[], serviceName: string) {
    ...
    if (agnosticAcct || agnosticRegion) {
      throw new ValidationError('CannotCannotcannotlookendpoint', 'Cannot look up VPC endpoint availability zones if account/region are not specified', this);
    }

    // The AWS call will fail if there is a Token in the service name
    if (agnosticService) {
      throw new ValidationError('CannotCannotcannotlookupservice', `Cannot lookup AZs for a service name with a Token: ${serviceName}`, this);
    }
    ...
    if (agnosticSubnets || agnosticSubnetList) {
      ...
      throw new ValidationError('Lookupsupportedazscannotfiltersubnets', `lookupSupportedAzs cannot filter on subnets with Token AZs: ${agnostic}`, this);
    }
  }
  ...
  private availableAvailabilityZones(serviceName: string): string[] {
    ...
    if (!Array.isArray(availableAZs)) {
      throw new ValidationError('Mustbediscoveredendpointservice', `Discovered AZs for endpoint service ${serviceName} must be an array`, this);
    }
    return availableAZs;
  }
}

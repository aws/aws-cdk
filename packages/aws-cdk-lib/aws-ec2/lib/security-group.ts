  ...
  public addEgressRule(peer: IPeer, connection: Port, description?: string, remoteRule?: boolean) {
    ...
    if (!isIpv6 && this.allowAllOutbound) {
      ...
      if (!remoteRule) { // Warn only if addEgressRule() was explicitely called
        Annotations.of(this).addWarningV2('EC2:SG:Ipv4IgnoreEgressRule', 'Ignoring Egress rule since \'allowAllOutbound\' is set to true; To add customized rules, set allowAllOutbound=false on the SecurityGroup');
      }
      return;
    ...
    if (isIpv6 && this.allowAllIpv6Outbound) {
      ...
      // rule.
      if (!remoteRule) { // Warn only if addEgressRule() was explicitely called
        Annotations.of(this).addWarningV2('EC2:SG:Ipv6IgnoreEgressRule', 'Ignoring Egress rule since \'allowAllIpv6Outbound\' is set to true; To add customized rules, set allowAllIpv6Outbound=false on the SecurityGroup');
      }
      return;
    }

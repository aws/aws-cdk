...
export class LaunchTemplate extends Resource implements ILaunchTemplate, iam.IGrantable, IConnectable {
  ...
  public static fromLaunchTemplateAttributes(scope: Construct, id: string, attrs: LaunchTemplateAttributes): ILaunchTemplate {
    ...
    if (haveId == haveName) {
      throw new ValidationError('Launchtemplate', 'LaunchTemplate.fromLaunchTemplateAttributes() requires exactly one of launchTemplateId or launchTemplateName be provided.', scope);
    }
    ...
    class Import extends Resource implements ILaunchTemplate {
      ...
      public get launchTemplateRef(): LaunchTemplateReference {
        if (!this.launchTemplateId) {
          throw new ValidationError('Launchtemplateidlaunchtemplate', 'You must set launchTemplateId in LaunchTemplate.fromLaunchTemplateAttributes() in order to use the LaunchTemplate in this API', this);
        }
  ...
  constructor(scope: Construct, id: string, props: LaunchTemplateProps = {}) {
    ...
    if (props.instanceProfile && props.role) {
      throw new ValidationError('CannotCannotprovidebothinstanceprofile', 'You cannot provide both an instanceProfile and a role', this);
    }
    ...
    if (props.versionDescription && !Token.isUnresolved(props.versionDescription) && props.versionDescription.length > 255) {
      throw new ValidationError('Mustbeversiondescriptionlessthan', `versionDescription must be less than or equal to 255 characters, got ${props.versionDescription.length}`, this);
    }
  ...
  public addSecurityGroup(securityGroup: ISecurityGroup): void {
    if (!this._connections) {
      throw new ValidationError('Launchtemplateonlyaddedsecuritygroup', 'LaunchTemplate can only be added a securityGroup if another securityGroup is initialized in the constructor.', this);
    }
  ...
  public get connections(): Connections {
    if (!this._connections) {
      throw new ValidationError('Launchtemplateonlyusediconnectable', 'LaunchTemplate can only be used as IConnectable if a securityGroup is provided when constructing it.', this);
    }
  ...
  public get grantPrincipal(): iam.IPrincipal {
    if (!this._grantPrincipal) {
      throw new ValidationError('Launchtemplateonlyusedigrantable', 'LaunchTemplate can only be used as IGrantable if a role is provided when constructing it.', this);
    }

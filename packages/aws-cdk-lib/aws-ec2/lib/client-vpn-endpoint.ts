...
export class ClientVpnEndpoint extends Resource implements IClientVpnEndpoint {
  ...
  constructor(scope: Construct, id: string, props: ClientVpnEndpointProps) {
    ...
    if (props.dnsServers && props.dnsServers.length > 2) {
      throw new ValidationError('Clientendpointservers', 'A client VPN endpoint can have up to two DNS servers', this);
    }

    if (props.logging == false && (props.logGroup || props.logStream)) {
      throw new ValidationError('CannotCannotSpecifyLogging', 'Cannot specify `logGroup` or `logStream` when logging is disabled', this);
    }
    ...
    if (props.clientConnectionHandler
      ...
      throw new ValidationError('MustBeNameLambdaFunction', 'The name of the Lambda function must begin with the `AWSClientVPN-` prefix', this);
    }
    ...
    if (Token.isUnresolved(subnetIds)) {
      throw new ValidationError('CannotCannotcannotassociatesubnets', 'Cannot associate subnets when VPC are imported from parameters or exports containing lists of subnet IDs.', this);
    }
...
function renderAuthenticationOptions(
  ...
  if (authenticationOptions.length === 0) {
    throw new UnscopedValidationError('ClientEndpointLeastAuthentication', 'A client VPN endpoint must use at least one authentication option');
  }

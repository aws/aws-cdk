...
export abstract class InitFile extends InitElement {
  ...
  public static fromString(fileName: string, content: string, options: InitFileOptions = {}): InitFile {
    if (!content) {
      throw new UnscopedValidationError('InitfileCannotCreateEmpty', `InitFile ${fileName}: cannot create empty file. Please supply at least one character of content.`);
    }
  ...
  public static symlink(fileName: string, target: string, options: InitFileOptions = {}): InitFile {
    ...
    if (mode && mode.slice(0, 3) !== '120') {
      throw new UnscopedValidationError('MustBeFileModeSymlinks', 'File mode for symlinks must begin with 120XXX');
    }
  ...
  protected _standardConfig(fileOptions: InitFileOptions, platform: InitPlatform, contentVars: Record<string, any>): Record<string, any> {
    if (platform === InitPlatform.WINDOWS) {
      if (fileOptions.group || fileOptions.owner || fileOptions.mode) {
        throw new UnscopedValidationError('Owner', 'Owner, group, and mode options not supported for Windows.');
      }
      return {
...
export class InitPackage extends InitElement {
  ...
  public _bind(options: InitBindOptions): InitElementConfig {
    if ((this.type === 'msi') !== (options.platform === InitPlatform.WINDOWS)) {
      if (this.type === 'msi') {
        throw new UnscopedValidationError('InstallersOnlySupportedWindows', 'MSI installers are only supported on Windows systems.');
      } else {
        throw new UnscopedValidationError('WindowsOnlySupportsPackage', 'Windows only supports the MSI package type');
      }
    }

    if (!this.packageName && !['rpm', 'msi'].includes(this.type)) {
      throw new UnscopedValidationError('MustBePackageNameSpecified', 'Package name must be specified for all package types besides RPM and MSI.');
    }

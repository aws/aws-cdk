...
abstract class VpcBase extends Resource implements IVpc {
  ...
  public enableVpnGateway(options: EnableVpnGatewayOptions): void {
    if (this.vpnGatewayId) {
      throw new ValidationError('VpnGatewayAlreadyEnabled', 'The VPN Gateway has already been enabled.', this);
    }
  ...
  private selectSubnetObjectsByName(groupName: string) {
    ...
    if (subnets.length === 0 && !this.incompleteSubnetDefinition) {
      ...
      throw new ValidationError('ThereSubnetGroupsName', `There are no subnet groups with name '${groupName}' in this VPC. Available names: ${names}`, this);
    }
  ...
  private selectSubnetObjectsByType(subnetType: SubnetType) {
    ...
    if (subnets.length === 0 && !this.incompleteSubnetDefinition) {
      ...
      throw new ValidationError('ThereSubnettypeSubnetGroups', `There are no '${subnetType}' subnet groups in this VPC. Available types: ${availableTypes}`, this);
    }
  ...
  private reifySelectionDefaults(placement: SubnetSelection): SubnetSelection {
    if (placement.subnetName !== undefined) {
      if (placement.subnetGroupName !== undefined) {
        throw new ValidationError('PleaseSubnetgroupnameSubnetnameDeprecated', 'Please use only \'subnetGroupName\' (\'subnetName\' is deprecated and has the same behavior)', this);
      } else {
    ...
    if (providedSelections.length > 1) {
      throw new ValidationError('OneProvidedselectionsSuppliedSubnet', `Only one of '${providedSelections}' can be supplied to subnet selection.`, this);
    }
...
export class Vpc extends VpcBase {
  ...
  public static fromLookup(scope: Construct, id: string, options: VpcLookupOptions): IVpc {
    if (Token.isUnresolved(options.vpcId)
      ...
      throw new ValidationError('ArgumentsVpcFromlookupConcrete', 'All arguments to Vpc.fromLookup() must be concrete (no Tokens)', scope);
    }
  ...
  constructor(scope: Construct, id: string, props: VpcProps = {}) {
    ...
    if (props.enableDnsHostnames && !props.enableDnsSupport) {
      throw new ValidationError('DnsHostnamesDnsSupport', 'To use DNS Hostnames, DNS Support must be enabled, however, it was explicitly disabled.', this);
    }

    if (props.availabilityZones && props.maxAzs) {
      throw new ValidationError('VpcSupportsAvailabilityzonesMaxazs', 'Vpc supports \'availabilityZones\' or \'maxAzs\', but not both.', this);
    }

    const cidrBlock = ifUndefined(props.cidr, Vpc.DEFAULT_CIDR_RANGE);
    if (Token.isUnresolved(cidrBlock)) {
      throw new ValidationError('CidrPropertyConcreteCidr', '\'cidr\' property must be a concrete CIDR string, got a Token (we need to parse it for automatic subdivision)', this);
    }

    if (props.ipAddresses && props.cidr) {
      throw new ValidationError('SupplyMostOneIpaddresses', 'supply at most one of ipAddresses or cidr', this);
    }
    ...
    if (!this.useIpv6) {
      for (const prop of ipv6OnlyProps) {
        if (props[prop] !== undefined) {
          throw new ValidationError('PropIpv6EnabledIpprotocol', `${prop} can only be set if IPv6 is enabled. Set ipProtocol to DUAL_STACK`, this);
        }
      }
    }
    ...
    if (props.availabilityZones) {
      ...
      if (!areGivenAzsSubsetOfStack) {
        throw new ValidationError('GivenVpcAvailabilityzonesProps', `Given VPC 'availabilityZones' ${props.availabilityZones} must be a subset of the stack's availability zones ${resolvedStackAzs}`, this);
      }
    ...
    if (props.vpnGateway && this.publicSubnets.length === 0 && this.privateSubnets.length === 0 && this.isolatedSubnets.length === 0) {
      throw new ValidationError('EnableVpnGatewayWhile', 'Can not enable the VPN gateway while the VPC has no subnets at all', this);
    }

    if ((props.vpnConnections || props.vpnGatewayAsn) && props.vpnGateway === false) {
      throw new ValidationError('SpecifyVpnconnectionsVpngatewayasnVpngateway', 'Cannot specify `vpnConnections` or `vpnGatewayAsn` when `vpnGateway` is set to false.', this);
    }
  ...
  private createNatGateways(provider: NatProvider, natCount: number, placement: SubnetSelection): void {
    ...
    for (const sub of natSubnets) {
      if (this.publicSubnets.indexOf(sub) === -1) {
        throw new ValidationError('NatgatewayplacementPlacementContainsNon', `natGatewayPlacement ${placement} contains non public subnet ${sub}`, this);
      }
    }
  ...
  private createSubnets() {
    ...
    if (allocatedSubnets.length != requestedSubnets.length) {
      throw new ValidationError('IncompleteSubnetAllocationResponse', 'Incomplete Subnet Allocation; response array dose not equal input array', this);
    }

    if (this.useIpv6) {
      if (this.ipv6SelectedCidr === undefined) {
        throw new ValidationError('Ipv6CidrBlockAssociated', 'No IPv6 CIDR block associated with this VPC could be found', this);
      }
      if (this.ipv6Addresses === undefined) {
        throw new ValidationError('Ipv6IpaddressesFound', 'No IPv6 IpAddresses were found', this);
      }
  ...
  private calculateMapPublicIpOnLaunch(subnetConfig: SubnetConfiguration) {
    if (subnetConfig.subnetType === SubnetType.PUBLIC) {
      ...
      if (subnetConfig.mapPublicIpOnLaunch !== undefined) {
        throw new ValidationError('SubnetconfigSubnettypeSubnetInclude', `${subnetConfig.subnetType} subnet cannot include mapPublicIpOnLaunch parameter`, this);
      }
      return false;
    }
  }
    ...
    allocatedSubnets.forEach((allocated, i) => {
      ...
      switch (subnetConfig.subnetType) {
        ...
        case SubnetType.ISOLATED:
          ...
          break;
        default:
          throw new ValidationError('UnrecognizedSubnetTypeSubnetconfig', `Unrecognized subnet type: ${subnetConfig.subnetType}`, this);
      }
...
export class Subnet extends Resource implements ISubnet {
  ...
  public addRoute(id: string, options: AddRouteOptions) {
    if (options.destinationCidrBlock && options.destinationIpv6CidrBlock) {
      throw new ValidationError('SpecifyDestinationcidrblockDestinationipv6cidrblock', 'Cannot specify both \'destinationCidrBlock\' and \'destinationIpv6CidrBlock\'', this);
    }
...
class ImportedVpc extends VpcBase {
  ...
  public get vpcCidrBlock(): string {
    if (this.cidr === undefined) {
      throw new ValidationError('PerformOperationVpccidrblockSupplied', 'Cannot perform this operation: \'vpcCidrBlock\' was not supplied when creating this VPC', this);
    }
    return this.cidr;
  }
}
...
class LookedUpVpc extends VpcBase {
  ...
  public get vpcCidrBlock(): string {
    if (this.cidr === undefined) {
      ...
      throw new ValidationError('PerformOperationVpccidrblockFound', 'Cannot perform this operation: \'vpcCidrBlock\' was not found when looking up this VPC. Use a newer version of the CDK CLI and clear the old context value.', this);
    }
    return this.cidr;
  }
...
class ImportedSubnet extends Resource implements ISubnet, IPublicSubnet, IPrivateSubnet {
  ...
  public get availabilityZone(): string {
    if (!this._availabilityZone) {
      throw new ValidationError('ReferenceSubnetAvailabilityZone', 'You cannot reference a Subnet\'s availability zone if it was not supplied. Add the availabilityZone when importing using Subnet.fromSubnetAttributes()', this);
    }
  ...
  public get ipv4CidrBlock(): string {
    if (!this._ipv4CidrBlock) {
      ...
      throw new ValidationError('ReferenceImportedSubnetIpv4', 'You cannot reference an imported Subnet\'s IPv4 CIDR if it was not supplied. Add the ipv4CidrBlock when importing using Subnet.fromSubnetAttributes()', this);
    }
...
function determineNatGatewayCount(requestedCount: number | undefined, subnetConfig: SubnetConfiguration[], azCount: number) {
  ...
  if (count === 0 && hasPrivateSubnets && !hasCustomEgress) {
    throw new UnscopedValidationError('WantNatGatewaysNatgateways', 'If you do not want NAT gateways (natGateways=0), make sure you don\'t configure any PRIVATE(_WITH_NAT) subnets in \'subnetConfiguration\' (make them PUBLIC or ISOLATED instead)');
  }

  if (count > 0 && !hasPublicSubnets) {
    throw new UnscopedValidationError('ConfigurePrivateSubnetsSubnetconfiguration', `If you configure PRIVATE subnets in 'subnetConfiguration', you must also configure PUBLIC subnets to put the NAT gateways into (got ${JSON.stringify(subnetConfig)}.`);
  }

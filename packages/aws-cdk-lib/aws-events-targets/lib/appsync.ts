...
export class AppSync implements events.IRuleTarget {
  ...
  public bind(rule: events.IRuleRef, _id?: string): events.RuleTargetConfig {
    ...
    if (!this.appsyncApi.modes.includes(appsync.AuthorizationType.IAM)) {
      throw new ValidationError('Awsiamauthorizationmodeenabled', 'You must have AWS_IAM authorization mode enabled on your API to configure an AppSync target', rule);
    }

    // make sure this is a 'public' (i.e.: 'GLOBAL') API
    if (this.appsyncApi.visibility !== appsync.Visibility.GLOBAL) {
      throw new ValidationError('MustBeYourVisibilityGlobal', 'Your API visibility must be "GLOBAL"', rule);
    }

    // make sure the EndpointArn is not blank
    if (this.appsyncApi.graphQLEndpointArn === '') {
      throw new ValidationError('Valid', 'You must have a valid `graphQLEndpointArn` set', rule);
    }

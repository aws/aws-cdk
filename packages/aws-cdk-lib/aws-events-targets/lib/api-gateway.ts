...
export class ApiGateway implements events.IRuleTarget {
  ...
  public get restApi(): api.RestApi {
    if (!api.RestApi.isRestApi(this._restApi)) {
      throw new ValidationError('Irestapirestapiconstruct', 'The iRestApi is not a RestApi construct, and cannot be retrieved this way.', this._restApi);
    }
  ...
  public bind(rule: events.IRuleRef, _id?: string): events.RuleTargetConfig { // FIXABLE
    ...
    if (wildcardCountsInPath !== (this.props?.pathParameterValues || []).length) {
      throw new ValidationError('Numberwildcardspathdoes', 'The number of wildcards in the path does not match the number of path pathParameterValues.', rule);
    }

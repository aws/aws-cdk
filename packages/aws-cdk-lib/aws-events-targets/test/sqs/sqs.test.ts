...
test('warns when using imported KMS key', () => {
  ...
  addSqsQueueTarget(stack, { encryptionMasterKey: importedKey });
...
test('does not warn when using CDK-managed KMS key', () => {
  ...
  addSqsQueueTarget(stack, { encryptionMasterKey: key });
...
test('does not warn when queue has no encryption', () => {
  const stack = new Stack();
  addSqsQueueTarget(stack);

  Annotations.fromStack(stack).hasNoWarning('/Default/MyRule', Match.anyValue());
});

function addSqsQueueTarget(stack: Stack, queueProps?: sqs.QueueProps): void {
  const queue = new sqs.Queue(stack, 'MyQueue', queueProps);
  const rule = new events.Rule(stack, 'MyRule', {
    schedule: events.Schedule.rate(Duration.hours(1)),
  });
  rule.addTarget(new targets.SqsQueue(queue));

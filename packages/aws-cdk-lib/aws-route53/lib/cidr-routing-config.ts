...
export class CidrRoutingConfig {
  ...
  private constructor(props: CidrRoutingConfigProps) {
    ...
    if (!Token.isUnresolved(props.collectionId) && !COLLECTION_ID_REGEX.test(props.collectionId)) {
      throw new UnscopedValidationError(lit`IsRequiredCollectionidRequiredValid`, `collectionId(${props.collectionId}) is required and must be a valid UUID in the format: xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx(8-4-4-4-12 hexadecimal digits)`);
    }
    if (!Token.isUnresolved(props.locationName) && (!props.locationName || !LOCATION_NAME_REGEX.test(props.locationName))) {
      throw new UnscopedValidationError(lit`IsRequiredLocationnameRequired116`, `locationName(${props.locationName}) is required and must be 1-16 characters long, containing only letters, numbers, underscores, hyphens, or asterisks`);
    }

...
export function validateZoneName(zoneName: string) {
  if (zoneName.length > 255) {
    throw new ValidationError('ZoneNameTooLong', 'zone name cannot be more than 255 bytes long');
  }
  if (zoneName.split('.').find(label => label.length > 63)) {
    throw new ValidationError('ZoneLabelTooLong', 'zone name labels cannot be more than 63 bytes long');
  }
  if (!zoneName.match(/^[a-z0-9!"#$%&'()*+,/:;<=>?@[\\\]^_`{|}~.-]+$/i)) {
    throw new ValidationError('InvalidZoneNameCharacters', 'zone names can only contain a-z, 0-9, -, ! " # $ % & \' ( ) * + , - / : ; < = > ? @ [ \ ] ^ _ ` { | } ~ .');
  }
}

  ...
  constructor(scope: Construct, id: string, props: EcsDeploymentGroupProps) {
    ...
    if (cdk.Resource.isOwnedResource(props.service)) {
      ...
      if (cfnSvc.deploymentController === undefined ||
        ...
        throw new ValidationError('EcsServiceAssociatedDeployment', 'The ECS service associated with the deployment group must use the CODE_DEPLOY deployment controller type', this);
      }

      if (cfnSvc.taskDefinition !== (props.service as ecs.BaseService).taskDefinition.family) {
        throw new ValidationError('EcsServiceAssociatedDeployment', 'The ECS service associated with the deployment group must specify the task definition using the task definition family name only. Otherwise, the task definition cannot be updated in the stack', this);
      }
    }

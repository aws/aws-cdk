...
export class Lambda implements ses.IReceiptRuleAction {
  ...
  public bind(rule: ses.IReceiptRule): ses.ReceiptRuleActionConfig {
    ...
    if (permission) { // The Lambda could be imported
      ...
      cdk.Annotations.of(rule).addWarningV2('SESActions:Lambda:AddInvokePermissions', 'This rule is using a Lambda action with an imported function. Ensure permission is given to SES to invoke that function.');
    }

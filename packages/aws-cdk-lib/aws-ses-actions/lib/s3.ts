...
export class S3 implements ses.IReceiptRuleAction {
  ...
  public bind(rule: ses.IReceiptRule): ses.ReceiptRuleActionConfig {
    ...
    if (policy) { // The bucket could be imported
      ...
      cdk.Annotations.of(rule).addWarningV2('SESActions:S3:AddBucketPermissions', 'This rule is using a S3 action with an imported bucket. Ensure permission is given to SES to write to that bucket.');
    }

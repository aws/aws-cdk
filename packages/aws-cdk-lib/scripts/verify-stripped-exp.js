...
async function main(tempDir) {
  ...
  try {
    ...
    if (invalidCfnModules.size > 0 || invalidModules.length > 0) {
      console.error('One or more experimental modules includes unexpected files. Experimental modules should only include autogenerated files.');
...
main(tempDir).then(
  ...
  (err) => {
    ...
    process.stderr.write(`Verification failed. Inspect working directory: '${tempDir}'`);
    process.exit(1);
  },
);

...
class FunctionUrlOriginWithOAC extends cloudfront.OriginBase {
  ...
  private validateAuthType(scope: Construct) {
    ...
    if (isAlwaysSigning && isAuthTypeIsNone) {
      throw new cdk.ValidationError('AuthtypeFunctionUrlAws', 'The authType of the Function URL must be set to AWS_IAM when origin access control signing method is SIGV4_ALWAYS.', scope);
    }
  }
}

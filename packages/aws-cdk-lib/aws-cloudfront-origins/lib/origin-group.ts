...
export class OriginGroup implements cloudfront.IOrigin {
  ...
  public bind(scope: Construct, options: cloudfront.OriginBindOptions): cloudfront.OriginBindConfig {
    ...
    if (primaryOriginConfig.failoverConfig) {
      throw new ValidationError('Origingroupcannotoriginfailover', 'An OriginGroup cannot use an Origin with its own failover configuration as its primary origin!', scope);
    }

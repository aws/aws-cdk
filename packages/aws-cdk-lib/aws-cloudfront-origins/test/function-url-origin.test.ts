...
test('Correctly configures CloudFront Distribution with Origin Access Identity (OAI)', () => {
  const fn = new lambda.Function(stack, 'MyFunction', {
    code: lambda.Code.fromInline('exports.handler = async () => {};'),
    handler: 'index.handler',
    runtime: lambda.Runtime.NODEJS_20_X,
  ...
  // Set up Distribution with OAI for Lambda Function URL
  ...
  new cloudfront.Distribution(stack, 'MyDistribution', {
    defaultBehavior: {
      origin: FunctionUrlOrigin.withOriginAccessIdentity(fnUrl, {
  ...
  // Validate that the CloudFront distribution is correctly configured with OAI
  template.hasResourceProperties('AWS::CloudFront::Distribution', {
    DistributionConfig: {
      Origins: Match.arrayWith([
        Match.objectLike({
  ...
  // Validate the OAI resource itself
...
test('Correctly configures CloudFront Distribution with automatic Origin Access Identity (OAI)', () => {
  const fn = new lambda.Function(stack, 'MyFunction', {
    code: lambda.Code.fromInline('exports.handler = async () => {};'),
    handler: 'index.handler',
    runtime: lambda.Runtime.NODEJS_20_X,
  ...
  // Set up Distribution with automatic OAI for Lambda Function URL
  new cloudfront.Distribution(stack, 'MyDistribution', {
    defaultBehavior: {
      origin: FunctionUrlOrigin.withOriginAccessIdentity(fnUrl),
    },
  ...
  // Validate that the CloudFront distribution is correctly configured with an automatically created OAI
  template.hasResourceProperties('AWS::CloudFront::Distribution', {
    DistributionConfig: {
      Origins: Match.arrayWith([
        Match.objectLike({
  ...
  // Validate the automatically created OAI resource
  template.hasResourceProperties('AWS::CloudFront::CloudFrontOriginAccessIdentity', {
    CloudFrontOriginAccessIdentityConfig: {
      Comment: 'OAI for MyDistributionOrigin1E3AAE2F3',
    },
  });
});

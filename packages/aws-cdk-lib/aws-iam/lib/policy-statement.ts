...
const ensureArrayOrUndefined = (field: any) => {
  ...
  if (typeof (field) !== 'string' && !Array.isArray(field)) {
    throw new UnscopedValidationError('MustBeFieldsEitherString', 'Fields must be either a string or an array of strings');
  }
...
export class PolicyStatement {
  ...
  public addActions(...actions: string[]) {
    ...
    if (actions.length > 0 && this._notAction.length > 0) {
      throw new UnscopedValidationError('CannotAddActionsPolicyStatement', 'Cannot add \'Actions\' to policy statement if \'NotActions\' have been added');
    }
  ...
  public addNotActions(...notActions: string[]) {
    ...
    if (notActions.length > 0 && this._action.length > 0) {
      throw new UnscopedValidationError('CannotAddActionsPolicyStatement', 'Cannot add \'NotActions\' to policy statement if \'Actions\' have been added');
    }
  ...
  public addPrincipals(...principals: IPrincipal[]) {
    ...
    if (principals.length > 0 && this._notPrincipals.length > 0) {
      throw new UnscopedValidationError('CannotAddPrincipalsPolicyStatement', 'Cannot add \'Principals\' to policy statement if \'NotPrincipals\' have been added');
    }
  ...
  public addNotPrincipals(...notPrincipals: IPrincipal[]) {
    ...
    if (notPrincipals.length > 0 && this._principals.length > 0) {
      throw new UnscopedValidationError('CannotAddPrincipalsPolicyStatement', 'Cannot add \'NotPrincipals\' to policy statement if \'Principals\' have been added');
    }
  ...
  private validatePolicyActions(actions: string[]) {
    ...
    for (const action of actions || []) {
      if (!cdk.Token.isUnresolved(action) && !/^(\*|[a-zA-Z0-9-]+:[a-zA-Z0-9*]+)$/.test(action)) {
        throw new UnscopedValidationError('ActionInvalid', `Action '${action}' is invalid. An action string consists of a service namespace, a colon, and the name of an action. Action names can include wildcards.`);
      }
    }
  }
  ...
  private validatePolicyPrincipal(principal: IPrincipal) {
    if (principal instanceof Group) {
      throw new UnscopedValidationError('CannotUseGroupAsPrincipal', 'Cannot use an IAM Group as the \'Principal\' or \'NotPrincipal\' in an IAM Policy');
    }
  }
  ...
  public addResources(...arns: string[]) {
    ...
    if (arns.length > 0 && this._notResource.length > 0) {
      throw new UnscopedValidationError('CannotAddResourcesPolicyStatement', 'Cannot add \'Resources\' to policy statement if \'NotResources\' have been added');
    }
  ...
  public addNotResources(...arns: string[]) {
    ...
    if (arns.length > 0 && this._resource.length > 0) {
      throw new UnscopedValidationError('CannotAddResourcesPolicyStatement', 'Cannot add \'NotResources\' to policy statement if \'Resources\' have been added');
    }
  ...
  private addPrincipalConditions(conditions: Conditions) {
    ...
    if (this.principalConditionsJson === undefined) {
      ...
      if (this.principalConditionsJson !== theseConditions) {
        throw new UnscopedValidationError('PrincipalsPolicystatementSameConditions', `All principals in a PolicyStatement must have the same Conditions (got '${this.principalConditionsJson}' and '${theseConditions}'). Use multiple statements instead.`);
      }
    }
  ...
  private assertNotFrozen(method: string) {
    if (this._frozen) {
      throw new UnscopedValidationError('FreezeCalledPolicystatementPreviously', `${method}: freeze() has been called on this PolicyStatement previously, so it can no longer be modified`);
    }
  }
}
...
class JsonPrincipal extends PrincipalBase {
  ...
  constructor(json: any = { }) {
    ...
    if (typeof(json) !== 'object') {
      throw new UnscopedValidationError('ShouldBeJsonPrincipalShould', `JSON IAM principal should be an object, got ${JSON.stringify(json)}`);
    }
...
export function deriveEstimateSizeOptions(scope: IConstruct): EstimateSizeOptions {
  ...
  if (typeof arnEstimate !== 'number') {
    throw new UnscopedValidationError('ShouldBeContextValueShould', `Context value ${ARN_SIZE_ESTIMATE_CONTEXT_KEY} should be a number, got ${JSON.stringify(arnEstimate)}`);
  }

...
export class RoleGrants {
  ...
  public assumeRole(identity: IPrincipal): Grant {
    ...
    if (identity instanceof ServicePrincipal || identity instanceof AccountPrincipal) {
      throw new ValidationError('CannotCannotcannotserviceaccount', 'Cannot use a service or account principal with grantAssumeRole, use assumeRolePolicy instead.', this.role);
    }

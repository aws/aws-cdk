...
/**
 * Extract the OIDC provider issuer from an OIDC provider reference.
 */
export function getOidcProviderIssuer(oidcProviderRef: IOIDCProviderRef): string {
  return Arn.extractResourceName(oidcProviderRef.oidcProviderRef.oidcProviderArn, 'oidc-provider');
}

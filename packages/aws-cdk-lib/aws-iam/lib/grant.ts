...
export class Grant implements IDependable {
  ...
  public static addToPrincipal(options: GrantOnPrincipalOptions): Grant {
    ...
    if (!addedToPrincipal.policyDependable) {
      throw new cdk.UnscopedValidationError('ContractViolationPrincipalReturns', 'Contract violation: when Principal returns statementAdded=true, it should return a dependable');
    }
  ...
  public assertSuccess(): void {
    if (!this.success) {
      throw new cdk.UnscopedValidationError('DescribegrantOptionsAddedEither', `${describeGrant(this.options)} could not be added on either identity or resource policy.`);
    }
  }

...
export class ManagedPolicy extends Resource implements IManagedPolicy, IGrantable {
  ...
  public get managedPolicyArn(): string {
    ...
    if (!this._resource) {
      throw new ValidationError('CannotCannotcannotaccessmanagedpolicyarn', 'Cannot access managedPolicyArn when synthesis is prevented', this);
    }
  ...
  public get managedPolicyName(): string {
    ...
    if (!this._resource) {
      throw new ValidationError('CannotCannotcannotaccessmanagedpolicyname', 'Cannot access managedPolicyName when synthesis is prevented', this);
    }
...
class ManagedPolicyGrantPrincipal implements IPrincipal {
  ...
  constructor(private _managedPolicy: ManagedPolicy) {
    ...
    const arn = Lazy.string({
      produce: () => {
        throw new ValidationError('Grantoperationneedsresource', 'This grant operation needs to add a resource policy so needs access to a principal. Grant permissions to a Role or User, instead of a ManagedPolicy.', _managedPolicy);
      },
    });
  ...
  public get assumeRoleAction(): string {
    ...
    throw new ValidationError('Grantoperationneedsresource', 'This grant operation needs to add a resource policy so needs access to a principal. Grant permissions to a Role or User, instead of a ManagedPolicy.', this._managedPolicy);
  }

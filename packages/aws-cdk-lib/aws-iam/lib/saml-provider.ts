...
export class SamlProvider extends Resource implements ISamlProvider {
  ...
  constructor(scope: Construct, id: string, props: SamlProviderProps) {
    ...
    if (props.name && !Token.isUnresolved(props.name) && !/^[\w+=,.@-]{1,128}$/.test(props.name)) {
      throw new ValidationError('InvalidInvalidinvalidsamlprovider', 'Invalid SAML provider name. The name must be a string of characters consisting of upper and lowercase alphanumeric characters with no spaces. You can also include any of the following characters: _+=, .@-. Length must be between 1 and 128 characters.', this);
    }

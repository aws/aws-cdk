...
export class Role extends Resource implements IRole {
  ...
  private splitLargePolicy() {
    ...
    if (mpCount > 20) {
      Annotations.of(this).addWarningV2('IAM:Role:PolicyTooLarge', `Policy too large: ${mpCount} exceeds the maximum of 20 managed policies attached to a Role`);
    } else if (mpCount > 10) {
      Annotations.of(this).addWarningV2('IAM:Role:PolicyLarge', `Policy large: ${mpCount} exceeds 10 managed policies attached to a Role, this requires a quota increase`);
    }

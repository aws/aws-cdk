...
class AssetBuildSpec extends BuildSpec {
  ...
  public toBuildSpec(scope?: Project): string {
    if (!scope) {
      throw new UnscopedValidationError(lit`AssetBuildSpecRequiresScopeArgument`, '`AssetBuildSpec` requires a `scope` argument');
    }
    ...
    if (!this.asset) {
      ...
      throw new UnscopedValidationError(lit`AssetAlreadyAssociatedWithAnotherStack`, `Asset is already associated with another stack '${Stack.of(this.asset).stackName}'. ` +
...
export function mergeBuildSpecs(lhs: BuildSpec, rhs: BuildSpec): BuildSpec {
  if (!(lhs instanceof ObjectBuildSpec) || !(rhs instanceof ObjectBuildSpec)) {
    throw new UnscopedValidationError(lit`CanOnlyMergeBuildSpecsFromObject`, 'Can only merge buildspecs created using BuildSpec.fromObject()');
  }

  if (lhs.spec.version === '0.1') {
    throw new UnscopedValidationError(lit`CannotExtendBuildSpecVersionZeroOne`, 'Cannot extend buildspec at version "0.1". Set the version to "0.2" or higher instead.');
  }
  if (lhs.spec.artifacts && rhs.spec.artifacts) {
    ...
    // single primary output into a buildspec with multiple outputs.
    // In case of multiple outputs they must have identifiers but we won't have that information.
    throw new UnscopedValidationError(lit`OnlyOneBuildSpecCanSpecifyArtifacts`, 'Only one build spec is allowed to specify artifacts.');
  }
...
function mergeDeep(lhs: any, rhs: any): any {
  ...
  if (isObject(lhs) && isObject(rhs)) {
    const ret: any = Object.assign(Object.create(null), lhs);

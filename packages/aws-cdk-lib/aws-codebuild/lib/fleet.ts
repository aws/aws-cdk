...
export interface FleetProps {
   ...
   *
   * This is only permitted if `computeType` is set to {@link FleetComputeType.ATTRIBUTE_BASED} or
   * {@link FleetComputeType.CUSTOM_INSTANCE_TYPE}. In such cases, this is required.
   *
   ...
   * builds assume the [Project Role]{@link Project.role} instead.
   *
   ...
   *
   * Only used if {@link vpc} is supplied.
   *
...
export interface ComputeConfiguration {
  /**
   * When using {@link FleetComputeType.ATTRIBUTE_BASED}, the amount of disk
   * space of the instance type included in your fleet. When using {@link FleetComputeType.CUSTOM_INSTANCE_TYPE},
  ...
  /**
   * When using {@link FleetComputeType.ATTRIBUTE_BASED}, the machine type of the instance type included in your fleet.
   *
   * @default - No requirement, the actual value will be based on the other selected configuration properties
   */
  ...
  /**
   * When using {@link FleetComputeType.ATTRIBUTE_BASED}, the amount of memory of the instance type included in your fleet.
   *
   * @default - No requirement, the actual value will be based on the other selected configuration properties
   */
  ...
  /**
   * When using {@link FleetComputeType.ATTRIBUTE_BASED}, the number of vCPUs of the instance type included in your fleet.
   *
   * @default - No requirement, the actual value will be based on the other selected configuration properties
   */
  ...
  /**
   * When using {@link FleetComputeType.CUSTOM_INSTANCE_TYPE}, the EC2 instance type to use for fleet instances.
   *
...
export class Fleet extends Resource implements IFleet {
  ...
  constructor(scope: Construct, id: string, props: FleetProps) {
    ...
    if (
      ...
      if (!props.computeConfiguration || !(props.computeConfiguration.disk || props.computeConfiguration.machineType || props.computeConfiguration.memory || 'vCpu' in props.computeConfiguration)) {
      ...
      } else if ('instanceType' in props.computeConfiguration) {
      ...
      computeConfiguration = {
        // If any of the numeric values are omitted, you get these kinds of
        // errors from CloudFormation:
        // > Resource handler returned message: "Cannot invoke "java.lang.Integer.intValue()" because the return value of "software.amazon.codebuild.fleet.ComputeConfiguration.getVCpu()" is null"
        disk: this.validatePositiveInteger(props.computeConfiguration?.disk?.toGibibytes() ?? 0, 'disk size'),
        machineType: props.computeConfiguration.machineType,
        memory: this.validatePositiveInteger(props.computeConfiguration?.memory?.toGibibytes() ?? 0, 'memory size'),
        vCpu: this.validatePositiveInteger(props.computeConfiguration?.vCpu ?? 0, 'vCPU count'),
      };
      ...
      computeConfiguration = {
        disk: this.validatePositiveInteger(props.computeConfiguration?.disk?.toGibibytes(), 'disk size'),
  ...
  private validatePositiveInteger(value: number | undefined, fieldName: string): number | undefined {

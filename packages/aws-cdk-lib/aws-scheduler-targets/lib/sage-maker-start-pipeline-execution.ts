...
export class SageMakerStartPipelineExecution extends ScheduleTargetBase implements IScheduleTarget {
  constructor(
    ...
    if (props.pipelineParameterList !== undefined && props.pipelineParameterList.length > 200) {
      throw new ValidationError('Mustbepipelineparameterlistlengthbetween', `pipelineParameterList length must be between 0 and 200, got ${props.pipelineParameterList.length}`, pipeline);
    }
  }

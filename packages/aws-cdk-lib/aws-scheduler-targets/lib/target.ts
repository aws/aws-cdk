...
import type { CfnSchedule } from '../../aws-scheduler/lib/scheduler.generated';
    ...
    const principal = new iam.ServicePrincipal('scheduler.amazonaws.com', {
      conditions: {
        StringEquals: {
          ...
          'aws:SourceArn': schedule.scheduleGroup?.scheduleGroupArn ?? Stack.of(schedule).formatArn({
            service: 'scheduler',
            resource: 'schedule-group',
            region: schedule.env.region,
            account: schedule.env.account,
            resourceName: schedule.scheduleGroup?.scheduleGroupName ?? 'default',
          }),
        },
      },
    });

...
export class EcsRunFargateTask extends EcsRunTask {
  ...
  protected bindBaseTargetConfig(_schedule: ISchedule): ScheduleTargetConfig {
    ...
    if (this.assignPublicIp && subnetSelection.subnetType !== ec2.SubnetType.PUBLIC) {
      throw new ValidationError('Shouldbeassignpublicipshouldtrue', 'assignPublicIp should be set to true only for public subnets', _schedule);
    }
...
export class EcsRunEc2Task extends EcsRunTask {
  ...
  protected bindBaseTargetConfig(_schedule: ISchedule): ScheduleTargetConfig {
    ...
    if (!taskDefinitionUsesAwsVpc && (this.props.securityGroups || this.props.vpcSubnets)) {
      throw new ValidationError('Securitygroupssubnetsonly', 'Security groups and subnets can only be used with awsvpc network mode', _schedule);
    }

...
export class EventBridgePutEvents extends ScheduleTargetBase implements IScheduleTarget {
  constructor(
    ...
    if (this.props.input) {
      throw new UnscopedValidationError(lit`ScheduleTargetBasePropsInput`, 'ScheduleTargetBaseProps.input is not supported for EventBridgePutEvents. Please use entry.detail instead.');
    }
  }

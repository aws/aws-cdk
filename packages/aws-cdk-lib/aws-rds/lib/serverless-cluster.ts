...
abstract class ServerlessClusterNew extends ServerlessClusterBase {
  ...
  constructor(scope: Construct, id: string, props: ServerlessClusterNewProps) {
    ...
    if (props.vpc === undefined) {
      if (props.vpcSubnets !== undefined) {
        throw new ValidationError('VpcRequiredVpcsubnetsServerlesscluster', 'A VPC is required to use vpcSubnets in ServerlessCluster. Please add a VPC or remove vpcSubnets', this);
      }
      if (props.subnetGroup !== undefined) {
        throw new ValidationError('VpcRequiredSubnetgroupServerlesscluster', 'A VPC is required to use subnetGroup in ServerlessCluster. Please add a VPC or remove subnetGroup', this);
      }
      if (props.securityGroups !== undefined) {
        throw new ValidationError('VpcRequiredSecuritygroupsServerlesscluster', 'A VPC is required to use securityGroups in ServerlessCluster. Please add a VPC or remove securityGroups', this);
      }
    }
    ...
    if (props.backupRetention) {
      ...
      if (backupRetentionDays < 1 || backupRetentionDays > 35) {
        throw new ValidationError('BackupRetentionPeriodDays', `backup retention period must be between 1 and 35 days. received: ${backupRetentionDays}`, this);
      }
    }
  ...
  private renderScalingConfiguration(options: ServerlessScalingOptions): CfnDBCluster.ScalingConfigurationProperty {
    ...
    if (minCapacity && maxCapacity && minCapacity > maxCapacity) {
      throw new ValidationError('MaximumCapacityGreaterEqual', 'maximum capacity must be greater than or equal to minimum capacity.', this);
    }
    ...
    if (secondsToAutoPause && (secondsToAutoPause < 300 || secondsToAutoPause > 86400)) {
      throw new ValidationError('AutoPauseTimeMinutes', 'auto pause time must be between 5 minutes and 1 day.', this);
    }

    if (timeout && (timeout < 60 || timeout > 600)) {
      throw new ValidationError('Timeout600SecondsGot', `timeout must be between 60 and 600 seconds, but got ${timeout} seconds.`, this);
    }
...
export class ServerlessCluster extends ServerlessClusterNew {
  ...
  public addRotationSingleUser(options: RotationSingleUserOptions = {}): secretsmanager.SecretRotation {
    ...
    if (this.vpc === undefined) {
      throw new ValidationError('AddSingleUserRotation', 'Cannot add single user rotation for a cluster without VPC.', this);
    }
  ...
  public addRotationMultiUser(id: string, options: RotationMultiUserOptions): secretsmanager.SecretRotation {
    ...
    if (this.vpc === undefined) {
      throw new ValidationError('AddMultiUserRotation', 'Cannot add multi user rotation for a cluster without VPC.', this);
    }

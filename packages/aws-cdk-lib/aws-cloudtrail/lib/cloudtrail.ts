...
export class Trail extends Resource {
  ...
  constructor(scope: Construct, id: string, props: TrailProps = {}) {
    ...
    if (props.kmsKey && props.encryptionKey) {
      throw new ValidationError('Bothkmskeyencryptionkeyspecified', 'Both kmsKey and encryptionKey must not be specified. Use only encryptionKey', this);
    }
  ...
  public addEventSelector(dataResourceType: DataResourceType, dataResourceValues: string[], options: AddEventSelectorOptions = {}) {
    if (dataResourceValues.length > 250) {
      throw new ValidationError('Maximumdataelementsevent', 'A maximum of 250 data elements can be in one event selector', this);
    }

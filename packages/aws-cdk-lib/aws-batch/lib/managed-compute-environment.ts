...
export class ManagedEc2EcsComputeEnvironment extends ManagedComputeEnvironmentBase implements IManagedEc2EcsComputeEnvironment {
  ...
  public static fromManagedEc2EcsComputeEnvironmentArn(
    ...
    class Import extends Resource implements IManagedEc2EcsComputeEnvironment {
      ...
      public addInstanceClass(_instanceClass: ec2.InstanceClass): void {
        throw new ValidationError('CannotCannotInstanceClass', `cannot add instance class to imported ComputeEnvironment '${id}'`, this);
      }
      public addInstanceType(_instanceType: ec2.InstanceType): void {
        throw new ValidationError('CannotCannotInstanceType', `cannot add instance type to imported ComputeEnvironment '${id}'`, this);
      }
    }
  ...
  constructor(scope: Construct, id: string, props: ManagedEc2EcsComputeEnvironmentProps) {
    ...
    if (props.defaultInstanceClasses && props.useOptimalInstanceClasses) {
      throw new ValidationError('CannotCannot', 'cannot use `defaultInstanceClasses` with `useOptimalInstanceClasses`. Please remove deprecated `useOptimalInstanceClasses`', this);
    }
    ...
    if (this.images?.find(image => image.imageType === EcsMachineImageType.ECS_AL2023) &&
      ...
      throw new ValidationError('Amazonlinux2023does', 'Amazon Linux 2023 does not support A1 instances.', this);
    }
...
export class ManagedEc2EksComputeEnvironment extends ManagedComputeEnvironmentBase implements IManagedEc2EksComputeEnvironment {
  ...
  constructor(scope: Construct, id: string, props: ManagedEc2EksComputeEnvironmentProps) {
    ...
    if (props.defaultInstanceClasses && props.useOptimalInstanceClasses) {
      throw new ValidationError('CannotCannot', 'cannot use `defaultInstanceClasses` with `useOptimalInstanceClasses`.', this);
    }
    ...
    if (this.allocationStrategy === AllocationStrategy.BEST_FIT) {
      throw new ValidationError('Managedec2ekscomputeenvironmentUsesInvalidAllocation', `ManagedEc2EksComputeEnvironment '${id}' uses invalid allocation strategy 'AllocationStrategy.BEST_FIT'`, this);
    }
    this.spotBidPercentage = props.spotBidPercentage;
    this.instanceTypes = props.instanceTypes ?? [];
    this.instanceClasses = props.instanceClasses ?? [];
...
function determineAllocationStrategy(scope: Construct, allocationStrategy?: AllocationStrategy, spot?: boolean): AllocationStrategy | undefined {
  ...
  if (!allocationStrategy) {
    ...
    throw new ValidationError('ManagedComputeenvironmentSpecifiesAllocationstrategy', `Managed ComputeEnvironment '${scope.node.id}' specifies 'AllocationStrategy.SPOT_PRICE_CAPACITY_OPTIMIZED' without using spot instances`, scope);
  } else if (allocationStrategy === AllocationStrategy.SPOT_CAPACITY_OPTIMIZED && !spot) {
    throw new ValidationError('ManagedComputeenvironmentSpecifiesAllocationstrategy', `Managed ComputeEnvironment '${scope.node.id}' specifies 'AllocationStrategy.SPOT_CAPACITY_OPTIMIZED' without using spot instances`, scope);
  }
...
function validateSpotConfig(scope: Construct, spot?: boolean, spotBidPercentage?: number, spotFleetRole?: iam.IRole): void {
  if (spotBidPercentage) {
    if (!spot) {
      throw new ValidationError('ManagedComputeenvironmentSpecifiesSpotbidpercentage', `Managed ComputeEnvironment '${scope.node.id}' specifies 'spotBidPercentage' without specifying 'spot'`, scope);
    }

    if (!Token.isUnresolved(spotBidPercentage)) {
      if (spotBidPercentage > 100) {
        throw new ValidationError('ManagedComputeenvironmentSpecifiesSpotbidpercentage', `Managed ComputeEnvironment '${scope.node.id}' specifies 'spotBidPercentage' > 100`, scope);
      } else if (spotBidPercentage < 0) {
        throw new ValidationError('ManagedComputeenvironmentSpecifiesSpotbidpercentage', `Managed ComputeEnvironment '${scope.node.id}' specifies 'spotBidPercentage' < 0`, scope);
      }
    }
  }
  ...
  if (spotFleetRole) {
    if (!spot) {
      throw new ValidationError('ManagedComputeenvironmentSpecifiesSpotfleetrole', `Managed ComputeEnvironment '${scope.node.id}' specifies 'spotFleetRole' without specifying 'spot'`, scope);
    }
  }
}
...
function validateVCpus(scope: Construct, minvCpus: number, maxvCpus: number): void {
  if (!Token.isUnresolved(minvCpus) && minvCpus < 0) {
    throw new ValidationError('ManagedComputeenvironmentMinvcpusMinvcpus', `Managed ComputeEnvironment '${scope.node.id}' has 'minvCpus' = ${minvCpus} < 0; 'minvCpus' cannot be less than zero`, scope);
  }
  if (!Token.isUnresolved(minvCpus) && !Token.isUnresolved(maxvCpus) && minvCpus > maxvCpus) {
    throw new ValidationError('ManagedComputeenvironmentMinvcpusMaxvcpus', `Managed ComputeEnvironment '${scope.node.id}' has 'minvCpus' = ${minvCpus} > 'maxvCpus' = ${maxvCpus}; 'minvCpus' cannot be greater than 'maxvCpus'`, scope);
  }
}
...
function asPlacementGroup(x: ec2.IPlacementGroupRef, scope: IConstruct): ec2.IPlacementGroup {
  ...
  throw new ValidationError('Providedplacementgroupinstance', `Provided placement group is not an instance of IPlacementGroup: ${x.constructor.name}`, scope);
}

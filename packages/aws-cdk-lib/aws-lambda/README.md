...
For deployment packages encrypted with a customer-managed KMS key, pass the key in
`sourceKMSKey`. CDK grants the Lambda service principal `kms:Decrypt` permission on
the key. If the source bucket or KMS key is imported and its policy cannot be modified
by CDK, configure the required permissions before deployment. See

...
`s3:GetObject` and `s3:GetObjectVersion` for the referenced object. The grant is
scoped to the function or layer name when a concrete name is specified. If the name
is unresolved or generated by CloudFormation, the grant is scoped to functions or
layers in the same account and CDK emits a warning. Specify `functionName` or
`layerVersionName` to scope the policy to a particular resource.

For deployment packages encrypted with a customer-managed KMS key, configure the
bucket with its `encryptionKey`. CDK grants the Lambda service principal `kms:Decrypt`
permission on that key. When importing an encrypted bucket, use
`Bucket.fromBucketAttributes()` and provide its `encryptionKey`; CDK cannot infer the
key from the bucket name or ARN. If the source bucket or bucket encryption key is
imported and its policy cannot be modified by CDK, configure the required permissions
before deployment. The `sourceKMSKey` option configures encryption of Lambda-managed
code storage and is not the key used to decrypt the original object in `REFERENCE` mode.
See
[Self-managed S3 code storage](https://docs.aws.amazon.com/lambda/latest/dg/configuration-self-managed-storage.html)
for the required bucket and key policies.
...
For a CDK-managed key, CDK adds the statements from the
[documented key policy](https://docs.aws.amazon.com/lambda/latest/dg/durable-encryption.html)
to the key's resource policy: the Lambda service principal (`kms:GenerateDataKey`, `kms:Decrypt`),
the function's execution role (`kms:Decrypt`), and the function author validation actions
(`kms:DescribeKey`, `kms:GenerateDataKey`, `kms:Decrypt`) for the account principal. The function
author statement delegates to IAM, so the identity deploying the function also needs IAM
permissions for those actions. For an imported key (`kms.Key.fromKeyArn`), CDK cannot modify the
key policy; author it yourself.

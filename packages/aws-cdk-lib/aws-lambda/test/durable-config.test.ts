...
describe('durable config', () => {
  ...
  test('DurableConfig with a CDK-managed kmsKey grants the Lambda service and execution role in the key policy', () => {
    const app = new cdk.App();
    const stack = new cdk.Stack(app, 'stack');
    const key = new kms.Key(stack, 'DurableKey');
    ...
    Template.fromStack(stack).hasResourceProperties('AWS::KMS::Key', {
      KeyPolicy: Match.objectLike({
        Statement: Match.arrayWith([
          ...
          Match.objectLike({
            Action: 'kms:Decrypt',
            Condition: { StringEquals: { 'kms:ViaService': { 'Fn::Join': ['', Match.arrayWith([{ Ref: 'AWS::URLSuffix' }])] } } },
          }),
          Match.objectLike({

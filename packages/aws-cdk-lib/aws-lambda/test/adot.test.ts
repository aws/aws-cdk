...
describe('ADOT Lambda Layer', () => {
  describe('when the stack region is specified and supported', () => {
    ...
    test('is added properly when the region information is available at synthesis time', () => {
      ...
      expect(layerArn).toEqual(
        'arn:aws:lambda:us-west-2:901920570463:layer:aws-otel-java-wrapper-amd64-ver-1-32-0:6',
      );
    });
  ...
  describe('when the Python ADOT version is specified', () => {
    ...
    test('is added properly when the region information is available at synthesis time', () => {
      const layerArn = lambda.AdotLambdaLayerPythonSdkVersion.V1_32_0.layerArn(fn.stack, fn.architecture);
        ...
        Value: {
          'Fn::Join': [
            ...
            [
              ...
              ':901920570463:layer:aws-otel-java-wrapper-arm64-ver-1-32-0:6',
            ],
          ],
        },
      });

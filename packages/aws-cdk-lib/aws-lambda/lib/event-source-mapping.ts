...
export class EventSourceMapping extends cdk.Resource implements IEventSourceMapping {
  ...
  constructor(scope: Construct, id: string, props: EventSourceMappingProps) {
    ...
    if (props.eventSourceArn == undefined && props.kafkaBootstrapServers == undefined) {
      throw new ValidationError(lit`EventSourceArnKafkaBootstrap`, 'Either eventSourceArn or kafkaBootstrapServers must be set', this);
    }

    if (props.eventSourceArn !== undefined && props.kafkaBootstrapServers !== undefined) {
      throw new ValidationError(lit`EventSourceArnKafkaBootstrap`, 'eventSourceArn and kafkaBootstrapServers are mutually exclusive', this);
    }
    ...
    if (props.kafkaBootstrapServers && (props.kafkaBootstrapServers?.length < 1)) {
      throw new ValidationError(lit`KafkaBootStrapServersEmpty`, 'kafkaBootStrapServers must not be empty if set', this);
    }

    if (props.maxBatchingWindow && props.maxBatchingWindow.toSeconds() > 300) {
      throw new ValidationError(lit`MaxBatchingWindowCannotSeconds`, `maxBatchingWindow cannot be over 300 seconds, got ${props.maxBatchingWindow.toSeconds()}`, this);
    }

    if (props.maxConcurrency && !cdk.Token.isUnresolved(props.maxConcurrency) && (props.maxConcurrency < 2 || props.maxConcurrency > 1000)) {
      throw new ValidationError(lit`MaxConcurrencyConcurrentInstances`, 'maxConcurrency must be between 2 and 1000 concurrent instances', this);
    }

    if (props.maxRecordAge && (props.maxRecordAge.toSeconds() < 60 || props.maxRecordAge.toDays({ integral: false }) > 7)) {
      throw new ValidationError(lit`MaxRecordAgeSecondsDays`, 'maxRecordAge must be between 60 seconds and 7 days inclusive', this);
    }
    ...
    props.retryAttempts !== undefined && cdk.withResolved(props.retryAttempts, (attempts) => {
      ...
      if (!(attempts === -1 || (attempts >= 0 && attempts <= 10000))) {
        throw new ValidationError(lit`RetryAttemptsInfiniteInclusive`, `retryAttempts must be -1 (for infinite) or between 0 and 10000 inclusive, got ${attempts}`, this);
      }
    });

    props.parallelizationFactor !== undefined && cdk.withResolved(props.parallelizationFactor, (factor) => {
      if (factor < 1 || factor > 10) {
        throw new ValidationError(lit`ParallelizationFactorInclusive`, `parallelizationFactor must be between 1 and 10 inclusive, got ${factor}`, this);
      }
    });

    if (props.tumblingWindow && !cdk.Token.isUnresolved(props.tumblingWindow) && props.tumblingWindow.toSeconds() > 900) {
      throw new ValidationError(lit`TumblingWindowCannotSeconds`, `tumblingWindow cannot be over 900 seconds, got ${props.tumblingWindow.toSeconds()}`, this);
    }

    if (props.startingPosition === StartingPosition.AT_TIMESTAMP && !props.startingPositionTimestamp) {
      throw new ValidationError(lit`StartingPositionTimestampProvidedStarting`, 'startingPositionTimestamp must be provided when startingPosition is AT_TIMESTAMP', this);
    }

    if (props.startingPosition !== StartingPosition.AT_TIMESTAMP && props.startingPositionTimestamp) {
      throw new ValidationError(lit`StartingPositionTimestampStartingPosition`, 'startingPositionTimestamp can only be used when startingPosition is AT_TIMESTAMP', this);
    }
    ...
    if (props.filterEncryption !== undefined && props.filters == undefined) {
      throw new ValidationError(lit`FilterCriteriaProvidedEnableSetting`, 'filter criteria must be provided to enable setting filter criteria encryption', this);
    }
  ...
  private validateKafkaConsumerGroupIdOrThrow(kafkaConsumerGroupId: string) {
    ...
    if (kafkaConsumerGroupId.length > 200 || kafkaConsumerGroupId.length < 1) {
      throw new ValidationError(lit`KafkaConsumerGroupIdValid`, 'kafkaConsumerGroupId must be a valid string between 1 and 200 characters', this);
    }

    const regex = new RegExp(/[a-zA-Z0-9-\/*:_+=.@-]*/);
    const patternMatch = regex.exec(kafkaConsumerGroupId);
    if (patternMatch === null || patternMatch[0] !== kafkaConsumerGroupId) {
      throw new ValidationError(lit`KafkaConsumerGroupIdContains`, 'kafkaConsumerGroupId contains invalid characters. Allowed values are "[a-zA-Z0-9-\/*:_+=.@-]"', this);
    }
  }
}

...
export class Version extends QualifiedFunctionBase implements IVersion {
  ...
  public static fromVersionArn(scope: Construct, id: string, versionArn: string): IVersion {
    ...
    class Import extends QualifiedFunctionBase implements IVersion {
      ...
      public get edgeArn(): string {
        if (version === '$LATEST') {
          throw new ValidationError('Latestfunctionversioncannot', '$LATEST function version cannot be used for Lambda@Edge', this);
        }
  ...
  private determineProvisionedConcurrency(props: VersionProps): CfnVersion.ProvisionedConcurrencyConfigurationProperty | undefined {
    ...
    if (props.provisionedConcurrentExecutions <= 0) {
      throw new ValidationError('Provisionedconcurrentexecutionsvaluegreaterthan', 'provisionedConcurrentExecutions must have value greater than or equal to 1', this);
    }
  ...
  private getFunctionScalingConfig(props: VersionProps): CfnVersion.FunctionScalingConfigProperty | undefined {
    ...
    if (minDefined && minExecutionEnvironments < 0) {
      throw new ValidationError('Mustbeminexecutionenvironmentsnonnegativeinteger', 'minExecutionEnvironments must be a non-negative integer.', this);
    }
    if (maxDefined && maxExecutionEnvironments < 0) {
      throw new ValidationError('Mustbemaxexecutionenvironmentsnonnegativeinteger', 'maxExecutionEnvironments must be a non-negative integer.', this);
    }

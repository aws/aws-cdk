...
export class Version extends QualifiedFunctionBase implements IVersion {
  ...
  private getFunctionScalingConfig(props: VersionProps): CfnVersion.FunctionScalingConfigProperty | undefined {
    ...
    if (minDefined && minExecutionEnvironments < 0) {
      throw new ValidationError('MinexecutionenvironmentsNonNegativeInteger', 'minExecutionEnvironments must be a non-negative integer.', this);
    }
    if (maxDefined && maxExecutionEnvironments < 0) {
      throw new ValidationError('MaxexecutionenvironmentsNonNegativeInteger', 'maxExecutionEnvironments must be a non-negative integer.', this);
    }

...
export class CapacityProvider extends CapacityProviderBase {
  ...
  private validateCapacityProviderProps(props: CapacityProviderProps) {
    ...
    if (props.maxVCpuCount !== undefined && !Token.isUnresolved(props.maxVCpuCount) && (props.maxVCpuCount < 12 || props.maxVCpuCount > 15000)) {
      throw new ValidationError('Maxvcpucount15000ValidationerrorcpnameProps', `maxVCpuCount must be between 12 and 15000, but ${validationErrorCPName} has ${props.maxVCpuCount}.`, this);
    }

    if (!Token.isUnresolved(props.subnets) && (props.subnets.length < 1 || props.subnets.length > 16)) {
      throw new ValidationError('SubnetsContainItemsValidationerrorcpname', `subnets must contain between 1 and 16 items but ${validationErrorCPName} has ${props.subnets.length} items.`, this);
    }

    if (!Token.isUnresolved(props.securityGroups) && (props.securityGroups.length < 1 || props.securityGroups.length > 5)) {
      throw new ValidationError('SecuritygroupsContainItemsValidationerrorcpname', `securityGroups must contain between 1 and 5 items but ${validationErrorCPName} has ${props.securityGroups.length} items.`, this);
    }
  ...
  private validateCapacityProviderName(name: string) {
    ...
    if (!/^([a-zA-Z0-9-_]+|arn:aws[a-zA-Z-]*:lambda:capacity-provider:[a-zA-Z0-9-_]+)$/.test(name)) {
      throw new ValidationError('CapacityprovidernameArnAlphanumericCharacters', `capacityProviderName must be an arn or have only alphanumeric characters, but did not: ${name}`, this);
    }
    if (name.length > 140) {
  ...
  private validateScalingPolicies(scalingOptions: ScalingOptions, validationErrorCPName: string) {
    if (scalingOptions?.scalingPolicies && !Token.isUnresolved(scalingOptions.scalingPolicies)) {
      if (scalingOptions.scalingPolicies.length < 1) {
        throw new ValidationError('ScalingoptionsLeastOnePolicy', `scalingOptions must have at least one policy when scalingMode is 'Manual', but ${validationErrorCPName} has ${scalingOptions.scalingPolicies.length} items.`, this);
      }

      if (scalingOptions.scalingPolicies.length > 10) {
        throw new ValidationError('ScalingoptionsMostTenPolicies', `scalingOptions can have at most ten policies when scalingMode is 'Manual', but ${validationErrorCPName} has ${scalingOptions.scalingPolicies.length} items.`, this);
      }
    }
  }
  ...
  private validateInstanceTypeFilter(instanceTypeFilter: InstanceTypeFilter, validationErrorCPName: string) {
    if (instanceTypeFilter?.allowedInstanceTypes && instanceTypeFilter.allowedInstanceTypes.length < 1) {
      throw new ValidationError('InstancetypefilterLeastOneInstancetype', `instanceTypeFilter must have at least one instanceType when configured, but ${validationErrorCPName} has ${instanceTypeFilter.allowedInstanceTypes.length} items.`, this);
    }

    if (instanceTypeFilter?.excludedInstanceTypes && instanceTypeFilter.excludedInstanceTypes.length < 1) {
      throw new ValidationError('InstancetypefilterLeastOneInstancetype', `instanceTypeFilter must have at least one instanceType when configured, but ${validationErrorCPName} has ${instanceTypeFilter.excludedInstanceTypes.length} items.`, this);
    }
  }
  ...
  private validateFunctionScalingConfig(minExecutionEnvironments?: number, maxExecutionEnvironments?: number) {
    ...
    if (minDefined && (minExecutionEnvironments < 0 || minExecutionEnvironments > 15000)) {
      throw new ValidationError('Minexecutionenvironments15000Minexecutionenvironments', `minExecutionEnvironments must be between 0 and 15000, but was ${minExecutionEnvironments}.`, this);
    }

    if (maxDefined && (maxExecutionEnvironments < 0 || maxExecutionEnvironments > 15000)) {
      throw new ValidationError('Maxexecutionenvironments15000Maxexecutionenvironments', `maxExecutionEnvironments must be between 0 and 15000, but was ${maxExecutionEnvironments}.`, this);
    }

    if (minDefined && maxDefined && minExecutionEnvironments > maxExecutionEnvironments) {
      throw new ValidationError('MinexecutionenvironmentsEqualMaxexecutionenvironments', 'minExecutionEnvironments must be less than or equal to maxExecutionEnvironments.', this);
    }
  }
}

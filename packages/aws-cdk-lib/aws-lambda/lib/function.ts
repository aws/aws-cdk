...
export class Function extends FunctionBase {
  ...
  constructor(scope: Construct, id: string, props: FunctionProps) {
    ...
    if (props.functionName && !Token.isUnresolved(props.functionName)) {
      if (props.functionName.length > 64) {
        throw new ValidationError('FunctionNameLongerCharacters', `Function name can not be longer than 64 characters but has ${props.functionName.length} characters.`, this);
      }
      if (!/^[a-zA-Z0-9-_]+$/.test(props.functionName)) {
        throw new ValidationError('FunctionNamePropsFunctionname', `Function name ${props.functionName} can contain only letters, numbers, hyphens, or underscores with no spaces.`, this);
      }
    }

    if (props.description && !Token.isUnresolved(props.description)) {
      if (props.description.length > 256) {
        throw new ValidationError('FunctionDescriptionLonger256', `Function description can not be longer than 256 characters but has ${props.description.length} characters.`, this);
      }
    }
    ...
    if (props.filesystem) {
      ...
      if (!Token.isUnresolved(config.localMountPath)) {
        if (!/^\/mnt\/[a-zA-Z0-9-_.]+$/.test(config.localMountPath)) {
          throw new ValidationError('LocalMountPathMatch', `Local mount path should match with ^/mnt/[a-zA-Z0-9-_.]+$ but given ${config.localMountPath}.`, this);
        }
        if (config.localMountPath.length > 160) {
          throw new ValidationError('LocalMountPathLonger', `Local mount path can not be longer than 160 characters but has ${config.localMountPath.length} characters.`, this);
        }
      }
      if (config.policies) {
    ...
    if (props.architecture && props.architectures !== undefined) {
      throw new ValidationError('EitherArchitectureArchitecturesSpecified', 'Either architecture or architectures must be specified but not both.', this);
    }
    if (props.architectures && props.architectures.length > 1) {
      throw new ValidationError('OneArchitectureSpecified', 'Only one architecture must be specified.', this);
    }
    ...
    if (props.ephemeralStorageSize && !props.ephemeralStorageSize.isUnresolved()
      ...
      throw new ValidationError('EphemeralStorageSize512', `Ephemeral storage size must be between 512 and 10240 MB, received ${props.ephemeralStorageSize}.`, this);
    }
    ...
    if (props.layers) {
      if (props.runtime === Runtime.FROM_IMAGE) {
        throw new ValidationError('LayersSupportedContainerImage', 'Layers are not supported for container image functions', this);
      }
    ...
    if (props.logRemovalPolicy) {
      if (props.logGroup) {
        throw new ValidationError('LogremovalpolicyLoggroupTogetherPlease', 'Cannot use `logRemovalPolicy` and `logGroup` together. Please set the removal policy on the logGroup directly', this);
      } else if (FeatureFlags.of(this).isEnabled(USE_CDK_MANAGED_LAMBDA_LOGGROUP)) {
        throw new ValidationError('LogremovalpolicyAwsCdkAws', 'Cannot use `logRemovalPolicy` and `@aws-cdk/aws-lambda:useCdkManagedLogGroup` flag together. Please set the removal policy on the automatically created log group directly', this);
      }
    }

    if (props.logRetention) {
      if (props.logGroup) {
        throw new ValidationError('CdkSupportSettingLogretention', 'CDK does not support setting logRetention and logGroup', this);
      }
    ...
    if (props.filesystem) {
      if (!props.vpc) {
        throw new ValidationError('ConfigureFilesystemWithoutConfiguring', 'Cannot configure \'filesystem\' without configuring a VPC.', this);
      }
      const config = props.filesystem.config;
  ...
  public addEnvironment(key: string, value: string, options?: EnvironmentOptions): this {
    ...
    if (reservedEnvironmentVariables.includes(key)) {
      throw new ValidationError('KeyEnvironmentVariableReserved', `${key} environment variable is reserved by the lambda runtime and can not be set manually. See https://docs.aws.amazon.com/lambda/latest/dg/configuration-envvars.html`, this);
    }
  ...
  private getLoggingConfig(props: FunctionProps): CfnFunction.LoggingConfigProperty | undefined {
    if (props.logFormat && props.loggingFormat) {
      throw new ValidationError('DefineLogformatLoggingformat', 'Only define LogFormat or LoggingFormat, not both.', this);
    }

    if (props.applicationLogLevel && props.applicationLogLevelV2) {
      throw new ValidationError('DefineApplicationloglevelApplicationloglevelv2', 'Only define applicationLogLevel or applicationLogLevelV2, not both.', this);
    }

    if (props.systemLogLevel && props.systemLogLevelV2) {
      throw new ValidationError('DefineSystemloglevelSystemloglevelv2', 'Only define systemLogLevel or systemLogLevelV2, not both.', this);
    }
    ...
    if (props.applicationLogLevel || props.applicationLogLevelV2 || props.systemLogLevel || props.systemLogLevelV2) {
      if (props.logFormat !== LogFormat.JSON && props.loggingFormat === undefined) {
        throw new ValidationError('ApplicationloglevelSystemloglevelLogformatLogformat', `To use ApplicationLogLevel and/or SystemLogLevel you must set LogFormat to '${LogFormat.JSON}', got '${props.logFormat}'.`, this);
      }

      if (props.loggingFormat !== LoggingFormat.JSON && props.logFormat === undefined) {
        throw new ValidationError('ApplicationloglevelSystemloglevelLoggingformatLoggingformat', `To use ApplicationLogLevel and/or SystemLogLevel you must set LoggingFormat to '${LoggingFormat.JSON}', got '${props.loggingFormat}'.`, this);
      }
    }
  ...
  public invalidateVersionBasedOn(x: string) {
    if (Token.isUnresolved(x)) {
      throw new ValidationError('InvalidateversiononInputContainUnresolved', 'invalidateVersionOn: input may not contain unresolved tokens', this);
    }
  ...
  public addLayers(...layers: ILayerVersion[]): void {
    for (const layer of layers) {
      if (this._layers.length === 5) {
        throw new ValidationError('UnableAddLayerLambda', 'Unable to add layer: this lambda function already uses 5 layers.', this);
      }
      if (layer.compatibleRuntimes && !layer.compatibleRuntimes.find(runtime => runtime.runtimeEquals(this.runtime))) {
        const runtimes = layer.compatibleRuntimes.map(runtime => runtime.name).join(', ');
        throw new ValidationError('LambdaFunctionUsesRuntime', `This lambda function uses a runtime that is incompatible with this layer (${this.runtime.name} is not in [${runtimes}])`, this);
      }
  ...
  public _checkEdgeCompatibility(): void {
    ...
    if (envKeys.length !== 0) {
      throw new ValidationError('FunctionNodePathContains', `The function ${this.node.path} contains environment variables [${envKeys}] and is not compatible with Lambda@Edge. \
Environment variables can be marked for removal when used in Lambda@Edge by setting the \'removeInEdge\' property in the \'addEnvironment()\' API.`, this);
  ...
  private configureAdotInstrumentation(props: FunctionProps): void {
    ...
    if (props.runtime === Runtime.FROM_IMAGE) {
      throw new ValidationError('AdotLambdaLayerConfigured', "ADOT Lambda layer can't be configured with container image package type", this);
    }
    ...
    if (this.runtime === Runtime.GO_1_X) {
      throw new ValidationError('RuntimeGo1SupportedAdot', 'Runtime go1.x is not supported by the ADOT Lambda Go SDK', this);
    }
    ...
    if (this.runtime.family === RuntimeFamily.PYTHON &&
      ...
      throw new ValidationError('PythonAdotLambdaLayer', 'Python Adot Lambda layer requires AdotLambdaExecWrapper.INSTRUMENT_HANDLER', this);
    }
  ...
  private configureVpc(props: FunctionProps): CfnFunction.VpcConfigProperty | undefined {
    if (props.securityGroup && props.securityGroups) {
      throw new ValidationError('OneFunctionPropsSecuritygroup', 'Only one of the function props, securityGroup or securityGroups, is allowed', this);
    }

    const hasSecurityGroups = props.securityGroups && props.securityGroups.length > 0;
    if (!props.vpc) {
      if (props.allowAllOutbound !== undefined) {
        throw new ValidationError('ConfigureAllowalloutboundWithoutConfiguring', 'Cannot configure \'allowAllOutbound\' without configuring a VPC', this);
      }
      if (props.securityGroup) {
        throw new ValidationError('ConfigureSecuritygroupWithoutConfiguring', 'Cannot configure \'securityGroup\' without configuring a VPC', this);
      }
      if (hasSecurityGroups) {
        throw new ValidationError('ConfigureSecuritygroupsWithoutConfiguring', 'Cannot configure \'securityGroups\' without configuring a VPC', this);
      }
      if (props.vpcSubnets) {
        throw new ValidationError('ConfigureVpcsubnetsWithoutConfiguring', 'Cannot configure \'vpcSubnets\' without configuring a VPC', this);
      }
      if (props.ipv6AllowedForDualStack) {
        throw new ValidationError('ConfigureIpv6allowedfordualstackWithoutConfiguring', 'Cannot configure \'ipv6AllowedForDualStack\' without configuring a VPC', this);
      }
      if (props.allowAllIpv6Outbound !== undefined) {
        throw new ValidationError('ConfigureAllowallipv6outboundWithoutConfiguring', 'Cannot configure \'allowAllIpv6Outbound\' without configuring a VPC', this);
      }
      return undefined;
    }
    ...
    if (props.allowAllOutbound !== undefined) {
      ...
      if (hasSecurityGroups) {
        throw new ValidationError('ConfigureAllowalloutboundDirectlySupplied', 'Configure \'allowAllOutbound\' directly on the supplied SecurityGroups.', this);
      }
    }

    if (props.allowAllIpv6Outbound !== undefined) {
      if (props.securityGroup) {
        throw new ValidationError('ConfigureAllowallipv6outboundDirectlySupplied', 'Configure \'allowAllIpv6Outbound\' directly on the supplied SecurityGroup.', this);
      }
      if (hasSecurityGroups) {
        throw new ValidationError('ConfigureAllowallipv6outboundDirectlySupplied', 'Configure \'allowAllIpv6Outbound\' directly on the supplied SecurityGroups.', this);
      }
    }
  ...
  private renderDurableConfig(config: DurableConfig): CfnFunction.DurableConfigProperty {
    ...
    if (!config.executionTimeout.isUnresolved() && (config.executionTimeout.toSeconds() < 1 || config.executionTimeout.toSeconds() > 31622400)) {
      throw new ValidationError('Executiontimeout31622400SecondsGot', `executionTimeout must be between 1 and 31622400 seconds, got ${config.executionTimeout.toSeconds()}`, this);
    }
    ...
    if (config.retentionPeriod) {
      if (!config.retentionPeriod.isUnresolved() && (config.retentionPeriod.toDays() < 1 || config.retentionPeriod.toDays() > 90)) {
        throw new ValidationError('RetentionperiodindaysDaysGotConfig', `retentionPeriodInDays must be between 1 and 90 days, got ${config.retentionPeriod.toDays()}`, this);
      }
  ...
  private configureSnapStart(props: FunctionProps): CfnFunction.SnapStartProperty | undefined {
    ...
    if (props.filesystem) {
      throw new ValidationError('SnapstartCurrentlySupportedEfs', 'SnapStart is currently not supported using EFS', this);
    }

    if (props.ephemeralStorageSize && props.ephemeralStorageSize?.toMebibytes() > 512) {
      throw new ValidationError('SnapstartCurrentlySupported512', 'SnapStart is currently not supported using more than 512 MiB Ephemeral Storage', this);
    }
  ...
  private buildDeadLetterQueue(props: FunctionProps): sqs.IQueue | sns.ITopic | undefined {
    ...
    if (props.deadLetterTopic && (props.deadLetterQueue || props.deadLetterQueueEnabled !== undefined)) {
      throw new ValidationError('DeadletterqueueDeadlettertopicSpecifiedTogether', 'deadLetterQueue and deadLetterTopic cannot be specified together at the same time', this);
    }
...
export function verifyCodeConfig(code: CodeConfig, props: FunctionProps) {
  ...
  if (codeType.filter(x => !!x).length !== 1) {
    throw new UnscopedValidationError('LambdaCodeSpecifyExactly', 'lambda.Code must specify exactly one of: "inlineCode", "s3Location", or "image"');
  }

  if (!!code.image === (props.handler !== Handler.FROM_IMAGE)) {
    throw new UnscopedValidationError('HandlerHandlerImageImage', 'handler must be `Handler.FROM_IMAGE` when using image asset for Lambda function');
  }

  if (!!code.image === (props.runtime !== Runtime.FROM_IMAGE)) {
    throw new UnscopedValidationError('RuntimeRuntimeImageImage', 'runtime must be `Runtime.FROM_IMAGE` when using image asset for Lambda function');
  }

  // if this is inline code, check that the runtime supports
  if (code.inlineCode && !props.runtime.supportsInlineCode) {
    throw new UnscopedValidationError('InlineSourceAllowedProps', `Inline source not allowed for ${props.runtime!.name}`);
  }
}

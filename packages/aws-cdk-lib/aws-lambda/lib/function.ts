...
Environment variables can be marked for removal when used in Lambda@Edge by setting the \'removeInEdge\' property in the \'addEnvironment()\' API.`, this);
  ...
  private configureVpc(props: FunctionProps): CfnFunction.VpcConfigProperty | undefined {
    ...
    if (props.allowAllOutbound !== undefined) {
      if (props.securityGroup) {
        throw new ValidationError('Configureallowalloutboundonsecuritygroup', 'ConfigureAllowAllOutboundOnSecurityGroup', 'Configure \'allowAllOutbound\' directly on the supplied SecurityGroup.', this);
      }
      if (hasSecurityGroups) {
        throw new ValidationError('Configureallowalloutboundonsecuritygroups', 'ConfigureAllowAllOutboundOnSecurityGroups', 'Configure \'allowAllOutbound\' directly on the supplied SecurityGroups.', this);
      }
    }

    if (props.allowAllIpv6Outbound !== undefined) {
      if (props.securityGroup) {
        throw new ValidationError('Configureallowallipv6outboundonsecuritygroup', 'ConfigureAllowAllIpv6OutboundOnSecurityGroup', 'Configure \'allowAllIpv6Outbound\' directly on the supplied SecurityGroup.', this);
      }
      if (hasSecurityGroups) {
        throw new ValidationError('Configureallowallipv6outboundonsecuritygroups', 'ConfigureAllowAllIpv6OutboundOnSecurityGroups', 'Configure \'allowAllIpv6Outbound\' directly on the supplied SecurityGroups.', this);
      }
    }

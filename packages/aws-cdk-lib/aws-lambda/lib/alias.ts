...
export class Alias extends QualifiedFunctionBase implements IAlias {
  ...
  private validateAdditionalWeights(weights: VersionWeight[]) {
    const total = weights.map(w => {
      if (w.weight < 0 || w.weight > 1) { throw new ValidationError('Mustbeadditionalversionweight', `Additional version weight must be between 0 and 1, got: ${w.weight}`, this); }
      return w.weight;
    }).reduce((a, x) => a + x);

    if (total > 1) {
      throw new ValidationError('Additionalversionweightsexceed', `Sum of additional version weights must not exceed 1, got: ${total}`, this);
    }
  }

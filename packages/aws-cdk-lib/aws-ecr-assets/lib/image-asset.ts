...
export class DockerImageAsset extends Construct implements IAsset {
  ...
  constructor(scope: Construct, id: string, props: DockerImageAssetProps) {
    ...
    if (!fs.existsSync(dir)) {
      throw new ValidationError('FindImageDirectoryDir', `Cannot find image directory at ${dir}`, this);
    }
    ...
    if (!fs.existsSync(file)) {
      throw new ValidationError('FindFileFile', `Cannot find file at ${file}`, this);
    }
...
function validateProps(props: DockerImageAssetProps) {
  for (const [key, value] of Object.entries(props)) {
    if (Token.isUnresolved(value)) {
      throw new UnscopedValidationError('TokenValueKeyValue', `Cannot use Token as value of '${key}': this value is used before deployment starts`);
    }
  }
...
function validateBuildProps(buildPropName: string, buildProps?: { [key: string]: string }) {
  for (const [key, value] of Object.entries(buildProps || {})) {
    if (Token.isUnresolved(key) || Token.isUnresolved(value)) {
      throw new UnscopedValidationError('TokensKeysValuesBuildpropname', `Cannot use tokens in keys or values of "${buildPropName}" since they are needed before deployment`);
    }
  }
}

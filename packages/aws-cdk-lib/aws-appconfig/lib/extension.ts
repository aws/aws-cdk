...
export class Extension extends Resource implements IExtension {
  ...
  public static fromExtensionArn(scope: Construct, id: string, extensionArn: string): IExtension {
    ...
    if (!parsedArn.resourceName) {
      throw new ValidationError('MissingMissingmissingrequiredextensionidextensionversionnumber', `Missing required /$/{extensionId}//$/{extensionVersionNumber} from configuration profile ARN: ${parsedArn.resourceName}`, scope);
    }

    const resourceName = parsedArn.resourceName.split('/');
    if (resourceName.length != 2 || !resourceName[0] || !resourceName[1]) {
      throw new ValidationError('MissingMissingshouldbemissingrequired', 'Missing required parameters for extension ARN: format should be /$/{extensionId}//$/{extensionVersionNumber}', scope);
    }

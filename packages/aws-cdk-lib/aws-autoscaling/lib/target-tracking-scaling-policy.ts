...
export class TargetTrackingScalingPolicy extends Construct {
  ...
  constructor(scope: Construct, id: string, props: TargetTrackingScalingPolicyProps) {
    ...
    if ((props.customMetric === undefined) === (props.predefinedMetric === undefined)) {
      throw new ValidationError('MustBeExactlyCustommetricPredefinedmetric', 'Exactly one of \'customMetric\' or \'predefinedMetric\' must be specified.', this);
    }

    if (props.predefinedMetric === PredefinedMetric.ALB_REQUEST_COUNT_PER_TARGET && !props.resourceLabel) {
      throw new ValidationError('Trackingalbrequestcountpertargetmetric', 'When tracking the ALBRequestCountPerTarget metric, the ALB identifier must be supplied in resourceLabel', this);
    }

    if (props.customMetric && !props.customMetric.toMetricConfig().metricStat) {
      throw new ValidationError('Onlydirectmetricssupported', 'Only direct metrics are supported for Target Tracking. Use Step Scaling or supply a Metric object.', this);
    }

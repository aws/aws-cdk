...
import { CfnSubscriptionFilter, FilterPattern, LogGroup, SubscriptionFilter, SubscriptionFilterSequencingAspect } from '../lib';

describe('subscription filter sequencing aspect', () => {
  test('deploys at most 5 subscription filters in parallel by default', () => {
    // GIVEN
    const stack = new Stack();
    const filters = [1, 2, 3, 4, 5, 6, 7].map((i) => addFilter(stack, `Filter${i}`, `filter-${i}`));

    // WHEN
    Aspects.of(stack).add(new SubscriptionFilterSequencingAspect());

    // THEN: filters 1-5 start immediately, 6 and 7 wait for 1 and 2
    const template = Template.fromStack(stack);
    for (const i of [1, 2, 3, 4, 5]) {
      template.hasResource('AWS::Logs::SubscriptionFilter', {
        Properties: Match.objectLike({ FilterName: `filter-${i}` }),
        DependsOn: Match.absent(),
      });
    }
    template.hasResource('AWS::Logs::SubscriptionFilter', {
      Properties: Match.objectLike({ FilterName: 'filter-6' }),
      DependsOn: [logicalIdOf(stack, filters[0])],
    });
    template.hasResource('AWS::Logs::SubscriptionFilter', {
      Properties: Match.objectLike({ FilterName: 'filter-7' }),
      DependsOn: [logicalIdOf(stack, filters[1])],
    });
  });

  test('sequences subscription filters into a single dependency chain with maxConcurrency 1', () => {
    // GIVEN
    const stack = new Stack();
    ...
    // WHEN
    Aspects.of(stack).add(new SubscriptionFilterSequencingAspect({ maxConcurrency: 1 }));
  ...
  test('sequences subscription filters created through addSubscriptionFilter', () => {
    // GIVEN
    const stack = new Stack();
  ...
  test('sequences raw CfnSubscriptionFilter resources', () => {
    // GIVEN
    const stack = new Stack();
  ...
  test('sequences subscription filters independently per stack when applied to the app', () => {
    // GIVEN
    const app = new App();
    ...
    // WHEN
    Aspects.of(app).add(new SubscriptionFilterSequencingAspect({ maxConcurrency: 1 }));
  ...
  test('splits subscription filters into parallel chains with maxConcurrency', () => {
    ...
    // WHEN
    Aspects.of(stack).add(new SubscriptionFilterSequencingAspect({ maxConcurrency: 2 }));
  ...
  test('does not add dependencies when maxConcurrency is at least the number of filters', () => {
    ...
    // WHEN
    Aspects.of(stack).add(new SubscriptionFilterSequencingAspect({ maxConcurrency: 3 }));
  ...
  test.each([0, -1, 1.5, NaN])('fails for invalid maxConcurrency %p', (maxConcurrency) => {
    expect(() => {
      new SubscriptionFilterSequencingAspect({ maxConcurrency });
    }).toThrow(/maxConcurrency must be a positive integer/);
  });

  test('fails when maxConcurrency is an unresolved token', () => {
    expect(() => {
      new SubscriptionFilterSequencingAspect({ maxConcurrency: Token.asNumber({ resolve: () => 1 }) });
  ...
  test('does not create self-dependencies when the same aspect is applied at multiple scopes', () => {
    ...
    const aspect = new SubscriptionFilterSequencingAspect({ maxConcurrency: 1 });

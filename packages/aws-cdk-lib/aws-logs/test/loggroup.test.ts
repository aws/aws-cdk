...
describe('log group', () => {
  ...
  test('when added to log groups, ARN principals are converted into canonical root ARN form in the resource policy', () => {
    // GIVEN
    const stack = new Stack();
    const lg = new LogGroup(stack, 'LogGroup');
  ...
  test('imported values are treated as if they are ARNs and converted to canonical root ARN form via CFN pseudo parameters', () => {
    // GIVEN
    const stack = new Stack();
    const lg = new LogGroup(stack, 'LogGroup');

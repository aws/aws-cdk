...
abstract class LogGroupBase extends Resource implements ILogGroup {
  ...
  private convertArnPrincipalToAccountId(principal: iam.IPrincipal) {
    if (principal.principalAccount) {
      // Return the full root ARN to match what CloudFormation stores after deployment.
      // This prevents false positive drift detection when comparing synthesized templates
      // against deployed resource policies.

    ...
    switch (this.type) {
      ...
      case ParserProcessorType.GROK:
        if (!props.grokOptions || !props.grokOptions.match) {
          throw new UnscopedValidationError(lit`IsRequiredMatchPatternRequired`, 'Match pattern is required for Grok parser');
        }
      ...
      case ParserProcessorType.OCSF:
        if (!props.parseToOCSFOptions) {
          throw new UnscopedValidationError(lit`ParseOptionsProvidedType`, 'parseToOCSFOptions must be provided for type OCSF');
        }
      ...
      default:
        throw new UnscopedValidationError(lit`UnsupportedParserProcessorType`, `Unsupported parser processor type: ${this.type}`);
    }
  }
...
export class VendedLogParser implements IProcessor {
  ...
  public _render(): any {
    switch (this.logType) {
      ...
      default:
        throw new UnscopedValidationError(lit`UnsupportedVendedLogType`, `Unsupported vended log type: ${this.logType}`);
    }
  }
}
    ...
    switch (this.type) {
      case StringMutatorType.LOWER_CASE:
        if (!props.lowerCaseKeys || props.lowerCaseKeys.length === 0) {
          throw new UnscopedValidationError(lit`MustBeLowercasekeysProvidedLowerCase`, 'lowerCaseKeys must be provided for LOWER_CASE string mutator');
        }
      ...
      case StringMutatorType.UPPER_CASE:
        if (!props.upperCaseKeys || props.upperCaseKeys.length === 0) {
          throw new UnscopedValidationError(lit`MustBeUppercasekeysProvidedUpperCase`, 'upperCaseKeys must be provided for UPPER_CASE string mutator');
        }
      ...
      case StringMutatorType.TRIM:
        if (!props.trimKeys || props.trimKeys.length === 0) {
          throw new UnscopedValidationError(lit`MustBeTrimkeysProvidedTrim`, 'trimKeys must be provided for TRIM string mutator');
        }
      ...
      case StringMutatorType.SPLIT:
        if (!props.splitOptions || !props.splitOptions.entries || props.splitOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`MustBeSplitoptionsProvidedSplit`, 'splitOptions must be provided for SPLIT string mutator');
        }
      ...
      case StringMutatorType.SUBSTITUTE:
        if (!props.substituteOptions || !props.substituteOptions.entries || props.substituteOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`SubstituteOptionsProvidedStringMutator`, 'substituteOptions must be provided for SUBSTITUTE string mutator');
        }
        this.substituteOptions = props.substituteOptions;
        break;

      default:
        throw new UnscopedValidationError(lit`UnsupportedStringMutatorType`, `Unsupported string mutator type: ${this.type}`);
    }
  }
    ...
    switch (this.type) {
      case JsonMutatorType.ADD_KEYS:
        if (!props.addKeysOptions || !props.addKeysOptions.entries || props.addKeysOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`MustBeAddkeysoptionsProvidedAddKeys`, 'addKeysOptions must be provided for ADD_KEYS JSON mutator');
        }
        this.addKeysOptions = {
      ...
      case JsonMutatorType.DELETE_KEYS:
        if (!props.deleteKeysOptions || !props.deleteKeysOptions.withKeys || props.deleteKeysOptions.withKeys.length === 0) {
          throw new UnscopedValidationError(lit`MustBeDeletekeysProvidedDeleteKeys`, 'deleteKeys must be provided for DELETE_KEYS JSON mutator');
        }
      ...
      case JsonMutatorType.MOVE_KEYS:
        if (!props.moveKeysOptions || !props.moveKeysOptions.entries || props.moveKeysOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`MustBeMovekeysoptionsProvidedMoveKeys`, 'moveKeysOptions must be provided for MOVE_KEYS JSON mutator');
        }
      ...
      case JsonMutatorType.RENAME_KEYS:
        if (!props.renameKeysOptions || !props.renameKeysOptions.entries || props.renameKeysOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`RenameKeysOptionsProvidedMutator`, 'renameKeysOptions must be provided for RENAME_KEYS JSON mutator');
        }
      ...
      case JsonMutatorType.COPY_VALUE:
        if (!props.copyValueOptions || !props.copyValueOptions.entries || props.copyValueOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`CopyValueOptionsProvidedMutator`, 'copyValueOptions must be provided for COPY_VALUE JSON mutator');
        }
      ...
      case JsonMutatorType.LIST_TO_MAP:
        if (!props.listToMapOptions || !props.listToMapOptions.source || !props.listToMapOptions.key) {
          throw new UnscopedValidationError(lit`ListMapOptionsSourceKey`, 'listToMapOptions with source and key must be provided for LIST_TO_MAP JSON mutator');
        }
        if (props.listToMapOptions.flatten && !props.listToMapOptions.flattenedElement) {
          throw new UnscopedValidationError(lit`ListMapOptionsFlattenedElement`, 'listToMapOptions flattenedElement must be provided when flatten is true for LIST_TO_MAP JSON mutator');
        }
      ...
      default:
        throw new UnscopedValidationError(lit`UnsupportedMutatorType`, `Unsupported JSON mutator type: ${this.type}`);
    }
  }
    ...
    switch (this.type) {
      case DataConverterType.TYPE_CONVERTER:
        if (!props.typeConverterOptions || !props.typeConverterOptions.entries || props.typeConverterOptions.entries.length === 0) {
          throw new UnscopedValidationError(lit`TypeConverterOptionsProvidedData`, 'typeConverterOptions must be provided for TYPE_CONVERTER data converter');
        }
      ...
      case DataConverterType.DATETIME_CONVERTER:
        if (!props.dateTimeConverterOptions || !props.dateTimeConverterOptions.source ||
          ...
          throw new UnscopedValidationError(lit`DateTimeConverterOptionsSource`, 'dateTimeConverterOptions with source, target and matchPatterns must be provided for DATETIME_CONVERTER data converter');
        }
      ...
      default:
        throw new UnscopedValidationError(lit`UnsupportedDataConverterType`, `Unsupported data converter type: ${this.type}`);
    }
  }
...
export class Transformer extends Resource {
  ...
  private validateProcessorCount(processors: Array<IProcessor>): void {
    ...
    if (processors.length === 0) {
      throw new ValidationError(lit`LeastOneProcessorRequiredTransformer`, 'At least one processor is required in a transformer', this);
    }

    if (processors.length > 20) {
      throw new ValidationError(lit`TransformerCannotProcessors`, 'A transformer cannot have more than 20 processors', this);
    }
  }
  /**
  ...
  private validateParserProcessors(processors: Array<IProcessor>): void {
    ...
    if (! (processors.at(0) instanceof ParserProcessor || processors.at(0) instanceof VendedLogParser)) {
      throw new ValidationError(lit`FirstProcessorTransformerParser`, 'First processor in a transformer must be a parser', this);
    }
    ...
    if (parserProcessors.length > 5) {
      throw new ValidationError(lit`TransformerCannotParserTypeProcessors`, 'A transformer cannot have more than 5 parser-type processors', this);
    }

    // Validate at most one vended parser processor exists
    if (vendedProcessors.length > 1) {
      throw new ValidationError(lit`OneVendedLogParserAllowed`, 'Only one vended log parser is allowed in a transformer', this);
    }
    ...
    if (vendedLogParserIndex > 0) {
      throw new ValidationError(lit`VendedLogParserFirstProcessor`, 'AWS vended log parser must be the first processor in a transformer', this);
    }
  }
  /**
  ...
  private validateUniqueProcessorTypes(processors: Array<IProcessor>): void {
    ...
    if (grokProcessors.length > 1) {
      throw new ValidationError(lit`OneGrokProcessorAllowedTransformer`, 'Only one grok processor is allowed in a transformer', this);
    }
    ...
    if (addKeysProcessors.length > 1) {
      throw new ValidationError(lit`OneAddKeysProcessorAllowed`, 'Only one addKeys processor is allowed in a transformer', this);
    }
    ...
    if (copyValueProcessors.length > 1) {
      throw new ValidationError(lit`OneCopyValueProcessorAllowed`, 'Only one copyValue processor is allowed in a transformer', this);
    }
    ...
    if (parseToOcsfProcessors.length > 1) {
      throw new ValidationError(lit`OneParseProcessorAllowedTransformer`, 'Only one parseToOCSF processor is allowed in a transformer', this);
    }
    if (parseToOcsfProcessors.length > 0) {
      ...
      if (parseToOcsfIndex != 0 ) {
        throw new ValidationError(lit`ParseProcessorFirstProcessorTransformer`, 'parseToOCSF processor must be the first processor in a transformer', this);
      }
    }
  }
  /**

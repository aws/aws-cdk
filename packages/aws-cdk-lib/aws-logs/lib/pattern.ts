...
export class FilterPattern {
  ...
  public static all(...patterns: JsonPattern[]): JsonPattern {
    if (patterns.length === 0) { throw new UnscopedValidationError(lit`SupplyLeastPattern`, 'Must supply at least one pattern, or use allEvents() to match all events.'); }
  ...
  public static any(...patterns: JsonPattern[]): JsonPattern {
    if (patterns.length === 0) { throw new UnscopedValidationError(lit`SupplyLeastPattern`, 'Must supply at least one pattern'); }
    if (patterns.length === 1) { return patterns[0]; }
    return new JSONAggregatePattern('||', patterns);
  }
...
class JSONAggregatePattern extends JsonPattern {
  public constructor(operator: string, patterns: JsonPattern[]) {
    if (operator !== '&&' && operator !== '||') {
      throw new UnscopedValidationError(lit`MustBeOperator`, 'Operator must be one of && or ||');
    }
...
export class SpaceDelimitedTextPattern implements IFilterPattern {
  ...
  public static construct(columns: string[]) {
    ...
    for (const column of columns) {
      if (!validColumnName(column)) {
        throw new UnscopedValidationError(lit`InvalidColumnName`, `Invalid column name: ${column}`);
      }
    }

    if (sum(columns.map(c => c === COL_ELLIPSIS ? 1 : 0)) > 1) {
      throw new UnscopedValidationError(lit`Most`, "Can use at most one '...' column");
    }
  ...
  public whereString(columnName: string, comparison: string, value: string): SpaceDelimitedTextPattern {
    if (columnName === COL_ELLIPSIS) {
      throw new UnscopedValidationError(lit`CanT`, "Can't use '...' in a restriction");
    }
    if (this.columns.indexOf(columnName) === -1) {
      throw new UnscopedValidationError(lit`ColumnRestrictionsColumns`, `Column in restrictions that is not in columns: ${columnName}`);
    }
...
function validateStringOperator(operator: string) {
  ...
  if (operator !== '=' && operator !== '!=') {
    throw new UnscopedValidationError(lit`InvalidComparisonOperator`, `Invalid comparison operator ('${operator}'), must be either '=' or '!='`);
  }
...
function validateNumericalOperator(operator: string) {
  ...
  if (VALID_OPERATORS.indexOf(operator) === -1) {
    throw new UnscopedValidationError(lit`InvalidComparisonOperator`, `Invalid comparison operator ('${operator}'), must be one of ${VALID_OPERATORS.join(', ')}`);
  }
...
function renderRestriction(column: string, restriction: ColumnRestriction) {
  if (restriction.numberValue !== undefined) {
    ...
    throw new UnscopedValidationError(lit`InvalidRestriction`, 'Invalid restriction');
  }
}

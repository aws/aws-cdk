...
export abstract class BaseListener extends Resource implements IListener {
  ...
  protected _setDefaultAction(action: IListenerAction) {
    ...
    if (this.defaultAction) {
      Annotations.of(this).addWarningV2('Elbv2:Listener:ExistingDefaultActionReplaced', 'A default Action already existed on this Listener and was replaced. Configure exactly one default Action.');
    }

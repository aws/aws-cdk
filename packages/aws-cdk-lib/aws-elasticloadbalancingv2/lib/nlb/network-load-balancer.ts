...
export class NetworkLoadBalancer extends BaseLoadBalancer implements INetworkLoadBalancer {
  ...
  constructor(scope: Construct, id: string, props: NetworkLoadBalancerProps) {
    ...
    if (props.subnetMappings && props.subnetMappings.length > 0) {
      if (props.internetFacing && props.subnetMappings.some(sm => sm.privateIpv4Address !== undefined)) {
        throw new ValidationError('SpecifyPrivateipv4addressInternetFacing', 'Cannot specify `privateIpv4Address` for a internet facing load balancer.', this);
      }
      if (props.internetFacing !== true && props.subnetMappings.some(sm => sm.allocationId !== undefined)) {
        throw new ValidationError('SpecifyAllocationidInternalLoad', 'Cannot specify `allocationId` for a internal load balancer.', this);
      }
      if (props.enablePrefixForIpv6SourceNat !== true && props.subnetMappings.some(sm => sm.sourceNatIpv6Prefix !== undefined)) {
        throw new ValidationError('SpecifySourcenatipv6prefixLoadBalancer', 'Cannot specify `sourceNatIpv6Prefix` for a load balancer that does not have `enablePrefixForIpv6SourceNat` enabled.', this);
      }
    }
    ...
    if (minimumCapacityUnit && !Token.isUnresolved(minimumCapacityUnit)) {
      ...
      if (!Number.isInteger(minimumCapacityUnit) || capacityUnitPerAz < 2750 || capacityUnitPerAz > 45000) {
        throw new ValidationError('MinimumcapacityunitPositiveValue2750', `'minimumCapacityUnit' must be a positive value between 2750 and 45000 per AZ for Network Load Balancer, got ${capacityUnitPerAz} LCU per AZ.`, this);
      }
    }
    ...
    if (props.securityGroups && props.disableSecurityGroups) {
      throw new ValidationError('SpecifySecuritygroupsDisablesecuritygroupsProperties', 'Cannot specify both `securityGroups` and `disableSecurityGroups` properties.', this);
    }
  ...
  public addListener(id: string, props: BaseNetworkListenerProps): NetworkListener {
    ...
    if (
      ...
      throw new ValidationError('AddListenerUdpProtocol', 'To add a listener with UDP protocol to a dual stack NLB, \'enablePrefixForIpv6SourceNat\' must be set to true.', this);
    }

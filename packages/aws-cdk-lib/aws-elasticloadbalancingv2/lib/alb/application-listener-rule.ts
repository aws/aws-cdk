...
export class ApplicationListenerRule extends Construct {
  ...
  constructor(scope: Construct, id: string, props: ApplicationListenerRuleProps) {
    ...
    if (this.conditions.length === 0 && !props.hostHeader && !hasPathPatterns) {
      throw new ValidationError('LeastOneConditionsHostheader', 'At least one of \'conditions\', \'hostHeader\', \'pathPattern\' or \'pathPatterns\' is required when defining a load balancing rule.', this);
    }
    ...
    if (providedActions.length > 1) {
      throw new ValidationError('ProvidedactionsSpecifiedTogetherSpecify', `'${providedActions}' specified together, specify only one`, this);
    }
    ...
    if (hasPathPatterns) {
      if (props.pathPattern && props.pathPatterns) {
        throw new ValidationError('PathpatternsPathpatternSpecifiedSpecify', 'Both `pathPatterns` and `pathPattern` are specified, specify only one', this);
      }
...
function validateFixedResponse(fixedResponse: FixedResponse) {
  if (fixedResponse.statusCode && !/^(2|4|5)\d\d$/.test(fixedResponse.statusCode)) {
    throw new UnscopedValidationError('Statuscode2xx4xx5xx', '`statusCode` must be 2XX, 4XX or 5XX.');
  }

  if (fixedResponse.messageBody && fixedResponse.messageBody.length > 1024) {
    throw new UnscopedValidationError('Messagebody1024Characters', '`messageBody` cannot have more than 1024 characters.');
  }
}
...
function validateRedirectResponse(redirectResponse: RedirectResponse) {
  if (redirectResponse.protocol && !/^(HTTPS?|#\{protocol\})$/i.test(redirectResponse.protocol)) {
    throw new UnscopedValidationError('ProtocolHttpHttpsProtocol', '`protocol` must be HTTP, HTTPS, or #{protocol}.');
  }

  if (!redirectResponse.statusCode || !/^HTTP_30[12]$/.test(redirectResponse.statusCode)) {
    throw new UnscopedValidationError('StatuscodeHttp301Http', '`statusCode` must be HTTP_301 or HTTP_302.');
  }
}

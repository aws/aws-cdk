...
describe('tests', () => {
  ...
  describe('attributes.target_group_health', () => {
    ...
    test.each([-1, 0])('dns_failover.minimum_healthy_targets.count cannot be %s', (count) => {
      expect(() => {
        new elbv2.ApplicationTargetGroup(stack, 'LB', {
          targetGroupHealth: {
            dnsFailoverMinimumHealthyTargetCount: count,
      ...
      }).toThrow(/DNS failover minimum healthy target count must be an integer greater than 0/);
    });
    ...
    test.each([0, 101, 99.5, -1])('dns_failover.minimum_healthy_targets.percentage cannot be %s', (percentage) => {
      expect(() => {
        new elbv2.ApplicationTargetGroup(stack, 'LB', {
          targetGroupHealth: {
            dnsFailoverMinimumHealthyTargetPercentage: percentage,
      ...
      }).toThrow(/DNS failover minimum healthy target percentage must be an integer from 1 to 100/);
    });
    ...
    test.each([-1, 0])('unhealthy_state_routing.minimum_healthy_targets.count cannot be %s', (count) => {
      ...
      }).toThrow(/Unhealthy state routing minimum healthy target count must be an integer greater than 0/);
    });
    ...
    test.each([0, 101, 99.5, -1])('unhealthy_state_routing.minimum_healthy_targets.percentage cannot be %s', (percentage) => {
      expect(() => {
        new elbv2.ApplicationTargetGroup(stack, 'LB', {
          targetGroupHealth: {
            unhealthyStateRoutingMinimumHealthyTargetPercentage: percentage,
      ...
      }).toThrow(/Unhealthy state routing minimum healthy target percentage must be an integer from 1 to 100/);
    });
  });
});

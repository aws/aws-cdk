...
describe('tests', () => {
  ...
  test.each(Object.values(elbv2.TargetGroupLoadBalancingAlgorithmType))('Custom Load balancer algorithm type can be %s', (type) => {
    // GIVEN
    const vpc = new ec2.Vpc(stack, 'VPC', {});

    // WHEN
    new elbv2.ApplicationTargetGroup(stack, 'TargetGroup', {
      loadBalancingAlgorithmType: type,
      vpc,
    });
  ...
  test.each([0, 300, 3600])('deregistration_delay.timeout_seconds can be %s', (time) => {
    // GIVEN
    const vpc = new ec2.Vpc(stack, 'VPC', {});

    new elbv2.ApplicationTargetGroup(stack, 'LB', {
      vpc,
      deregistrationDelay: cdk.Duration.seconds(time),
    });
  ...
  test.each([3601])('deregistration_delay.timeout_seconds cannot be %s', (time) => {
    expect(() => {
      new elbv2.ApplicationTargetGroup(stack, 'LB', {
        deregistrationDelay: cdk.Duration.seconds(time),
      });
    }).toThrow(/deregistration_delay.timeout_seconds must be an integer from 0 to 3600/);
  });

  test.each(['on', 0, 101, 99.5, -1])('unhealthy_state_routing.minimum_healthy_targets.percentage cannot be %s', (percentage) => {
    expect(() => {
      new elbv2.ApplicationTargetGroup(stack, 'LB', {
        unhealthyStateRoutingMinimumHealthyTargetPercentage: percentage,
      });
    }).toThrow(/unhealthy_state_routing.minimum_healthy_targets.percentage must be an integer from 1 to 100/);
  });

  describe('attributes.target_group_health', () => {
    test.each(['off', 1, 5])('dns_failover.minimum_healthy_targets.count can be %s', (count) => {
      // GIVEN
      const vpc = new ec2.Vpc(stack, 'VPC', {});

      new elbv2.ApplicationTargetGroup(stack, 'LB', {
        vpc,
        dnsFailoverMinimumHealthyTargetCount: count,
      });
    ...
    test.each(['on', -1, 0])('dns_failover.minimum_healthy_targets.count cannot be %s', (count) => {
      expect(() => {
        new elbv2.ApplicationTargetGroup(stack, 'LB', {
          dnsFailoverMinimumHealthyTargetCount: count,
        });
      }).toThrow(/target_group_health.dns_failover.minimum_healthy_targets.count must be an integer greater than 0/);
    });

    test.each(['off', 1, 50, 100])('dns_failover.minimum_healthy_targets.percentage can be %s', (percentage) => {
      // GIVEN
      const vpc = new ec2.Vpc(stack, 'VPC', {});

      new elbv2.ApplicationTargetGroup(stack, 'LB', {
        vpc,
        dnsFailoverMinimumHealthyTargetPercentage: percentage,
      });
    ...
    test.each(['on', 0, 101, 99.5, -1])('dns_failover.minimum_healthy_targets.percentage cannot be %s', (percentage) => {
      expect(() => {
        new elbv2.ApplicationTargetGroup(stack, 'LB', {
          dnsFailoverMinimumHealthyTargetPercentage: percentage,
        });
      }).toThrow(/target_group_health.dns_failover.minimum_healthy_targets.percentage must be an integer from 1 to 100/);
    });
    ...
    test.each([1, 5])('unhealthy_state_routing.minimum_healthy_targets.count can be %s', (count) => {
      ...
      new elbv2.ApplicationTargetGroup(stack, 'LB', {
        vpc,
        unhealthyStateRoutingMinimumHealthyTargetCount: count,
      });
    ...
    test.each(['off', 1, 50, 100])('unhealthy_state_routing.minimum_healthy_targets.percentage can be %s', (percentage) => {
      // GIVEN
      const vpc = new ec2.Vpc(stack, 'VPC', {});

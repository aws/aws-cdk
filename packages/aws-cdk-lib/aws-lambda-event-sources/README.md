...
**Important**: Observability features (`LoggingConfig` and `MetricsConfig`) are only available when using provisioned mode. 
...
Note: This will similarly work for `SelfManagedKafkaEventSource` but the example only shows setup for
`MEventSource`.
...
Note: This will simi but the example only shows setup for

    ...
    if (props.provisionedPollerConfig) {
      ...
      if (minimumPollers != undefined) {
        if (minimumPollers < 1 || minimumPollers > 200) {
          throw new UnscopedValidationError('MinimumProvisionedPollers200', 'Minimum provisioned pollers must be between 1 and 200 inclusive');
        }
      }
      if (maximumPollers != undefined) {
        if (maximumPollers < 1 || maximumPollers > 2000) {
          throw new UnscopedValidationError('MaximumProvisionedPollers2000', 'Maximum provisioned pollers must be between 1 and 2000 inclusive');
        }
      }
      if (minimumPollers != undefined && maximumPollers != undefined) {
        if (minimumPollers > maximumPollers) {
          throw new UnscopedValidationError('MinimumProvisionedPollersEqual', 'Minimum provisioned pollers must be less than or equal to maximum provisioned pollers');
        }
      }
    }
  }

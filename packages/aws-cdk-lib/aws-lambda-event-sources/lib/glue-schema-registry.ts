  ...
  private getRegistryProps(props: GlueSchemaRegistryProps, _target: IEventSourceMapping) {
    ...
    if (props.schemaRegistryArn) {
      ...
      if (!glueRegistryMatch) {
        throw new ValidationError('Schemaregistryarnmatch', `schemaRegistryArn ${this.props.schemaRegistryArn} must match ${GLUE_SCHEMA_REGISTRY_ARN_REGEX}`, _target);
      }
      return {
    ...
    throw new ValidationError('Mustbeschemaregistryarnschemaregistrypassed', 'one of schemaRegistryArn or schemaRegistry must be passed', _target);
  }

...
export class DynamoEventSource extends StreamEventSource {
  ...
  constructor(private readonly table: dynamodb.ITable, props: DynamoEventSourceProps) {
    ...
    if (this.props.batchSize !== undefined
      ...
      throw new ValidationError('Mustbemaximumbatchsize', `Maximum batch size must be between 1 and 10000 inclusive (given ${this.props.batchSize})`, table);
    }
  }

  public bind(target: lambda.IFunction) {
    if (!this.table.tableStreamArn) {
      throw new ValidationError('Mustbedynamodbstreamsenabled', `DynamoDB Streams must be enabled on the table ${this.table.node.path}`, this.table);
    }
  ...
  public get eventSourceMappingId(): string {
    if (!this._eventSourceMappingId) {
      throw new ValidationError('Dynamoeventsourceboundeventsource', 'DynamoEventSource is not yet bound to an event source mapping', this.table);
    }

...
export class Canary extends cdk.Resource implements ec2.IConnectable {
  ...
  public constructor(scope: Construct, id: string, props: CanaryProps) {
    ...
    if (props.cleanup === Cleanup.LAMBDA && props.provisionedResourceCleanup) {
      throw new ValidationError('SpecifyProvisionedresourcecleanupCleanupCleanup', 'Cannot specify `provisionedResourceCleanup` when `cleanup` is set to `Cleanup.LAMBDA`. Use only `provisionedResourceCleanup`.', this);
    }
  ...
  private validateDryRunAndUpdate(runtime: Runtime, dryRunAndUpdate?: boolean) {
    ...
    if (!match || match.length < 3 || MIN_SUPPORTED_VERSIONS[match[1]] === undefined || parseFloat(match[2]) < MIN_SUPPORTED_VERSIONS[match[1]]) {
      throw new ValidationError('DryrunandupdateSupportedCanaryRuntime', `dryRunAndUpdate is only supported for canary runtime versions 'syn-nodejs-puppeteer-10.0+', 'syn-nodejs-playwright-2.0+', or 'syn-python-selenium-5.1+', got: ${runtimeName}`, this);
    }
  }
  ...
  private validateBrowserConfigs(runtime: Runtime, browserConfigs?: BrowserType[]) {
    ...
    if (browserConfigs.length > 2) {
      throw new ValidationError('SpecifyBrowserConfigurationsGot', `You can specify up to 2 browser configurations, got: ${browserConfigs.length}.`, this);
    }

    if (browserConfigs.length === 0) {
      throw new ValidationError('BrowserconfigsContainLeastOne', 'browserConfigs must contain at least one browser type if specified.', this);
    }
    ...
    if (hasFirefox) {
      ...
      if (isSeleniumRuntime) {
        throw new ValidationError('FirefoxBrowserSupportedPython', 'Firefox browser is not supported with Python Selenium runtimes. Use Chrome instead or switch to a Node.js runtime with Puppeteer or Playwright.', this);
      }
    }
  }
  ...
  public get connections(): ec2.Connections {
    if (!this._connections) {
      throw new ValidationError('VpcAssociatedCanariesSecurity', 'Only VPC-associated Canaries have security groups to manage. Supply the "vpc" parameter when creating the Canary.', this);
    }
  ...
  private validateHandler(handler: string, runtime: Runtime) {
    ...
    if (oldRuntimes.includes(runtime)) {
      if (!handler.match(/^[0-9A-Za-z_\\-]+\.handler*$/)) {
        throw new ValidationError('CanaryHandlerSpecifiedFilename', `Canary Handler must be specified as \'fileName.handler\' for legacy runtimes, received ${handler}`, this);
      }
    } else {
      if (!handler.match(/^([0-9a-zA-Z_-]+\/)*[0-9A-Za-z_\\-]+\.[A-Za-z_][A-Za-z0-9_]*$/)) {
        throw new ValidationError('CanaryHandlerSpecifiedEither', `Canary Handler must be specified either as \'fileName.handler\', \'fileName.functionName\', or \'folder/fileName.functionName\', received ${handler}`, this);
      }
    }
    if (handler.length < 1 || handler.length > 128) {
      throw new ValidationError('CanaryHandlerLength128', `Canary Handler length must be between 1 and 128, received ${handler.length}`, this);
    }
  }
  ...
  private createRunConfig(props: CanaryProps): CfnCanary.RunConfigProperty | undefined {
    ...
    if (
      ...
      throw new ValidationError('EnableActiveTracingCanaries', `You can only enable active tracing for canaries that use canary runtime version 'syn-nodejs-2.0' or later and are not using the Playwright runtime, got ${props.runtime.name}.`, this);
    }
    ...
    if (!cdk.Token.isUnresolved(props.memory) && props.memory !== undefined) {
      ...
      if (memoryInMb % 64 !== 0) {
        throw new ValidationError('MemoryMultipleMibGot', `\`memory\` must be a multiple of 64 MiB, got ${memoryInMb} MiB.`, this);
      }
      if (memoryInMb < 960 || memoryInMb > 3008) {
        throw new ValidationError('Memory960Mib3008', `\`memory\` must be between 960 MiB and 3008 MiB, got ${memoryInMb} MiB.`, this);
      }
    }
    ...
    if (!cdk.Token.isUnresolved(props.timeout) && props.timeout !== undefined) {
      ...
      if (timeoutInMillis % 1000 !== 0) {
        throw new ValidationError('TimeoutIntegerRepresentingSeconds', `\`timeout\` must be set as an integer representing seconds, got ${timeoutInMillis} milliseconds.`, this);
      }

      timeoutInSeconds = props.timeout.toSeconds();
      if (timeoutInSeconds < 3 || timeoutInSeconds > 840) {
        throw new ValidationError('TimeoutSeconds840Seconds', `\`timeout\` must be between 3 seconds and 840 seconds, got ${timeoutInSeconds} seconds.`, this);
      }
    }
  ...
  private createVpcConfig(props: CanaryProps): CfnCanary.VPCConfigProperty | undefined {
    if (!props.vpc) {
      if (props.vpcSubnets != null || props.securityGroups != null) {
        throw new ValidationError('ProvideVpcPropVpc', "You must provide the 'vpc' prop when using VPC-related properties.", this);
      }
    ...
    if (subnetIds.length < 1) {
      throw new ValidationError('MatchingSubnetsFoundVpc', 'No matching subnets found in the VPC.', this);
    }
  ...
  private createArtifactConfig(props: CanaryProps): CfnCanary.ArtifactConfigProperty | undefined {
    ...
    if (
      ...
      throw new ValidationError('CustomerManagedKmsKey', `A customer-managed KMS key was provided, but the encryption mode is not set to SSE-KMS, got: ${props.artifactS3EncryptionMode}.`, this);
    }
    ...
    if (!isNodeRuntime && props.artifactS3EncryptionMode) {
      throw new ValidationError('ArtifactEncryptionSupportedCanaries', `Artifact encryption is only supported for canaries that use Synthetics runtime version \`syn-nodejs-puppeteer-3.3\` or later and the Playwright runtime, got ${props.runtime.name}.`, this);
    }
...
function validateName(name: string) {
  if (name.length > 255) {
    throw new UnscopedValidationError('CanaryNameTooLarge', `Canary name is too large, must be between 1 and 255 characters, but is ${name.length} (got "${name}")`);
  }
  if (!nameRegex.test(name)) {
    throw new UnscopedValidationError('CanaryNameLowercaseNumbers', `Canary name must be lowercase, numbers, hyphens, or underscores (got "${name}")`);
  }
}

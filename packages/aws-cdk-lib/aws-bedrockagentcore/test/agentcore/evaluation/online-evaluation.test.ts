...
describe('OnlineEvaluationConfig', () => {
  ...
  describe('creation with minimal props', () => {
    test('creates OnlineEvaluationConfig with CloudWatch Logs data source', () => {
      ...
      template.hasResourceProperties('AWS::BedrockAgentCore::OnlineEvaluationConfig', {
        ...
        EvaluationExecutionRoleArn: { 'Fn::GetAtt': ['TestEvaluationExecutionRole29188CE3', 'Arn'] },
        Rule: {
    ...
    test('creates OnlineEvaluationConfig with AgentCore Runtime endpoint data source', () => {
      ...
      new OnlineEvaluationConfig(stack, 'TestEvaluation', {
        onlineEvaluationConfigName: 'test_evaluation',
        evaluators: [EvaluatorSelector.builtin(BuiltinEvaluator.CORRECTNESS)],
    ...
    test('creates OnlineEvaluationConfig with all options', () => {
      ...
      new OnlineEvaluationConfig(stack, 'TestEvaluation', {
        ...
        evaluators: [
          ...
          EvaluatorSelector.builtin(BuiltinEvaluator.FAITHFULNESS),
        ],
        dataSource: DataSourceConfig.fromCloudWatchLogs({
          logGroupNames: ['/aws/bedrock-agentcore/my-agent'],
          serviceNames: ['my-agent.default'],
      ...
      template.hasResourceProperties('AWS::BedrockAgentCore::OnlineEvaluationConfig', {
        ...
        EvaluationExecutionRoleArn: { 'Fn::GetAtt': ['ExecutionRole605A040B', 'Arn'] },
        Rule: {
  ...
  // These tests verify the EvaluatorSelector API contract (evaluatorId mapping and bind() output).
  // Template-based assertions for evaluator IDs rendering in CFN are covered in 'creates with minimal properties' and 'creates with all properties'.
  describe('evaluator references', () => {
    test('creates built-in evaluator reference', () => {
      const evaluator = EvaluatorSelector.builtin(BuiltinEvaluator.HELPFULNESS);
    ...
    test('supports all built-in evaluators', () => {
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.GOAL_SUCCESS_RATE).evaluatorId).toBe('Builtin.GoalSuccessRate');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.HELPFULNESS).evaluatorId).toBe('Builtin.Helpfulness');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.CORRECTNESS).evaluatorId).toBe('Builtin.Correctness');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.FAITHFULNESS).evaluatorId).toBe('Builtin.Faithfulness');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.HARMFULNESS).evaluatorId).toBe('Builtin.Harmfulness');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.STEREOTYPING).evaluatorId).toBe('Builtin.Stereotyping');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.REFUSAL).evaluatorId).toBe('Builtin.Refusal');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.TOOL_SELECTION_ACCURACY).evaluatorId).toBe('Builtin.ToolSelectionAccuracy');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.TOOL_PARAMETER_ACCURACY).evaluatorId).toBe('Builtin.ToolParameterAccuracy');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.COHERENCE).evaluatorId).toBe('Builtin.Coherence');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.RESPONSE_RELEVANCE).evaluatorId).toBe('Builtin.ResponseRelevance');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.CONCISENESS).evaluatorId).toBe('Builtin.Conciseness');
      expect(EvaluatorSelector.builtin(BuiltinEvaluator.INSTRUCTION_FOLLOWING).evaluatorId).toBe('Builtin.InstructionFollowing');
    });
  });

  // These tests verify the DataSourceConfig.bind() API contract (log group name format, service name computation).
  // Template-based assertions for data source rendering in CFN are covered in 'creates with minimal properties'.
  // IAM scoping of runtime data sources is verified via template in 'runtime data source IAM scoping'.
  ...
  describe('validation', () => {
    ...
    test('throws error for too many evaluators', () => {
      expect(() => {
        new OnlineEvaluationConfig(stack, 'TestEvaluation', {
          onlineEvaluationConfigName: 'test_evaluation',
          evaluators: Array(11).fill(EvaluatorSelector.builtin(BuiltinEvaluator.HELPFULNESS)),
          dataSource: DataSourceConfig.fromCloudWatchLogs({
            logGroupNames: ['/aws/log-group'],
            serviceNames: ['service'],
          }),
  ...
  describe('IAM role', () => {
    ...
    test('uses provided execution role', () => {
      ...
      const template = Template.fromStack(stack);
      template.hasResourceProperties('AWS::BedrockAgentCore::OnlineEvaluationConfig', {
        EvaluationExecutionRoleArn: { 'Fn::GetAtt': ['CustomRole6D8E6809', 'Arn'] },
      });
  ...
  // These tests verify the contract of each import factory method (ARN parsing, ID extraction, attribute passthrough).
  // The 'imported config ARN is used correctly in grants' test below verifies the imported ARN flows into CFN via template assertion.
  describe('import methods', () => {
    ...
    test('imported config ARN is used correctly in grants', () => {
      const imported = OnlineEvaluationConfig.fromOnlineEvaluationConfigArn(
        stack,
        'Imported',
        'arn:aws:bedrock-agentcore:us-east-1:123456789012:online-evaluation-config/my-config-id',
      ...
      imported.grant(role, 'bedrock-agentcore:GetOnlineEvaluationConfig');
      ...
      template.hasResourceProperties('AWS::IAM::Policy', {
        PolicyDocument: {
          Statement: Match.arrayWith([
            Match.objectLike({
              Resource: 'arn:aws:bedrock-agentcore:us-east-1:123456789012:online-evaluation-config/my-config-id',
            }),
          ]),
        },
      });

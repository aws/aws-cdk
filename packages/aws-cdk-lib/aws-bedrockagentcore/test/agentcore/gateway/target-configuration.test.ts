...
// These tests verify internal configuration object APIs (.create(), _render(), validation).
// Template-based assertions for target configurations rendering in CFN are in target.test.ts
// ('Should render full API Gateway target configuration in CFN', 'Should render full Lambda target configuration in CFN').
describe('Target Configuration Tests', () => {
  ...
  describe('ApiGatewayTargetConfiguration', () => {
    ...
    describe('Bind and render', () => {
      ...
      test('Should render configuration correctly without metadata', () => {
        ...
        expect(rendered.mcp.apiGateway.apiGatewayToolConfiguration.toolFilters).toEqual([
      ...
      test('Should render configuration correctly with tool overrides', () => {
        ...
        const rendered = config._render();
        expect(rendered.mcp.apiGateway.apiGatewayToolConfiguration.toolOverrides).toEqual([
          { path: '/pets/{petId}', method: 'GET', name: 'get_pet_by_id', description: 'Get a pet' },
        ]);
      });
  ...
  describe('LambdaTargetConfiguration', () => {
    ...
    test('Should render Lambda configuration correctly', () => {
      ...
      expect(rendered.mcp.lambda.lambdaArn).toBe(fn.functionArn);
      expect(rendered.mcp.lambda.toolSchema.inlinePayload).toEqual([
        {
          name: 'test_tool',
          description: 'Test tool',
  ...
  describe('Additional Branch Coverage Tests', () => {
    ...
    describe('Target type verification', () => {
      ...
      test('Should have correct target type for API Gateway configuration', () => {
        ...
        expect(config.targetType).toBe('API_GATEWAY');
      });
    });
    ...
    describe('Render with varying metadata configurations', () => {
      test('Should render with all three metadata arrays present', () => {
        ...
        expect(rendered.mcp.apiGateway.metadataConfiguration).toEqual({
          allowedQueryParameters: ['q1', 'q2'],
          allowedRequestHeaders: ['h1', 'h2'],
          allowedResponseHeaders: ['r1', 'r2'],
        });

...
// These tests verify the internal API contracts of GatewayCustomClaim and GatewayAuthorizer (_render() output, validation).
// Template-based assertion for authorizer + custom claims rendering in CFN is in gateway-coverage.test.ts ('custom JWT authorizer with custom claims renders in template').

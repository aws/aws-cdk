...
describe('PolicyEngine grant methods tests', () => {
  ...
  test('grantEvaluateForGateway scopes to this gateway when the name is an auto-generated Lazy', () => {
    const newApp = new cdk.App();
    ...
    // No gatewayName provided, so the name is a Lazy. `Token.isUnresolved` is true for it, but it
    // resolves during synthesis, so the grant must still name this one gateway rather than widen to
    // every gateway in the account. This is the default path.
    ...
    // The account-wide scope must not appear: that is the defect this test guards.
    expect(JSON.stringify(template.toJSON())).not.toContain('gateway/*');
  });

  test('grantEvaluateForGateway lowercases the gateway name, because the service does so in the ARN', () => {
    const newApp = new cdk.App();
    const newStack = new cdk.Stack(newApp, 'mixed-case-stack', {
      env: { account: '123456789012', region: 'us-east-1' },
    });
    const newEngine = new PolicyEngine(newStack, 'mixed-case-engine', {
      policyEngineName: 'mixed_case_engine',
    });
    const newRole = new iam.Role(newStack, 'mixed-case-role', {
      assumedBy: new iam.ServicePrincipal('bedrock-agentcore.amazonaws.com'),
    });
    const gateway = new Gateway(newStack, 'mixed-case-gateway', {
      gatewayName: 'MixedCaseGateway',
      role: newRole,
    });

    newEngine.grantEvaluateForGateway(newRole, gateway);

    // IAM compares resource ARNs case sensitively, so a pattern carrying the caller's casing would
    // never match the ARN the service creates.
    const rendered = JSON.stringify(Template.fromStack(newStack).toJSON());
    expect(rendered).toContain(':gateway/mixedcasegateway-??????????');
    expect(rendered).not.toContain(':gateway/MixedCaseGateway-??????????');
  });

  test('grantEvaluateForGateway falls back to a wildcard only for a deploy-time gateway name', () => {
    const newApp = new cdk.App();
    const newStack = new cdk.Stack(newApp, 'param-gateway-stack', {
      env: { account: '123456789012', region: 'us-east-1' },
    });
    const newEngine = new PolicyEngine(newStack, 'param-engine', {
      policyEngineName: 'param_engine',
    });
    const newRole = new iam.Role(newStack, 'param-role', {
      assumedBy: new iam.ServicePrincipal('bedrock-agentcore.amazonaws.com'),
    });
    // A CfnParameter value exists only at deploy time. It resolves to a CloudFormation intrinsic
    // rather than a string, so nothing at synthesis can narrow it and the wildcard is unavoidable.
    const nameParam = new cdk.CfnParameter(newStack, 'GatewayNameParam', { type: 'String' });
    const gateway = new Gateway(newStack, 'param-gateway', {
      gatewayName: nameParam.valueAsString,
      role: newRole,
    });

    newEngine.grantEvaluateForGateway(newRole, gateway);

    expect(JSON.stringify(Template.fromStack(newStack).toJSON())).toContain(':gateway/*');
  });
        ...
        Statement: Match.arrayWith([
          Match.objectLike({
            ...
            Resource: Match.arrayWith([
              ...
              Match.objectLike({
                'Fn::Join': ['', Match.arrayWith([{ Ref: 'AWS::Partition' }, Match.stringLikeRegexp(':bedrock-agentcore:.*:gateway/auth-gateway-\\?{10}')])],
              }),
            ]),
          }),
        ]),
...
describe('PolicyEngine metrics tests', () => {
  ...
  test('metric with custom dimensions', () => {
    const metric = policyEngine.metric('CustomMetric', { dimensionsMap: { CustomDimension: 'value' } });
    expect(metric.namespace).toBe('AWS/Bedrock-AgentCore');
    expect(metric.metricName).toBe('CustomMetric');
    expect(metric.dimensions).toEqual({ CustomDimension: 'value', Resource: policyEngine.policyEngineArn });
  });
});
...
describe('PolicyEngine confused deputy protection', () => {
  test('grantPrincipal scopes the bedrock-agentcore service principal with aws:SourceAccount and aws:SourceArn', () => {
    ...
    // Granting the engine (as grantee) against a resource-based policy exercises its
    // grantPrincipal, routing the scoped service principal into the KMS key policy.
    key.grantDecrypt(engine);
              ...
              ArnLike: {
                'aws:SourceArn': {
                  'Fn::Join': Match.arrayWith([
                    Match.arrayWith([Match.stringLikeRegexp('policy-engine/test_engine-\\*')]),
                  ]),
                },
              },
            },

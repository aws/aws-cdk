...
describe('Policy default tests', () => {
  ...
  test('Policy should depend on PolicyEngine', () => {
    template.hasResource('AWS::BedrockAgentCore::Policy', {
      DependsOn: ['testengineE9EB0227'],
    });
  });
});
...
describe('Policy metrics tests', () => {
  ...
  test('metric with custom dimensions', () => {
    const metric = policy.metric('CustomMetric', { dimensionsMap: { CustomDimension: 'value' } });
    expect(metric.namespace).toBe('AWS/Bedrock-AgentCore');
    expect(metric.metricName).toBe('CustomMetric');
    expect(metric.dimensions).toEqual({ CustomDimension: 'value', Resource: policy.policyArn });
  });
});
...
describe('Policy confused deputy protection', () => {
  test('grantPrincipal scopes the bedrock-agentcore service principal with aws:SourceAccount and aws:SourceArn', () => {
    const app = new cdk.App();
    const stack = new cdk.Stack(app, 'stack', {
      env: { account: '123456789012', region: 'us-east-1' },
    });
    const engine = new PolicyEngine(stack, 'Engine', { policyEngineName: 'test_engine' });
    ...
    // Granting the policy (as grantee) against a resource-based policy exercises its
    // grantPrincipal, routing the scoped service principal into the KMS key policy.
    key.grantDecrypt(policy);
              ...
              ArnLike: {
                'aws:SourceArn': {
                  'Fn::Join': Match.arrayWith([
                    Match.arrayWith([Match.stringLikeRegexp('/policy/test_policy-\\*')]),
                  ]),
                },
              },
            },
...
describe('PolicyValidationMode.of() escape hatch', () => {
  test('renders custom validation mode in the template', () => {
    // `ValidationMode` is an enumerated CFN property, so an unmodelled value synthesizes with a warning.
    const app = new cdk.App({ postCliContext: { '@aws-cdk/core:strictCfnValidateErrors': false } });
    const stack = new cdk.Stack(app, 'PolicyValidationModeOfStack');
    const policyEngine = new PolicyEngine(stack, 'PolicyEngine', {
      policyEngineName: 'of_validation_engine',
    });

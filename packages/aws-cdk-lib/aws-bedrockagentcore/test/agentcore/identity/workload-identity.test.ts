...
describe('WorkloadIdentity', () => {
  test('synthesizes expected CloudFormation resource', () => {
    ...
    new WorkloadIdentity(stack, 'MyIdentity', {
  ...
  test('auto-generates physical name when not specified', () => {
    ...
    template.resourceCountIs('AWS::BedrockAgentCore::WorkloadIdentity', 1);
    template.hasResourceProperties('AWS::BedrockAgentCore::WorkloadIdentity', {
      Name: Match.stringLikeRegexp('TestStackMyIdentity.*'),
    });
  });
  ...
  test('grantRead scopes actions to identity ARN and parent resources', () => {
    ...
    const wi = new WorkloadIdentity(stack, 'MyIdentity', {
        ...
        Statement: Match.arrayWith([
          Match.objectLike({
            ...
            Resource: Match.arrayWith([
              { 'Fn::GetAtt': [Match.stringLikeRegexp('MyIdentity.*'), 'WorkloadIdentityArn'] },
            ]),
          }),
        ]),
      },
  ...
  test('grantAdmin grants control plane permissions', () => {
    ...
    template.hasResourceProperties('AWS::IAM::Policy', {
      PolicyDocument: {
        Statement: Match.arrayWith([
          Match.objectLike({
            Action: Match.arrayWith(WorkloadIdentityPerms.ADMIN_PERMS),
            Effect: 'Allow',
          }),
        ]),
      },
  ...
  test('grantUse grants data plane token-minting permissions', () => {
    ...
    template.hasResourceProperties('AWS::IAM::Policy', {
      PolicyDocument: {
        Statement: Match.arrayWith([
          Match.objectLike({
            Action: Match.arrayWith(WorkloadIdentityPerms.USE_PERMS),
            Effect: 'Allow',
            Resource: Match.arrayWith([
              Match.objectLike({ 'Fn::Join': Match.arrayWith([Match.arrayWith([Match.stringLikeRegexp('workload-identity-directory/default')])]) }),
            ]),
          }),
        ]),
      },
  ...
  // Tests below verify import behavior (construct property accessors).
  // No CFN synthesis — no template counterpart applicable.

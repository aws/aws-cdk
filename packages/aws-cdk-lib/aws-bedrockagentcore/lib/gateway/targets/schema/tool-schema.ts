 ...
 */
export class SchemaDefinitionType {
  /** String type */
  public static readonly STRING = new SchemaDefinitionType('string');
  /** Number type */
  public static readonly NUMBER = new SchemaDefinitionType('number');
  /** Object type */
  public static readonly OBJECT = new SchemaDefinitionType('object');
  /** Array type */
  public static readonly ARRAY = new SchemaDefinitionType('array');
  /** Boolean type */
  public static readonly BOOLEAN = new SchemaDefinitionType('boolean');
  /** Integer type */
  public static readonly INTEGER = new SchemaDefinitionType('integer');

  /**
   * Use a custom schema definition type not yet defined in this class.
   * @param value The type string value
   */
  public static of(value: string): SchemaDefinitionType {
    return new SchemaDefinitionType(value);
  }
...
export class InlineToolSchema extends ToolSchema {
  ...
  public _render(): any {
    return {
      inlinePayload: this.schema.map(tool => ({
        ...tool,
        inputSchema: this.renderSchemaDefinition(tool.inputSchema),
        ...(tool.outputSchema && { outputSchema: this.renderSchemaDefinition(tool.outputSchema) }),
      })),
    };
  }

  private renderSchemaDefinition(def: SchemaDefinition): any {
    return {
      type: def.type.value,
      ...(def.description && { description: def.description }),
      ...(def.items && { items: this.renderSchemaDefinition(def.items) }),
      ...(def.properties && {
        properties: Object.fromEntries(
          Object.entries(def.properties).map(([k, v]) => [k, this.renderSchemaDefinition(v)]),
        ),
      }),

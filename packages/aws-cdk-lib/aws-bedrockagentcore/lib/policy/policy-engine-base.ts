...
export abstract class PolicyEngineBase extends Resource implements IPolicyEngine {
  ...
  public grantEvaluateForGateway(grantee: iam.IGrantable, gateway: IGateway): iam.Grant {
    ...
    // using gateway.gatewayName here to avoid a circular dependency between PolicyEngine and Gateway that
    ...
    // is unknowable at synthesis. Using `${gateway.gatewayName}-*` allows us to correctly match the actual ARN at runtime.
    const gatewayResourceName = Token.isUnresolved(gateway.gatewayName) ? '*' : `${gateway.gatewayName}-*`;

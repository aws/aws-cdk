...
import type { CfnPolicyEngineProps } from '../../../aws-bedrockagentcore';
...
export class PolicyEngine extends PolicyEngineBase {
  ...
  constructor(scope: Construct, id: string, props: PolicyEngineProps = {}) {
    ...
    // this PolicyEngine (for example a customer-managed KMS key policy) only allows
    // the AgentCore service to act on behalf of THIS policy engine, not any other
    // AgentCore resource in the account. This is the cross-service confused deputy
    // protection AWS recommends for bedrock-agentcore, which prescribes the full
    // aws:SourceArn when it is known and a wildcard only for the unknown portion of
    // the ARN otherwise. The engine ARN is `policy-engine/<name>-<random suffix>`:
    // the name and its `-` separator are known, and only the suffix the service
    // appends at creation time is unknown, so we wildcard just that suffix as
    // `<name>-*`. Keeping the `-` literal stops a same-prefixed sibling (an engine
    // named `foo` would otherwise also match `foobar-...`) from satisfying it.
      ...
      conditions: {
        ...
        ArnLike: {
          'aws:SourceArn': Arn.format(
            { service: 'bedrock-agentcore', resource: 'policy-engine', resourceName: `${this.policyEngineName}-*` },
            Stack.of(this),
          ),
        },
      },

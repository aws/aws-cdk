...
export interface OnlineEvaluationConfigProps extends OnlineEvaluationBaseProps {
   ...
   * @maximum 10
   */
  readonly evaluators: EvaluatorSelector[];
...
export class OnlineEvaluationConfig extends OnlineEvaluationBase {
  ...
  public static readonly PROPERTY_INJECTION_ID: string =
    'aws-cdk-lib.aws-bedrockagentcore.OnlineEvaluationConfig';
  ...
  constructor(scope: Construct, id: string, props: OnlineEvaluationConfigProps) {
    super(scope, id, { physicalName: props.onlineEvaluationConfigName });
  ...
  private buildRuleConfig(props: OnlineEvaluationConfigProps): bedrockagentcore.CfnOnlineEvaluationConfig.RuleProperty {
    return {
      samplingConfig: { samplingPercentage: props.samplingPercentage ?? 10 },
      sessionConfig: { sessionTimeoutMinutes: props.sessionTimeout?.toMinutes() ?? 15 },
      ...(props.filters && props.filters.length > 0 ? {
        filters: props.filters.map((filter) => ({
          key: filter.key,
          operator: filter.operator.value,
          value: filter.value._bind(),
        })),
      } : {}),
    };
  }

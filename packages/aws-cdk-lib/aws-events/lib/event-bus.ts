...
import { md5hash, memoizedGetter } from '../../core/lib/helpers-internal';
...
export class EventBus extends EventBusBase {
  ...
  /**
   * Counter for statements added without a sid, to give each a distinct generated one
   */
  private autoGeneratedStatementCount = 0;
   ...
   * `AWS::Events::EventBusPolicy` resource, so it must be unique within the account and
   * region across all stacks. With the `@aws-cdk/aws-events:eventBusPolicyUniqueStatementId`
   * feature flag enabled, the statement id is suffixed with a hash unique to this event
   * bus so that statements with the same `sid` in different stacks do not collide, and
   * the statement's `sid` becomes optional: when omitted, a `Statement<N>` sid is
   * generated, so avoid using that form for your own sids on the same bus. Without the
   * flag, the `sid` is required and used as-is, and deploying the same `sid` twice in
   * one account fails.
   */
  @MethodMetadata()
  public addToResourcePolicy(statement: iam.PolicyStatement): iam.AddToResourcePolicyResult {
    if (FeatureFlags.of(this).isEnabled(cxapi.EVENT_BUS_POLICY_UNIQUE_STATEMENT_ID)) {
      return this.addUniqueStatement(statement);
    }
  ...
  /**
   * Adds the statement with a StatementId that is unique within the account and region.
   *
   * The construct id keeps the legacy `cdk-<sid>` form so that enabling the feature flag
   * does not change logical IDs; only the physical `StatementId` gains the unique suffix,
   * which CloudFormation handles as an in-place replacement of the policy resource.
   */
  private addUniqueStatement(statement: iam.PolicyStatement): iam.AddToResourcePolicyResult {
    const sid = statement.sid ?? `Statement${this.autoGeneratedStatementCount++}`;
    const constructId = `cdk-${sid}`.slice(0, 64);

    // Unique per (stack, bus): Names.uniqueId includes the stack name and construct path
    const busSuffix = md5hash(Names.uniqueId(this)).slice(0, 8);
    // 64-char CloudFormation limit: 55 for the legacy prefix + '-' + 8-char hash
    const statementId = `${`cdk-${sid}`.slice(0, 55)}-${busSuffix}`;

    statement.sid = statementId;
    const policy = new EventBusPolicy(this, constructId, {
      eventBus: this,
      statement: statement.toJSON(),
      statementId,
    });

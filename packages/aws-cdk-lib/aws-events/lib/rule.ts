...
export interface RuleProps extends EventCommonOptions {
   ...
   *
   * To set a user-defined id on a target (for example, to align with
   * infrastructure created outside of CDK), call `addTarget(target, { id })`
   * instead of declaring the target here.
   *
   * @default - No targets.
   */
...
export interface AddTargetOptions {
   ...
   * hyphens (`-`), and underscores (`_`). The identifier must be between
   * 1 and 64 characters long.
   *
   * Setting this lets you align the EventBridge target `Id` with
   * infrastructure created outside of CDK (for example, by the
   * `PutTargets` API), avoiding spurious drift detection.
   *
   * Note that the id supplied here is only used for the CloudFormation
   * target `Id` field. Construct identifiers and resource names derived
   * from the target binding (such as IAM roles or log groups created by
   * the target) continue to use the auto-generated id.
   *
   * @default - a unique id is auto-generated for each target (e.g. `Target0`, `Target1`, ...)
   */
  readonly id?: string;
}
...
export class Rule extends Resource implements IRule {
   ...
   * @param options Options for the target, such as a user-defined target id.
   */
  @MethodMetadata()
  public addTarget(target?: IRuleTarget, options: AddTargetOptions = {}): void {
    if (!target) { return; }
    ...
    if (userId !== undefined && !Token.isUnresolved(userId)) {
      if (userId.length < 1 || userId.length > 64) {
        throw new ValidationError(
          lit`InvalidTargetId`,
          `Target id must be between 1 and 64 characters. Received: ${JSON.stringify(userId)}`,
          this,
        );
      }
      if (!/^[\.\-_A-Za-z0-9]+$/.test(userId)) {
        throw new ValidationError(
          lit`InvalidTargetId`,
          `Target id ${JSON.stringify(userId)} can contain only letters, numbers, periods, hyphens, or underscores with no spaces.`,
          this,
        );
      }
    }
    ...
    const id = userId ?? (targetProps.id || autoGeneratedId);

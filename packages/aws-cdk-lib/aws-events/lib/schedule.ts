  ...
  public static rate(duration: Duration): Schedule {
    if (duration.isUnresolved()) {
      ...
      if (validDurationUnit.indexOf(duration.unitLabel()) === -1) {
        throw new UnscopedValidationError(lit`InvalidScheduleUnit`, "Allowed units for scheduling are: 'minute', 'minutes', 'hour', 'hours', 'day', 'days'");
      }
      return new LiteralSchedule(`rate(${duration.formatTokenToNumber()})`);
    }
    if (duration.toMinutes() === 0) {
      throw new UnscopedValidationError(lit`DurationCannotBeZero`, 'Duration cannot be 0');
    }

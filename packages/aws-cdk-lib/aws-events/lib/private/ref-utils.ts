...
export function toIConnection(connection: IConnectionRef): IConnection {
  if (!('connectionArn' in connection) || !('connectionName' in connection)) {
    throw new UnscopedValidationError('ConnectionInstanceShouldImplement', `'connection' instance should implement IConnection, but doesn't: ${connection.constructor.name}`);
  }

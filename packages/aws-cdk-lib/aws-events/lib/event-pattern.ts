...
export class Match implements IResolvable {
  ...
  public static cidr(range: string): string[] {
    ...
    if (!ipv4Regex.test(range) && !ipv6Regex.test(range)) {
      throw new UnscopedValidationError('InvalidInvalidAddressRange', `Invalid IP address range: ${range}`);
    }
  ...
  public static anythingBut(...values: any[]): string[] {
    if (values.length === 0) {
      throw new UnscopedValidationError('MustBeAnythingbutMatchersNonEmpty', 'anythingBut matchers must be non-empty lists');
    }
    ...
    if (!(allNumbers || allStrings)) {
      throw new UnscopedValidationError('MustBeAnythingbutMatchersLists', 'anythingBut matchers must be lists that contain only strings or only numbers.');
    }
  ...
  public static interval(lower: number, upper: number): string[] {
    if (lower > upper) {
      throw new UnscopedValidationError('InvalidInvalidInterval', `Invalid interval: [${lower}, ${upper}]`);
    }
  ...
  private merge(matchers: any[]): any {
    ...
    if (!matchers.every(matcher => matcher?.numeric)) {
      throw new UnscopedValidationError('OnlyNumericMatchersMerged', 'Only numeric matchers can be merged into a single matcher.');
    }

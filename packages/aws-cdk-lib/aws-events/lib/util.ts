...
export function mergeEventPattern(dest: any, src: any) {
  ...
  function mergeObject(destObj: any, srcObj: any) {
    if (typeof(srcObj) !== 'object') {
      throw new UnscopedValidationError('InvalidInvalidEventPattern', `Invalid event pattern '${JSON.stringify(srcObj)}', expecting an object or an array`);
    }
    ...
    for (const field of Object.keys(srcObj)) {
      ...
      if (typeof(srcValue) !== 'object') {
        throw new UnscopedValidationError('InvalidInvalidEventPattern', `Invalid event pattern field { ${field}: ${JSON.stringify(srcValue)} }. All fields must be arrays`);
      }

...
describe('rule', () => {
  ...
  test('rule displays warning when minutes are not included in cron', () => {
    ...
    Annotations.fromStack(stack).hasWarning('/Default/MyRule', "Events:Schedule:WillRunEveryMinute: cron: If you don't pass 'minute', by default the event runs every minute. Pass 'minute: '*'' if that's what you intend, or 'minute: 0' to run once per hour instead.");
  });

...
export class ScalableTarget extends Resource implements IScalableTarget {
  ...
  public static fromScalableTargetId(scope: Construct, id: string, scalableTargetId: string): IScalableTarget {
    class Import extends Resource implements IScalableTarget {
      ...
      public get scalableTargetRef(): ScalableTargetReference {
        throw new ValidationError('CannotCannotcannotaccessscalabletargetref', 'Cannot access scalableTargetRef on a ScalableTarget imported by ID only. Use ScalableTarget.fromScalableTargetAttributes() instead.', this);
      }
    }
  ...
  constructor(scope: Construct, id: string, props: ScalableTargetProps) {
    ...
    withResolved(props.minCapacity, props.maxCapacity, (min, max) => {
      if (max < min) {
        throw new ValidationError('Shouldbemincapacityshouldlower', `minCapacity (${props.minCapacity}) should be lower than maxCapacity (${props.maxCapacity})`, scope);
      }
    });
  ...
  public scaleOnSchedule(id: string, action: ScalingSchedule) {
    if (action.minCapacity === undefined && action.maxCapacity === undefined) {
      throw new ValidationError('Supplyleastmincapacitymaxcapacity', `You must supply at least one of minCapacity or maxCapacity, got ${JSON.stringify(action)}`, this);
    }

...
export class StepScalingPolicy extends Construct {
  ...
  constructor(scope: Construct, id: string, props: StepScalingPolicyProps) {
    ...
    if (props.scalingSteps.length < 2) {
      throw new ValidationError(lit`SupplyLeastIntervalsAutoscaling`, 'You must supply at least 2 intervals for autoscaling', scope);
    }

    if (props.scalingSteps.length > 40) {
      throw new ValidationError(lit`ScalingstepsMostSteps`, `'scalingSteps' can have at most 40 steps, got ${props.scalingSteps.length}`, scope);
    }

    if (props.evaluationPeriods !== undefined && !cdk.Token.isUnresolved(props.evaluationPeriods) && props.evaluationPeriods < 1) {
      throw new ValidationError(lit`EvaluationPeriodsCannotLess`, `evaluationPeriods cannot be less than 1, got: ${props.evaluationPeriods}`, scope);
    }
    if (props.datapointsToAlarm !== undefined) {
      if (props.evaluationPeriods === undefined) {
        throw new ValidationError(lit`EvaluationPeriodsSetDatapointsAlarm`, 'evaluationPeriods must be set if datapointsToAlarm is set', scope);
      }
      if (!cdk.Token.isUnresolved(props.datapointsToAlarm) && props.datapointsToAlarm < 1) {
        throw new ValidationError(lit`DatapointsAlarmCannotLess`, `datapointsToAlarm cannot be less than 1, got: ${props.datapointsToAlarm}`, scope);
      }
      if (!cdk.Token.isUnresolved(props.datapointsToAlarm)
        ...
        throw new ValidationError(lit`DatapointsAlarmLessEqualEvaluation`, `datapointsToAlarm must be less than or equal to evaluationPeriods, got datapointsToAlarm: ${props.datapointsToAlarm}, evaluationPeriods: ${props.evaluationPeriods}`, scope);
      }
    }

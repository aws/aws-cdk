...
describe('cron', () => {
  ...
  test('test europe/london cron', () => {
    expect(appscaling.Schedule.cron({ hour: '18', minute: '0', timeZone: TimeZone.EUROPE_LONDON }).timeZone?.timezoneName).toEqual('Europe/London');
  });
});
...
describe('rate', () => {
  test('rate must be whole number of minutes', () => {
    ...
    }).toThrow(/0.13456 must be a whole number of minutes/);
  });

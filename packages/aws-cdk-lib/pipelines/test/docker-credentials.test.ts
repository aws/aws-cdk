...
describe('ExternalDockerCredential', () => {
  ...
  test('maximum example includes all expected properties', () => {
    const roleArn = 'arn:aws:iam::012345678901:role/MyRole';
    const creds = cdkp.DockerCredential.customRegistry('example.com', secret, {
  ...
  describe('grantRead', () => {
    ...
    test('grants read access to the secret to the assumeRole if provided', () => {
      const assumeRole = iam.Role.fromRoleArn(stack, 'Role', 'arn:aws:iam::012345678901:role/MyRole');
      ...
      Template.fromStack(stack).hasResourceProperties('AWS::IAM::Policy', {
        PolicyDocument: {
          Statement: [{
            ...
            Effect: 'Allow',
            Resource: 'arn:aws:iam::012345678901:role/MyRole',
          }],
          Version: '2012-10-17',
        },
        Users: [{ Ref: 'User00B015A1' }],
...
describe('EcrDockerCredential', () => {
  ...
  beforeEach(() => {
    repo = ecr.Repository.fromRepositoryArn(stack, 'Repo', 'arn:aws:ecr:eu-west-1:012345678901:repository/Repo');
  });
  ...
  test('minimal example only renders ecrRepository', () => {
    ...
    expect(stack.resolve(Object.keys(config))).toEqual([{
      'Fn::Select': [
        0, {
          'Fn::Split': ['/', {
            'Fn::Join': ['', ['012345678901.dkr.ecr.eu-west-1.', { Ref: 'AWS::URLSuffix' }, '/Repo']],
          }],
        },
      ],
    }]);
  ...
  describe('grantRead', () => {
      ...
      Template.fromStack(stack).hasResourceProperties('AWS::IAM::Policy', {
        PolicyDocument: {
          Statement: [{
            ...
            Effect: 'Allow',
            Resource: 'arn:aws:ecr:eu-west-1:012345678901:repository/Repo',
          },
          {
    ...
    test('grants pull access to multiple repos if provided', () => {
      const repo2 = ecr.Repository.fromRepositoryArn(stack, 'Repo2', 'arn:aws:ecr:eu-west-1:012345678901:repository/Repo2');
      ...
      Template.fromStack(stack).hasResourceProperties('AWS::IAM::Policy', {
        PolicyDocument: {
          ...
          },
          {
            ...
            Effect: 'Allow',
            Resource: 'arn:aws:ecr:eu-west-1:012345678901:repository/Repo2',
          }]),
          Version: '2012-10-17',
        },
        Users: [{ Ref: 'User00B015A1' }],
...
describe('dockerCredentialsInstallCommands', () => {
  const secretArn = 'arn:aws:secretsmanager:eu-west-1:012345678901:secret:mySecret-012345';
  let secret: secretsmanager.ISecret;

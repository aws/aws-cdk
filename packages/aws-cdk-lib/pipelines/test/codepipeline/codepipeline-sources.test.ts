...
import { CodeCommitTrigger, GitHubTrigger } from '../../../aws-codepipeline-actions';
import * as ecr from '../../../aws-ecr';
import { AnyPrincipal, Role } from '../../../aws-iam';
import * as s3 from '../../../aws-s3';
import { SecretValue, Stack, Token } from '../../../core';
    ...
    Stages: Match.arrayWith([
      {
        ...
        Actions: [
          Match.objectLike({
            OutputArtifacts: [{ Name: 'owner_my_repo_Source' }],
          }),
        ],
      },
    ]),
    ...
    Stages: Match.arrayWith([
      {
        ...
        Actions: [
          Match.objectLike({
            OutputArtifacts: [{ Name: artifactId }],
          }),
        ],
      },
    ]),
...
test('can use source attributes in pipeline', () => {
  ...
  new ModernTestGitHubNpmPipeline(pipelineStack, 'Pipeline', {
    ...
    synth: new cdkp.ShellStep('Synth', {
      ...
      commands: ['echo "Click here: $GITHUB_URL"'],
    }),
    selfMutation: false,
  });
    ...
    Stages: Match.arrayWith([
      {
        ...
        Actions: [
          Match.objectLike({
            ...
            Name: { Ref: Match.anyValue() },
            RoleArn: {
              'Fn::GetAtt': [Match.stringLikeRegexp('TestRole.*'), 'Arn'],
            },
          }),
        ],
      },

...
export class CodeBuildFactory implements ICodePipelineActionFactory {
  ...
  public get project(): codebuild.IProject {
    if (!this._project) {
      throw new UnscopedValidationError('ProjectBecomesAvailableAfter', 'Project becomes available after produce() has been called');
    }
  ...
  public produceAction(stage: codepipeline.IStage, options: ProduceActionOptions): CodePipelineActionFactoryResult {
    ...
    if ((!projectOptions.buildEnvironment?.privileged || projectOptions.vpc === undefined) &&
      ...
      throw new UnscopedValidationError('SettingFilesystemlocationsRequiresVpc', 'Setting fileSystemLocations requires a vpc to be set and privileged to be set to true.');
    }
    ...
    if (!inputArtifact) {
      ...
      throw new UnscopedValidationError('CodebuildActionStepidRequires', `CodeBuild action '${this.stepId}' requires an input (and the pipeline doesn't have a Source to fall back to). Add an input or a pipeline source.`);
    }
    ...
    if (this.props.passBuildSpecViaCloudAssembly) {
      ...
      if (typeof fileContents !== 'string') {
        throw new UnscopedValidationError('BuildspecContainsCloudformationReferences', `This BuildSpec contains CloudFormation references and is supported by publishInParallel=false: ${JSON.stringify(fileContents, undefined, 2)}`);
      }
...
function renderArtifactsBuildSpec(artifactMap: ArtifactMap, outputs: FileSetLocation[]) {
  ...
  for (const output of outputs) {
    ...
    if (!art.artifactName) {
      throw new UnscopedValidationError('GiveOutputArtifactName', 'You must give the output artifact a name');
    }

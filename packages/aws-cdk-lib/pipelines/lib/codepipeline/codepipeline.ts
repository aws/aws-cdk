...
export interface CodePipelineProps {
   ...
   * to store the pipeline's artifact.
   *
   * Only used when the artifact bucket is created by the pipeline itself,
   * i.e. when neither `artifactBucket` nor `codePipeline` is provided.
   *
   ...
   * automatically created to store the pipeline's artifact when the bucket
   * is removed.
   *
   * Only used when the artifact bucket is created by the pipeline itself,
   * i.e. when neither `artifactBucket` nor `codePipeline` is provided.
...
export class CodePipeline extends PipelineBase {
  ...
  public get selfMutationProject(): cb.IProject {
    ...
    if (!this._selfMutationProject) {
      throw new UnscopedValidationError(lit`SelfMutationProjectSinceSelf`, 'No selfMutationProject since the selfMutation property was set to false');
    }
  ...
  public get pipeline(): cp.Pipeline {
    if (!this._pipeline) {
      throw new UnscopedValidationError(lit`PipelineCreated`, 'Pipeline not created yet');
    }
  ...
  protected doBuildPipeline(): void {
    if (this._pipeline) {
      throw new ValidationError(lit`PipelineAlreadyCreated`, 'Pipeline already created', this);
    }
    ...
    if (this.props.codePipeline) {
      if (this.props.pipelineName) {
        throw new ValidationError(lit`CannotSetPipelineNameExisting`, 'Cannot set \'pipelineName\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      if (this.props.crossAccountKeys !== undefined) {
        throw new ValidationError(lit`CannotSetCrossAccountKeys`, 'Cannot set \'crossAccountKeys\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      if (this.props.enableKeyRotation !== undefined) {
        throw new ValidationError(lit`CannotSetEnableKeyRotation`, 'Cannot set \'enableKeyRotation\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      if (this.props.crossRegionReplicationBuckets !== undefined) {
        throw new ValidationError(lit`CannotSetCrossRegionReplication`, 'Cannot set \'crossRegionReplicationBuckets\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      if (this.props.reuseCrossRegionSupportStacks !== undefined) {
        throw new ValidationError(lit`CannotSetReuseCrossRegion`, 'Cannot set \'reuseCrossRegionSupportStacks\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      if (this.props.role !== undefined) {
        throw new ValidationError(lit`CannotSetRoleExistingCode`, 'Cannot set \'role\' if an existing CodePipeline is given using \'codePipeline\'', this);
      }
      ...
      if (this.props.artifactBucket !== undefined
        && (this.props.artifactBucketRemovalPolicy !== undefined || this.props.artifactBucketAutoDeleteObjects !== undefined)) {
        throw new ValidationError(lit`CannotSetArtifactBucketRemovalPolicyWithBucket`, 'Cannot set \'artifactBucketRemovalPolicy\' or \'artifactBucketAutoDeleteObjects\' when \'artifactBucket\' is specified. Configure the removal policy on the provided bucket directly.', this);
      }
  ...
  private get myCxAsmRoot(): string {
    if (!this._myCxAsmRoot) {
      throw new ValidationError(lit`CanTReadMycxasmrootBuild`, 'Can\'t read \'myCxAsmRoot\' if build deployment not called yet', this);
    }
  ...
  private pipelineStagesAndActionsFromGraph(structure: PipelineGraph) {
    ...
    for (const stageNode of flatten(structure.graph.sortedChildren())) {
      if (!isGraph(stageNode)) {
        throw new ValidationError(lit`MustBeTopLevelChildrenGraphs`, `Top-level children must be graphs, got '${stageNode}'`, this);
      }
  ...
  private actionFromNode(node: AGraphNode): ICodePipelineActionFactory {
    switch (node.data?.type) {
      ...
      case 'stack-group':
      case undefined:
        throw new ValidationError(lit`ActionNodeExpectGroupNodes`, `actionFromNode: did not expect to get group nodes: ${node.data?.type}`, this);
      ...
      default:
        throw new ValidationError(lit`CodepipelineDoesSupportGraph`, `CodePipeline does not support graph nodes of type '${node.data?.type}'. You are probably using a feature this CDK Pipelines implementation does not support.`, this);
    }
  }
  ...
  private actionFromStep(node: AGraphNode, step: Step): ICodePipelineActionFactory {
    ...
    throw new ValidationError(lit`DeploymentStepSupportedCodepipelineBacked`, `Deployment step '${step}' is not supported for CodePipeline-backed pipelines`, this);
  }
  ...
  private publishAssetsAction(node: AGraphNode, assets: StackAsset[]): ICodePipelineActionFactory {
    ...
    if (assets.some(a => a.assetType !== assetType)) {
      throw new ValidationError(lit`AssetsSinglePublishingStepType`, 'All assets in a single publishing step must be of the same type', this);
    }

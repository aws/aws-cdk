...
export class PipelineGraph {
  ...
  constructor(public readonly pipeline: PipelineBase, props: PipelineGraphProps = {}) {
    ...
    if (!cloudAssembly) {
      throw new ValidationError('SynthStepProduceCloud', `The synth step must produce the cloud assembly artifact, but doesn't: ${pipeline.synth}`, this.pipeline);
    }
  ...
  private addStage(stage: StageDeployment): AGraph {
    ...
    for (const stack of stage.stacks) {
      ...
      if (stack.changeSet.length > 0) {
        if (prepareNode) {
          ...
          throw new ValidationError('ChangesetStepsStackStack', `Cannot use \'changeSet\' steps for stack \'${stack.stackName}\': the pipeline does not support them or they have been disabled`, this.pipeline);
        }
      }
      ...
      if (this.publishTemplate) {
        if (!stack.templateAsset) {
          throw new ValidationError('PublishtemplateEnabledStackStack', `"publishTemplate" is enabled, but stack ${stack.stackArtifactId} does not have a template asset`, this.pipeline);
        }
    ...
    for (const stack of stage.stacks) {
      for (const dep of stack.stackDependencies) {
        ...
        if (!stackNode) {
          throw new ValidationError('FindNodeStackStackname', `cannot find node for ${stack.stackName}`, this.pipeline);
        }
        if (!depNode) {
          throw new ValidationError('FindNodeDepStackname', `cannot find node for ${dep.stackName}`, this.pipeline);
        }
  ...
  private addMissingDependencyNodes() {
    ...
    while (attempts-- > 0) {
      ...
      for (const [step, builder] of unsatisfied) {
        ...
        if (!parent) {
          throw new ValidationError('ConsumerDoesnParentGraph', `Consumer doesn't have a parent graph: ${leftMostConsumer}`, this.pipeline);
        }
    ...
    throw new ValidationError('RecursionDepthTooLarge', [
  ...
  private publishAsset(stackAsset: StackAsset): AGraphNode {
    ...
    if (data?.type !== 'publish-assets') {
      throw new ValidationError('AssetnodeWrongDataType', `${assetNode} has the wrong data.type: ${data?.type}`, this.pipeline);
    }

...
export class Pipeline extends PipelineBase {
  ...
  constructor(scope: Construct, id: string, props: PipelineProps = {}) {
    ...
    if (props.artifactBucket && props.crossRegionReplicationBuckets) {
      throw new ValidationError('OnlyArtifactBucketOrCrossRegionReplicationBucketsSpecified', 'Only one of artifactBucket and crossRegionReplicationBuckets can be specified!', this);
    }
    ...
    if (this.enableKeyRotation && !this.crossAccountKeys) {
      throw new ValidationError('CrossAccountKeysRequiredForKeyRotation', "Setting 'enableKeyRotation' to true also requires 'crossAccountKeys' to be enabled", this);
    }
    ...
    if (
      ...
      throw new ValidationError('ExecutionModeRequiresV2Pipeline', `${props.executionMode} execution mode can only be used with V2 pipelines, \`PipelineType.V2\` must be specified for \`pipelineType\``, this);
    }
  ...
  public addStage(props: StageOptions): IStage {
    ...
    if (this._stages.find(s => s.stageName === props.stageName)) {
      throw new ValidationError('StageDuplicateNameAdded', `Stage with duplicate name '${props.stageName}' added to the Pipeline`, this);
    }
  ...
  public addVariable(variable: Variable): Variable {
    ...
    if (this.variables.find(v => v.variableName === variable.variableName)) {
      throw new ValidationError('VariableDuplicateNameAdded', `Variable with duplicate name '${variable.variableName}' added to the Pipeline`, this);
    }
  ...
  public stage(stageName: string): IStage {
    ...
    throw new ValidationError('PipelineDoesNotContainStage', `Pipeline does not contain a stage named '${stageName}'. Available stages: ${this._stages.map(s => s.stageName).join(', ')}`, this);
  }
  ...
  private ensureReplicationResourcesExistFor(action: RichAction): CrossRegionInfo {
    ...
    if (action.actionProperties.category === ActionCategory.SOURCE) {
      throw new ValidationError('SourceActionMustBeSameRegion', `Source action '${action.actionProperties.actionName}' must be in the same region as the pipeline`, this);
    }
  ...
  private createSupportResourcesForRegion(otherStack: Stack | undefined, actionRegion: string): CrossRegionSupport {
    ...
    if (Token.isUnresolved(pipelineAccount)) {
      throw new ValidationError('ExplicitAccountRequiredForCrossRegion', "You need to specify an explicit account when using CodePipeline's cross-region support", this);
    }
  ...
  private getRoleFromActionPropsOrGenerateIfCrossAccount(stage: Stage, action: RichAction): iam.IRole | undefined {
    ...
    if (action.isCrossAccount) {
      ...
      if (!artifactBucket.encryptionKey) {
        throw new ValidationError('ArtifactBucketMustHaveKmsKey',
    ...
    if (action.actionProperties.role) {
      if (this.isAwsOwned(action)) {
        ...
        throw new ValidationError('RoleNotSupportedForNonAwsActions',
  ...
  private getOtherStackIfActionIsCrossAccount(action: IAction): Stack | undefined {
    ...
    if (Token.isUnresolved(targetAccount)) {
      if (Token.isUnresolved(this.env.account)) {
        ...
        throw new ValidationError('AccountPropertyMustBeConcrete', `The 'account' property must be a concrete value (action: '${action.actionProperties.actionName}')`, this);
      }
    }
    ...
    if (Token.isUnresolved(this.env.account)) {
      throw new ValidationError('PipelineStackMustHaveExplicitAccount', 'Pipeline stack which uses cross-environment actions must have an explicitly set account', this);
    }
  ...
  private calculateInsertIndexFromPlacement(placement: StagePlacement): number {
    ...
    if (providedPlacementProps.length > 1) {
      throw new ValidationError('PlacementPropertiesConflict',
    ...
    if (placement.rightBefore !== undefined) {
      ...
      if (targetIndex === -1) {
        throw new ValidationError('StageNotFoundForRightBefore',
          'Error adding Stage to the Pipeline: ' +
    ...
    if (placement.justAfter !== undefined) {
      ...
      if (targetIndex === -1) {
        throw new ValidationError('StageNotFoundForJustAfter',
          'Error adding Stage to the Pipeline: ' +
  ...
  private requireRegion(): string {
    ...
    if (Token.isUnresolved(region)) {
      throw new ValidationError('PipelineStackMustHaveExplicitRegion', 'Pipeline stack which uses cross-environment actions must have an explicitly set region', this);
    }
    return region;
  }
  ...
  private supportScope(): CdkStage {
    ...
    if (!scope) {
      throw new ValidationError('PipelineStackMustBePartOfCdkApp', 'Pipeline stack which uses cross-environment actions must be part of a CDK App or Stage', this);
    }
    return scope;
  }
}

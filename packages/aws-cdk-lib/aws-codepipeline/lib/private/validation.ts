...
function ValidateGitPushFilter(pushFilter: GitPushFilter[], actionName: string) {
  ...
  pushFilter.forEach(filter => {
    ...
    validateTagsAndBranches(filter, actionName);
...
const validateFilePathsWithBranches = (filter: GitPushFilter | GitPullRequestFilter, actionName: string) => {
  if (!hasBranches(filter) && hasFilePaths(filter)) {
    throw new UnscopedValidationError(
      `cannot specify filePaths without branches in '${typeof(filter)}' for sourceAction with name '${actionName}'`,
    );
  }
};

const validateTagsAndBranches = (filter: GitPushFilter, actionName: string) => {
  if (hasTags(filter) && hasBranches(filter)) {
    throw new UnscopedValidationError(
      `cannot specify both tags and branches in GitpushFilter for sourceAction with name '${actionName}'`,
    );
  }
};

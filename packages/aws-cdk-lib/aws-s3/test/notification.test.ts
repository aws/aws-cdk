...
describe('notification', () => {
  ...
  test('custom resource depends on handler IAM policy to prevent race condition on removal', () => {
    ...
    // When removing a notification from one of multiple buckets, each bucket's
    // Custom::S3BucketNotifications must have its own dedicated IAM policy.
    // This ensures removal is a delete (not a shared policy update), so
    // CloudFormation correctly orders custom resource deletion before policy deletion.
    const stack = new cdk.Stack();
  ...
  test('multiple buckets in same stack result in separate per-bucket policies', () => {
    ...
    // Each policy should have scoped permissions (not wildcards)
    for (const policyId of handlerPolicies) {
      expect(allPolicies[policyId].Properties.PolicyDocument.Statement).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            Action: expect.stringContaining('s3:'),
            Effect: 'Allow',

...
export abstract class BucketBase extends Resource implements IBucket, IEncryptedResource {
  ...
  public addReplicationPolicy(roleArn: string, accessControlTransition?: boolean, account?: string) {
    ...
    if (accessControlTransition) {
      if (!account) {
        throw new ValidationError('Mustbeaccountspecifiedoverride', 'account must be specified to override ownership access control transition', this);
      }
      results.push(this.addToResourcePolicy(new iam.PolicyStatement({
...
export class Bucket extends BucketBase {
  ...
  public static fromBucketAttributes(scope: Construct, id: string, attrs: BucketAttributes): IBucket {
    ...
    if (!bucketName) {
      throw new ValidationError('Isrequiredbucketnamerequired', 'Bucket name is required', scope);
    }
  ...
  public static validateBucketName(physicalName: string, allowLegacyBucketNaming: boolean = false): void {
    ...
    if (errors.length > 0) {
      throw new UnscopedValidationError('InvalidInvalidBucketName', `Invalid S3 bucket name (value: ${bucketName})${EOL}${errors.join(EOL)}`);
    }
  }
  ...
  constructor(scope: Construct, id: string, props: BucketProps = {}) {
    ...
    if (props.enforceSSL) {
      ...
      throw new ValidationError('MustBeEnforcesslEnabledMinimumtlsversion', '\'enforceSSL\' must be enabled for \'minimumTLSVersion\' to be applied', this);
    }
    ...
    if (props.publicReadAccess) {
      if (props.blockPublicAccess === undefined) {
        throw new ValidationError('CannotCannotPublicreadaccessProperty', 'Cannot use \'publicReadAccess\' property on a bucket without allowing bucket-level public access through \'blockPublicAccess\' property.', this);
      }
    ...
    if (props.autoDeleteObjects) {
      if (props.removalPolicy !== RemovalPolicy.DESTROY) {
        throw new ValidationError('CannotCannotAutodeleteobjectsProperty', 'Cannot use \'autoDeleteObjects\' property on a bucket without setting removal policy to \'DESTROY\'.', this);
      }
  ...
  private parseEncryption(props: BucketProps): {
    ...
    if (props.bucketKeyEnabled && encryptionType === BucketEncryption.UNENCRYPTED) {
      throw new ValidationError('BucketkeyenabledSpecified', `bucketKeyEnabled is specified, so 'encryption' must be set to KMS, DSSE or S3 (value: ${encryptionType})`, this);
    }
  ...
  private parseLifecycleConfiguration(): CfnBucket.LifecycleConfigurationProperty | undefined {
    ...
    function parseLifecycleRule(rule: LifecycleRule): CfnBucket.RuleProperty {
      ...
      && (rule.expiration || rule.expirationDate || self.parseTagFilters(rule.tagFilters))) {
        ...
        throw new ValidationError('Expiredobjectdeletemarkercannotspecifiedexpiration', 'ExpiredObjectDeleteMarker cannot be specified with expiration, ExpirationDate, or TagFilters.', self);
      }
      ...
      if (rule.transitions) {
        for (const transition of rule.transitions) {
          ...
          if (!hasTransitionDate && !hasTransitionAfter) {
            throw new ValidationError('Mustbeexactlytransitiondatetransitionafter', 'Exactly one of transitionDate or transitionAfter must be specified in lifecycle rule transition', self);
          }
  ...
  private parseServerAccessLogs(props: BucketProps): CfnBucket.LoggingConfigurationProperty | undefined {
    ...
    if (
      ...
      throw new ValidationError('Defaultbucketencryptionmanaged', 'Default bucket encryption with KMS managed or DSSE managed key is not supported for Server Access Logging target buckets', this);
    }
  ...
  private parseObjectLockConfig(props: BucketProps): CfnBucket.ObjectLockConfigurationProperty | undefined {
    ...
    if (objectLockEnabled === false && objectLockDefaultRetention) {
      throw new ValidationError('Mustbeobjectlockenabled', 'Object Lock must be enabled to configure default retention settings', this);
    }
  ...
  private renderWebsiteConfiguration(props: BucketProps): CfnBucket.WebsiteConfigurationProperty | undefined {
    ...
    if (props.websiteErrorDocument && !props.websiteIndexDocument) {
      throw new ValidationError('IsRequiredWebsiteindexdocumentRequiredWebsiteerrordocument', '"websiteIndexDocument" is required if "websiteErrorDocument" is set', this);
    }

    if (props.websiteRedirect && (props.websiteErrorDocument || props.websiteIndexDocument || props.websiteRoutingRules)) {
      throw new ValidationError('Websiteindexdocument', '"websiteIndexDocument", "websiteErrorDocument" and, "websiteRoutingRules" cannot be set if "websiteRedirect" is used', this);
    }
    ...
    const routingRules = props.websiteRoutingRules ? props.websiteRoutingRules.map<CfnBucket.RoutingRuleProperty>((rule) => {
      if (rule.condition && rule.condition.httpErrorCodeReturnedEquals == null && rule.condition.keyPrefixEquals == null) {
        throw new ValidationError('Conditionpropertycannotempty', 'The condition property cannot be an empty object', this);
      }
  ...
  private renderReplicationConfiguration(props: BucketProps): CfnBucket.ReplicationConfigurationProperty | undefined {
    ...
    if (replicationRulesIsEmpty && props.replicationRole) {
      throw new ValidationError('CannotCannotcannotspecifyreplicationrole', 'cannot specify replicationRole when replicationRules is empty', this);
    }
    ...
    props.replicationRules.forEach(rule => {
      if (rule.replicationTimeControl && !rule.metrics) {
        throw new ValidationError('MustBeReplicationtimecontrolmetricsEnabledReplicationtimecontrol', '\'replicationTimeControlMetrics\' must be enabled when \'replicationTimeControl\' is enabled.', this);
      }
      if (rule.deleteMarkerReplication && rule.filter?.tags) {
        throw new ValidationError('FilterCannotSpecifiedDeletemarkerreplication', 'tag filter cannot be specified when \'deleteMarkerReplication\' is enabled.', this);
      }
    });
    ...
    return {
      ...
      rules: props.replicationRules.map((rule) => {
        ...
        if (isCrossAccount) {
          ...
          throw new ValidationError('Accesscontroltranslationonlysupportedcrossaccount', 'accessControlTranslation is only supported for cross-account replication', this);
        }
  ...
  private allowLogDelivery(from: IBucket, prefix?: string) {
    if (FeatureFlags.of(this).isEnabled(cxapi.S3_SERVER_ACCESS_LOGS_USE_BUCKET_POLICY)) {
      ...
      throw new ValidationError('CannotCannotEnableDelivery', "Cannot enable log delivery to this bucket because the bucket's ACL has been set and can't be changed", this);
    } else {
  ...
  private parseInventoryConfiguration(): CfnBucket.InventoryConfigurationProperty[] | undefined {
    ...
    return this.inventories.map((inventory, index) => {
      ...
      if (inventory.inventoryId !== undefined && (inventory.inventoryId.length > 64 || inventoryIdValidationRegex.test(inventory.inventoryId))) {
        throw new ValidationError('Inventoryidshouldexceedcharacters', `inventoryId should not exceed 64 characters and should not contain special characters except . and -, got ${inventory.inventoryId}`, this);
      }

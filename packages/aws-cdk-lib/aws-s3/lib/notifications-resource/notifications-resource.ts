...
function renderFilters(filters: NotificationKeyFilter[], scope: BucketNotifications): Filter | undefined {
  ...
  for (const rule of filters) {
    if (!rule.suffix && !rule.prefix) {
      throw new ValidationError('NotificationkeyfilterSpecifyAndOr', 'NotificationKeyFilter must specify `prefix` and/or `suffix`', scope);
    }

    if (rule.suffix) {
      if (hasSuffix) {
        throw new ValidationError('CannotCannotcannotspecifymore', 'Cannot specify more than one suffix rule in a filter.', scope);
      }
    ...
    if (rule.prefix) {
      if (hasPrefix) {
        throw new ValidationError('CannotCannotcannotspecifymore', 'Cannot specify more than one prefix rule in a filter.', scope);
      }

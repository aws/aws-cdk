...
Rolldown is configured through a `rolldown.config.{mts,mjs,ts,cts,js,cjs}` file that you author yourself — CDK does not re-expose rolldown's config options as props. The construct handles entry resolution, Docker orchestration, `nodeModules` install, and asset hashing, and invokes rolldown as `rolldown -c <rolldownConfigFile> --cwd <projectRoot> --input <entry> --dir <outputDir> --entry-file-names index.js`. `entry` on `NodejsFunction` drives rolldown's input.

By default the construct looks for `rolldown.config.<ext>` in the project root, trying `.mts`, `.mjs`, `.ts`, `.cts`, `.js`, and `.cjs` in that order. CDK forces the output filename to `index.js` so the default `index.handler` entry point resolves the config controls the format and any plugins.

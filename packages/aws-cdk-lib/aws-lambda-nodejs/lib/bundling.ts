...
export class Bundling implements cdk.BundlingOptions {
  ...
  constructor(scope: IConstruct, private readonly props: BundlingProps) {
    ...
    if (this.relativeDepsLockFilePath.includes('..')) {
      throw new ValidationError('ExpectedDepslockfilepathPropsDepslockfilepath', `Expected depsLockFilePath: ${props.depsLockFilePath} to be under projectRoot: ${this.projectRoot} (${this.relativeDepsLockFilePath})`, scope);
    }
    ...
    if (props.preCompilation && !/\.tsx?$/.test(props.entry)) {
      throw new ValidationError('PrecompilationUsedTypescriptFiles', 'preCompilation can only be used with typescript files', scope);
    }

    if (props.format === OutputFormat.ESM && !isEsmRuntime(props.runtime)) {
      throw new ValidationError('EcmascriptModuleOutputFormat', `ECMAScript module output format is not supported by the ${props.runtime.name} runtime`, scope);
    }
  ...
  private createBundlingCommand(scope: IConstruct, options: BundlingCommandOptions): string {
    ...
    if (this.props.preCompilation) {
      ...
      if (!tsconfig) {
        throw new ValidationError('FindTsconfigJsonPrecompilation', 'Cannot find a `tsconfig.json` but `preCompilation` is set to `true`, please specify it via `tsconfig`', scope);
      }
    ...
    if (this.props.sourceMap === false && this.props.sourceMapMode) {
      throw new ValidationError('SourcemapmodeUsedSourcemapFalse', 'sourceMapMode cannot be used when sourceMap is false', scope);
    }
    ...
    if (this.props.nodeModules) {
      ...
      if (!pkgPath) {
        throw new ValidationError('FindPackageJsonProject', 'Cannot find a `package.json` in this project. Using `nodeModules` requires a `package.json`.', scope);
      }
    ...
    return {
      tryBundle(outputDir: string) {
        ...
        if (!Bundling.esbuildInstallation.version.startsWith(`${ESBUILD_MAJOR_VERSION}.`)) {
          throw new ValidationError('ExpectedEsbuildVersionEsbuild', `Expected esbuild version ${ESBUILD_MAJOR_VERSION}.x but got ${Bundling.esbuildInstallation.version}`, scope);
        }
...
function toTarget(scope: IConstruct, runtime: Runtime): string {
  ...
  if (!match) {
    throw new ValidationError('ExtractVersionRuntime', 'Cannot extract version from runtime.', scope);
  }

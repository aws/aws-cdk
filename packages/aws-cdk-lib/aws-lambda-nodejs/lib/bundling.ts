...
export class Bundling implements cdk.BundlingOptions {
  ...
  constructor(scope: IConstruct, private readonly props: BundlingProps) {
    ...
    if (this.relativeDepsLockFilePath.includes('..')) {
      throw new ValidationError('Expecteddepslockfilepathunderprojectroot', `Expected depsLockFilePath: ${props.depsLockFilePath} to be under projectRoot: ${this.projectRoot} (${this.relativeDepsLockFilePath})`, scope);
    }
    ...
    if (props.preCompilation && !/\.tsx?$/.test(props.entry)) {
      throw new ValidationError('Precompilationonlyusedtypescript', 'preCompilation can only be used with typescript files', scope);
    }
  ...
  private createBundlingCommand(scope: IConstruct, options: BundlingCommandOptions): string {
    ...
    if (this.props.preCompilation) {
      ...
      if (!tsconfig) {
        throw new ValidationError('CannotCannotFind', 'Cannot find a `tsconfig.json` but `preCompilation` is set to `true`, please specify it via `tsconfig`', scope);
      }
    ...
    if (this.props.sourceMap === false && this.props.sourceMapMode) {
      throw new ValidationError('Sourcemapmodecannotusedsourcemap', 'sourceMapMode cannot be used when sourceMap is false', scope);
    }
    ...
    if (this.props.nodeModules) {
      ...
      if (!pkgPath) {
        throw new ValidationError('CannotCannotFindProject', 'Cannot find a `package.json` in this project. Using `nodeModules` requires a `package.json`.', scope);
      }
...
function toTarget(scope: IConstruct, runtime: Runtime): string {
  ...
  if (!match) {
    throw new ValidationError('CannotCannotcannotextractversion', 'Cannot extract version from runtime.', scope);
  }

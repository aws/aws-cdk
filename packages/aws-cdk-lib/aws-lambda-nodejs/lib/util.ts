...
export function exec(cmd: string, args: string[], options?: SpawnSyncOptions) {
  ...
  if (proc.status !== 0) {
    if (proc.stdout || proc.stderr) {
      throw new UnscopedValidationError('StatusProcStatusStdout', `[Status ${proc.status}] stdout: ${proc.stdout?.toString().trim()}\n\n\nstderr: ${proc.stderr?.toString().trim()}`);
    }
    throw new UnscopedValidationError('CmdArgsJoinOptions', `${cmd} ${args.join(' ')} ${options?.cwd ? `run in directory ${options.cwd}` : ''} exited with status ${proc.status}`);
  }
...
export function extractDependencies(pkgPath: string, modules: string[]): { [key: string]: string } {
  ...
  for (const mod of modules) {
    ...
    if (!version) {
      throw new UnscopedValidationError('ExtractVersionModuleMod', `Cannot extract version for module '${mod}'. Check that it's referenced in your package.json or installed.`);
    }
...
export function getTsconfigCompilerOptions(tsconfigPath: string): string {
  ...
  Object.keys(options).sort().forEach((key: string) => {
    ...
    if (type === 'boolean') {
      ...
      throw new UnscopedValidationError('MissingSupportCompileroptionKey', `Missing support for compilerOption: [${key}]: { ${type}, ${value}} \n`);
    }
  });

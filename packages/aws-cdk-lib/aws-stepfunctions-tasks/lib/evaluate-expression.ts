...
export class EvaluateExpression extends sfn.TaskStateBase {
  ...
  constructor(scope: Construct, id: string, private readonly props: EvaluateExpressionProps) {
    ...
    // A path placed as literal text inside a plain string or template literal used to be
    // substituted in place by the old handler; it no longer resolves, so fail fast at synth
    // instead of failing at runtime or silently evaluating to the wrong value.

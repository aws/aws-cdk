...
export class EcsRunTaskBase implements ec2.IConnectable, sfn.IStepFunctionsTask {
  ...
  constructor(private readonly props: EcsRunTaskBaseProps) {
    ...
    if (this.integrationPattern === sfn.ServiceIntegrationPattern.WAIT_FOR_TASK_TOKEN
      ...
      throw new cdk.UnscopedValidationError('TaskTokenRequiredLeast', 'Task Token is required in at least one `containerOverrides.environment` for callback. Use JsonPath.taskToken to set the token.');
    }

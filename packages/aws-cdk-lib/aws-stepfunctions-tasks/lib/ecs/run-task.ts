...
export class CapacityProviderOptions {
  ...
  public static custom(capacityProviderStrategy: ecs.CapacityProviderStrategy[]): CapacityProviderOptions {
    if (capacityProviderStrategy.length < 1 || capacityProviderStrategy.length > 20) {
      throw new cdk.UnscopedValidationError('CapacityProviderStrategyContain', `Capacity provider strategy must contain between 1 and 20 capacity providers, got ${capacityProviderStrategy.length}`,
      );
    }
...
export class EcsEc2LaunchTarget implements IEcsLaunchTarget {
  ...
  public bind(task: EcsRunTask, launchTargetOptions: LaunchTargetBindOptions): EcsLaunchTargetConfig {
    if (!launchTargetOptions.taskDefinition.isEc2Compatible) {
      throw new ValidationError('SuppliedTaskdefinitionCompatibleEc2', 'Supplied TaskDefinition is not compatible with EC2', task);
    }

    if (!launchTargetOptions.cluster?.hasEc2Capacity) {
      throw new ValidationError('ClusterServiceNeedsEc2', 'Cluster for this service needs Ec2 capacity. Call addCapacity() on the cluster.', task);
    }
...
export class EcsRunTask extends sfn.TaskStateBase implements ec2.IConnectable {
  ...
  constructor(scope: Construct, id: string, private readonly props: EcsRunTaskProps) {
    ...
    if (this.integrationPattern === sfn.IntegrationPattern.WAIT_FOR_TASK_TOKEN
      ...
      throw new ValidationError('TaskTokenRequiredLeast', 'Task Token is required in at least one `containerOverrides.environment` for callback. Use JsonPath.taskToken to set the token.', this);
    }

    if (!this.props.taskDefinition.defaultContainer) {
      throw new ValidationError('TaskdefinitionLeastOneEssential', 'A TaskDefinition must have at least one essential container', this);
    }
  ...
  private validateNoNetworkingProps() {
    if (this.props.subnets !== undefined || this.props.securityGroups !== undefined) {
      throw new ValidationError('SuppliedTaskdefinitionNetworkmodeAws', `Supplied TaskDefinition must have 'networkMode' of 'AWS_VPC' to use 'vpcSubnets' and 'securityGroup'. Received: ${this.props.taskDefinition.networkMode}`, this,
      );
    }
  }

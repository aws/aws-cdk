...
export class EventBridgeSchedulerTarget {
  ...
  private validateProps(props: EventBridgeSchedulerTargetProps) {
    if (props.input !== undefined && !Token.isUnresolved(props.input) && props.input.length < 1) {
      throw new UnscopedValidationError('MustBeInputLeastCharacter', 'Input must be at least 1 character long.');
    }
    ...
    if (props.retryPolicy) {
      if (
        ...
        throw new UnscopedValidationError('MustBeMaximumretryattemptsIntegerBetween', `MaximumRetryAttempts must be an integer between 0 and 185, got ${props.retryPolicy.maximumRetryAttempts}`);
      }
      if (
        ...
        throw new UnscopedValidationError('MustBeMaximumeventageinsecondsBetween86400', 'MaximumEventAgeInSeconds must be between 60 and 86400 seconds');
      }
    }
  }
}
...
export class EventBridgeSchedulerCreateScheduleTask extends sfn.TaskStateBase {
  ...
  private validateProps(props: EventBridgeSchedulerCreateScheduleTaskProps) {
    if (props.startDate && props.endDate && props.startDate >= props.endDate) {
      throw new ValidationError('MustBeStartdateBeforeEnddate', '\'startDate\' must be before \'endDate\'', this);
    }
    ...
    if (props.clientToken !== undefined && !Token.isUnresolved(props.clientToken)) {
      if (props.clientToken.length > 64 || props.clientToken.length < 1) {
        throw new ValidationError('Mustbeclienttokenbetweencharacters', `ClientToken must be between 1 and 64 characters long. Got: ${props.clientToken.length}`, this);
      }
    ...
    if (props.description !== undefined && !Token.isUnresolved(props.description) && props.description.length > 512) {
      throw new ValidationError('Mustbedescriptionlessthan', `Description must be less than 512 characters long. Got: ${props.description.length}`, this);
    }
    ...
    if (props.flexibleTimeWindow && (
      ...
      throw new ValidationError('Mustbeflexibletimewindowbetween1440', 'FlexibleTimeWindow must be between 1 and 1440 minutes', this);
    }

    if (props.groupName !== undefined && !Token.isUnresolved(props.groupName)) {
      if (props.groupName.length < 1 || props.groupName.length > 64) {
        throw new ValidationError('Mustbegroupnamebetweencharacters', `GroupName must be between 1 and 64 characters long. Got: ${props.groupName.length}`, this);
      }
    ...
    if (props.timezone !== undefined && !Token.isUnresolved(props.timezone) && (props.timezone.length < 1 || props.timezone.length > 50)) {
      throw new ValidationError('Mustbetimezonebetweencharacters', `Timezone must be between 1 and 50 characters long. Got: ${props.timezone.length}`, this);
    }
  }
}

...
export class EventBridgeSchedulerTarget {
  ...
  private validateProps(props: EventBridgeSchedulerTargetProps) {
    if (props.input !== undefined && !Token.isUnresolved(props.input) && props.input.length < 1) {
      throw new UnscopedValidationError('InputMustBeAtLeastOneCharacter', 'Input must be at least 1 character long.');
    }
    ...
    if (props.retryPolicy) {
      if (
        ...
        throw new UnscopedValidationError('MaximumRetryAttemptsMustBeBetween0And185', `MaximumRetryAttempts must be an integer between 0 and 185, got ${props.retryPolicy.maximumRetryAttempts}`);
      }
      if (
        ...
        throw new UnscopedValidationError('MaximumEventAgeInSecondsMustBeBetween60And86400', 'MaximumEventAgeInSeconds must be between 60 and 86400 seconds');
      }
    }
  }
}
...
export class EventBridgeSchedulerCreateScheduleTask extends sfn.TaskStateBase {
  ...
  private validateProps(props: EventBridgeSchedulerCreateScheduleTaskProps) {
    if (props.startDate && props.endDate && props.startDate >= props.endDate) {
      throw new ValidationError('StartDateMustBeBeforeEndDate', '\'startDate\' must be before \'endDate\'', this);
    }
    ...
    if (props.clientToken !== undefined && !Token.isUnresolved(props.clientToken)) {
      if (props.clientToken.length > 64 || props.clientToken.length < 1) {
        throw new ValidationError('ClientTokenMustBeBetween1And64Characters', `ClientToken must be between 1 and 64 characters long. Got: ${props.clientToken.length}`, this);
      }
      if (!/^[a-zA-Z0-9-_]+$/.test(props.clientToken)) {
        throw new ValidationError('ClientTokenMustConsistOfAlphanumericCharacters', `ClientToken must consist of alphanumeric characters, dashes, and underscores only, Got: ${props.clientToken}`, this);
      }
    }

    if (props.description !== undefined && !Token.isUnresolved(props.description) && props.description.length > 512) {
      throw new ValidationError('DescriptionMustBeLessThan512Characters', `Description must be less than 512 characters long. Got: ${props.description.length}`, this);
    }
    ...
    if (props.flexibleTimeWindow && (
      ...
      throw new ValidationError('FlexibleTimeWindowMustBeBetween1And1440Minutes', 'FlexibleTimeWindow must be between 1 and 1440 minutes', this);
    }

    if (props.groupName !== undefined && !Token.isUnresolved(props.groupName)) {
      if (props.groupName.length < 1 || props.groupName.length > 64) {
        throw new ValidationError('GroupNameMustBeBetween1And64Characters', `GroupName must be between 1 and 64 characters long. Got: ${props.groupName.length}`, this);
      }
      if (!/^[a-zA-Z0-9-_.]+$/.test(props.groupName)) {
        throw new ValidationError('GroupNameMustConsistOfAlphanumericCharacters', `GroupName must consist of alphanumeric characters, dashes, underscores, and periods only, Got: ${props.groupName}`, this);
      }
    }

    if (props.timezone !== undefined && !Token.isUnresolved(props.timezone) && (props.timezone.length < 1 || props.timezone.length > 50)) {
      throw new ValidationError('TimezoneMustBeBetween1And50Characters', `Timezone must be between 1 and 50 characters long. Got: ${props.timezone.length}`, this);
    }
  }
}

...
export class RunLambdaTask implements sfn.IStepFunctionsTask {
  ...
  constructor(private readonly lambdaFunction: lambda.IFunction, private readonly props: RunLambdaTaskProps = {}) {
    ...
    if (!supportedPatterns.includes(this.integrationPattern)) {
      throw new ValidationError('InvalidInvalidinvalidserviceintegration', `Invalid Service Integration Pattern: ${this.integrationPattern} is not supported to call Lambda.`, lambdaFunction);
    }

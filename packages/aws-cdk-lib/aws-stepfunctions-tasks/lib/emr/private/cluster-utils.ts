...
export function InstanceTypeConfigPropertyToJson(property: EmrCreateCluster.InstanceTypeConfigProperty) {
  if (property.bidPrice && property.bidPriceAsPercentageOfOnDemandPrice) {
    throw new UnscopedValidationError('CannotCannotSpecifyBoth', 'Cannot specify both bidPrice and bidPriceAsPercentageOfOnDemandPrice');
  }
...
function SpotProvisioningSpecificationPropertyToJson(property?: EmrCreateCluster.SpotProvisioningSpecificationProperty) {
  ...
  if ((property.timeout && property.timeoutDurationMinutes) || (!property.timeout && !property.timeoutDurationMinutes)) {
    throw new UnscopedValidationError('MustBeTimeoutTimeoutdurationminutesSpecified', 'one of timeout and timeoutDurationMinutes must be specified');
  }
  ...
  if (timeout !== undefined && !cdk.Token.isUnresolved(timeout) && (timeout < 5 || timeout > 1440)) {
    throw new UnscopedValidationError('MustBeTimeoutBetween1440', `timeout must be between 5 and 1440 minutes, got ${timeout} minutes.`);
  }
...
export function InstanceFleetConfigPropertyToJson(property: EmrCreateCluster.InstanceFleetConfigProperty) {
  if (!property.targetSpotCapacity && !property.targetOnDemandCapacity) {
    throw new UnscopedValidationError('ShouldBeLeastTargetspotcapacityTargetondemandcapacity', 'At least one of targetSpotCapacity and targetOnDemandCapacity should be greater than 0');
  }
  if (property.instanceFleetType === EmrCreateCluster.InstanceRoleType.MASTER) {
    if (property.targetSpotCapacity && property.targetOnDemandCapacity) {
      throw new UnscopedValidationError('MasterInstanceFleet', 'For a master instance fleet, only one of targetSpotCapacity and targetOnDemandCapacity can be specified');
    }
    if (property.targetSpotCapacity && property.targetSpotCapacity !== 1) {
      throw new UnscopedValidationError('MasterInstanceFleet', `For a master instance fleet, targetSpotCapacity cannot be a number other than 1, got ${property.targetSpotCapacity}`);
    }
    if (property.targetOnDemandCapacity && property.targetOnDemandCapacity !== 1) {
      throw new UnscopedValidationError('MasterInstanceFleet', `For a master instance fleet, targetOnDemandCapacity cannot be a number other than 1, got ${property.targetOnDemandCapacity}`);
    }
  }
  return {

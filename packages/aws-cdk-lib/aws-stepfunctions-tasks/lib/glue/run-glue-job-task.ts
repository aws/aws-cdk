...
export class RunGlueJobTask implements sfn.IStepFunctionsTask {
  ...
  constructor(private readonly glueJobName: string, private readonly props: RunGlueJobTaskProps = {}) {
    ...
    if (!supportedPatterns.includes(this.integrationPattern)) {
      throw new UnscopedValidationError('InvalidInvalidServiceIntegration', `Invalid Service Integration Pattern: ${this.integrationPattern} is not supported to call Glue.`);
    }
  }

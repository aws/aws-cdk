  ...
  public static enable(identifier: string, version: number): Guardrail {
    if (!Token.isUnresolved(version)) {
      if (version < 1 || version > 99999999) {
        throw new UnscopedValidationError('Version99999999GotVersion', `\`version\` must be between 1 and 99999999, got ${version}.`);
      }
    }
  ...
  private constructor(public readonly guardrailIdentifier: string, public readonly guardrailVersion: string) {
    if (!Token.isUnresolved(guardrailIdentifier)) {
      ...
      if (guardrailIdentifier.startsWith('arn:')) {
        ...
        if (!arn.resourceName) {
          throw new UnscopedValidationError('InvalidArnFormatArn', `Invalid ARN format. The ARN of Guradrail should have the format: \`arn:<partition>:bedrock:<region>:<account-id>:guardrail/<guardrail-name>\`, got ${guardrailIdentifier}.`);
        }
      ...
      if (!guardrailPattern.test(gurdrailId)) {
        throw new UnscopedValidationError('GuardrailContainLowercaseLetters', `The id of Guardrail must contain only lowercase letters and numbers, got ${gurdrailId}.`);
      }

      if (guardrailIdentifier.length > 2048) {
        throw new UnscopedValidationError('GuardrailidentifierLength2048Got', `\`guardrailIdentifier\` length must be between 0 and 2048, got ${guardrailIdentifier.length}.`);
      }
    }
  }
}

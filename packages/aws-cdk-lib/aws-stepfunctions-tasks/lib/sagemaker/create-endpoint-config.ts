...
export class SageMakerCreateEndpointConfig extends sfn.TaskStateBase {
  ...
  private validateProductionVariants() {
    if (this.props.productionVariants.length < 1 || this.props.productionVariants.length > 10) {
      throw new ValidationError('Specifyproductionvariantsendpoint', 'Must specify from 1 to 10 production variants per endpoint configuration', this);
    }
    this.props.productionVariants.forEach((variant) => {
      if (variant.initialInstanceCount && variant.initialInstanceCount < 1) throw new ValidationError('Defineleastinstance', 'Must define at least one instance', this);

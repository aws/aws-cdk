...
export class EventBridgePutEvents extends sfn.TaskStateBase {
  ...
  constructor(scope: Construct, id: string, private readonly props: EventBridgePutEventsProps) {
    ...
    if (this.integrationPattern === sfn.IntegrationPattern.WAIT_FOR_TASK_TOKEN) {
      if (!sfn.FieldUtils.containsTaskToken(props.entries.map(entry => entry.detail))) {
        throw new cdk.ValidationError('TaskTokenRequiredEntries', 'Task Token is required in `entries`. Use JsonPath.taskToken to set the token.', this);
      }
    }
  ...
  private validateEntries(): void {
    if (this.props.entries.length <= 0) {
      throw new cdk.ValidationError('ValuePropertyEntriesNon', 'Value for property `entries` must be a non-empty array.', this);
    }
    if (this.props.entries.some(e => e.source.startsWith('aws.'))) {
      throw new cdk.ValidationError('EventSourceStartAws', 'Event source cannot start with "aws."', this);
    }
  }
}

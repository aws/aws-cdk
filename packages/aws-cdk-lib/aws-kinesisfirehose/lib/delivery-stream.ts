...
export class DeliveryStream extends DeliveryStreamBase {
  ...
  static fromDeliveryStreamAttributes(scope: Construct, id: string, attrs: DeliveryStreamAttributes): IDeliveryStream {
    if (!attrs.deliveryStreamName && !attrs.deliveryStreamArn) {
      throw new cdk.ValidationError('DeliveryStreamNameOrArnRequired', 'Either deliveryStreamName or deliveryStreamArn must be provided in DeliveryStreamAttributes', scope);
    }
    ...
    if (!deliveryStreamName) {
      throw new cdk.ValidationError('DeliveryStreamNameNotFoundInArn', `No delivery stream name found in ARN: '${attrs.deliveryStreamArn}'`, scope);
    }
  ...
  constructor(scope: Construct, id: string, props: DeliveryStreamProps) {
    ...
    if (
      ...
      throw new cdk.ValidationError('ServerSideEncryptionNotSupportedWithKinesisSource', 'Requested server-side encryption but delivery stream source is a Kinesis data stream. Specify server-side encryption on the data stream instead.', this);
    }

...
export function createBufferingHints(
  ...
  if (intervalInSeconds > 900) {
    throw new cdk.ValidationError('BufferingInterval900Seconds', `Buffering interval must be less than 900 seconds. Buffering interval provided was ${intervalInSeconds} seconds.`, scope);
  }
  ...
  if (sizeInMBs < 1 || sizeInMBs > 128) {
    throw new cdk.ValidationError('BufferingSize128Mibs', `Buffering size must be between 1 and 128 MiBs. Buffering size provided was ${sizeInMBs} MiBs.`, scope);
  }

  if (dataFormatConversionConfiguration?.enabled && sizeInMBs < 64) {
    throw new cdk.ValidationError('DataFormatConversionEnabled', `When data format conversion is enabled, buffering size must be at least 64 MiBs. Buffering size provided was ${sizeInMBs} MiBs.`, scope);
  }
...
export function createProcessingConfig(
  ...
  if (processorTypes.has('CloudWatchLogProcessing') && !processorTypes.has('Decompression')) {
    throw new cdk.ValidationError('CloudwatchlogprocessorEnabledDecompressionprocessor', 'CloudWatchLogProcessor can only be enabled with DecompressionProcessor', scope);
  }

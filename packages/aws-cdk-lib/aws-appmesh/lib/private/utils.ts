 ...
 */
export function renderTlsClientPolicy(
  scope: Construct,
  tlsClientPolicy: TlsClientPolicy | undefined
 ...
 */
export function renderListenerTlsOptions(
  scope: Construct,
  listenerTls: ListenerTlsOptions | undefined
...
export function validateHttpMatchArrayLength(headers?: HeaderMatch[], queryParameters?: QueryParameterMatch[]) {
  ...
  if (headers && (headers.length < MIN_LENGTH || headers.length > MAX_LENGTH)) {
    throw new Error(
      `Number of headers provided for matching must be between ${MIN_LENGTH} and ${MAX_LENGTH}, got: ${headers.length}`
    );
  }
...
export function validateGrpcMatchArrayLength(metadata?: HeaderMatch[]): void {
  ...
  if (metadata && (metadata.length < MIN_LENGTH || metadata.length > MAX_LENGTH)) {
    throw new Error(
      `Number of metadata provided for matching must be between ${MIN_LENGTH} and ${MAX_LENGTH}, got: ${metadata.length}`
    );
  }
}
...
export function validateGrpcRouteMatch(match: GrpcRouteMatch): void {
  if (
    match.serviceName === undefined &&
    match.metadata === undefined &&
    match.methodName === undefined &&

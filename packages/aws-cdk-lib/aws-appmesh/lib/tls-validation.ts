...
class TlsValidationAcmTrust extends TlsValidationTrust {
  ...
  public bind(scope: Construct): TlsValidationTrustConfig {
    if (this.certificateAuthorities.length === 0) {
      throw new ValidationError('Provideleastcertificateauthority', 'you must provide at least one Certificate Authority when creating an ACM Trust ClientPolicy', scope);
    } else {
      return {
        tlsValidationTrust: {
          acm: {

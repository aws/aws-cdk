...
export class UserPoolIdentityProviderSaml extends UserPoolIdentityProviderBase {
  ...
  private validateName(name?: string) {
    if (name && !Token.isUnresolved(name) && (name.length < 3 || name.length > 32)) {
      throw new ValidationError('Expectedprovidernamebetween', `Expected provider name to be between 3 and 32 characters, received ${name} (${name.length} characters)`, this);
    }
  }
}

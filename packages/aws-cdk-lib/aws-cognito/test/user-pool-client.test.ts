...
describe('User Pool Client', () => {
  ...
  test('client name', () => {
    ...
    expect(() => client2.userPoolClientName).toThrow(
      /available only if specified on the UserPoolClient during initialization/
    );
  });
  ...
  describe('Client with secret', () => {
    test('generate secret', () => {
      ...
      Template.fromStack(stack).hasResourceProperties('AWS::Lambda::Function', {
        Role: {
          'Fn::GetAtt': ['AWS679f53fac002430cb0da5b7982bd2287ServiceRoleC1EA0FF2', 'Arn'],
        },
    ...
    test('explicitly disable secret generation', () => {
      ...
      // THEN
      expect(() => clientWithoutSecret.userPoolClientSecret).toThrow(
  ...
  test('OAuth scopes - resource server', () => {
    ...
    pool.addClient('Client', {
      oAuth: {
        flows: { clientCredentials: true },
        scopes: [OAuthScope.resourceServer(resourceServer, scope)],
      },
    });
    ...
    Template.fromStack(stack).hasResourceProperties('AWS::Cognito::UserPoolClient', {
      AllowedOAuthScopes: [
        {
          'Fn::Join': ['', [stack.resolve(resourceServer.userPoolResourceServerId), '/scope-name']],
        },
      ],
    });
  });
  ...
  test('default supportedIdentityProviders', () => {
    ...
    // THEN
    Template.fromStack(stack).hasResourceProperties('AWS::Cognito::UserPoolClient', {
      SupportedIdentityProviders: ['userpool-idp', 'COGNITO'],
    });
  });
  ...
  describe('auth session validity', () => {
    ...
    test.each([
      ...
      expect(() => {
        ...
        `authSessionValidity: Must be a duration between 3 minutes and 15 minutes (inclusive); received ${validity.toHumanString()}.`
      );
    });

    test.each([Duration.minutes(3), Duration.minutes(9), Duration.minutes(15)])(
      'validates authSessionValidity is a duration between 3 and 15 minutes (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
  ...
  describe('token validity', () => {
    ...
    test.each([Duration.minutes(0), Duration.minutes(4), Duration.days(1).plus(Duration.minutes(1)), Duration.days(2)])(
      'validates accessTokenValidity is a duration between 5 minutes and 1 day',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
        expect(() => {
          ...
          `accessTokenValidity: Must be a duration between 5 minutes and 1 day (inclusive); received ${validity.toHumanString()}.`
        );
      }
    );

    test.each([Duration.minutes(0), Duration.minutes(4), Duration.days(1).plus(Duration.minutes(1)), Duration.days(2)])(
      'validates idTokenValidity is a duration between 5 minutes and 1 day',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
        expect(() => {
          ...
          `idTokenValidity: Must be a duration between 5 minutes and 1 day (inclusive); received ${validity.toHumanString()}.`
        );
      }
    );

    test.each([Duration.hours(1).plus(Duration.minutes(1)), Duration.hours(12), Duration.days(1)])(
      'validates accessTokenValidity is not greater than refresh token expiration',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
        expect(() => {
          ...
          `accessTokenValidity: Must be a duration between 5 minutes and 1 hour (inclusive); received ${validity.toHumanString()}.`
        );
      }
    );

    test.each([Duration.hours(1).plus(Duration.minutes(1)), Duration.hours(12), Duration.days(1)])(
      'validates idTokenValidity is not greater than refresh token expiration',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
        expect(() => {
          ...
          `idTokenValidity: Must be a duration between 5 minutes and 1 hour (inclusive); received ${validity.toHumanString()}.`
        );
      }
    );
    ...
    test.each([
      ...
      expect(() => {
        ...
        `refreshTokenValidity: Must be a duration between 1 hour and 3650 days (inclusive); received ${validity.toHumanString()}.`
      );
    });

    test.each([Duration.minutes(5), Duration.minutes(60), Duration.days(1)])(
      'validates accessTokenValidity is a duration between 5 minutes and 1 day (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
    ...
    test.each([Duration.minutes(5), Duration.minutes(60), Duration.days(1)])(
      'validates idTokenValidity is a duration between 5 minutes and 1 day (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
    ...
    test.each([Duration.minutes(60), Duration.minutes(120), Duration.days(365), Duration.days(10 * 365)])(
      'validates refreshTokenValidity is a duration between 1 hour and 10 years (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
    ...
    test.each([Duration.minutes(5), Duration.minutes(60), Duration.hours(1)])(
      'validates accessTokenValidity is not greater than refresh token expiration (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
    ...
    test.each([Duration.minutes(5), Duration.minutes(60), Duration.hours(1)])(
      'validates idTokenValidity is not greater than refresh token expiration (valid)',
      (validity) => {
        const stack = new Stack();
        const pool = new UserPool(stack, 'Pool');
  ...
  test('cannot create when invalid defaultRedirectUri is set', () => {
    ...
    expect(
      ...
      `defaultRedirectUri must match the \`^(?=.{1,1024}$)[\p{L}\p{M}\p{S}\p{N}\p{P}]+$\` pattern, got ${invalidUrl}`
    );
  });
});

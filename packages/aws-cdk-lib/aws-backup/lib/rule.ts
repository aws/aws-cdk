...
export class BackupPlanRule {
  ...
  constructor(props: BackupPlanRuleProps) {
    if (props.deleteAfter && props.moveToColdStorageAfter &&
      ...
      throw new UnscopedValidationError('DeleteafterGreaterMovetocoldstorageafter', '`deleteAfter` must be greater than `moveToColdStorageAfter`');
    }

    if (props.scheduleExpression && !/^cron/.test(props.scheduleExpression.expressionString)) {
      throw new UnscopedValidationError('ScheduleexpressionTypeCron', '`scheduleExpression` must be of type `cron`');
    }
    ...
    if (props.enableContinuousBackup && props.moveToColdStorageAfter) {
      throw new UnscopedValidationError('MovetocoldstorageafterSpecifiedEnablecontinuousbackupEnabled', '`moveToColdStorageAfter` must not be specified if `enableContinuousBackup` is enabled');
    }

    if (props.enableContinuousBackup && props.deleteAfter &&
      (props.deleteAfter?.toDays() < 1 || props.deleteAfter?.toDays() > 35)) {
      throw new UnscopedValidationError('DeleteafterDaysEnablecontinuousbackupEnabled', `'deleteAfter' must be between 1 and 35 days if 'enableContinuousBackup' is enabled, but got ${props.deleteAfter.toHumanString()}`);
    }
    ...
    if (props.copyActions && props.copyActions.length > 0) {
      props.copyActions.forEach(copyAction => {
        if (copyAction.deleteAfter && !Token.isUnresolved(copyAction.deleteAfter) &&
          ...
          throw new UnscopedValidationError('DeleteafterLeastDaysLater', [

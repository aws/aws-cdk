...
export class BackupVault extends BackupVaultBase {
  ...
  public static fromBackupVaultArn(scope: Construct, id: string, backupVaultArn: string): IBackupVault {
    ...
    if (parsedArn.arnFormat !== ArnFormat.COLON_RESOURCE_NAME) {
      throw new ValidationError('Backupvaultwrongformat', `Backup Vault Arn ${backupVaultArn} has the wrong format, expected ${ArnFormat.COLON_RESOURCE_NAME}.`, scope);
    }
    if (!parsedArn.resourceName) {
      throw new ValidationError('Backupvaultdoesresource', `Backup Vault Arn ${backupVaultArn} does not have a resource name.`, scope);
    }
...
function renderLockConfiguration(scope: Construct, config?: LockConfiguration): CfnBackupVault.LockConfigurationTypeProperty | undefined {
  ...
  if (config.changeableFor && config.changeableFor.toHours() < 72) {
    throw new ValidationError('Backupenforces72hourcoolingoff', `AWS Backup enforces a 72-hour cooling-off period before Vault Lock takes effect and becomes immutable, got ${config.changeableFor.toHours()} hours`, scope);
  }
  ...
  if (config.maxRetention) {
    ...
    if (config.maxRetention.toDays() <= config.minRetention.toDays()) {
      throw new ValidationError('Mustbemaximumretentionperiod', `The maximum retention period (${config.maxRetention.toDays()} days) must be greater than the minimum retention period (${config.minRetention.toDays()} days)`, scope);
    }
  }

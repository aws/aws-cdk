...
test('with notifications', () => {
  ...
  new BackupVault(stack, 'Vault', {
    ...
    notificationEvents: [BackupVaultEvents.BACKUP_JOB_COMPLETED, BackupVaultEvents.COPY_JOB_FAILED],
  });

  // THEN
  Template.fromStack(stack).hasResourceProperties('AWS::Backup::BackupVault', {
    Notifications: {
      BackupVaultEvents: ['BACKUP_JOB_COMPLETED', 'COPY_JOB_FAILED'],
      SNSTopicArn: {
        Ref: 'TopicBFC7AF6E',
      },
    },
...
test('import from arn should throw if arn format is incorrect', () => {
  ...
  expect(() => BackupVault.fromBackupVaultArn(stack, 'Vault', vaultArn)).toThrow(

...
| [@aws-cdk/core:cdkDiffUseChangeSetPropertyValuesByDefault](#aws-cdkcorecdkdiffusechangesetpropertyvaluesbydefault) | When enabled, make `cdk diff` default to using values from DescribeChangeSet(IncludePropertyValues: true) for resource property differences. | 2.144.0 | (fix) |
...
```json
{
  "context": {
    ...
    "@aws-cdk/core:cdkDiffUseChangeSetPropertyValuesByDefault": true
  }
}
```
<!-- END json -->
...
### @aws-cdk/core:cdkDiffUseChangeSetPropertyValuesByDefault

*When enabled, make `cdk diff` default to using values from DescribeChangeSet(IncludePropertyValues: true) for resource property differences.* (fix)

...
Enable this feature flag to allow the CDK to track changes in Redshift columns through their `id` attribute. This is a breaking change, as the `name` attribute was currently being used to track changes to Redshift columns. Column `id`s will be required, however their impact on the CDK application will not take place until setting this feature flag.

...
describe('Boostrap Roles', () => {
  test('default bootstrap role name is always under 64 characters', () => {
    ...
    expect(firstFile.destinations['000000000000-us-east-1-ec7c3d8f'].assumeRoleArn).toEqual('arn:${AWS::Partition}:iam::000000000000:role/cdk-super-long-app-id-th-file-role-us-east-1');
  });
  ...
  test('can supply existing arn for bucket staging role', () => {
    ...
    const manifest: cxschema.AssetManifest = JSON.parse(fs.readFileSync(manifestArtifact.file, { encoding: 'utf-8' }));
    const firstFile: any = (manifest.files ? manifest.files[Object.keys(manifest.files)[0]] : undefined) ?? {};
    expect(firstFile.destinations['000000000000-us-east-1-ec7c3d8f'].assumeRoleArn).toEqual('arn:aws:iam::123456789012:role/S3Access');
  });
  ...
  test('can provide existing arn for image staging role', () => {
    ...
    expect(firstFile.destinations['000000000000-us-east-1-ec7c3d8f'].assumeRoleArn).toEqual('arn:aws:iam::123456789012:role/ECRAccess');
  });

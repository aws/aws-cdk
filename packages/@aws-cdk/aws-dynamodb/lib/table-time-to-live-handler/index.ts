...
import { disableTimeToLive, enableTimeToLive, isTimeToLiveStable } from './utils';
...
export async function onEventHandler(event: OnEventRequest): Promise<OnEventResponse> {
  ...
  if (event.RequestType === 'Create' || event.RequestType === 'Update') {
    ...
    switch (currentTtl.TimeToLiveDescription?.TimeToLiveStatus) {
      case 'ENABLING':
        throw new Error('Can not change time to live while it is beeing enabled.');
        break;
      case 'DISABLING':
        throw new Error('Can not change time to live while it is beeing disabled.');
        break;
      case 'ENABLED':
...
export async function isCompleteHandler(event: IsCompleteRequest): Promise<IsCompleteResponse> {
  ...
  switch (event.RequestType) {
    ...
    case 'Update':
      ...
      return { IsComplete: tableActive && await isTimeToLiveStable(event) };
    case 'Delete':

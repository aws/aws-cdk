...
describe('default properties', () => {
  ...
  test('can use PhysicalName.GENERATE_IF_NEEDED as the Table name', () => {
    ...
    Template.fromStack(stack).hasResourceProperties('AWS::DynamoDB::Table', {
      TableName: Match.absentProperty(),
    });
  });
});
...
describe('global', () => {
  test('create replicas', () => {
    ...
    expect(Template.fromStack(stack).toJSON().Conditions).toEqual({
  ...
  test('allows PROVISIONED billing mode when auto-scaled writes with a policy are specified', () => {
    ...
    Template.fromStack(stack).hasResourceProperties('AWS::DynamoDB::Table', {
      BillingMode: Match.absentProperty(), // PROVISIONED is the default
    });
  });
  ...
  test('no conditions when region is known', () => {
    ...
    // THEN
    expect(Template.fromStack(stack).toJSON().Conditions).toBeUndefined();
  });

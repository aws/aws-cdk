          ...
          { PolicyDocument:
           { Statement:
            [ { Action: [ 'dynamodb:DescribeTable', 'dynamodb:UpdateTable' ],
              ...
              { Action: [ 'cloudwatch:DeleteAlarms', 'cloudwatch:DescribeAlarms', 'cloudwatch:GetMetricStatistics',
...
function testGrant(test: Test, expectedActions: string[], invocation: (user: iam.IPrincipal, table: Table) => void) {
  ...
  // THEN
  const action = expectedActions.length > 1 ? expectedActions.map(a => `dynamodb:${a}`).sort() : `dynamodb:${expectedActions[0]}`;

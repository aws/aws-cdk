...
test('findUp', () => {
  ...
  expect(findUp('README.md')).toMatch(/aws-lambda-python$/);

  // Non existing file
  expect(findUp('non-existing-file.unknown')).toBe(undefined);

  // Starting at a specific path
  expect(findUp('util.test.ts', path.join(__dirname, 'integ-handlers'))).toMatch(/aws-lambda-python\/test$/);
  ...
  expect(findUp('util.test.ts', 'test/integ-handlers')).toMatch(/aws-lambda-python\/test$/);
});

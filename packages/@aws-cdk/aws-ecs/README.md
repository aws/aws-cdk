...
const ecsService = new ecs.LoadBalancedEc2Service(this, 'Service', {
  cluster,
  memoryLimitMiB: 512,
  image: ecs.ContainerImage.fromRepository("amazon/amazon-ecs-sample"),
});
```
...
* `ecs.ContainerImage.fromRepository(imageName)`: use a public image.
* `ecs.ContainerImage.fromRepository(imageName, { credentials: mySecret })`: use a private image that requires credentials.

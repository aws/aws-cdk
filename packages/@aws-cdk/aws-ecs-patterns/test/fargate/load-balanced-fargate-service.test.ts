...
test('setting executionRole updated taskDefinition with given execution role', () => {
  ...
  // THEN
  const serviceTaskDefinition = Template.fromStack(stack).toJSON().Resources.ServiceTaskDef1922A00F;
...
test('setting servicename updates service name with given name', () => {
  ...
  // THEN
  const serviceTaskDefinition = Template.fromStack(stack).toJSON().Resources.Service9571FDD8;
...
test('domainName and domainZone not required for HTTPS listener with provided cert', () => {
  ...
  // THEN
  expect(Template.fromStack(stack).findResources('AWS::Route53::RecordSet')).toEqual({});
});

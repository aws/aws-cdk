...
import { App, DefaultStackSynthesizer, ImageAsset, IgnoreMode, Lazy, LegacyStackSynthesizer, Stack, Stage } from '../../../lib';
...
describe('image asset', () => {
  ...
  testFutureBehavior('asset.repository.grantPull can be used to grant a principal permissions to use the image', flags, App, (app) => {
    // GIVEN
    const stack = new Stack(app);
    const asset = new ImageAsset(stack, 'Image', {
      directory: path.join(__dirname, 'demo-image'),
    });

    // THEN
    expect(asset.repositoryName).toBe('aws-cdk/assets');
  });
  ...
  test('fails if using tokens in build args keys or values', () => {
    ...
    // THEN
    expect(() => new ImageAsset(stack, 'MyAsset1', {
      directory: path.join(__dirname, 'demo-image'),
      buildArgs: { [token]: 'value' },
    })).toThrow(expected);

    expect(() => new ImageAsset(stack, 'MyAsset2', {
      directory: path.join(__dirname, 'demo-image'),
      buildArgs: { key: token },
    })).toThrow(expected);
  ...
  testFutureBehavior('docker build options are included in the asset id', flags, App, (app) => {
    ...
    const asset1 = new ImageAsset(stack, 'Asset1', { directory });
    const asset2 = new ImageAsset(stack, 'Asset2', { directory, file: 'Dockerfile.Custom' });
    const asset3 = new ImageAsset(stack, 'Asset3', { directory, target: 'NonDefaultTarget' });
    const asset4 = new ImageAsset(stack, 'Asset4', { directory, buildArgs: { opt1: '123', opt2: 'boom' } });
    const asset5 = new ImageAsset(stack, 'Asset5', { directory, file: 'Dockerfile.Custom', target: 'NonDefaultTarget' });
    const asset6 = new ImageAsset(stack, 'Asset6', { directory, extraHash: 'random-extra' });
    const asset7 = new ImageAsset(stack, 'Asset7', { directory, repositoryName: 'foo' });
...
testFutureBehavior('nested assemblies share assets: legacy synth edition', flags, App, (app) => {
  ...
  // WHEN
  new ImageAsset(stack1, 'Image', { directory: path.join(__dirname, 'demo-image') });
  new ImageAsset(stack2, 'Image', { directory: path.join(__dirname, 'demo-image') });

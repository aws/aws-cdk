...
export async function toCloudFormation(stack: Stack): Promise<any> {
  const synthesizedTemplate = (await synthesize(stack, { skipValidation: true })).getStackByName(stack.stackName).template;

...
import { CfnDBProxy, CfnDBProxyTargetGroup, CfnDBProxyEndpoint } from './rds.generated';
...
export interface IDatabaseProxy extends cdk.IResource {
  ...
  /**
   * Add an endpoint with the give properties to the proxy.
   *
   * @param options the properties if the endpoint
   */
  addEndpoint(scope: Construct, id: string, options: DatabaseProxyEndpointOptions): IDatabaseProxyEndpoint;
}
...
abstract class DatabaseProxyBase extends cdk.Resource implements IDatabaseProxy {
  ...
  /**
   * Collection of all the proxy endpoints of this proxy
   */
  ...
  public addEndpoint(scope: Construct, id: string, options: DatabaseProxyEndpointOptions): IDatabaseProxyEndpoint {
    const endpoint = new DatabaseProxyEndpoint(scope, id, {
      ...options,
      dbProxy: this,
    });
...
export enum TargetRole {
  /**
   * Role for a proxy endpoint that can be used for both reading and writing.
   */
  READ_WRITE = 'READ_WRITE',

  /**
   * Role for a proxy endpoint that can only be used for reading.
   */
  READ_ONLY = 'READ_ONLY',
}
...
export interface DatabaseProxyEndpointOptions {
  /**
   * The identifier of the DB proxy endpoint.
   *
   *  @default - Generated by CloudFormation (recommended)
   */
  ...
  /**
   * One or more VPC security groups to associate with the new proxy endpoint.
   *
   * @default - Create a new SecurityGroup
   */
  readonly securityGroups?: ec2.ISecurityGroup[];
  ...
  /**
   * The VPC to associate with the new proxy endpoint.
   */
  readonly vpc: ec2.IVpc;
   ...
   *
   * @default - READ_WRITE (TODO documentation does not say!).
   */
 ...
 * DB Proxy Endpoint
 */
export interface IDatabaseProxyEndpoint extends cdk.IResource {
  /**
...
abstract class DatabaseProxyEndpointBase extends cdk.Resource implements IDatabaseProxyEndpoint {
...
export class DatabaseProxyEndpoint extends DatabaseProxyEndpointBase {
  ...
  /**
   * Indicates wether this
   *
   * @attribute
   */
  public readonly isDefault: cdk.IResolvable;
  ...
  private readonly resource: CfnDBProxyEndpoint;

  constructor(scope: Construct, id: string, props: DatabaseProxyEndpointProps) {
    super(scope, id, { physicalName: props.dbProxyEndpointName || id });

    const securityGroups = props.securityGroups ?? [
      new ec2.SecurityGroup(this, 'ProxyEndPointSecurityGroup', {
        description: 'SecurityGroup for Database Proxy Endpoint',
        vpc: props.vpc,
      }),
    ];
    this.connections = new ec2.Connections({ securityGroups });
    ...
    this.resource = new CfnDBProxyEndpoint(this, 'Resource', {
      dbProxyEndpointName: this.physicalName,
    ...
    this.dbProxyEndpointName = this.resource.ref;
    this.dbProxyEndpointArn = this.resource.attrDbProxyEndpointArn;
    this.endpoint = this.resource.attrEndpoint;
    this.vpcId = this.resource.attrVpcId;
    this.isDefault = this.resource.attrIsDefault;

    // TODO add dependency to the proxy?
    // TODO what about DeletePolicy
  }
}

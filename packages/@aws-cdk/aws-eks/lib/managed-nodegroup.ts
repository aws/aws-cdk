...
export interface NodegroupOptions {
  ...
  /**
   * The instance types to use for your node group. Currently, you can specify a single instance type for a node group.
   * If you specify multiple instance types, only the first one will be selected. The default value for this parameter
   * is `t3.medium`. If you choose a GPU instance type, be sure to specify the `AL2_x86_64_GPU` with the amiType
   * parameter.
   *
   * @default - t3.medium
   */
   ...
   * makes calls to AWS APIs on your behalf. Worker nodes receive permissions for these API calls through an IAM
   * instance profile and associated policies. Before you can launch worker nodes and register them into a cluster,
   ...
   * specify an Amazon EC2 SSH key but do not specify a source security group when you create a managed
   * node group, then port 22 on the worker nodes is opened to the internet (0.0.0.0/0)
   *
   * @default - disabled
   */
...
export class Nodegroup extends Resource implements INodegroup {
  ...
  constructor(scope: Construct, id: string, props: NodegroupProps ) {
    ...
    const resource = new CfnNodegroup(this, 'Resource', {
      ...
      instanceTypes: props.instanceTypes ? [ props.instanceTypes[0].toString() ] : undefined,
      labels: props.labels,

...
def helm_handler(event, context):
    ...
    subprocess.check_call(
        ["aws", "eks", "update-kubeconfig", "--role-arn", role_arn, "--name", cluster_name, "--kubeconfig", kubeconfig]
    )
...
def get_oci_cmd(repository, version):
    ...
    if re.fullmatch(private_ecr_pattern, registry) is not None:
        ...
        logger.info("Found AWS public repository, will use us-east-1 region as deployment")
        cmnd = [

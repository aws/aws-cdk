...
test('Can map an older ALB app version to helm chart version', () => {
  const { stack } = testFixture();
...
test('When multiple helm chart versions exist for a given app version, the newest one is used', () => {
  const { stack } = testFixture();

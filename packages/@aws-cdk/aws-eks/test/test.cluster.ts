...
export = {
  ...
  'outputs': {
    ...
    'enableDockerBridge as true should be set correctly'(test: Test) {
      // GIVEN
      const { stack, vpc } = testFixture();

      // WHEN
      const cluster = new eks.Cluster(stack, 'Cluster', { vpc, kubectlEnabled: false, defaultCapacity: 0, version: CLUSTER_VERSION });
    ...
    'enableDockerBridge as false should be set correctly'(test: Test) {
      // GIVEN
      const { stack, vpc } = testFixture();

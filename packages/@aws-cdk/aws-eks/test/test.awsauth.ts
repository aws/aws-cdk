...
export = {
  ...
  'throws when adding a role from a different stack'(test: Test) {
    ...
    try {
      ...
      test.equal(err.message, 'RoleStackRole6729D0A7 must be in the ClusterStack stack');
    }
  ...
  'throws when adding a user from a different stack'(test: Test) {
    ...
    try {
      ...
      test.equal(err.message, 'UserStackUser0406F94E must be in the ClusterStack stack');
    }

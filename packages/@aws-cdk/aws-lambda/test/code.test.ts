...
describe('code', () => {
  describe('lambda.Code.fromInline', () => {
    ...
    test('fails if larger than 4096 bytes', () => {
      expect(() => defineFunction(lambda.Code.fromInline(generateRandomString(4097)), lambda.Runtime.NODEJS_12_X))
...
function defineFunction(code: lambda.Code, runtime: lambda.Runtime = lambda.Runtime.NODEJS_12_X) {
  const stack = new cdk.Stack();

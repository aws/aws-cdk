...
The `awslambda.Code` class includes static convenience methods for various types of
runtime code.

 * `awslambda.Code.fromBucket(bucket, key[, objectVersion])` - specify an S3 object
   that contains the archive of your runtime code.
 * `awslambda.Code.fromInline(code)` - inline the handle code as a string. This is
   limited to supported runtimes and the code cannot exceed 4KiB.
 * `awslambda.Code.fromAsset(path)` - specify a directory or a .zip file in the local
...
The `fn.currentVersion` property can be used to obtain a `awslambda.Version`
...
> uses either `awslambda.Code.fromAsset` or `awslambda.Code.fromInline`. Other types
> of code providers (such as `awslambda.Code.fromBucket`) require that you define a
> `awslambda.Version` resource directly since the CDK is unable to determine if
...
> NOTE: The `fn.latestVersion` property returns a `awslambda.IVersion` which
...
> normally want to use `awslambda.currentVersion`.

### Layers

The `awslambda.LayerVersion` class can be used to define Lambda layers and manage
...
const fn = new awslambda.Function(this, 'MyFunction', {
    runtime: awslambda.Runtime.NODEJS_10_X,
    handler: 'index.handler',
    code: awslambda.Code.fromInline('exports.handler = function(event, ctx, cb) { return cb(null, "hi"); }'),
...
See [the AWS documentation](https://docs.aws.amazon.com/codeguru/latest/profiler-ug/setting-up-awslambda.html)
...
const fn = new awslambda.Function(stack, 'MyLambda', {
  code,
  handler,
  runtime,
  vpc,
  // mount the access point to /mnt/msg in the lambda runtime enironment
  filesystem: awslambda.FileSystem.fromEfsAccessPoint(accessPoint, '/mnt/msg'),
});
```
...
### Bundling Asset Code
When using `awslambda.Code.fromAsset(path)` it is possible to bundle the code by running a
...
Example with Python:
```ts
new awslambda.Function(this, 'Function', {
  code: awslambda.Code.fromAsset(path.join(__dirname, 'my-python-handler'), {
    bundling: {
      image: awslambda.Runtime.PYTHON_3_6.bundlingDockerImage,
      command: [
        'bash', '-c', `
...
new awslambda.Function(this, 'Function', {
  code: awslambda.Code.fromAsset('/path/to/handler', {
    bundling: {

...
export class Nodegroup extends Resource implements INodegroup {
  ...
  constructor(scope: Construct, id: string, props: NodegroupProps) {
    ...
    withResolved(this.desiredSize, this.maxSize, (desired, max) => {
      ...
      if (desired > max) {
        throw new ValidationError('DesiredCapacityDesiredGreater', `Desired capacity ${desired} can't be greater than max size ${max}`, this);
      }
    });
    ...
    withResolved(this.desiredSize, this.minSize, (desired, min) => {
      if (desired === undefined) {return ;}
      if (desired < min) {
        throw new ValidationError('MinimumCapacityMinGreater', `Minimum capacity ${min} can't be greater than desired size ${desired}`, this);
      }
    });
    ...
    if (props.launchTemplateSpec && props.diskSize) {
      ...
      throw new ValidationError('DisksizeSpecifiedWithinLaunch', 'diskSize must be specified within the launch template', this);
    }

    if (props.instanceType && props.instanceTypes) {
      throw new ValidationError('InstancetypeDeprecatedPleaseInstancetypes', '"instanceType is deprecated, please use "instanceTypes" only.', this);
    }
    ...
    if (instanceTypes && instanceTypes.length > 0) {
      ...
      if (props.amiType && !possibleAmiTypes.includes(props.amiType)) {
        throw new ValidationError('SpecifiedAmiMatchInstance', `The specified AMI does not match the instance types architecture, either specify one of ${possibleAmiTypes.join(', ').toUpperCase()} or don't specify any`, this);
      }
      ...
      instanceTypes.filter(isWindowsSupportedInstanceType).length < instanceTypes.length) {
        throw new ValidationError('SpecifiedInstancetypeSupportWindows', 'The specified instanceType does not support Windows workloads. '
  ...
  private validateUpdateConfig(maxUnavailable?: number, maxUnavailablePercentage?: number) {
    ...
    if (maxUnavailablePercentage && (maxUnavailablePercentage < 1 || maxUnavailablePercentage > 100)) {
      throw new ValidationError('Maxunavailablepercentage100GotMaxunavailablepercentage', `maxUnavailablePercentage must be between 1 and 100, got ${maxUnavailablePercentage}`, this);
    }
    if (maxUnavailable) {
      if (maxUnavailable > this.maxSize) {
        throw new ValidationError('MaxunavailableLowerMaxsizeMaxsize', `maxUnavailable must be lower than maxSize (${this.maxSize}), got ${maxUnavailable}`, this);
      }
      if (maxUnavailable < 1 || maxUnavailable > 100) {
        throw new ValidationError('Maxunavailable100GotMaxunavailable', `maxUnavailable must be between 1 and 100, got ${maxUnavailable}`, this);
      }
    }
  }
}
...
function getPossibleAmiTypes(instanceTypes: InstanceType[]): NodegroupAmiType[] {
  ...
  if (architectures.size === 0) { // protective code, the current implementation will never result in this.
    throw new UnscopedValidationError('DetermineAnyAmiType', `Cannot determine any ami type compatible with instance types: ${instanceTypes.map(i => i.toString()).join(', ')}`);
  }

  if (architectures.size > 1) {
    throw new UnscopedValidationError('InstancetypesDifferentArchitecturesAllowed', 'instanceTypes of different architectures is not allowed');
  }

...
To do so, pass the `oidcProviderNative` property when you import the cluster into the application.
...
Compared to `eks.OpenIdConnectProvider`, `eks.OidcProviderNative` uses L1 CFN resource instead of a custom resource to create the IAM OidcProvider.

...
test('non-200 status code throws an error', async () => {
  ...
  await expect(
    lambda.handler({ RequestType: 'Create', ResourceProperties: mockResourceProperties, ...mockRequest })
  ).rejects.toMatchObject({ message: 'Trigger handler failed with status code 500' });
...
test('retry with access denied exception', async () => {
  ...
  expect(lambdaMock).toHaveReceivedNthCommandWith(1, InvokeCommand, {
    FunctionName: handlerArn,
    InvocationType: 'Event',
  });
  expect(lambdaMock).toHaveReceivedNthCommandWith(2, InvokeCommand, {
    FunctionName: handlerArn,
    InvocationType: 'Event',
  });
});
...
describe('function error', () => {
  const makeTest = (payload: string | undefined, expectedError: string) => {
    return async () => {
      ...
      await expect(
        lambda.handler({ RequestType: 'Create', ResourceProperties: mockResourceProperties, ...mockRequest })
      ).rejects.toMatchObject({ message: expectedError });
  ...
  test(
    'with stack trace',
    makeTest('{"errorMessage": "my error", "trace": "my stack trace"}', 'my error\nmy stack trace')
  );
});

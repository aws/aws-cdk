...
import { RuntimeDeterminer } from '../../../lib/custom-resources-framework/utils/runtime-determiner';

const DEFAULT_RUNTIME = Runtime.NODEJS_18_X;
...
describe('latest runtime', () => {
  test('selects default runtime', () => {
    ...
    // WHEN
    const latestRuntime = RuntimeDeterminer.determineLatestRuntime(runtimes, DEFAULT_RUNTIME);
  ...
  test('throws if no runtimes are specified', () => {
    ...
    // WHEN / THEN
    expect(() => {
      RuntimeDeterminer.determineLatestRuntime(runtimes, DEFAULT_RUNTIME);

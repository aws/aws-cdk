...
>(
  ...
  if (props.recipe && !props.recipe._isContainerRecipe() && props.imageScanningEcrRepository !== undefined) {
    throw new cdk.ValidationError('imageScanningEcrRepository is only supported for container recipe builds', scope);
  }

  if (props.recipe && !props.recipe._isContainerRecipe() && props.imageScanningEcrTags !== undefined) {

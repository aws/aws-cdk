...
export const defaultExecutionRolePolicy = <PropsT extends ImagePipelineProps | ImageProps>(
  ...
  if (!hasProps || !props.recipe || (props.recipe._isImageRecipe() && props.distributionConfiguration !== undefined)) {
  ...
  if (!hasProps || props.imageScanningEnabled !== false) {
    ...
    if (!hasProps || !props.recipe || props.recipe._isContainerRecipe()) {
      policies.push(
        new iam.PolicyStatement({
          effect: iam.Effect.ALLOW,

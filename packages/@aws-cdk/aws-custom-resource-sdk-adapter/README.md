...
In general, input types expected by SDKv3 are more permissive than what was expected by SDKv2. One such example is the regression to number types with respect to AWS SDKv2. More specifically, in SDKv2, number types that are accidentally passed as strings will be silently converted to the right type. SDKv3, however, will not do the conversion which causes the server call to fail because of mismatched types. For example,
...
Originally, the `AwsCustomResource` construct advertised that the service argument could be in any one of the following formats:
* The SDK v2 constructor name: APIGateway
* The SDK v2 constructor name in all lower case: apigateway (primary format)

Similarly, the action argument was advertised as being permitted in any of the following formats:

...
async function updateExportsAndEntryPoints(modules: ModuleMap, pkgPath: string) {
  ...
  await fs.writeFile(servicesIndexFilePath, indexStatements.sort((l1, l2) => l1.localeCompare(l2)).join('\n') + '\n');
}

async function submodules(modules: ModuleMap, outPath: string) {
  for (const [moduleName, submodule] of Object.entries(modules)) {
...
async function ensureSubmodule(submodule: ModuleMapEntry, outPath: string) {
  ...
  if (!existsSync(mixinsIndex)) {
    ...
    lines.push(...submodule.files
      .map((f) => path.relative(modulePath, path.join(outPath, f)))
      .map((f) => `export * from './${f.replace('.ts', '')}';`));
    await fs.writeFile(mixinsIndex, lines.join('\n') + '\n');
  }

...
export interface S3OriginConfig {
     ...
     */
    readonly originAccessIdentity?: cloudfront.CloudFrontOriginAccessIdentityResource
}
...
export interface Behavior {
     ...
     *
     */
    forwardedValues?: cloudfront.DistributionResource.ForwardedValuesProperty;
...
export interface CloudFrontWebDistributionProps {
     ...
     */
    errorConfigurations?: cloudfront.DistributionResource.CustomErrorResponseProperty[];
}
...
export class CloudFrontWebDistribution extends Construct {
    ...
    constructor(parent: Construct, name: string, props: CloudFrontWebDistributionProps) {
        ...
        const distributionConfig: cloudfront.DistributionResource.DistributionConfigProperty = {
        ...
        const origins: cloudfront.DistributionResource.OriginProperty[] = [];
        ...
        for (const originConfig of props.originConfigs) {
            ...
            const originHeaders: cloudfront.DistributionResource.OriginCustomHeaderProperty[] = [];
            if (originConfig.originHeaders) {
                Object.keys(originConfig.originHeaders).forEach(key => {
                    const oHeader: cloudfront.DistributionResource.OriginCustomHeaderProperty = {
                        headerName: key,
            ...
            const originProperty: cloudfront.DistributionResource.OriginProperty = {
                id: originId,
        ...
        const otherBehaviors: cloudfront.DistributionResource.CacheBehaviorProperty[] = [];
        for (const behavior of behaviors.filter(b => !b.isDefaultBehavior)) {
            if (!behavior.pathPattern) {
                throw new Error("pathPattern is required for all non-default behaviors");
            }
            otherBehaviors.push(this.toBehavior(behavior) as cloudfront.DistributionResource.CacheBehaviorProperty);
        }

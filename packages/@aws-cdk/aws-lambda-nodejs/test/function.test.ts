...
import { NodejsFunction, NodejsEdgeFunction } from '../lib';
...
describe('NodejsEdgeFunction', () => {
  ...
  test('NodejsEdgeFunction with .ts handler', () => {
    // WHEN
    new NodejsEdgeFunction(stack, 'handler1');
  ...
  test('NodejsEdgeFunction with .js handler', () => {
    // WHEN
    new NodejsEdgeFunction(stack, 'handler2');
  ...
  test('NodejsEdgeFunction with .mjs handler', () => {
    // WHEN
    new NodejsEdgeFunction(stack, 'handler3');
  ...
  test('NodejsEdgeFunction with container env vars', () => {
    // WHEN
    new NodejsEdgeFunction(stack, 'handler1', {
      bundling: {
        environment: {
  ...
  test('throws when entry is not js/ts', () => {
    expect(() => new NodejsEdgeFunction(stack, 'Fn', {
      entry: 'handler.py',
    })).toThrow(/Only JavaScript or TypeScript entry files are supported/);
  });
  ...
  test('throws when entry cannot be automatically found', () => {
    expect(() => new NodejsEdgeFunction(stack, 'Fn')).toThrow(/Cannot find handler file .*function.test.Fn.ts, .*function.test.Fn.js or .*function.test.Fn.mjs/);
  });

  test('throws with the wrong runtime family', () => {
    expect(() => new NodejsEdgeFunction(stack, 'handler1', {
      runtime: Runtime.PYTHON_3_8,
    })).toThrow(/Only `NODEJS` runtimes are supported/);
  });
  ...
  test('resolves entry to an absolute path', () => {
    // WHEN
    new NodejsEdgeFunction(stack, 'fn', {
      entry: 'lib/index.ts',
    });

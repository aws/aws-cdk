...
test('use api gateway rest api as an event rule target', () => {
  ...
  // WHEN
  rule1.addTarget(new targets.ApiGateway(restApi));
...
test('set http header parameters', () => {
  // GIVEN
  const stack = new cdk.Stack();
  const restApi = newTestRestApi(stack);
  const rule1 = new events.Rule(stack, 'Rule', {
  ...
  rule1.addTarget(new targets.ApiGateway(restApi, { headerParameters }));

  // THEN
  expect(stack).toHaveResourceLike('AWS::Events::Rule', {
    Targets: [
      {
        HttpParameters: { HeaderParameters: headerParameters },
        Id: 'Target0',
      },
    ],
  });
...
function newTestRestApi(scope: constructs.Construct, suffix = '') {
  ...
  const restApi = new api.LambdaRestApi(scope, `MyLambdaRestApi${suffix}`, {

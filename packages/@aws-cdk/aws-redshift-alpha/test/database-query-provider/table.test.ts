...
const stackId =
  'arn:aws:cloudformation:us-east-1:788445345501:stack/aws-cdk-redshift-cluster-database/e782bf70-b8f4-11ed-8c6a-111111111111';
...
describe('update', () => {
  ...
  describe('table name', () => {
    test('does not replace if PhysicalResourceId is old format', async () => {
      ...
      expect(mockExecuteStatement).not.toHaveBeenCalledWith(
        expect.objectContaining({
    ...
    expect(mockExecuteStatement).toHaveBeenCalledWith(
      expect.objectContaining({
        Sql: expect.stringMatching(
          new RegExp(`ALTER TABLE ${newResourceProperties.tableName.prefix}.+ DROP COLUMN col1`)
        ),
      })
    );
  });
  ...
  describe('column name', () => {
    test('does not replace if column name changed', async () => {
      const newEvent = {
        ...
        OldResourceProperties: {
          ...event.OldResourceProperties,
          tableColumns: [{ id: 'col1', name: 'col1', dataType: 'varchar(1)' }],
        },
      };
      const newTableColumnName = 'col2';
      const newResourceProperties: ResourcePropertiesType = {
        ...resourceProperties,
        tableColumns: [{ id: 'col1', name: newTableColumnName, dataType: 'varchar(1)' }],
      };
    ...
    test('does not replace if column id assigned, from undefined', async () => {
      const newEvent = {
        ...
        OldResourceProperties: {
          ...event.OldResourceProperties,
          tableColumns: [{ name: 'col1', dataType: 'varchar(1)' }],
        },
      };
      const newResourceProperties: ResourcePropertiesType = {
        ...resourceProperties,
  ...
  describe('column encoding', () => {
    ...
    test('adds a comma between multiple statements', async () => {
      ...
      const newResourceProperties = {
        ...resourceProperties,
        tableColumns: [
          { name: 'col1', dataType: 'varchar(1)', encoding: ColumnEncoding.RAW },
          { name: 'col2', dataType: 'varchar(1)', encoding: ColumnEncoding.RAW },
        ],
      };

...
export async function handler(
  ...
  if (event.RequestType === 'Create') {
    ...
    return {
      PhysicalResourceId: makePhysicalId(
        tableName,
        tableAndClusterProps,
    ...
    await dropTable(
      event.PhysicalResourceId.includes(event.StackId.substring(event.StackId.length - 12))
        ? tableName
...
async function createTable(
  ...
  const tableColumnsString = tableColumns
    .map((column) => `${column.name} ${column.dataType}${getEncodingColumnString(column)}`)
    .join();
  ...
  if (tableAndClusterProps.tableComment) {
    await executeStatement(
      `COMMENT ON TABLE ${tableName} IS '${tableAndClusterProps.tableComment}'`,
      tableAndClusterProps
    );
  }
...
async function updateTable(
  ...
  if (
    tableAndClusterProps.clusterName !== oldClusterProps.clusterName ||
    tableAndClusterProps.databaseName !== oldClusterProps.databaseName
  ) {
  ...
  if (columnDeletions.length > 0) {
    alterationStatements.push(
      ...columnDeletions.map((column) => `ALTER TABLE ${tableName} DROP COLUMN ${column.name}`)
    );
  }
  ...
  const columnEncoding = tableColumns
    .filter((column) => {
      return oldTableColumns.some(
        (oldColumn) => column.name === oldColumn.name && column.encoding !== oldColumn.encoding
      );
    })
  ...
  const columnComments = tableColumns
    .filter((column) => {
      return oldTableColumns.some(
        (oldColumn) => column.name === oldColumn.name && column.comment !== oldColumn.comment
      );
    })
    .map(
      (column) =>
  ...
  if (useColumnIds) {
    const columnNameUpdates = tableColumns.reduce(
      (updates, column) => {
    ...
    if (Object.keys(columnNameUpdates).length > 0) {
      alterationStatements.push(
        ...Object.entries(columnNameUpdates).map(
          ([oldName, newName]) => `ALTER TABLE ${tableName} RENAME COLUMN ${oldName} TO ${newName}`
        )
      );
    }
  }
  ...
  if (
    (oldSortStyle === newSortStyle && !areColumnsEqual(oldSortKeyColumns, newSortKeyColumns)) ||

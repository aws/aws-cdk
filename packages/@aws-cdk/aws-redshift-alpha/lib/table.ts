...
export class Table extends TableBase {
  ...
  private validateDistKeyColumns(columns: Column[]): void {
    try {
      ...
      throw new cdk.ValidationError('OneColumnConfiguredDistkey', 'Only one column can be configured as distKey.', this);
    }
  }
  ...
  private validateDistStyle(distStyle: TableDistStyle, columns: Column[]): void {
    ...
    if (distKeyColumn && distStyle !== TableDistStyle.KEY) {
      throw new cdk.ValidationError('TablediststyleKeyConfiguredDistkey', `Only 'TableDistStyle.KEY' can be configured when distKey is also configured. Found ${distStyle}`, this);
    }
    if (!distKeyColumn && distStyle === TableDistStyle.KEY) {
      throw new cdk.ValidationError('DiststyleTablediststyleKeyConfigured', 'distStyle of "TableDistStyle.KEY" can only be configured when distKey is also configured.', this);
    }
  }
  ...
  private validateSortStyle(sortStyle: TableSortStyle, columns: Column[]): void {
    ...
    if (sortKeyColumns.length === 0 && sortStyle !== TableSortStyle.AUTO) {
      throw new cdk.ValidationError('SortstyleSortstyleConfiguredSortkey', `sortStyle of '${sortStyle}' can only be configured when sortKey is also configured.`, this);
    }
    if (sortKeyColumns.length > 0 && sortStyle === TableSortStyle.AUTO) {
      throw new cdk.ValidationError('SortstyleTablesortstyleAutoConfigured', `sortStyle of '${TableSortStyle.AUTO}' cannot be configured when sortKey is also configured.`, this);
    }
  }
    ...
    for (let i = 0; i < columns.length; i++) {
      ...
      if (column.id) {
        if (columnIds.has(column.id)) {
          throw new cdk.ValidationError('ColumnColumnUnique', `Column id '${column.id}' is not unique.`, this);
        }
        columnIds.add(column.id);
      } else {
        if (columnIds.has(column.name)) {
          throw new cdk.ValidationError('ColumnNameColumnName', `Column name '${column.name}' is not unique amongst the column ids.`, this);
        }

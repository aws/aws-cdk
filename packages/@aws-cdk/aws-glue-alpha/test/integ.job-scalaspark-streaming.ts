...
const stack = new cdk.Stack(app, 'aws-glue-job-scalaspark-streaming');
...
new glue.ScalaSparkStreamingJob(stack, 'BasicScalaSparkStreamingJob', {
  script: jar_file,
  role: iam_role,
  className: job_class,
});

new glue.ScalaSparkStreamingJob(stack, 'OverrideScalaSparkStreamingJob', {
  script: jar_file,
  className: job_class,
  role: iam_role,

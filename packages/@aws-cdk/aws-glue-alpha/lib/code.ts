...
export class AssetCode extends Code {
  ...
  constructor(private readonly path: string, private readonly options: s3assets.AssetOptions = { }) {
    ...
    if (fs.lstatSync(this.path).isDirectory()) {
      throw new cdk.UnscopedValidationError('CodePathPathDirectory', `Code path ${this.path} is a directory. Only files are supported`);
    }
  }
  ...
  public bind(scope: constructs.Construct, grantable: iam.IGrantable): CodeConfig {
    ...
    if (!this.asset) {
      ...
      throw new cdk.UnscopedValidationError('AssetAlreadyAssociatedAnother', `Asset is already associated with another stack '${cdk.Stack.of(this.asset).stackName}'. ` +

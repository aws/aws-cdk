...
describe('Fleet base', () => {
  describe('FleetBase.fromFleetAttributes()', () => {
    ...
    describe('for a fleet in a different account and region', () => {
      let func: gamelift.IFleet;

      beforeEach(() => {
        func = gamelift.FleetBase.fromFleetAttributes(stack, 'iFunc', { fleetArn });
      });

      test("the fleet's region is taken from the ARN", () => {
        expect(func.env.region).toBe('fleet-region');
      });

      test("the fleet's account is taken from the ARN", () => {
        expect(func.env.account).toBe('123456789012');
      });
    });
  });
});
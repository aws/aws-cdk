...
import { Lazy, Resource, Aspects, Annotations } from '@aws-cdk/core';
...
export interface BackupSelectionOptions {
  ...
  /**
   * Set this flag to true to disable the addition of all managed policies for
   * either the autogenerated or provided IAM role. Note - you will need to
   * configure the role yourself to provide permissions for the desired backup
   * and restore operations against your resource selections.
   * @default false
   */
  readonly customRole?: boolean;
}
...
export class BackupSelection extends Resource implements iam.IGrantable {
  ...
  constructor(scope: Construct, id: string, props: BackupSelectionProps) {
    ...
    // Only add managed policies if the customRole field is false
    if (!props.customRole) {
      ...
      // Provide deployment warnings if customRole is enabled but any of the
      // permission flags are true
      if (props.allowRestores) {
        Annotations.of(this).addWarning('Both the customRole and allowRestores flag was set to true - no restore managed policy was added.');
      }
      if (props.allowS3Backup) {
        Annotations.of(this).addWarning('Both the customRole and allowS3Backup flag was set to true - no S3 backup managed policy was added.');
      }
      if (props.allowS3Restores) {
        Annotations.of(this).addWarning('Both the customRole and allowS3Restores flag was set to true - no S3 restore managed policy was added.');
      }
    }

...
import { CfnResource, Construct, Legacy, Stack } from '@aws-cdk/core';
...
export class LambdaDestination implements s3.IBucketNotificationDestination {
  ...
  public bind(_scope: Construct, bucket: s3.IBucket): s3.BucketNotificationDestinationConfig {
    const permissionId = `AllowBucketNotificationsTo${Legacy.nodeUniqueId(this.fn.permissionsNode)}`;

    if (!Construct.isConstruct(bucket)) {
      throw new Error(`LambdaDestination for function ${Legacy.nodeUniqueId(this.fn.permissionsNode)} can only be configured on a

...
export = {
  'Pipeline artifacts KMS support': {
    'KMS key is used in output artifacts (cross region)'(test: Test) {
      ...
      const pipeline = new codepipeline.Pipeline(stack, 'MyPipeline', {
        artifactsStore: new codepipeline.ArtifactsStore(stack, 'PipelineDefaultStore', {
          artifactsKeyAliasArn: `arn:aws:kms:us-west-2:111122223333:alias/pipeline-bucket-alias`
        }),
        crossRegionArtifactsStores: {
          'us-west-1': new codepipeline.ImportedArtifactsStore(stack, 'Imported-us-west-1', {
            ...
            artifactsKeyAliasArn: `arn:aws:kms:us-west-1:111122223333:alias/sfo-replication-bucket-alias`
          })
        }
      });
      ...
      test.equal(pipeline.artifactsStores['us-west-1'].artifactsKeyAliasArn,
    ...
    'KMS key is used in output artifacts (single region)'(test: Test) {
      ...
      test.equal(pipeline.artifactsStore.artifactsKeyAliasArn, 'arn:aws:kms:us-west-2:111122223333:alias/pipeline-bucket-alias');
      test.done();
    },

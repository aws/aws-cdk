...
test('legacyPipeline has self-mutation stage', () => {
  // THEN
  expect(pipelineStack).toHaveResourceLike('AWS::CodePipeline::Pipeline', {
    Stages: arrayWith({
      Name: 'UpdatePipeline',
...
test('selfmutation stage correctly identifies nested assembly of legacyPipeline stack', () => {
...
test('can control fix/CLI version used in legacyPipeline selfupdate', () => {
  // WHEN
...
test('changing CLI version leads to a different legacyPipeline structure (restarting it)', () => {
  // GIVEN
  const stack2 = new Stack(app, 'Stack2', { env: legacyTestUtil.PIPELINE_ENV });
...
test('tags get reflected in legacyPipeline', () => {
  // WHEN

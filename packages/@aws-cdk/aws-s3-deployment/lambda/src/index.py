...
def handler(event, context):
    ...
    try:
        ...
        try:
            ...
            deleteOnSync        = props.get('DeleteMissingFiles', 'true') == 'true'
        ...
        if request_type == "Update" or request_type == "Create":
            s3_deploy(s3_source_zips, s3_dest, user_metadata, system_metadata, deleteOnSync)
...
def s3_deploy(s3_source_zips, s3_dest, user_metadata, system_metadata, deleteOnSync):
    ...
    aws_command(*s3_command, contents_dir, s3_dest, *create_metadata_args(user_metadata, system_metadata))

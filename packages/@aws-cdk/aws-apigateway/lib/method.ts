...
export class Method extends cdk.Construct {
  ...
  constructor(scope: cdk.Construct, scid: string, props: MethodProps) {
    super(scope, scid);
  ...
  public get methodArn(): string {
    if (!this.restApi.deploymentStage) {
      throw new Error(
        `Unable to determine ARN for method "${this.node.scid}" since there is no stage associated with this API.\n` +

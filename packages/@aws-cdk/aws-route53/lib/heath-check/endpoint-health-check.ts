...
export class EndpointHealthCheckProtocol {
    ...
    public static http(resourcePath = '/', searchString?: string): EndpointHealthCheckProtocol {
        return new EndpointHealthCheckProtocol({ type: searchString ? EndpointHealthCheckType.HTTP_STR_MATCH : EndpointHealthCheckType.HTTP, resourcePath, searchString });
    }
    ...
    public static https(resourcePath = '/', searchString?: string, enableSni = true): EndpointHealthCheckProtocol {
        return new EndpointHealthCheckProtocol({ type: searchString ? EndpointHealthCheckType.HTTPS_STR_MATCH : EndpointHealthCheckType.HTTPS, resourcePath, searchString, enableSni });
    }
...
export class EndpointHealthCheck extends HealthCheck {
    public static ipAddress(scope: Construct, id: string, endpointProps: IPAddressHealthCheckOptions, protocol: EndpointHealthCheckProtocol, options: AdvancedEndpointHealthCheckOptions = {}) {
        return new EndpointHealthCheck(scope, id, { ...endpointProps, ...protocol.options, ...options });
    }

    public static domainName(scope: Construct, id: string, fullyQualifiedDomainName: string, protocol: EndpointHealthCheckProtocol, options: AdvancedEndpointHealthCheckOptions = {}) {

...
export class ObservabilityConfiguration extends cdk.Resource implements IObservabilityConfiguration {
  ...
  public static fromArn(scope: Construct, id: string, observabilityConfigurationArn: string): IObservabilityConfiguration {
    ...
    if (!resourceParts || resourceParts.length < 3) {
      throw new cdk.UnscopedValidationError('UnexpectedArnFormatObservabilityconfigurationarn', `Unexpected ARN format: ${observabilityConfigurationArn}.`);
    }
  ...
  public constructor(scope: Construct, id: string, props: ObservabilityConfigurationProps) {
    ...
    if (props.observabilityConfigurationName !== undefined && !cdk.Token.isUnresolved(props.observabilityConfigurationName)) {
      if (props.observabilityConfigurationName.length < 4 || props.observabilityConfigurationName.length > 32) {
        throw new cdk.ValidationError('ObservabilityconfigurationnameCharactersGotProps', `\`observabilityConfigurationName\` must be between 4 and 32 characters, got: ${props.observabilityConfigurationName.length} characters.`, this,
        );
      }

      if (!/^[A-Za-z0-9][A-Za-z0-9\-_]*$/.test(props.observabilityConfigurationName)) {
        throw new cdk.ValidationError('ObservabilityconfigurationnameStartAlphanumericCharacter', `\`observabilityConfigurationName\` must start with an alphanumeric character and contain only alphanumeric characters, hyphens, or underscores after that, got: ${props.observabilityConfigurationName}.`, this,
        );
      }
    }

...
export class Cpu {
  ...
  public static of(unit: string): Cpu {
    ...
    if (!isValidValue) {
      throw new cdk.UnscopedValidationError('CpuValueInvalid', 'CPU value is invalid');
    }
...
export class Memory {
  ...
  public static of(unit: string): Memory {
    ...
    if (!isValidValue) {
      throw new cdk.UnscopedValidationError('MemoryValueInvalid', 'Memory value is invalid');
    }
  ...
  private constructor(
    ...
    if (this.healthCheckProtocolType === HealthCheckProtocolType.HTTP) {
      if (this.path !== undefined && this.path.length === 0) {
        throw new cdk.UnscopedValidationError('PathLengthGreater', 'path length must be greater than 0');
      }
    ...
    if (this.healthyThreshold < 1 || this.healthyThreshold > 20) {
      throw new cdk.UnscopedValidationError('HealthythresholdGotHealthythreshold', `healthyThreshold must be between 1 and 20, got ${this.healthyThreshold}`);
    }
    if (this.unhealthyThreshold < 1 || this.unhealthyThreshold > 20) {
      throw new cdk.UnscopedValidationError('UnhealthythresholdGotUnhealthythreshold', `unhealthyThreshold must be between 1 and 20, got ${this.unhealthyThreshold}`);
    }
    if (this.interval.toSeconds() < 1 || this.interval.toSeconds() > 20) {
      throw new cdk.UnscopedValidationError('IntervalSecondsGotInterval', `interval must be between 1 and 20 seconds, got ${this.interval.toSeconds()}`);
    }
    if (this.timeout.toSeconds() < 1 || this.timeout.toSeconds() > 20) {
      throw new cdk.UnscopedValidationError('TimeoutSecondsGotTimeout', `timeout must be between 1 and 20 seconds, got ${this.timeout.toSeconds()}`);
    }
  }
...
export class Service extends cdk.Resource implements IService, iam.IGrantable {
  ...
  public constructor(scope: Construct, id: string, props: ServiceProps) {
    ...
    if (this.source.codeRepository?.codeConfiguration.configurationSource == ConfigurationSourceType.REPOSITORY &&
      ...
      throw new cdk.ValidationError('ConfigurationvaluesProvidedConfigurationsourceRepository', 'configurationValues cannot be provided if the ConfigurationSource is Repository', this);
    }
    ...
    if (props.serviceName !== undefined && !cdk.Token.isUnresolved(props.serviceName)) {
      if (props.serviceName.length < 4 || props.serviceName.length > 40) {
        throw new cdk.ValidationError('ServicenameCharactersGotProps', `\`serviceName\` must be between 4 and 40 characters, got: ${props.serviceName.length} characters.`, this,
        );
      }

      if (!/^[A-Za-z0-9][A-Za-z0-9\-_]*$/.test(props.serviceName)) {
        throw new cdk.ValidationError('ServicenameStartAlphanumericCharacter', `\`serviceName\` must start with an alphanumeric character and contain only alphanumeric characters, hyphens, or underscores after that, got: ${props.serviceName}.`, this,
        );
      }
    }
  ...
  public addEnvironmentVariable(name: string, value: string) {
    if (name.startsWith('AWSAPPRUNNER')) {
      throw new cdk.ValidationError('EnvironmentVariableKeyName', `Environment variable key ${name} with a prefix of AWSAPPRUNNER is not allowed`, this);
    }
  ...
  public addSecret(name: string, secret: Secret) {
    if (name.startsWith('AWSAPPRUNNER')) {
      throw new cdk.ValidationError('EnvironmentSecretKeyName', `Environment secret key ${name} with a prefix of AWSAPPRUNNER is not allowed`, this);
    }
  ...
  private getEnvironmentVariables(): { [key: string]: string } {
    ...
    if (codeEnv.every(el => el !== undefined) || imageEnv.every(el => el !== undefined)) {
      throw new cdk.ValidationError('EnvironmentvariablesEnvironmentPropertiesPlease', [

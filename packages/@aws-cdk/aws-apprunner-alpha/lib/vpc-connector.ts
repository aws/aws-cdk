  ...
  public constructor(scope: Construct, id: string, props: VpcConnectorProps) {
    ...
    if (props.vpcConnectorName !== undefined && !cdk.Token.isUnresolved(props.vpcConnectorName)) {
      if (props.vpcConnectorName.length < 4 || props.vpcConnectorName.length > 40) {
        throw new cdk.ValidationError('VpcconnectornameCharactersGotProps', `\`vpcConnectorName\` must be between 4 and 40 characters, got: ${props.vpcConnectorName.length} characters.`, this,
        );
      }

      if (!/^[A-Za-z0-9][A-Za-z0-9\-_]*$/.test(props.vpcConnectorName)) {
        throw new cdk.ValidationError('VpcconnectornameStartAlphanumericCharacter', `\`vpcConnectorName\` must start with an alphanumeric character and contain only alphanumeric characters, hyphens, or underscores after that, got: ${props.vpcConnectorName}.`, this,
        );
      }
    }

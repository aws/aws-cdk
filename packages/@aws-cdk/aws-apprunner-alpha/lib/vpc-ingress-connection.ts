  ...
  public constructor(scope: Construct, id: string, props: VpcIngressConnectionProps) {
    ...
    if (props.vpcIngressConnectionName !== undefined && !cdk.Token.isUnresolved(props.vpcIngressConnectionName)) {
      if (props.vpcIngressConnectionName.length < 4 || props.vpcIngressConnectionName.length > 40) {
        throw new cdk.ValidationError('VpcingressconnectionnameCharactersGotProps', `\`vpcIngressConnectionName\` must be between 4 and 40 characters, got: ${props.vpcIngressConnectionName.length} characters.`, this,
        );
      }

      if (!/^[A-Za-z0-9][A-Za-z0-9\-_]*$/.test(props.vpcIngressConnectionName)) {
        throw new cdk.ValidationError('VpcingressconnectionnameStartAlphanumericCharacter', `\`vpcIngressConnectionName\` must start with an alphanumeric character and contain only alphanumeric characters, hyphens, or underscores after that, got: ${props.vpcIngressConnectionName}.`, this,
        );
      }
    }

...
test.each([['MyObservabilityConfiguration'], ['my-observability-configuration_1']])(
  'create a ObservabilityConfiguration with all properties (name: %s)',
...
test.each([['tes'], ['test-observability-configuration-name-over-limitation']])(
  'observabilityConfigurationName length is invalid (name: %s)',
  (observabilityConfigurationName: string) => {
    expect(() => {
      new ObservabilityConfiguration(stack, 'ObservabilityConfiguration', {
        observabilityConfigurationName,
      ...
      `\`observabilityConfigurationName\` must be between 4 and 32 characters, got: ${observabilityConfigurationName.length} characters.`
    );
  }
);

test.each([['-test'], ['test-?'], ['test-\\']])(
  'observabilityConfigurationName includes invalid characters (name: %s)',
  (observabilityConfigurationName: string) => {
    expect(() => {
      new ObservabilityConfiguration(stack, 'ObservabilityConfiguration', {
        observabilityConfigurationName,
      ...
      `\`observabilityConfigurationName\` must start with an alphanumeric character and contain only alphanumeric characters, hyphens, or underscores after that, got: ${observabilityConfigurationName}.`
    );
  }
);

...
to have one API mapping with undefined `mappingKey`. If more than one API mappings are required, it's mandatory to specify `mappingKey` for all of them. In the sample above, the custom domain is associated

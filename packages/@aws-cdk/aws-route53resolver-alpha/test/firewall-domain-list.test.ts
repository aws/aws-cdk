...
test('domain list from strings', () => {
  ...
  new FirewallDomainList(stack, 'List', {
    domains: FirewallDomains.fromList(['first-domain.com', 'second-domain.net', '*.wildcard.com']),
  });

  // THEN
  Template.fromStack(stack).hasResourceProperties('AWS::Route53Resolver::FirewallDomainList', {
    Domains: ['first-domain.com', 'second-domain.net', '*.wildcard.com'],
  });
});
...
test('domain list from asset', () => {
  ...
  Template.fromStack(stack).hasResourceProperties('AWS::Route53Resolver::FirewallDomainList', {
    DomainFileUrl: {
      'Fn::Sub':
        's3://cdk-hnb659fds-assets-${AWS::AccountId}-${AWS::Region}/e820b3f07bf66854be0dfd6f3ec357a10d644f2011069e5ad07d42f4f89ed35a.txt',
    },
  });
});

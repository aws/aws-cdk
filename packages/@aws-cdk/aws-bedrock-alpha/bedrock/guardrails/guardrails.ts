...
export class Guardrail extends GuardrailBase {
  ...
  private validateRegexFilter(filter: filters.RegexFilter, index?: number): void {
    ...
    if (filter.name !== undefined && !Token.isUnresolved(filter.name)) {
      if (filter.name.length < 1) {
        throw new ValidationError(`${prefix}: The field name is ${filter.name.length} characters long but must be at least 1 characters`, this, this);
      }
      if (filter.name.length > 100) {
        throw new ValidationError(`${prefix}: The field name is ${filter.name.length} characters long but must be less than or equal to 100 characters`, this, this);
      }
    }
    ...
    if (filter.description !== undefined && !Token.isUnresolved(filter.description)) {
      if (filter.description.length < 1) {
        throw new ValidationError(`${prefix}: The field description is ${filter.description.length} characters long but must be at least 1 characters`, this, this);
      }
      if (filter.description.length > 1000) {
        throw new ValidationError(`${prefix}: The field description is ${filter.description.length} characters long but must be less than or equal to 1000 characters`, this, this);
      }
    }
    ...
    if (filter.pattern !== undefined && !Token.isUnresolved(filter.pattern)) {
      if (filter.pattern.length < 1) {
        throw new ValidationError(`${prefix}: The field pattern is ${filter.pattern.length} characters long but must be at least 1 characters`, this, this);
      }
    }
    ...
    if (filter.action !== undefined && !Token.isUnresolved(filter.action) && !Object.values(filters.GuardrailAction).includes(filter.action)) {
      throw new ValidationError(`${prefix}: action must be a valid GuardrailAction value`, this, this);
    }
    ...
    if (filter.inputAction !== undefined &&
      ...
      throw new ValidationError(`${prefix}: inputAction must be a valid GuardrailAction value`, this, this);
    }
    ...
    if (filter.outputAction !== undefined &&
      ...
      throw new ValidationError(`${prefix}: outputAction must be a valid GuardrailAction value`, this, this);
    }
    ...
    if (filter.inputEnabled !== undefined &&
      ...
      throw new ValidationError(`${prefix}: inputEnabled must be a boolean value`, this, this);
    }
    ...
    if (filter.outputEnabled !== undefined &&
      ...
      throw new ValidationError(`${prefix}: outputEnabled must be a boolean value`, this, this);
    }
  ...
  private validateMessagingProperty(value: string | undefined, propertyName: string): void {
    if (value !== undefined && !Token.isUnresolved(value)) {
      if (value.length < 1) {
        throw new ValidationError(`Invalid ${propertyName}: The field ${propertyName} is ${value.length} characters long but must be at least 1 characters`, this, this);
      }
      if (value.length > 500) {
        throw new ValidationError(`Invalid ${propertyName}: The field ${propertyName} is ${value.length} characters long but must be less than or equal to 500 characters`, this, this);
      }
    }
  }
  ...
  private validateContentFilters(contentFilters?: filters.ContentFilter[]): void {
    ...
    contentFilters.forEach((filter, index) => {
      ...
      if (!filter.type) {
        throw new ValidationError(`${prefix}: type is required`, this, this);
      }
      ...
      if (filter.inputStrength !== undefined && !Token.isUnresolved(filter.inputStrength)) {
        if (!Object.values(filters.ContentFilterStrength).includes(filter.inputStrength)) {
          throw new ValidationError(`${prefix}: inputStrength must be a valid ContentFilterStrength value`, this, this);
        }
      }
      ...
      if (filter.outputStrength !== undefined && !Token.isUnresolved(filter.outputStrength)) {
        if (!Object.values(filters.ContentFilterStrength).includes(filter.outputStrength)) {
          throw new ValidationError(`${prefix}: outputStrength must be a valid ContentFilterStrength value`, this, this);
        }
      }
      ...
      if (filter.inputModalities) {
        filter.inputModalities.forEach((modality, modalityIndex) => {
          if (!Object.values(filters.ModalityType).includes(modality)) {
            throw new ValidationError(`${prefix}: inputModalities[${modalityIndex}] must be a valid ModalityType value`, this, this);
          }
        });
      }
      ...
      if (filter.outputModalities) {
        filter.outputModalities.forEach((modality, modalityIndex) => {
          if (!Object.values(filters.ModalityType).includes(modality)) {
            throw new ValidationError(`${prefix}: outputModalities[${modalityIndex}] must be a valid ModalityType value`, this, this);
          }
        });
      }
  ...
  private validatePiiFilters(piiFilters?: filters.PIIFilter[]): void {
    ...
    piiFilters.forEach((filter, index) => {
      ...
      if (!filter.action) {
        throw new ValidationError(`${prefix}: action is required`, this, this);
      }
  ...
  private validateDeniedTopics(deniedTopics?: filters.Topic[]): void {
    ...
    deniedTopics.forEach((topic, index) => {
      ...
      if (!topic.name) {
        throw new ValidationError(`${prefix}: name is required`, this, this);
      }

      if (!topic.definition) {
        throw new ValidationError(`${prefix}: definition is required`, this, this);
      }

      // Validate name length
      if (!Token.isUnresolved(topic.name) && topic.name.length > 100) {
        throw new ValidationError(`${prefix}: name must be 100 characters or less`, this, this);
      }

      // Validate definition length
      if (!Token.isUnresolved(topic.definition) && topic.definition.length > 1000) {
        throw new ValidationError(`${prefix}: definition must be 1000 characters or less`, this, this);
      }

      // Validate examples if provided
      if (topic.examples) {
        if (topic.examples.length > 100) {
          throw new ValidationError(`${prefix}: examples array cannot contain more than 100 examples`, this, this);
        }

        topic.examples.forEach((example, exampleIndex) => {
          if (!Token.isUnresolved(example) && example.length > 100) {
            throw new ValidationError(`${prefix}: examples[${exampleIndex}] must be 100 characters or less`, this, this);
          }
        });
      }
  ...
  private validateContextualGroundingFilters(contextualGroundingFilters?: filters.ContextualGroundingFilter[]): void {
    ...
    contextualGroundingFilters.forEach((filter, index) => {
      ...
      if (filter.threshold === undefined) {
        throw new ValidationError(`${prefix}: threshold is required`, this, this);
      }

      // Validate type
      if (!Object.values(filters.ContextualGroundingFilterType).includes(filter.type)) {
        throw new ValidationError(`${prefix}: type must be a valid ContextualGroundingFilterType value`, this, this);
      }

      // Validate threshold range
      if (!Token.isUnresolved(filter.threshold)) {
        if (filter.threshold < 0 || filter.threshold > 0.99) {
          throw new ValidationError(`${prefix}: threshold must be between 0 and 0.99`, this, this);
        }
      }
      ...
      if (filter.enabled !== undefined &&
        ...
        throw new ValidationError(`${prefix}: enabled must be a boolean value`, this, this);
      }
  ...
  private validateWordFilters(wordFilters?: filters.WordFilter[]): void {
    ...
    wordFilters.forEach((filter, index) => {
      ...
      // Validate that the filter has required properties
      if (!filter.text) {
        throw new ValidationError(`${prefix}: text is required`, this, this);
      }

      // Validate text length
      if (!Token.isUnresolved(filter.text) && filter.text.length > 100) {
        throw new ValidationError(`${prefix}: text must be 100 characters or less`, this, this);
      }
  ...
  private validateManagedWordListFilters(managedWordListFilters?: filters.ManagedWordFilter[]): void {
    ...
    managedWordListFilters.forEach((filter, index) => {
      ...
      if (filter.type !== undefined && !Object.values(filters.ManagedWordFilterType).includes(filter.type)) {
        throw new ValidationError(`${prefix}: type must be a valid ManagedWordFilterType value`, this, this);
      }
  ...
  private validateSingleContentFilter(filter: filters.ContentFilter): void {
    ...
    try {
      ...
      if (error instanceof ValidationError) {
        ...
        throw new ValidationError(message, this, this);
      }
      throw error;
    }
  }

...
export class Agent extends AgentBase implements IAgent {
  ...
  constructor(scope: Construct, id: string, props: AgentProps) {
    ...
    if (props.instruction !== undefined &&
      ...
      throw new ValidationError('instruction must be at least 40 characters', this);
    }

    // Validate orchestration configuration
    if (props.orchestrationType === OrchestrationType.CUSTOM_ORCHESTRATION && !props.customOrchestration) {
      throw new ValidationError('customOrchestration must be provided when orchestrationType is CUSTOM_ORCHESTRATION', this);
    }

    if (props.customOrchestration && props.orchestrationType !== OrchestrationType.CUSTOM_ORCHESTRATION) {
      throw new ValidationError('customOrchestration can only be provided when orchestrationType is CUSTOM_ORCHESTRATION', this);
    }

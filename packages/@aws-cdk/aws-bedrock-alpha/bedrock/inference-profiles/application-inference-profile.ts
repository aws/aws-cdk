...
export class ApplicationInferenceProfile extends InferenceProfileBase implements IBedrockInvokable {
  ...
  private validateProps(props: ApplicationInferenceProfileProps): void {
    ...
    if (!props.applicationInferenceProfileName || props.applicationInferenceProfileName.trim() === '') {
      throw new ValidationError('BEDROCK_APPLICATION_INFERENCE_PROFILE_NAME_REQUIRED', 'applicationInferenceProfileName is required and cannot be empty', this);
    }

    // Validate applicationInferenceProfileName length
    if (props.applicationInferenceProfileName.length > 64) {
      throw new ValidationError('BEDROCK_APPLICATION_INFERENCE_PROFILE_NAME_TOO_LONG', 'applicationInferenceProfileName cannot exceed 64 characters', this);
    }
    ...
    if (!namePattern.test(props.applicationInferenceProfileName)) {
      throw new ValidationError(
        'BEDROCK_APPLICATION_INFERENCE_PROFILE_NAME_INVALID_PATTERN',
    ...
    if (!props.modelSource) {
      throw new ValidationError('BEDROCK_APPLICATION_INFERENCE_PROFILE_MODEL_SOURCE_REQUIRED', 'modelSource is required', this);
    }

    // Validate description length if provided
    if (props.description !== undefined && props.description.length > 200) {
      throw new ValidationError('BEDROCK_APPLICATION_INFERENCE_PROFILE_DESCRIPTION_TOO_LONG', 'description cannot exceed 200 characters', this);
    }
    ...
    if (props.description !== undefined && props.description !== '') {
      ...
      if (!descriptionPattern.test(props.description)) {
        throw new ValidationError(
          'BEDROCK_APPLICATION_INFERENCE_PROFILE_DESCRIPTION_INVALID_PATTERN',

...
export interface ExtendedDataSourceProps {
	 ...
	 * @default - No config
	 */
	readonly openSearchServiceConfig?:
	 ...
	 * @default - No config
	 */
	readonly relationalDatabaseConfig?:
...
export abstract class BaseDataSource extends Construct {
	...
	constructor(
		...
		const supportedName = Token.isUnresolved(name)
			? name
			: name.replace(/[\W]+/g, '')
 ...
 */
export abstract class BackedDataSource

...
- `Directive.cognito(groups: string[], isAdditional: boolean)` sets a type or field's 
  authorization to be validated through `Cognito User Pools`
  - `groups` the name of the cognito groups to give access to
  - `isAdditional` if using cognito authorization as an additional mode, set this to be true

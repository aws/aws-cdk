...
export interface IModel extends cdk.IResource, iam.IGrantable, ec2.IConnectable {
    ...
    /**
     * Whether or not this Model was bound to a VPC
     *
...
export class Model extends ModelBase {
     ...
     */
    public static fromModelAttributes(
        scope: cdk.Construct,
        id: string,
    ...
    constructor(scope: cdk.Construct, id: string, props: ModelProps) {
        ...
        const model = new CfnModel(this, "Model", {
            ...
            primaryContainer:
                containers.length === 1 ? this.renderContainer(containers[0]) : undefined,
            vpcConfig: cdk.Lazy.anyValue({ produce: () => this.renderVpcConfig() }),
            containers:
                containers.length === 1
                    ? undefined
                    : containers.map((c) => this.renderContainer(c)),
        });
    ...
    private renderContainer(container: ContainerDefinition): CfnModel.ContainerDefinitionProperty {
        return {
            ...
            modelDataUrl: container.modelData
                ? container.modelData.bind(this, this).uri
                : undefined,
        };
    }

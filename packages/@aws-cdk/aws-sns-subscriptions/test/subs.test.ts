...
testFutureBehavior('can create lambda subscriptions with same ids in different scopes',
  { [cxapi.SNS_SUBSCRIPTIONS_UNIQUE_LAMBDA_SUBSCRIPTION_ID]: true },
  App,
  (app) => {
    const testStack = new Stack(app, 'stack');
    const fction = new lambda.Function(testStack, 'MyFunc', {
      runtime: lambda.Runtime.NODEJS_14_X,
      handler: 'index.handler',
      code: lambda.Code.fromInline('exports.handler = function(e, c, cb) { return cb() }'),
    });

    const resource1 = new Construct(testStack, 'scope1');
    const topic1 = new sns.Topic(resource1, 'topic');
    const resource2 = new Construct(testStack, 'scope2');
    const topic2 = new sns.Topic(resource2, 'topic');

    topic1.addSubscription(new subs.LambdaSubscription(fction));
    topic2.addSubscription(new subs.LambdaSubscription(fction));

    Template.fromStack(testStack).templateMatches({
      'Resources': {
        'scope1topicCD0AC840': {
          'Type': 'AWS::SNS::Topic',
        },
        ...
        'MyFuncstackscope1topic59B8CC7497672669': {
          'Type': 'AWS::SNS::Subscription',
          'Properties': {
            'Endpoint': {
              'Fn::GetAtt': [
        ...
        'MyFuncAllowInvokestackscope2topicD1533E1C76F71B57': {
          'Type': 'AWS::Lambda::Permission',
          'Properties': {
            'Action': 'lambda:InvokeFunction',
            'FunctionName': {
        ...
        'MyFuncstackscope2topicD1533E1CAA05A9D1': {
          'Type': 'AWS::SNS::Subscription',
          'Properties': {
            'Endpoint': {
              'Fn::GetAtt': [
        ...
        'MyFuncAllowInvokestackscope1topic59B8CC7420223131': {
          'Type': 'AWS::Lambda::Permission',
          'Properties': {
            'Action': 'lambda:InvokeFunction',
            'FunctionName': {
...
testLegacyBehavior('cannot create lambda subscriptions with same ids in different scopes',
  App,
  (app) => {
    const testStack = new Stack(app, 'stack');
    const fction = new lambda.Function(testStack, 'MyFunc', {
    ...
    expect(() => topic2.addSubscription(new subs.LambdaSubscription(fction)))
      .toThrowError(/A subscription with id \"topic\" already exists under the scope stack\/MyFunc/);
  });

...
export class DefaultValidationPlugin implements IPolicyValidationPlugin {
  public readonly name = 'ValidationEngine (Default)';
    ...
    for (const templatePath of context.templatePaths) {
      ...
      for (const diag of report.diagnostics) {
        ...
        if (existing) {
          ...
          const qualifiedId = `default::${diag.ruleId}`;
          const description = severity === 'fatal'
            ? diag.message
            : `${diag.message} [ack: ${qualifiedId}]`;
          violationMap.set(key, {
            ruleName: qualifiedId,
            description,
            severity,

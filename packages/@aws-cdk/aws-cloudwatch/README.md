...
Like other constructs, `IInsightRule` has a `metric()` function to create a Metric Object for the [metrics generated by the Insight Rule](https://docs.aws.amazon.com/AmazonCloudWatch/latest/monitoring/ContributorInsights-GraphReportData.html).

```ts
declare const insightRule: cloudwatch.IInsightRule;

const uniqueContributors = insightRule.metric('UniqueContributors');
const average = insightRule.metric('Average');
```

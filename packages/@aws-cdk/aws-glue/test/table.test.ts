...
test('table with CSV format', () => {
  ...
  const table = new glue.Table(stack, 'Table', {
    ...
    compressed: true,
    dataFormat: glue.DataFormat.csvWithSeparator(glue.CsvSeparator.TAB),
  });
  equal(table.encryptionKey, undefined);
  equal(table.bucket.encryptionKey, undefined);
...
test('table with XML format', () => {
  ...
  const table = new glue.Table(stack, 'Table', {
    ...
    compressed: true,
    dataFormat: glue.DataFormat.xmlWithRowTag('row'),
  });
  equal(table.encryptionKey, undefined);
  equal(table.bucket.encryptionKey, undefined);

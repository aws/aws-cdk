...
export abstract class Inspector {
  public aroundAssert?: (cb: () => Promise<any>) => Promise<any>;
  ...
  public notTo(assertion: Assertion<this>): Promise<any> {
    return this.to(not(assertion));
  }

  abstract get value(): Promise<any>;

  private _to(assertion: Assertion<this>): Promise<any> {
    return assertion.assertOrThrow(this);
  }
}

export class StackInspector extends Inspector {

  private readonly template: Promise<{ [key: string]: any }>;

  constructor(public readonly stack: Promise<api.CloudFormationStackArtifact | object>) {
    super();

    this.template = stack.then(s => s instanceof api.CloudFormationStackArtifact ? s.template : s);
  }

  public at(path: string | string[]): StackPathInspector {
    const strPath = typeof path === 'string' ? path : path.join('/');
    return new StackPathInspector(this.stack.then(stack => {
  ...
  public get value(): Promise<{ [key: string]: any }> {
    return this.template;
  }
}

export class StackPathInspector extends Inspector {
  constructor(public readonly stack: Promise<api.CloudFormationStackArtifact>, public readonly path: string) {
    super();
  }

  public get value(): Promise<{ [key: string]: any } | undefined> {
    return this.stack.then(stack => {
      ...
      const md = metadata[this.path] || metadata[`/${stack.id}${this.path}`];

...
export = {
  'Vpc.fromLookup()': {
    ...
    'selectSubnets onePerAz fails when AZ is not defined'(test: Test) {
      const previous = mockVpcContextProviderWith(test, {
        vpcId: 'vpc-1234',
        subnetGroups: [
          {
      ...
      // THEN
      test.throws(() => vpc.selectSubnets({ subnetType: SubnetType.PRIVATE, onePerAz: true }));

      restoreContextProvider(previous);
      test.done();
    },

    'selectSubnets withoud onePerAz when AZ is not defined'(test: Test) {
      const previous = mockVpcContextProviderWith(test, {
        vpcId: 'vpc-1234',
        subnetGroups: [
          {
      ...
      // THEN: we got 4 subnets
      test.deepEqual(subnets.subnets.length, 4);

...
Use the `DataFirehosePutRecord` target to put a record to an Amazon Data Firehose delivery stream.
...
new Schedule(this, 'Schedule', {
  schedule: ScheduleExpression.rate(Duration.minutes(60)),
  target: new targets.DataFirehosePutRecord(deliveryStream, {
    input: ScheduleTargetInput.fromObject(payload),
  }),
});
```

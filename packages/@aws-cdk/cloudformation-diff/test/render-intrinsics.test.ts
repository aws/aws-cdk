...
test('resolves Ref', () => expect(renderIntrinsics({ Ref: 'SomeLogicalId' })).toEqual('${SomeLogicalId}'));

test('resolves Fn::GetAtt', () =>
  expect(renderIntrinsics({ 'Fn::GetAtt': ['SomeLogicalId', 'Attribute'] })).toEqual('${SomeLogicalId.Attribute}'));

test('resolves Fn::Join', () => expect(renderIntrinsics({ 'Fn::Join': ['/', ['a', 'b', 'c']] })).toEqual('a/b/c'));
...
test('does not resolve Fn::Join if the second argument is not a list literal', () =>
  expect(renderIntrinsics({ 'Fn::Join': ['/', { Ref: 'ListParameter' }] })).toEqual(
...
test('deep resolves intrinsics in array', () =>
  expect(renderIntrinsics([{ Ref: 'SomeLogicalId' }, 'Do not replace'])).toEqual([
    '${SomeLogicalId}',
    'Do not replace',
  ]));
...
test('removes NoValue from array', () =>
  expect(renderIntrinsics([{ Ref: 'SomeLogicalId' }, { Ref: 'AWS::NoValue' }])).toEqual(['${SomeLogicalId}']));

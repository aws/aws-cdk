...
test('equality is reflexive', () => {
  fc.assert(
    fc.property(arbitraryRule, (statement) => {
...
test('equality is symmetric', () => {
  fc.assert(
    fc.property(twoArbitraryRules, (s) => {
...
test('can describe protocol', () => {
  ...
  expect(new SecurityGroupRule({ IpProtocol: 'tcp', FromPort: 10, ToPort: 20 }).describeProtocol()).toEqual(
    'TCP 10-20'
  );

...
import type {
  DescribeChangeSetOutput as DescribeChangeSet,
  ResourceChangeDetail as RCD,
...
export class TemplateAndChangeSetDiffMerger {
  public static determineChangeSetReplacementMode(
    propertyChange: ChangeSetResourceChangeDetail
  ): types.ReplacementModes {
   ...
   */
  public overrideDiffResourceChangeImpactWithChangeSetChangeImpact(
    logicalId: string,
    change: types.ResourceDifference
  ) {
    ...
    change.forEachDifference(
      (type: 'Property' | 'Other', name: string, value: types.Difference<any> | types.PropertyDifference<any>) => {
        if (type === 'Property') {
          ...
          const changingPropertyCausesResourceReplacement = (this.changeSetResources[logicalId]
  ...
  public addImportInformationFromChangeset(

...
import { Lazy, Names, Resource, UnscopedValidationError, Token } from 'aws-cdk-lib/core';
...
export interface TableBucketProps {
   ...
   *
   * When omitted, the auto-generated name is derived from the construct path and is
   * therefore not stable across stack or construct renames — pass an explicit name
   * when you need a fixed identifier across deployments.
   *
   * @default - generated by CDK from the stack and construct path.
   */
  readonly tableBucketName?: string;
...
export class TableBucket extends TableBucketBase implements ITaggableV2 {
  ...
  constructor(scope: Construct, id: string, props: TableBucketProps = {}) {
    super(scope, id, {
      physicalName: props.tableBucketName ?? Lazy.string({
    ...
    this.resource = new s3tables.CfnTableBucket(this, id, {
      tableBucketName: this.physicalName,
      unreferencedFileRemoval: {
  ...
  private parseEncryption(props: TableBucketProps): {
    ...
    if (encryptionType === TableBucketEncryption.KMS) {
      if ( key === undefined ) {
        ...
        this.allowTablesMaintenanceAccessToKey(key, this.physicalName);
      }
      return {
        bucketEncryption: {
          kmsKeyArn: key.keyArn,

...
var AssertionHandler = class extends CustomResourceHandler {
  async processEvent(request2) {
    ...
    switch (request2.assertionType) {
      case "objectLike":
        if (typeof actual === "string") {
          actual = JSON.parse(actual);
        }
        matcher = Match2.objectLike(expected);
        break;
      case "equals":
        matcher = Match2.exact(expected);
        break;
      case "arrayWith":
        matcher = Match2.arrayWith(expected);
        break;
      default:
        throw new Error(`Unsupported query type ${request2.assertionType}`);
    }
...
var SdkHandler = class extends CustomResourceHandler {
  async processEvent(request2) {

...
new StateMachine(stack, 'StateMachine', {
  definition: new Task(stack, 'TrainTask', {
    task: new SagemakerTrainTask(stack, {
      ...
      inputDataConfig: [{ channelName: 'InputData', dataSource: { s3DataSource: { s3Location: S3Location.inBucket(trainingData, 'data/') } } }],
      outputDataConfig: { s3OutputLocation: S3Location.inBucket(trainingData, 'result/') },

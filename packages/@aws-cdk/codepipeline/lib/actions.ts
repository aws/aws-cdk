...
export abstract class Action extends Construct {
    ...
    public validate(): string[] {
        return validateArtifactBounds('input', this.inputArtifacts, this.artifactBounds.minInputs,
                this.artifactBounds.maxInputs, this.category, this.provider).concat(
            validateArtifactBounds('output', this.outputArtifacts, this.artifactBounds.minOutputs,
    ...
    public onStateChange(name: string, target?: IEventRuleTarget, options?: EventRuleProps) {
        const rule = new EventRule(this.stage.pipeline, name, options);

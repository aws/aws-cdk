...
export class SubnetV2 extends Resource implements ISubnetV2 {
  ...
  constructor(scope: Construct, id: string, props: SubnetV2Props) {
    ...
    if (!checkCidrRanges(props.vpc, props.ipv4CidrBlock.cidr)) {
      throw new ValidationError('CidrBlockWithinRange', 'CIDR block should be within the range of VPC', this);
    }
    ...
    if (overlap || overlapIpv6) {
      throw new ValidationError('CidrBlockOverlapExisting', 'CIDR block should not overlap with existing subnet blocks', this);
    }

    if (props.assignIpv6AddressOnCreation && !props.ipv6CidrBlock) {
      throw new ValidationError('Ipv6CidrBlockRequired', 'IPv6 CIDR block is required when assigning IPv6 address on creation', this);
    }

    if (props.mapPublicIpOnLaunch === true && props.subnetType !== SubnetType.PUBLIC) {
      throw new ValidationError('MappubliciponlaunchTruePublicSubnets', 'mapPublicIpOnLaunch can only be set to true for public subnets', this);
    }
...
function storeSubnetToVpcByType(vpc: IVpcV2, subnet: SubnetV2, type: SubnetType) {
  ...
  if (findFunctionType) {
    ...
    throw new UnscopedValidationError('UnsupportedSubnetTypeType', `Unsupported subnet type: ${type}`);
  }
...
function validateSupportIpv6(vpc: IVpcV2) {
  ...
  secondaryAddress.ipv6IpamPoolId !== undefined || secondaryAddress.ipv6Pool !== undefined)) {
      ...
      throw new UnscopedValidationError('Ipv6VpcEnableIpv6', 'To use IPv6, the VPC must enable IPv6 support.');
    }
  } else {return false;}
}
...
function checkCidrRanges(vpc: IVpcV2, cidrRange: string) {
  ...
  if (allCidrs.length === 0) {
    throw new UnscopedValidationError('SecondaryAddressAttachedVpc', 'No secondary IP address attached to VPC');
  }

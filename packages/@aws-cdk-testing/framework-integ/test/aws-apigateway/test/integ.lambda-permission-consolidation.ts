...
class LambdaPermissionConsolidationStack extends Stack {
  ...
  constructor(scope: Construct) {
    ...
    // Add 6 methods to trigger permission consolidation
    const methods = ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD'];
    methods.forEach(method => {
      this.api.root.addMethod(method, new LambdaIntegration(fn));
    });

    // Add another method after consolidation to verify it still works
    this.api.root.addMethod('OPTIONS', new LambdaIntegration(fn));
  }
}

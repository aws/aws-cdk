 ...
 * 8. agentcore.PolicyEngine.addPolicy() convenience method
...
const basicEngine = new agentcore.PolicyEngine(stack, 'BasicPolicyEngine', {
  policyEngineName: 'basic_engine_integ_test',
  description: 'Basic policy engine for integration testing',
});

new agentcore.Policy(stack, 'BasicPolicy', {
...
const encryptedEngine = new agentcore.PolicyEngine(stack, 'EncryptedPolicyEngine', {
...
new agentcore.Policy(stack, 'RawCedarPolicy', {
  policyEngine: encryptedEngine,
  policyName: 'raw_cedar_policy',
  definition: 'permit(principal, action in [AgentCore::Action::"GetData", AgentCore::Action::"PutData"], resource is AgentCore::Gateway);',
...
new agentcore.Policy(stack, 'BuilderSimplePolicy', {
  policyEngine: encryptedEngine,
...
new agentcore.Policy(stack, 'BuilderConditionsPolicy', {
  policyEngine: encryptedEngine,
...
new agentcore.Policy(stack, 'BuilderComplexPolicy', {
  policyEngine: encryptedEngine,
...
new agentcore.Policy(stack, 'BuilderForbidPolicy', {
  policyEngine: encryptedEngine,
...
const engineWithAddedPolicies = new agentcore.PolicyEngine(stack, 'EngineWithAddedPolicies', {
...
const autoNamedEngine = new agentcore.PolicyEngine(stack, 'AutoNamedEngine', {
  description: 'PolicyEngine with auto-generated name',
});

new agentcore.Policy(stack, 'AutoNamedPolicy', {
...
const gatewayEngine = new agentcore.PolicyEngine(stack, 'GatewayPolicyEngine', {
...
new agentcore.Gateway(stack, 'PolicyGateway', {
  gatewayName: 'policy-integ-gateway',
  policyEngineConfiguration: {
    policyEngine: gatewayEngine,
    mode: agentcore.PolicyEngineMode.LOG_ONLY,
  },
});

...
const stack = new cdk.Stack(app, 'integ-distribution-tenant-managed-certificate', {
  env: {
    region: 'us-east-1',
    account,
  },
...
const cnameRecord = new route53.CnameRecord(stack, 'Record', {
  domainName: connectionGroup.routingEndpoint,
  zone: hostedZone,
  recordName: `integ.${hostedZoneName}`,
});

const distributionTenant = new cloudfront.DistributionTenant(stack, 'dist-tenant', {
  distributionId: distribution.distributionId,
  domains: [`integ.${hostedZoneName}`],
  ...
  managedCertificateRequest: {
    primaryDomainName: `integ.${hostedZoneName}`,
...
distribution.node.addDependency(cnameRecord);
distributionTenant.node.addDependency(distribution);

new IntegTest(app, 'distribution-tenant-managed-certificate', {
  testCases: [stack],
  diffAssets: true,
  enableLookups: true,
});

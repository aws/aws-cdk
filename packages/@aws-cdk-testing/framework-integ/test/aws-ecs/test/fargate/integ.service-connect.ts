...
describeServicesCall.expect(integ.ExpectedResult.objectLike({
  services: integ.Match.arrayWith([
    integ.Match.objectLike({
      deployments: integ.Match.arrayWith([
        integ.Match.objectLike({
          serviceConnectConfiguration: {
            enabled: true,
            namespace: integ.Match.stringLikeRegexp('.*'),
            services: integ.Match.arrayWith([
              integ.Match.objectLike({
                portName: 'api',
              }),
            ]),
            logConfiguration: integ.Match.objectLike({
              logDriver: 'awslogs',
            }),
            accessLogConfiguration: {
              format: 'JSON',

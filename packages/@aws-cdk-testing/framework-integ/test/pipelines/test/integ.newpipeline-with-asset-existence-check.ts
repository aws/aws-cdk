...
/**
 * Integration test for pipeline asset existence checking.
 *
 * This test verifies that when checkAssetExistence is enabled, the pipeline
 * generates the appropriate CodeBuild commands to check S3 and ECR before
 * publishing assets.
 *
 * Stack verification steps:
 * 1. Verify the pipeline synthesizes successfully with checkAssetExistence enabled
...
class AppStage extends Stage {
  constructor(scope: Construct, id: string, props?: StageProps) {
    ...
    // Add file assets to test S3 existence checking
    ...
    // Add a Docker image asset to test ECR existence checking
...
const stack = new PipelineStack(app, 'PipelineStackWithAssetExistenceCheck');

new IntegTest(app, 'PipelineAssetExistenceCheckTest', {
  testCases: [stack],
});

...
def get_chart_from_oci(tmpdir, repository=None, version=None):
    ...
    while retry > 0:
        try:
            ...
            if 'ecr_login' in commands and 'registry_login' in commands:
                logger.info("Running login command: %s", commands['ecr_login'])
                logger.info("Running registry login command: %s", commands['registry_login'])
                ...
                p2 = Popen(commands['registry_login'], stdin=p1.stdout, stdout=PIPE, stderr=PIPE, cwd=tmpdir)
                ...
                if p2.returncode != 0:
                    ...
                    raise subprocess.CalledProcessError(p2.returncode, commands['registry_login'], p2_err)

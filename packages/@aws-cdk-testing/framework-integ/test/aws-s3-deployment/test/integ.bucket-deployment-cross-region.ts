...
function deterministicBucketName(testName: string): string {
  const hash = crypto.createHash('sha256').update(testName).digest('hex').substring(0, 12);
  return `cdk-s3-deploy-cr-${hash}`;
}

function createTestCase(props: CreateTestCaseProps) {
  const bucketName = deterministicBucketName(props.testName);
  ...
  const bucketStack = new BucketStack(props.app, `${props.testName}BucketStack`, {
    ...
    env: { account, region: props.destinationBucketRegion },
  });
  ...
  const deploymentStack = new BucketDeploymentStack(props.app, `${props.testName}Stack`, {
    ...
    env: { account, region: props.deploymentRegion },
  });

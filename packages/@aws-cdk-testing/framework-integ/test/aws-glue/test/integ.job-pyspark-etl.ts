...
const stack = new cdk.Stack(app, 'aws-glue-job-pyspark-etl');
...
new glue.PySparkEtlJob(stack, 'BasicPySparkETLJob', {
  script: script,
  role: iam_role,
});

new glue.PySparkEtlJob(stack, 'OverridePySparkETLJob', {
  script: script,
  role: iam_role,

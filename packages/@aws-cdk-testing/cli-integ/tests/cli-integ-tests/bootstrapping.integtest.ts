...
integTest('can bootstrap without execution', withoutBootstrap(async (fixture) => {
  ...
  const resp = await fixture.aws.cloudFormation.send(new DescribeStacksCommand({
...
integTest('can remove customPermissionsBoundary', withoutBootstrap(async (fixture) => {
  ...
  let policyArn;
  try {
    const policy = await fixture.aws.iam.send(new CreatePolicyCommand({
    ...
    const response2 = await fixture.aws.cloudFormation.send(new DescribeStacksCommand({ StackName: bootstrapStackName }));
    expect(
    ...
    const role = await fixture.aws.iam.send(new GetRoleCommand({ RoleName: `cdk-${fixture.qualifier}-cfn-exec-role-${account}-${region}` }));
    if (!role.Role) {
...
integTest('create ECR with tag IMMUTABILITY to set on', withoutBootstrap(async (fixture) => {
  ...
  const ecrResponse = await fixture.aws.ecr.send(new DescribeRepositoriesCommand({
    repositoryNames: [

import { AwsAccountId, Construct, FnConcat, PolicyStatement, ServicePrincipal, Token } from 'aws-cdk';
...
import { Role } from '../../aws-cdk-iam/lib/role';
...
export class CloudTrail extends Construct {
    ...
    constructor(parent: Construct, name: string, props: CloudTrailProps = {}) {
        ...
        const s3bucket = new Bucket(this, 'S3', {encryption: BucketEncryption.Unencrypted});
        ...
        s3bucket.addToResourcePolicy(new PolicyStatement()
            .addResource(s3bucket.arnForObjects(new FnConcat('/AWSLogs/', new AwsAccountId())))
        ...
        if (props.sendToCloudWatchLogs) {
            ...
            const logsRole = new Role(this, 'LogsRole', {assumedBy: new ServicePrincipal(cloudTrailPrincipal) });

            const streamArn = new FnConcat(this.cloudWatchLogsGroupArn, ":log-stream:*");
            logsRole.addToPolicy(new PolicyStatement()

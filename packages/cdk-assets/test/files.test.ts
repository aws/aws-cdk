...
import { mockAws, mockedApiFailure, mockedApiResult, mockS3Upload } from './mock-aws';
import { mockSpawn } from './mock-child_process';
import * as mockfs from './mock-fs';
import { AssetPublishing, AssetManifest } from '../lib';

const ABS_PATH = '/external/cdk.out/some_external_file';
...
describe('file upload tests', () => {
  ...
  test('do nothing if external asset file exists already', async () => {
    const pub = new AssetPublishing(AssetManifest.fromPath(mockfs.path('/external/cdk.out')), { aws });
    aws.mockS3.listObjectsV2 = mockedApiResult({ Contents: [{ Key: 'some_key' }] });
  ...
  test('upload external asset correctly', async () => {
    const pub = new AssetPublishing(AssetManifest.fromPath(mockfs.path('/external/cdk.out')), { aws });
    aws.mockS3.listObjectsV2 = mockedApiResult({ Contents: undefined });
    aws.mockS3.upload = mockS3Upload('ZIP_FILE_THAT_IS_DEFINITELY_NOT_EMPTY');
    const expectAllSpawns = mockSpawn({ commandLine: ['sometool'], stdout: `${mockfs.path(ABS_PATH)}` });

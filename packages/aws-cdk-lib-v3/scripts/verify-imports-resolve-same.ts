...
async function main() {
  ...
  await withTemporaryDirectory(async (tmpDir) => {
    ...
    await fs.symlink(path.resolve(__dirname, '..'), path.join(tmpDir, 'node_modules', 'aws-cdk-lib-v3'));

    const import1 = 'import { aws_lambda } from "aws-cdk-lib-v3";';
    const import2 = 'import * as aws_lambda from "aws-cdk-lib-v3/aws-lambda";';

    const src1 = await compileAndResolve(path.join(tmpDir, 'program1.ts'), import1, 'aws_lambda');
    const src2 = await compileAndResolve(path.join(tmpDir, 'program2.ts'), import2, 'aws_lambda');

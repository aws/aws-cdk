  ...
  return yargs
    ...
    .command('deploy [STACKS..]', 'Deploys the stack(s) named STACKS into your AWS account', (yargs: Argv) =>
      yargs
        ...
        .option('progress', {
          type: 'string',
          choices: [StackActivityProgress.BAR, StackActivityProgress.EVENTS],
          desc: 'Display mode for stack activity events',
        })
        .option('rollback', {
          type: 'boolean',
          desc:
            "Rollback stack to stable state on failure. Defaults to 'true', iterate more rapidly with --no-rollback or -R. " +
        ...
        .option('hotswap', {
          type: 'boolean',
          desc:
            "Attempts to perform a 'hotswap' deployment, " +
        ...
        .option('watch', {
          type: 'boolean',
          desc:
            'Continuously observe the project files, ' +
    ...
    .command('import [STACK]', 'Import existing resource(s) into the given STACK', (yargs: Argv) =>
      yargs
        ...
        .option('record-resource-mapping', {
          ...
          requiresArg: true,
          desc:
            'If specified, CDK will generate a mapping of existing physical resources to CDK resources to be imported as. The mapping ' +
        ...
        .option('resource-mapping', {
          ...
          requiresArg: true,
          desc:
            'If specified, CDK will use the given file to map physical resources to CDK resources for import, instead of interactively ' +
    ...
    .command('migrate', false /* hidden from "cdk --help" */, (yargs: Argv) =>
      yargs
        .option('stack-name', {
          type: 'string',
          alias: 'n',
        ...
        .option('language', {
          ...
          choices: MIGRATE_SUPPORTED_LANGUAGES,
        })
...
export async function exec(args: string[], synthesizer?: Synthesizer): Promise<number | void> {
  ...
  if (shouldDisplayNotices()) {
    void refreshNotices().catch((e) => debug(`Could not refresh notices: ${e}`));
  }

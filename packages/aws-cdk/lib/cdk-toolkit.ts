      ...
      for (const stack of stacks.stackArtifacts) {
        ...
        if (options.changeSet) {
          ...
          try {
            ...
            debug(e.message);
            stream.write(
              'Checking if the stack exists before creating the changeset has failed, will base the diff on template differences (run again with -v to see the reason)\n',
            );
            stackExists = false;
          }
      ...
      if (scanType == TemplateSourceOptions.SCAN) {
        ...
        if (templateId) {
          ...
          cfn = new CfnTemplateGeneratorProvider(
            await TemplateGeneratorClient.build(this.props.sdkProvider, environment),
          );

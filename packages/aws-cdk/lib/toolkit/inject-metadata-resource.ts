...
/**
 * A CloudAssemblySource that is injecting the metadata resource based on the assembly version.
 *
 ...
 */
export class CloudAssemblyWithMetadataResource implements ICloudAssemblySource {
  /**
   ...
   */
  public static TEMPLATE_INCLUDES_ANALYTICS_SCHEMA_VERSION = '6.0.0';
  ...
  public async produce(): Promise<cxapi.CloudAssembly> {
    ...
    if (!semver.gte(assembly.version, CloudAssemblyWithMetadataResource.TEMPLATE_INCLUDES_ANALYTICS_SCHEMA_VERSION)) {
      // @deprecate(v2): the framework now manages its own analytics. For
      // Cloud Assemblies *older* than when we introduced this feature, have
      // the CLI add it. Otherwise, do nothing.

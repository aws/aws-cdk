...
import { Command as _Command } from '@smithy/smithy-client';
...
/** The list of languages supported by the built-in cdk-from-cfn binary. */
...
export async function readFromStack(
  ...
  const cloudFormation = (await sdkProvider.forEnvironment(environment, Mode.ForReading)).cloudFormation();
...
export async function generateTemplate(options: GenerateTemplateOptions): Promise<GenerateTemplateOutput> {
  const cfn = new CfnTemplateGeneratorProvider(
    await TemplateGeneratorClient.build(options.sdkProvider, options.environment),
  );
...
export async function scanProgressBar(scanId: string, cfn: CfnTemplateGeneratorProvider) {
  ...
  let curScan: DescribeResourceScanCommandOutput = { Status: ScanStatus.IN_PROGRESS, $metadata: {} };
...
interface ICommand {
  new (input: any): _Command<any, any, any, any, any>;
}
export class TemplateGeneratorClient {
  public static async build(sdkProvider: SdkProvider, environment: Environment) {
    const client = (await sdkProvider.forEnvironment(environment, Mode.ForReading)).cloudFormation();
    const sdk = await sdkProvider.forEnvironment(environment, Mode.ForReading);
    sdk.appendCustomUserAgent('cdk-migrate');

    return new TemplateGeneratorClient(client);
  }

  constructor(private readonly client: CloudFormationClient) {}

  public async call(input: any, Command: ICommand) {
    const _command = new Command(input);
    return this.client.send(_command);
  }
}
...
export class CfnTemplateGeneratorProvider {
  private generator: TemplateGeneratorClient;
  constructor(generator: TemplateGeneratorClient) {
  ...
  async getResourceScanRelatedResources(
    ...
    for (const chunk of chunks(resources, 100)) {
      ...
      const res = await this.generator.call(
        {
          ResourceScanId: scanId,
          Resources: chunk,
        },
      ...
      while (nextToken) {
        const nextRelatedResources = await this.generator.call(
          {
            ResourceScanId: scanId,
  ...
  async listResourceScans() {
    return this.generator.call({}, ListResourceScansCommand);
  }
  ...
  async listResourceScanResources(scanId: string, filters: string[] = []): Promise<ScannedResourceIdentifier[]> {
    ...
    if (filters.length > 0) {
      ...
      for (const filter of filters) {
        ...
        const resources = await this.generator.call(resourceScanInputs, ListResourceScansCommand);
        ...
        while (nextToken) {
          ...
          const nextResources = await this.generator.call(resourceScanInputs, ListResourceScansCommand);
      ...
      const resources = await await this.generator.call(resourceScanInputs, ListResourceScansCommand);
      resourceList = resourceList!.concat(resources.Resources ?? []);
      let nextToken = resources.NextToken;
  ...
  async describeResourceScan(scanId: string): Promise<DescribeResourceScanCommandOutput> {
    return this.generator.call({ ResourceScanId: scanId }, DescribeResourceScanCommand);
  }
  ...
  async describeGeneratedTemplate(templateId: string): Promise<DescribeGeneratedTemplateCommandOutput> {
    const generatedTemplate = await this.generator.call(
  ...
  async createGeneratedTemplate(stackName: string, resources: ResourceDefinition[]) {
    const createTemplateOutput = await this.generator.call(
      {
        Resources: resources,

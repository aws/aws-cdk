...
export class EnvironmentResources {
  ...
  public async prepareEcrRepository(repositoryName: string): Promise<EcrRepositoryInfo> {
    ...
    try {
      ...
      const describeResponse = await ecr.send(new DescribeRepositoriesCommand({ repositoryNames: [repositoryName] }));
    ...
    const response = await ecr.send(new CreateRepositoryCommand({ repositoryName, tags: [assetTag] }));

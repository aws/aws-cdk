...
export class CloudFormationStack {
   ...
   */
  public static doesNotExist(cfn: CloudFormationClient, stackName: string) {
   ...
   */
  public static fromStaticInformation(cfn: CloudFormationClient, stackName: string, stack: Stack) {
  ...
  public async template(): Promise<Template> {
    ...
    if (this._template === undefined) {
      const commnand = new GetTemplateCommand({
        StackName: this.stackName,
        TemplateStage: this.retrieveProcessedTemplate ? 'Processed' : 'Original',
      });
      const response = await this.cfn.send(commnand);
...
async function describeChangeSet(
  ...
  const describeOne = new DescribeChangeSetCommand({
    StackName: stackName,
    ChangeSetName: changeSetName,
  });
  const response = await cfn.send(describeOne);

  // If fetchAll is true, traverse all pages from the change set description.
  while (fetchAll && response.NextToken != null) {
    const describeAll = new DescribeChangeSetCommand({
      StackName: stackName,
      ChangeSetName: response.ChangeSetId ?? changeSetName,
      NextToken: response.NextToken,
    });
    const nextPage = await cfn.send(describeAll);
...
async function createChangeSet(options: CreateChangeSetOptions): Promise<DescribeChangeSetCommandOutput> {
  ...
  const changeSet = await options.cfn.send(command);
...
export async function cleanupOldChangeset(changeSetName: string, stackName: string, cfn: CloudFormationClient) {
 ...
 */
export async function stabilizeStack(cfn: CloudFormationClient, stackName: string) {

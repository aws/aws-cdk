...
export async function waitForChangeSet(
  ...
  await debug('Waiting for changeset %s on stack %s to finish creating...', changeSetName, stackName);
  const ret = await waitFor(async () => {
    ...
    if (description.Status === 'CREATE_PENDING' || description.Status === 'CREATE_IN_PROGRESS') {
      await debug('Changeset %s on stack %s is still creating', changeSetName, stackName);
      return undefined;
    }
...
export async function createDiffChangeSet(
  ...
  for (const resource of Object.values(options.stack.template.Resources ?? {})) {
    if ((resource as any).Type === 'AWS::CloudFormation::Stack') {
      ...
      await debug('This stack contains one or more nested stacks, falling back to template-only diff...');
...
async function createChangeSet(options: CreateChangeSetOptions): Promise<DescribeChangeSetCommandOutput> {
  ...
  await debug(`Attempting to create ChangeSet with name ${options.changeSetName} for stack ${options.stack.stackName}`);
...
export async function stabilizeStack(cfn: ICloudFormationClient, stackName: string) {
  await debug('Waiting for stack %s to finish creating or updating...', stackName);
  return waitFor(async () => {
    const stack = await CloudFormationStack.lookup(cfn, stackName);
    if (!stack.exists) {
      await debug('Stack %s does not exist', stackName);
      return null;
    }
    const status = stack.stackStatus;
    if (status.isInProgress) {
      await debug('Stack %s has an ongoing operation in progress and is not stable (%s)', stackName, status);
      return undefined;
      ...
      await debug('Stack %s is in REVIEW_IN_PROGRESS state. Considering this is a stable status (%s)', stackName, status);
    }

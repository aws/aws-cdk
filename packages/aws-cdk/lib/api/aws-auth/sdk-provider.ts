...
export interface SdkForEnvironmentv3 {
  /**
   * The SDK for the given environment
   */
  readonly sdk: ISDKv3;
 ...
 * lookup assume role options are used when context providers perform lookups.
 */
export async function initContextProviderSdkv3(aws: SdkProviderv3, options: cxschema.ContextLookupRoleOptions): Promise<ISDKv3> {
  const account = options.account;
  const region = options.region;

...
export class SdkProvider {
  ...
  public static async withAwsCliCompatibleDefaults(options: SdkProviderOptions = {}) {
    const credentials = await AwsCliCompatible.credentialChainBuilder({
    ...
    return new SdkProvider(credentials, region, requestHandler);
  }
  ...
  public async forEnvironment(
    ...
    if (options?.assumeRoleArn === undefined) {
      ...
      return new SDK(baseCreds.credentials, env.region, this.requestHandler);
    }

    // We will proceed to AssumeRole using whatever we've been given.

    try {
      return await this.withAssumedRole(baseCreds, options.assumeRoleArn, options.assumeRoleExternalId, env.region);
    } catch (err: any) {

import { RegisterTaskDefinitionCommand, UpdateServiceCommand, waitUntilServicesStable } from '@aws-sdk/client-ecs';
import {
  ChangeHotswapResult,
  classifyChanges,
  ...
  if (namesOfHotswappableChanges.length > 0) {
    ...
    ret.push({
      ...
      apply: async (sdk: ISDK) => {
        ...
        const command = new RegisterTaskDefinitionCommand(lowercasedTaskDef);
        const registerTaskDefResponse = await sdk.ecs().send(command);
        ...
        for (const ecsService of ecsServicesReferencingTaskDef) {
          ...
          clusterPromises.push({
            promise: sdk.ecs().send(updateServiceCommand),
            ecsService: ecsService,
          });
        }
        await Promise.all(
          Object.values(servicePerClusterUpdates).map((clusterUpdates) => {
        ...
        await Promise.all(
          Object.entries(servicePerClusterUpdates).map(async ([clusterName, serviceUpdates]) => {
            return waitUntilServicesStable(
              ...
              {
                cluster: clusterName,
                services: serviceUpdates.map((service) => service.ecsService.serviceArn),
              },
            );
          }),
        );

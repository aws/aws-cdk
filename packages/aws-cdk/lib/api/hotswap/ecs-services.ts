...
//import { ChangeHotswapImpact, ChangeHotswapResult, HotswapOperation, HotswappableChangeCandidate, HotswapType, lowerCaseFirstCharacter, transformObjectKeys } from './common';
...
//): Promise<ChangeHotswapResult> {
  ...
  //for (const updatedPropName in change.propertyUpdates) {
    ...
    //if (updatedPropName !== 'ContainerDefinitions' && hotswapType === HotswapType.HOTSWAP) {
      //return ChangeHotswapImpact.REQUIRES_FULL_DEPLOYMENT;
  ...
  //if (ecsServicesReferencingTaskDef.length === 0 ||
      //(resourcesReferencingTaskDef.length > ecsServicesReferencingTaskDef.length &&
        //hotswapType === HotswapType.HOTSWAP)) {
  ...
  //// when in `HotswapType.HOTSWAP_ONLY` mode use the old
  //// props since other, non-hotswappable props may have changed.
  //// Including the new ones here would include them in the SDK call
  //// in apply(), which may cause resource replacement

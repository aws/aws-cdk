...
export async function deployStack(options: DeployStackOptions): Promise<DeployStackResult> {
  ...
  if (cloudFormationStack.stackStatus.isCreationFailure) {
    ...
    await cfn.send(new DeleteStackCommand({ StackName: deployName }));
...
class FullCloudFormationDeployment {
  ...
  private async createChangeSet(changeSetName: string, willExecute: boolean) {
    ...
    const changeSet = await this.cfn.send(
...
export async function destroyStack(options: DestroyStackOptions) {
  ...
  try {
    await cfn.send(new DeleteStackCommand({ StackName: deployName, RoleARN: options.roleArn }));

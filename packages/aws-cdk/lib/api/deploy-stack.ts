...
import { changeSetHasNoChanges, CloudFormationStack, StackParameters, TemplateParameters, waitForChangeSet, waitForStackAfterDeploy, waitForStackAfterDelete } from './util/cloudformation';
...
export async function deployStack(options: DeployStackOptions): Promise<DeployStackResult> {
  ...
  if (cloudFormationStack.stackStatus.isCreationFailure) {
    ...
    const deletedStack = await waitForStackAfterDelete(cfn, deployName);
  ...
  if (execute) {
    ...
    try {
      const finalStack = await waitForStackAfterDeploy(cfn, deployName);
...
export async function destroyStack(options: DestroyStackOptions) {
  ...
  try {
    ...
    const destroyedStack = await waitForStackAfterDelete(cfn, deployName);

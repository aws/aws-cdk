...
cleanup() {
  # this file is generated by the test so lets clean it up.
  rm -rf ${integ_test_dir}/docker/Dockerfile.Custom
}

trap cleanup EXIT INT

# before we synth we need to create the custom docker file since there 
# is a stack using it. (DockerStackWithCustomFile)
cp ${integ_test_dir}/docker/Dockerfile ${integ_test_dir}/docker/Dockerfile.Custom
...
# delete the original custom docker file that was used during synth.
# this verifies that the assemly has a copy of it and that the manifest uses
# relative paths to reference to it.
rm ${integ_test_dir}/docker/Dockerfile.Custom

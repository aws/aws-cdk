...
test('prints 0/4 progress report, when addActivity is called with an "IN_PROGRESS" ResourceStatus', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`0/4 |10:17:06 PM | ${reset('IN_PROGRESS         ')} | AWS::CloudFormation::Stack | ${reset(bold('stack1'))}`);
});
...
test('prints 1/4 progress report, when addActivity is called with an "UPDATE_COMPLETE" ResourceStatus', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`1/4 |10:17:06 PM | ${green('UPDATE_COMPLETE     ')} | AWS::CloudFormation::Stack | ${green(bold('stack1'))}`);
});
...
test('prints 1/4 progress report, when addActivity is called with an "UPDATE_COMPLETE_CLEAN_IN_PROGRESS" ResourceStatus', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`1/4 |10:17:06 PM | ${green('UPDATE_COMPLETE_CLEA')} | AWS::CloudFormation::Stack | ${green(bold('stack1'))}`);
});
...
test('prints 1/4 progress report, when addActivity is called with an "ROLLBACK_COMPLETE_CLEAN_IN_PROGRESS" ResourceStatus', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`1/4 |10:17:06 PM | ${yellow('ROLLBACK_COMPLETE_CL')} | AWS::CloudFormation::Stack | ${yellow(bold('stack1'))}`);
});
...
test('prints 0/4 progress report, when addActivity is called with an "UPDATE_FAILED" ResourceStatus', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`0/4 |10:17:06 PM | ${red('UPDATE_FAILED       ')} | AWS::CloudFormation::Stack | ${red(bold('stack1'))}`);
});
...
test('does not print "Failed Resources:" list, when all deployments are successful', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`0/2 |10:17:06 PM | ${reset('IN_PROGRESS         ')} | AWS::CloudFormation::Stack | ${reset(bold('stack1'))}`);
  expect(output[1].trim()).toStrictEqual(`1/2 |10:17:06 PM | ${green('UPDATE_COMPLETE     ')} | AWS::CloudFormation::Stack | ${green(bold('stack1'))}`);
  expect(output[2].trim()).toStrictEqual(`2/2 |10:17:06 PM | ${green('UPDATE_COMPLETE     ')} | AWS::CloudFormation::Stack | ${green(bold('stack2'))}`);
});
...
test('prints "Failed Resources:" list, when at least one deployment fails', () => {
  ...
  expect(output[0].trim()).toStrictEqual(`0/2 |10:17:06 PM | ${reset('IN_PROGRESS         ')} | AWS::CloudFormation::Stack | ${reset(bold('stack1'))}`);
  expect(output[1].trim()).toStrictEqual(`0/2 |10:17:06 PM | ${red('UPDATE_FAILED       ')} | AWS::CloudFormation::Stack | ${red(bold('stack1'))}`);
  expect(output[2].trim()).toStrictEqual('Failed resources:');
  expect(output[3].trim()).toStrictEqual(`10:17:06 PM | ${red('UPDATE_FAILED       ')} | AWS::CloudFormation::Stack | ${red(bold('stack1'))}`);
});

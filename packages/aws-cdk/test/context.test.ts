...
test('load context from both files if available', async () => {
  ...
  // WHEN
  const config = await Configuration.get({ readUserContext: false }).load();
...
test('can save and load', async () => {
  // GIVEN
  const config1 = await Configuration.get({ readUserContext: false }).load();
  config1.context.set('some_key', 'some_value');
  await config1.saveContext();
  expect(config1.context.get('some_key')).toEqual('some_value');

  // WHEN
  const config2 = await Configuration.get({ readUserContext: false }).load();

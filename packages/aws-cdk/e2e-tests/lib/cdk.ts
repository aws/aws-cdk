...
export function withCdkApp<A extends TestContext & AwsContext>(block: (context: TestFixture) => Promise<void>) {
  return async (context: A) => {
    ...
    await cloneDirectory(path.join(__dirname, 'fixture-apps', 'default'), integTestDir, context.output);
...
export function withMonolithicCfnIncludeCdkApp<A extends TestContext>(block: (context: TestFixture) => Promise<void>) {
  return async (context: A) => {
    ...
    await cloneDirectory(path.join(__dirname, 'fixture-apps', 'cfn-include'), integTestDir, context.output);
    const fixture = new TestFixture(
      integTestDir,
      stackNamePrefix,
      context.output,

...
import { data, debug, error, PluginHost, print, SDK, serializeStructure, setVerbose } from '@aws-cdk/toolchain-common';
import { Configuration, Settings } from '@aws-cdk/toolchain-common';
// import { destroyStack } from '../lib/api/deploy-stack';
import { CDKToolkit } from '../lib';
import { DEFAULT_TOOLKIT_STACK_NAME } from '../lib/api/deployment-target';
...
async function parseCommandLineArguments() {
  return yargs
    ...
    .option('outdir', { type: 'string', alias: 'o', desc: 'REQUIRED: Path of synthesized application (cdk outdir)', requiresArg: true })
...
async function initCommandLine() {
  ...
  const commandOptions = { args: argv, configuration, aws };
  ...
  async function main(command: string, args: any): Promise<number | string | {} | void> {
    ...
    async function cli(): Promise<CDKToolkit> {
      // const outFile = await fs.readJSON(args.outDir);
